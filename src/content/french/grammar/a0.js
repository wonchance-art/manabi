    ...
    sections: [
      {
        ...
        body:
          "프랑스어는 스페인어·이탈리아어와 함께 **라틴어에서 갈라져 나온 로망스어**예요. 라틴어가 2천 년에 걸쳐 변한 모습이 지금의 프랑스어죠.\n\n" +
          "이게 왜 중요하냐면 — 영어 어휘의 **절반 가까이가 프랑스어·라틴어에서 온 말**이기 때문이에요. 1066년 프랑스 노르망디 공작이 영국을 정복한 뒤 약 300년간 영국 지배층이 프랑스어를 썼고, 그때 수만 개의 프랑스어 단어가 영어로 흘러들어갔어요.\n\n" +
          "그래서 영어를 B1 정도 해두신 분이라면, 프랑스어 단어의 상당수를 **이미 알고 있는 상태**로 시작하는 거예요. 이 레퍼런스에서는 그런 연결고리를 🌱 표시로 짚어드릴게요.",
        examples: [
      ...
      },
      {
        ...
        body:
          "솔직하게 시작할게요. 한국어 화자에게 프랑스어의 난관은 크게 셋이에요.\n\n" +
          "**1. 발음** — 한국어에 없는 모음([y], 비모음)과 r 소리. 하지만 규칙이 매우 일관적이라 규칙만 익히면 처음 보는 단어도 읽을 수 있어요. (영어보다 훨씬 규칙적이에요!)\n\n" +
          "**2. 명사의 성(性)** — 모든 명사가 남성 아니면 여성. 한국어에도 영어에도 없는 개념이라 처음엔 당황스럽지만, 어미를 보면 80% 이상 추측 가능한 패턴이 있어요.\n\n" +
          "**3. 동사 활용** — 인칭마다 동사 모양이 바뀌어요. 다만 한국어도 어미가 화려하게 변하는 언어죠('가다, 가요, 갑니다, 가세요'). 변화 자체는 익숙한 개념이에요.\n\n" +
          "반대로 **쉬운 점**도 있어요. 존댓말(tu/vous) 개념은 한국어와 닮아 직관적이고, 리에종(연음)은 한국어 연음과 같은 원리이고, 어순은 영어와 거의 같아요.",
        pitfall: "프랑스어 발음이 어렵다는 말에 겁먹지 마세요. '불규칙해서 어려운' 영어와 달리 프랑스어는 '소리가 낯설 뿐 규칙은 정직한' 언어예요. A0 발음 챕터 2개만 제대로 넘기면 평생 자산이 됩니다.",
      },
      {
        heading: "이 레퍼런스 사용법",
        body:
          "문법 레퍼런스는 A0부터 C2까지 **학습 순서대로** 배열되어 있어요. 위에서 아래로 읽어 내려가면 자연스럽게 단계가 올라갑니다.\n\n" +
          "각 챕터의 예문은 그 레벨까지 배운 문법·어휘만으로 만들었으니, 모르는 게 나와도 '아직 안 배운 것'이 아니라 '지금 배우는 것'이에요. 어휘 파트는 레벨별·주제별로 정리되어 있고, 명사는 항상 관사(le/la)와 함께 실어요 — 성별까지 한 덩어리로 외우는 게 정석이거든요.\n\n" +
    ...
    sections: [
      {
        ...
        body:
          "프랑스어 알파벳은 영어와 똑같은 26자예요. 다만 글자의 **이름**이 달라요. 철자를 불러줄 일(이름 스펠링, 예약 확인)이 많으니 이름부터 익혀두면 좋아요.\n\n" +
          "특히 헷갈리는 것만 추리면: **e**는 '으'[ə], **g**는 '제'[ʒe], **j**는 '지'[ʒi], **w**는 '두블르베'(double v), **y**는 '이그렉'(그리스의 i)이에요. g와 j는 영어와 이름이 서로 뒤바뀐 느낌이라 한국 학습자들이 정말 자주 헷갈려요.",
        table: {
      ...
      },
      {
        ...
        body:
          "프랑스어에서 글자 위아래에 붙는 기호를 **악상**이라고 해요. 장식이 아니라 **발음이나 의미를 바꾸는 정식 철자**예요. 다섯 가지만 알면 됩니다.\n\n" +
          "**é (악상 테귀)** — '에'로 발음이 정해져요.\n" +
          "**è (악상 그라브)** — '애'에 가까운 열린 '에'.\n" +
          "**ê (악상 시르콩플렉스)** — 모자 모양. 발음은 è와 비슷해요. 역사적으로 뒤에 s가 있었다는 흔적이에요.\n" +
          "**ë, ï (트레마)** — '앞 글자와 따로 발음하라'는 분리 신호.\n" +
          "**ç (세디유)** — c를 a, o, u 앞에서도 [s]로 읽게 만들어요.",
        examples: [
      ...
      },
      {
        heading: "한 가지 더 — 합자 œ",
        body:
          "o와 e가 붙은 **œ**라는 글자도 가끔 만나요. sœur(누나/언니/여동생), cœur(심장), œuf(달걀) 같은 기초 단어에 나오죠. 발음은 [œ] — 입은 '오' 모양인데 혀는 '에' 위치, 한국어 '외'를 짧게 발음하는 느낌에 가까워요.",
        examples: [
    ...
    sections: [
      {
        ...
        body:
          "한국 학습자가 가장 먼저 부딪히는 벽이에요. 프랑스어 **ou**는 한국어 'ㅜ'와 같아서 쉬워요. 문제는 **u**.\n\n" +
          "u [y]는 한국어에 없는 소리예요. 만드는 법: **'이~'라고 길게 소리 내면서, 혀는 그대로 두고 입술만 'ㅜ' 모양으로 동그랗게** 오므리세요. 입술은 '우', 혀는 '이' — 이 어색한 조합이 바로 [y]입니다.\n\n" +
          "이 둘을 구별 못 하면 단어가 통째로 바뀌어요. tu(너)와 tout(전부), rue(길)와 roue(바퀴)처럼요.",
        examples: [
      ...
      },
      {
        ...
        body:
          "an/en, on, in/ain 같은 철자는 **모음+ㄴ받침이 아니라**, 코로 공기를 흘려보내며 내는 **하나의 모음**이에요. 이걸 비모음이라고 해요.\n\n" +
          "**an/en [ɑ̃]** — 입을 크게 벌린 '엉'에 가까운 콧소리. (앙X 엉O에 가까움)\n" +
          "**on [ɔ̃]** — 입술을 동그랗게 한 '옹' 콧소리.\n" +
          "**in/ain/ein [ɛ̃]** — '앵'에 가까운 콧소리.\n\n" +
          "핵심: 끝에서 혀가 입천장에 닿으면 안 돼요. 한국어 'ㄴ' 받침은 혀끝이 닿지만, 비모음은 **혀가 어디에도 닿지 않고 코로만** 울려요.",
        examples: [
