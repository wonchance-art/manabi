...
export async function GET(request){try{const p=new URL(request.url).searchParams;return reply(await getTeacherBoards(p.get('rootId'),p.get('day'),{from:p.get('from'),to:p.get('to'),cursor:p.get('cursor')}));}catch(error){return errorReply(error);}}
