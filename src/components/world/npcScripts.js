...
export const NPC_SCRIPTS = {
  ...
  ramen: {
    ...
    // 만남 주석(rfc-vocab-encounter §4.1): refs = 이 스텝에서 노출되는 정본 표제어,
    // answerRefs = 정답 발화가 요구하는 표현(계약 2 — 선행 refs ∪ assumedRefs 안에서만).
    lang: 'ja',
    // 기반 챕터 전제: ラーメン·替え玉 = ot-10-ramen, お願いします = ot-06-konbini(편의점에서 학습).
    assumedRefs: ['ラーメン', 'お願いします', '替え玉'],
    steps: [
  ...
  shrine: {
    ...
    lang: 'ja',
    // 기반 챕터 전제: ください = N5 기초 문형 「〜を ください」(ot-06-konbini 계열에서 학습).
    assumedRefs: ['ください'],
    steps: [
      { t: 'narr', text: '붉은 鳥居(도리이)를 지나자, 방울 곁에 선 미코상이 미소 짓는다.', refs: ['鳥居'] },
      {
        t: 'say', who: 'npc',
        ja: 'ようこそ。まずは 二礼二拍手一礼。',
  ...
  konbini: {
    ...
    lang: 'ja',
    // 기반 챕터 전제(ot-07-konbini): 만능 두 마디 お願いします·大丈夫(です)를 가르치는 챕터.
    assumedRefs: ['お願いします', '大丈夫'],
    steps: [
      { t: 'narr', text: '자동문이 스르륵 — 사방에서 いらっしゃいませ!가 쏟아진다. 대답 대신 가벼운 목례. 계산대 옆 온장고엔 김 오르는 肉まん(니쿠만·이미 쪄서 보온 판매), 나는 도시락(おべんとう)을 계산대에 올린다.', refs: ['いらっしゃいませ', 'お弁当'] },
      {
        t: 'say', who: 'npc',
      ...
      },
      {
        ...
        // 이 스크립트는 두 마디가 쌍으로 학습 대상이라, 오답 쪽(大丈夫です)도 노출로 기록한다.
        refs: ['お願いします', '大丈夫'],
        answerRefs: ['お願いします'],
      },
      {
  ...
  izakaya: {
    ...
    lang: 'ja',
    // 기반 챕터 전제(ot-08-izakaya): とりあえず生で의 生=生ビール, 점원 부르기 すみません.
    assumedRefs: ['生ビール', 'すみません'],
    steps: [
      { t: 'narr', text: '드르륵 — 미닫이문을 열자 いらっしゃいませ!가 쏟아진다. 목례로 받고 카운터석에 앉는다.', refs: ['いらっしゃいませ'] },
      {
        t: 'say', who: 'npc',
  ...
  ekiin: {
    ...
    lang: 'ja',
    // 기반 챕터 전제(ot-11-densha): 행선지 〜行き, 부르기 すみません(ot-08에서 학습).
    assumedRefs: ['すみません', '行き'],
    steps: [
      ...
      },
      {
        ...
        refs: ['ICカード', 'カード', 'どうぞ', '優先席'],
      },
      { t: 'say', who: 'me', ja: 'どうも。', yomi: 'どうも (도-모)', ko: '감사합니다(가볍게).', refs: ['どうも'] },
    ],
  ...
  menzei: {
    ...
    lang: 'ja',
    // 기반 챕터 전제(ot-12-menzei): 만능 お願いします(ot-07·ot-10 기학습), 지시어 これ(N5 기초).
    assumedRefs: ['お願いします', 'これ'],
    steps: [
      { t: 'narr', text: '과자와 화장품을 한 바구니. 계산대 옆 免税(めんぜい)·TAX FREE 간판의 창구로 향한다.', refs: ['免税'] },
      {
        t: 'ask', mode: 'type',
  ...
  'tokyo-yamanote-west-cafe': {
    ...
    lang: 'ja',
    // 전제: 사양의 만능 大丈夫(です)는 ot-07 편의점에서 기학습(why 문구가 명시).
    assumedRefs: ['大丈夫'],
    steps: [
      ...
      },
      {
        ...
        // ホット는 이 스텝이 신출 지점 — 노출 기록만(아이스 대비어 アイス도 함께 노출).
        refs: ['コーヒー', 'ホット', 'アイス'],
      },
      {
        t: 'say', who: 'npc',
  ...
  'tokyo-central-east-bookstore': {
    ...
    lang: 'ja',
    // 전제: すみません(ot-08 기학습), 의문어 どこ·いくら(N5 기초).
    assumedRefs: ['すみません', 'どこ', 'いくら'],
    steps: [
      ...
      },
      {
        ...
        // サイズ는 이 스텝이 신출 지점(선행 노출 없음) — 노출 기록만 하고 answerRefs는 달지 않는다.
        refs: ['サイズ'],
      },
      {
        t: 'say', who: 'npc',
  ...
  'osaka-north-hubs-transfer': {
    ...
    lang: 'ja',
    // 전제: ください(N5 기초 — ot-07·ot-10 기학습, why 문구가 명시).
    assumedRefs: ['ください'],
    steps: [
