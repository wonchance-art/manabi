...
//   軍艦島(端島)은 앞바다 섬이라 sea 타일 위 view-only 마커(백두산처럼 다가가지 않고 보는 연출).
const KYUSHU_SPOTS = [
  // 福岡
  ...
  // 長崎
  { id: 'glover-garden', name: '글로버 정원', tile: [124, 328], desc: '개항기 외국인 거류지의 서양식 저택들을 모아 공개하는 언덕 위 정원 「グラバー園」(グラバーえん).' },
  { id: 'oura-church', name: '오우라 천주당', tile: [124, 327], desc: '현존 일본 最古(최고)의 목조 고딕 양식 성당 「大浦天主堂」(おおうらてんしゅどう). 세계문화유산.' },
  { id: 'gunkanjima', name: '군함도(端島)', tile: [124, 329], desc: '해저 탄광으로 번성 후 폐광된 무인도 「端島／軍艦島」(はしま／ぐんかんじま). 군함을 닮은 외형으로 불리며 세계문화유산 — 앞바다 섬이라 배로만 가요.' },
  ...
  // 宮崎
  { id: 'takachiho', name: '다카치호 협곡', tile: [152, 328], desc: '화산 용암이 굳어 형성된 주상절리 협곡 「高千穂峡」(たかちほきょう). 마나이 폭포(真名井の滝)와 보트 놀이로 알려져요.' },
...
export const WORLD_NODES = [
  ...
  ...NAMED_PEAKS,
  // 규슈 대표 관광지 21종(福岡~鹿児島) — 전국맵 규슈 지역 채우기(마커+desc).
  ...KYUSHU_SPOTS,
