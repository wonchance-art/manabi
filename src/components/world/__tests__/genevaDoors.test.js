...
import { GENEVA_DOORS, genevaDoorById } from '../genevaDoors.js';
...
// 제네바 프랑스어 도어(fr-13~15) — 챕터 실존·track 라우팅·프랑스어권 3세트(파리·MSM·마르세유)와
// 비중복 고정. 배선 보행성은 cityDoorSets 계약이 담당.

describe('제네바 도어 3종 — 프랑스어 트랙 연결 계약', () => {
  const frenchSlugs = new Set((FRENCH.ALL_CHAPTERS ?? []).map((chapter) => chapter.slug));

  it('id fr-13~15, 챕터 실존·라우팅·2축+문화 사실', () => {
    expect(GENEVA_DOORS.map((door) => door.id)).toEqual(['fr-13', 'fr-14', 'fr-15']);
    for (const door of GENEVA_DOORS) {
  ...
  it('프랑스어권 기존 3세트와 챕터 비중복', () => {
    const used = new Set([...PARIS_DOORS, ...MSM_DOORS, ...MARSEILLE_DOORS].map((door) => door.chapter));
    for (const door of GENEVA_DOORS) expect(used.has(door.chapter), door.id).toBe(false);
  });

  it('genevaDoorById — 존재/부재', () => {
    expect(genevaDoorById('fr-14')?.name).toBe('퐁뒤 식당');
    expect(genevaDoorById('fr-99')).toBeNull();
  });
});
