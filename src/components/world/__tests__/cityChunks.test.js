...
import {
  CHUNK_TILES, chunkKey, chunkDims, chunkTileBounds, visibleChunks, chunkCapacity, ChunkLRU, planChunkUpdate,
} from '../cityChunks.js';
...
describe('대형 스트레스 — 600×500 맵 카메라 팬(상주 텍스처 상한·가시성)', () => {
  ...
  // 씬 refreshChunks 와 동일 경로(planChunkUpdate: bake 전 선축출)를 순수하게 재현.
  //   반환: 이 스텝의 peak(=선축출 후 새 청크를 하나씩 bake 하는 동안의 최대 상주 수).
  const applyStep = (lru, resident, visKeys) => {
    const plan = planChunkUpdate(lru, resident, visKeys, cap);
    for (const key of plan.toDestroy) { resident.delete(key); lru.delete(key); }
    let peak = resident.size;            // 선축출 직후
    for (const key of visKeys) {
      if (!resident.has(key)) { resident.add(key); peak = Math.max(peak, resident.size); } // bake 1개씩
      lru.touch(key);
    }
    return { peak, cap: plan.cap };
  };

  it('맵 대각선 팬 내내 상주 청크 ≤ cap(매 스텝 peak), 가시 청크는 항상 상주', () => {
    const lru = new ChunkLRU();
    ...
    for (let s = 0; s <= steps; s++) {
      const t = s / steps;
      const vis = visibleChunks(mkView(t * (COLS - 10) * TILE, t * (ROWS - 9) * TILE), opts);
      const visKeys = vis.map((c) => c.key);
      const { peak: stepPeak, cap: effCap } = applyStep(lru, resident, visKeys);
      expect(stepPeak).toBeLessThanOrEqual(effCap);          // bake 도중에도 상한 준수
      for (const k of visKeys) expect(resident.has(k)).toBe(true);
  ...
  it('P2: cap 가득 찬 상태 → 안 겹치는 위치 순간이동 시 bake 중 peak 도 cap 초과 안 함', () => {
    const lru = new ChunkLRU();
    const resident = new Set();
    // A: 좌상단 구역을 대각선으로 팬하며(계속 새 청크 방문) 상주를 cap 까지 포화시킨다.
    for (let s = 0; s <= 60; s++) {
      const p = (s / 60) * 200 * TILE;
      applyStep(lru, resident, visibleChunks(mkView(p, p), opts).map((c) => c.key));
    }
    const filled = resident.size;
    expect(filled).toBeGreaterThanOrEqual(12);               // 상주가 cap 근처로 참
    // B: 완전히 안 겹치는 우하단으로 직행 — 선축출이 없으면 filled + N 로 순간 초과.
    const visB = visibleChunks(mkView(560 * TILE, 460 * TILE), opts).map((c) => c.key);
    expect(visB.some((k) => resident.has(k))).toBe(false);   // 겹침 0
    const { peak, cap: effCap } = applyStep(lru, resident, visB);
    expect(filled + visB.length).toBeGreaterThan(effCap);    // 선축출 없으면 초과했을 상황(전제)
    expect(peak).toBeLessThanOrEqual(effCap);                // ★ bake 도중 최대 상주도 cap 이하
    expect(resident.size).toBeLessThanOrEqual(effCap);
    for (const k of visB) expect(resident.has(k)).toBe(true);
  });

  it('P1: 가로수(청크 장식)는 상주 청크 수에만 비례 — 대형 전원 PARK 여도 면적 무관', () => {
    // 600×500 전부 PARK 인 최악(트리 후보 최다). 씬은 가시 청크 범위만 트리를 생성한다.
    const treeTilesInChunk = (cx, cy) => {
      const b = chunkTileBounds(cx, cy, COLS, ROWS);
      return (b.x1 - b.x0) * (b.y1 - b.y0);               // 청크당 상한(전부 PARK 가정)
    };
    const lru = new ChunkLRU();
    const resident = new Set();
    let peakTrees = 0;
    for (let s = 0; s <= 400; s++) {
      const t = s / 400;
      const visKeys = visibleChunks(mkView(t * (COLS - 10) * TILE, t * (ROWS - 9) * TILE), opts).map((c) => c.key);
      applyStep(lru, resident, visKeys);
      let trees = 0;
      for (const key of resident) { const [cx, cy] = key.split(',').map(Number); trees += treeTilesInChunk(cx, cy); }
      peakTrees = Math.max(peakTrees, trees);
    }
    const perChunkMax = CHUNK_TILES * CHUNK_TILES;         // 256
    // 상주 트리 ≤ cap × 청크당상한. 전역 배치라면 COLS×ROWS(=30만) — 그 1/10 미만이어야.
    expect(peakTrees).toBeLessThanOrEqual(cap * perChunkMax);
    expect(peakTrees).toBeLessThan((COLS * ROWS) / 10);
  });
