...
//   geo v2.1(건물 실 래스터)에서 부두 인근 보행칸으로 재배치. **ENTRANCE 와 같은 x열(239) 북쪽**에
//   두어 스폰→북진(위로) 직진만으로 EXIT 에 닿게 한다(전국맵→시내→EXIT 복귀 e2e 스모크 계약).
//   x=239 는 y60..78 이 끊김 없는 보행 세로 회랑 — 그 북단 2칸을 EXIT 로 승격(밟으면 전국맵 복귀).
const EXIT_TILES = [[239, 61], [239, 62]];

// 도시 진입 스폰(하카타항 부두 인근 보행칸 — 페리에서 내려 도심을 향해 down). 직행 재접속 폴백.
//   EXIT 와 같은 x열(239)·8칸 남쪽 — 스폰 후 위로 직진하면 부두 EXIT(북단)에 닿는다. 메인 성분·전역 도달.
export const ENTRANCE = { x: 239, y: 70, facing: 'down' };
...
export const PROPS = [
  ...
  { kind: 'bus', tile: [296, 149] },        // キャナルシティ 버스터미널(내부 컷어웨이 · 노드 라벨 가림 회피, Codex #93)
