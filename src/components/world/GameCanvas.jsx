...
import { tokyoPilotDialogForAnchor } from './tokyoPilotDialogs';
...
export default function GameCanvas({ userId = null, devGuest = false, nickname = '나', pet = { key: 'dog', emoji: '🐕', level: 1, mood: 'happy' }, avatar = DEFAULT_AVATAR, controlsRef = null, initialSpawn = null, canAccessPreviewRegions = false, onOpenChapter = null, onOpenReading = null, onOpenDictionary = null, onAvatarChange = null }) {
  ...
  const tokyoPilotControllerRef = useRef(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      tokyoPilotControllerRef.current = tokyoPilotController;
      const tokyoPilotCtx = (sceneId) => ({
        ...
        onAnchorChange: (anchor) => {
          ...
          const dialog = tokyoPilotDialogForAnchor(anchor?.id, tokyoPilotController.snapshot().phase);
          setNearNode(dialog);
        },
      });
    ...
    return () => {
      ...
      tokyoPilotControllerRef.current = null;
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', background: '#0b0d08' }}>
      ...
      {npcDialog && (
        <NpcDialog
          ...
          stepStart={npcDialog.node?.stepStart}
          stepEnd={npcDialog.node?.stepEnd}
          ...
          completionNote={npcDialog.node?.noStamp === true
            ...
            : undefined}
          onComplete={() => {
            if (npcDialog.node?.pilotInteraction) {
              tokyoPilotControllerRef.current?.resolveLanguage({
                understood: true,
                minimal: false,
                recovered: false,
                skipped: false,
