...
export default function BoardCloudPanel({mode,active,rootId,day,store,onBackup,onBeforeAction,onOpen,pages=[],onCopy,onPick}){
 const [sourceDay,setSourceDay]=useState(null);
 const historyList=useRef(null);
 useEffect(()=>{if(!active)setSourceDay(null);},[active]);
 ...
 if(mode==='history'&&sourceDay&&active)return <BoardReusePanel key={`${rootId}:${sourceDay}`} rootId={rootId} sourceDay={sourceDay} day={day} pages={pages} blocked={store.conflict} onCopy={onCopy} onPick={onPick} onBack={()=>{setSourceDay(null);requestAnimationFrame(()=>historyList.current?.querySelector(`[data-source-day="${sourceDay}"]`)?.focus());}}/>;
 return <div className="board-cloud-panel"><label>날짜 찾기<input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="예: 2026-09"/></label>{error?<p role="alert">{error} <button onClick={()=>setAttempt(n=>n+1)}>다시 불러오기</button></p>:rows===null?<p role="status">지난 설명판을 불러오고 있어요…</p>:<><ul ref={historyList} className="board-cloud-history">{rows.filter(r=>r.day.includes(query.trim())).map(row=><li key={row.id}><div className="board-cloud-history-row"><button disabled={busy||row.day===day} aria-current={row.day===day?'date':undefined} onClick={()=>act(()=>onOpen(row.day))}><span><b>{row.day}{row.day===day?' · 현재 수업':''}</b><small>{row.pages}개 판</small></span><span aria-hidden="true">↗</span></button>{row.day!==day&&<button className="board-reuse-open" data-source-day={row.day} aria-label={`${row.day} 페이지 가져오기`} disabled={busy} onClick={()=>setSourceDay(row.day)}>가져오기</button>}</div></li>)}</ul>{!rows.length&&<p>계정에 저장한 설명판이 아직 없어요.</p>}{!!rows.length&&!rows.some(r=>r.day.includes(query.trim()))&&<p>해당 날짜의 설명판이 없어요.</p>}{truncated&&<p>최근 100개 수업을 표시합니다.</p>}</>}</div>;
}
