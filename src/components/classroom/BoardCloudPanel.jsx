...
export default function BoardCloudPanel({mode,active,rootId,day,store,onBackup,onBeforeAction,onOpen,pages=[],onCopy}){
 ...
 if(mode==='history'&&sourceDay&&active)return <BoardReusePanel key={`${rootId}:${sourceDay}`} rootId={rootId} sourceDay={sourceDay} day={day} pages={pages} blocked={store.conflict} onCopy={onCopy} onBack={()=>{setSourceDay(null);requestAnimationFrame(()=>historyList.current?.querySelector(`[data-source-day="${sourceDay}"]`)?.focus());}}/>;
