...
import {readClassMaterial} from '../../lib/classDirectStudy';
import {localViewerHref} from '../../lib/classBoard';
import {buildVocabRow} from '../../lib/vocabIO';
...
export default function ClassSaveResume({user}){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const copy=await readClassMaterial(`local:${intent.materialId}`,intent.team);
      if(!active)return;
      if(intent.revision && intent.revision!==copy.contentRevision)throw new Error('수업 자료가 바뀌었어요. 수업으로 돌아가 뜻을 확인한 뒤 다시 담아 주세요.');
      ...
      if(!copy.raw_text.includes(quote)||!quote.includes(word.text))throw new Error('원문이 바뀌었어요. 수업 자료에서 표현을 다시 선택해 주세요. 저장 요청은 보관되어 있어요.');
      const row=buildVocabRow({userId:user.id,surface:word.text,base:word.base,meaning:word.meaning,pos:word.pos,reading:word.reading,language:word.language,sourceSentence:quote,grade:intent.grade});
      if(!active)return;
      const source={kind:'class',team:intent.team,materialId:String(intent.materialId),revision:copy.contentRevision,quote,surface:word.text,...(exactToken?{tokenId:intent.tokenId}:{})};
      await saveContext({word:row,source,grade:intent.grade,...(confirm?{confirmId:confirm.id,confirmMeaning:confirm.meaning}:{})});wordSaved=true;
      ...
      const url=new URL(localViewerHref(intent.materialId,intent.team),'https://manabi.invalid');
    ...
    })().catch(error=>{if(active){setMessage(error.code==='meaning_conflict'?'기존 카드와 같은 의미인지 확인해 주세요. 아직 새 문맥은 추가하지 않았어요.':wordSaved?'단어는 저장됐지만 문맥 연결을 마치지 못했어요. 다시 시도해 주세요.':error.message||'저장을 마치지 못했어요. 다시 시도해 주세요.');if(error.code==='meaning_conflict')setConflict(error);}})
