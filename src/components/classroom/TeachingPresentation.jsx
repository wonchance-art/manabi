...
export default function TeachingPresentation({entry,lang,onClose,onRecord,recordState}) {
 const dialog=useRef(null),[hideMeaning,setHideMeaning]=useState(false);
 useEffect(()=>{
  const back=document.activeElement,old=document.body.style.overflow,position={top:window.scrollY,left:window.scrollX,behavior:'instant'};document.body.style.overflow='hidden';
  ...
  return()=>{document.body.style.overflow=old;values.forEach(([el,value])=>{el.inert=value;});window.removeEventListener('keydown',hold,true);back?.focus({preventScroll:true});window.scrollTo(position);requestAnimationFrame(()=>window.scrollTo(position));};
 },[onClose]);
 ...
 <footer><button aria-pressed={hideMeaning} onClick={()=>setHideMeaning(v=>!v)}>{hideMeaning?'뜻 보이기':'뜻 가리기'}</button>{onRecord&&<button onClick={onRecord} disabled={!!recordState}>{recordState||'수업 노트에도 기록'}</button>}<span>설명을 닫으면 읽던 위치로 돌아갑니다.</span></footer></section></div>,document.body);
}
