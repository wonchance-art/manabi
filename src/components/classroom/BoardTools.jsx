...
const tools=[['text','글자'],['arrow','화살표'],['laser','레이저']];
export default function BoardTools({active,style,onTool,onStyle}) {
  const hasStrokeSettings=!['eraser','laser','hand'].includes(active);
  return <div className="board-tools" role="toolbar" aria-label="필기 도구">
    <div className="board-icon-grid" aria-label="추가 필기 도구">{tools.map(([type,label])=><BoardIconButton key={type} icon={type} label={label} aria-pressed={active===type} onClick={()=>onTool(type)}/>)}</div>
    {active==='eraser'&&<p className="board-menu-caption">지우개는 선이나 요소를 통째로 지워요.</p>}
    {hasStrokeSettings&&<div className="board-style-options" role="group" aria-label="펜과 선택한 요소의 선 설정">
    ...
    <div className="board-style-options"><span>도형·이동</span><div>{[['rectangle','네모'],['ellipse','원'],['line','직선'],['hand','화면 이동']].map(([type,label])=><BoardIconButton key={type} icon={type} label={label} aria-pressed={active===type} onClick={()=>onTool(type)}/>)}</div>
    </div>
  </div>;
}
