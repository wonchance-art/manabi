...
export default function BookReader({ book, sectionIndex, preview = false }) {
  ...
  return <div ref={root} className={`book-reader${focus ? ' is-focused' : ''}`} onClick={interact}>
    ...
    {unit === 'cover' ? <BookHome book={book} preview={preview} progress={progress} hasProgress={hasProgress} ready={ready} /> : <div className="manabi-reader-page">
      <div className="manabi-reader-toolbar"><LibraryReturnLink/><a href={bookHref(book.edition)}>← 책으로</a><span>일본어 · N5{lesson ? ` / ${String(lesson.number).padStart(2, '0')}과` : ''}</span><div><a href={bookHref(book.edition, 'reference-start')}>찾아보기</a><Link href={`/books/japanese-n5/review?edition=${book.edition}`}>담은 표현</Link>{lesson && <button type="button" onClick={() => openPanel('materials')}>내 자료</button>}<button type="button" aria-pressed={focus} onClick={() => setFocus(!focus)}>{focus ? '기본 보기' : '집중 읽기'}</button></div></div>
