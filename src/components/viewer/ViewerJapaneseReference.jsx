...
export default function ViewerJapaneseReference({userId,word,meaning,dictEntry,loading,dictError,jaTable,formError}) {
  const [requested,setRequested]=useState(false);
  const glyphForm=jaTable?toJaForm(word,jaTable):null;
  const dictionary=japaneseReferenceForMeaning(dictEntry,meaning);
  ...
  return <section className="reader-japanese" aria-label="일본어 대조">
    <div className="reader-japanese__row">
      ...
      {glyphForm?<span lang="ja" className="reader-japanese__form">{glyphForm}</span>:<span role="status">{formError?'자형을 불러오지 못했어요.':'불러오는 중…'}</span>}
      {same&&query.data&&<small title="현재 한국어 뜻을 기준으로 찾은 AI 대응어">AI</small>}
    </div>
    {ref&&!same&&<div className="reader-japanese__row"><span className="reader-japanese__label">같은 뜻</span><strong lang="ja">{ref.form}</strong>{query.data&&<small title="현재 한국어 뜻을 기준으로 찾은 AI 대응어">AI</small>}</div>}
