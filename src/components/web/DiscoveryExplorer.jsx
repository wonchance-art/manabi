...
export default function DiscoveryExplorer({ documents, regions, topics }) {
  ...
  function change(patch) {
    ...
    window.history.replaceState(null, '', `/discover${suffix ? `?${suffix}` : ''}#reading-index`);
  }
  return <>
    <section className="discovery-language-entry" aria-labelledby="foreign-reading-title"><div><p className="manabi-eyebrow">READ IN ANOTHER LANGUAGE</p><h2 id="foreign-reading-title">이번엔, 원어로.</h2><p>학습 언어와 난이도에 맞는 공개 자료를 찾아보세요.</p></div><div className="discovery-language-links">{BOOK_SERIES.map(series => <Link key={series.id} href={`/materials?tab=public&lang=${series.language}`} style={{ '--series-color': series.color }}><span>{series.language}</span><strong>{series.name}</strong><i aria-hidden="true">↗</i></Link>)}</div></section>
