...
describe('상하이 APAC 도시 게이트', () => {
  it('인민광장 입구를 결정적으로 투영하고 원점의 체크인된 보행 타일에 도착한다', () => {
    const node = getNode('shanghai');
    ...
    expect(node).toMatchObject({
      ...
      lat: 31.2323,
      arrivalOffset: [0, 0],
      gate: { type: 'city', to: 'shanghai' },
    });
    expect([projected.x, projected.y]).toEqual([1347, 736]);
    expect(node.overworldTile).toEqual([1347, 736]);
