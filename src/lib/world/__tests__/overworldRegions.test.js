...
describe('오버월드 지역 레지스트리', () => {
  ...
  it('레만호 로잔역 게이트가 체크인된 보행 타일과 독립 상호작용 반경에 도착한다', () => {
    const region = overworldRegionById('emea');
    const leman = getNode('leman-riviera');
    const projected = projectOverworldRegionCoordinate(region, leman.lon, leman.lat);
    const tile = {
      x: projected.x + leman.arrivalOffset[0],
      y: projected.y + leman.arrivalOffset[1],
    };

    expect(leman).toMatchObject({ lon: 6.6291, lat: 46.5167, arrivalOffset: [0, 0] });
    expect(projected).toEqual({ x: 279, y: 481 });
    expect(tile).toEqual({ x: 279, y: 481 });
    expect(leman.overworldTile).toEqual([tile.x, tile.y]);
    expect(checkedInGateCell(region, { tile })).toMatchObject({
      valid: true,
      collision: 0,
      viewOnly: 0,
    ...
    const existingNodes = [
      ...ALL_WORLD_NODES
        .filter(({ id, regionId }) => id !== leman.id && regionId === region.id)
        .map(({ id, overworldTile: existingTile }) => ({ id, tile: existingTile })),
      ...EMEA_RAIL_NETWORK.hubs.map(({ id, tile: existingTile }) => ({ id, tile: existingTile })),
      { id: region.gate.id, tile: [region.gate.tile.x, region.gate.tile.y] },
      { id: region.airGate.id, tile: [region.airGate.tile.x, region.airGate.tile.y] },
  ...
  it('리옹 파르디외역 게이트가 체크인된 보행 타일과 독립 상호작용 반경에 도착한다', () => {
    const region = overworldRegionById('emea');
    const lyon = getNode('lyon');
    const projected = projectOverworldRegionCoordinate(region, lyon.lon, lyon.lat);
    const tile = {
      x: projected.x + lyon.arrivalOffset[0],
      y: projected.y + lyon.arrivalOffset[1],
    };

    expect(lyon).toMatchObject({ lon: 4.8595, lat: 45.7606, arrivalOffset: [0, 0] });
    expect(projected).toEqual({ x: 251, y: 500 });
    expect(tile).toEqual({ x: 251, y: 500 });
    expect(lyon.overworldTile).toEqual([tile.x, tile.y]);
    expect(checkedInGateCell(region, { tile })).toMatchObject({
      valid: true,
      collision: 0,
      viewOnly: 0,
    ...
    const existingNodes = [
      ...ALL_WORLD_NODES
        .filter(({ id, regionId }) => id !== lyon.id && regionId === region.id)
        .map(({ id, overworldTile: existingTile }) => ({ id, tile: existingTile })),
      ...EMEA_RAIL_NETWORK.hubs.map(({ id, tile: existingTile }) => ({ id, tile: existingTile })),
      { id: region.gate.id, tile: [region.gate.tile.x, region.gate.tile.y] },
      { id: region.airGate.id, tile: [region.airGate.tile.x, region.airGate.tile.y] },
  ...
  it('보르도 생장역 게이트가 체크인된 보행 타일과 독립 상호작용 반경에 도착한다', () => {
    const region = overworldRegionById('emea');
    const bordeaux = getNode('bordeaux');
    const projected = projectOverworldRegionCoordinate(region, bordeaux.lon, bordeaux.lat);
    const tile = {
      x: projected.x + bordeaux.arrivalOffset[0],
      y: projected.y + bordeaux.arrivalOffset[1],
    };

    expect(bordeaux).toMatchObject({ lon: -0.556, lat: 44.8256, arrivalOffset: [0, 0] });
    expect(projected).toEqual({ x: 165, y: 523 });
    expect(tile).toEqual({ x: 165, y: 523 });
    expect(bordeaux.overworldTile).toEqual([tile.x, tile.y]);
    expect(checkedInGateCell(region, { tile })).toMatchObject({
      valid: true,
      collision: 0,
      viewOnly: 0,
    ...
    const existingNodes = [
      ...ALL_WORLD_NODES
        .filter(({ id, regionId }) => id !== bordeaux.id && regionId === region.id)
        .map(({ id, overworldTile: existingTile }) => ({ id, tile: existingTile })),
      ...EMEA_RAIL_NETWORK.hubs.map(({ id, tile: existingTile }) => ({ id, tile: existingTile })),
      { id: region.gate.id, tile: [region.gate.tile.x, region.gate.tile.y] },
      { id: region.airGate.id, tile: [region.airGate.tile.x, region.airGate.tile.y] },
  ...
  it('스트라스부르역 게이트가 체크인된 보행 타일과 독립 상호작용 반경에 도착한다', () => {
    const region = overworldRegionById('emea');
    const strasbourg = getNode('strasbourg');
    const projected = projectOverworldRegionCoordinate(region, strasbourg.lon, strasbourg.lat);
    const tile = {
      x: projected.x + strasbourg.arrivalOffset[0],
      y: projected.y + strasbourg.arrivalOffset[1],
    };

    expect(strasbourg).toMatchObject({ lon: 7.735, lat: 48.585, arrivalOffset: [0, 0] });
    expect(projected).toEqual({ x: 296, y: 430 });
    expect(tile).toEqual({ x: 296, y: 430 });
    expect(strasbourg.overworldTile).toEqual([tile.x, tile.y]);
    expect(checkedInGateCell(region, { tile })).toMatchObject({
      valid: true,
      collision: 0,
      viewOnly: 0,
    ...
    const existingNodes = [
      ...ALL_WORLD_NODES
        .filter(({ id, regionId }) => id !== strasbourg.id && regionId === region.id)
        .map(({ id, overworldTile: existingTile }) => ({ id, tile: existingTile })),
      ...EMEA_RAIL_NETWORK.hubs.map(({ id, tile: existingTile }) => ({ id, tile: existingTile })),
      { id: region.gate.id, tile: [region.gate.tile.x, region.gate.tile.y] },
      { id: region.airGate.id, tile: [region.airGate.tile.x, region.airGate.tile.y] },
