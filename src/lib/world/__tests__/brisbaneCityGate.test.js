...
describe('브리즈번 APAC 도시 게이트', () => {
  it('센트럴역 입구를 결정적으로 투영하고 원점의 체크인된 보행 타일에 도착한다', () => {
    const node = getNode('brisbane');
    ...
    expect(node).toMatchObject({
      ...
      lat: -27.466,
      arrivalOffset: [0, 0],
      gate: { type: 'city', to: 'brisbane' },
    });
    expect([projected.x, projected.y]).toEqual([2039, 2186]);
    expect(node.overworldTile).toEqual([2039, 2186]);
