...
describe('멜버른 APAC 도시 게이트', () => {
  it('플린더스 스트리트역 좌표를 결정적으로 투영하고 원점의 체크인된 보행 타일에 도착한다', () => {
    const node = getNode('melbourne');
    ...
    expect(node).toMatchObject({
      ...
      lat: -37.8183,
      arrivalOffset: [0, 0],
      gate: { type: 'city', to: 'melbourne' },
    });
    expect([projected.x, projected.y]).toEqual([1862, 2442]);
    expect(node.overworldTile).toEqual([1862, 2442]);
