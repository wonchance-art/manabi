...
export const ZH_POS_FIX = {
  ...
  // ── 분석기 라운드 9 (2026-09-02) — 어휘 정답지 대조. 어휘 표제어 6,985개를 자기 예문에서 토큰화해 콘텐츠 pos와
  //    대조하니 단일 토큰 6,034 중 불일치 981. 절반은 분류 체계 차이(콘텐츠 명사 ⊃ 시간사·방위사·처소사, 개사=전치사)
  //    라 손대지 않고, jieba **사전 태그 자체가 틀린** 것(문맥 태그 = 고립 태그)만 단어별로 싣는다. 성씨 겸용 1자
  //    (金·熊·钟·云·富·蓝·帅·咸·美·青)는 정밀도 원칙상 뺐다 — 金先生의 金은 nr이 맞다.
  // 고유명사 태그로 오는 보통명사 (nr/ns → n)
  红绿灯: { tag: 'n' }, 友谊: { tag: 'n' }, 孙子: { tag: 'n' }, 麦克风: { tag: 'n' }, 雷达: { tag: 'n' }, 蓝天: { tag: 'n' },
  海湾: { tag: 'n' }, 商城: { tag: 'n' }, 外卖: { tag: 'n' }, // 外卖/v(팔아치우다) — 학습 텍스트의 外卖는 배달 음식
  // 고유명사 태그로 오는 동사 (ns/nz/nr → v)
  回国: { tag: 'v' }, 长大: { tag: 'v' }, 登山: { tag: 'v' }, 上门: { tag: 'v' }, 入门: { tag: 'v' }, 上市: { tag: 'v' }, 上台: { tag: 'v' },
  抽奖: { tag: 'v' }, 中奖: { tag: 'v' }, 中毒: { tag: 'v' }, 曝光: { tag: 'v' }, 敬仰: { tag: 'v' }, 布满: { tag: 'v' }, 养老: { tag: 'v' },
  // 고유명사 태그로 오는 형용사 (nr/ns → a) · 高兴/b는 구별사가 아니다(我很高兴 — 술어가 된다)
  高明: { tag: 'a' }, 爱国: { tag: 'a' }, 长寿: { tag: 'a' }, 安宁: { tag: 'a' }, 吉利: { tag: 'a' }, 高兴: { tag: 'a' },
  // 수사 접두로 m이 붙은 부사 (一起 HSK1이 「수사」로 떴다). 一道는 一道菜(m)가 있어 뺐다.
  一起: { tag: 'd' }, 十分: { tag: 'd' }, 一下子: { tag: 'd' }, 万一: { tag: 'd' }, 大多: { tag: 'd' }, 多次: { tag: 'd' },
  整天: { tag: 'd' }, 大声: { tag: 'd' }, 万分: { tag: 'd' }, 四处: { tag: 'd' }, 大都: { tag: 'd' },
  // 구별사 b로 오는 명사 · 시간사 t로 오는 동사 · HMM x로 오는 동사 1자
  年级: { tag: 'n' }, 主任: { tag: 'n' },
  过年: { tag: 'v' }, 上来: { tag: 'v' }, 下来: { tag: 'v' }, 下去: { tag: 'v' },
  留: { tag: 'v' }, 结: { tag: 'v' }, 牵: { tag: 'v' }, 赖: { tag: 'v' }, 悬: { tag: 'v' },
};

/** 이합사 표(V-O) 토큰이 n으로 오면 동사가 기본, 명사는 후보 — 라운드 9. 근거는 splitZhCompound 위 ④ 주석. */
const SEP_POS_FIX = { tag: 'v', posAll: '동사·명사' };
...
export function fixZhTagged(tagged) {
  ...
  //    수제 층 → HSK 수확층 → 이합사 표 순으로 조회(수제 우선). baseForm(③)은 통과 보존.
  //    이합사 표 규칙(라운드 9): jieba 사전은 V-O 이합사 다수를 n으로 싣는다(走路·跑步·唱歌·上学·生病 — 코퍼스 220건,
  //    어휘 정답지 65건 중 콘텐츠 동사 55·명사 4·형용사 6). 동사를 기본으로, 명사(结果·讲话)는 후보로 실어 문맥 판별기가
  //    짚게 한다 — 정답지 대조 gain 55 · loss 0. n일 때만: v·vn·a로 오는 것(生气/a)은 그대로.
  return marked.map((e) => {
    const fix = ZH_POS_FIX[e.word] ?? ZH_POS_FIX_HSK[e.word] ?? (e.tag === 'n' && ZH_SEPARABLE[e.word] ? SEP_POS_FIX : undefined);
    if (!fix) return e;
    return {
      word: e.word,
      tag: fix.tag,
