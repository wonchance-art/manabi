...
import { mergeJaCompounds } from './jaCompoundMerge';
...
export async function tokenizeJaLine(line) {
  ...
  // 복합명사 재병합(라운드 10): kuromoji가 낸 映画|館·誕生|日·お|皿를 정본 읽기 색인의 표제어로 다시 합친다(수사 창 제외).
  const tokens = mergeJaCompounds(segmentKanaTokens(fixJaTokens(tokenizer.tokenize(line))));
