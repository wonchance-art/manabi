...
describe('⑥-c 수사 + 양사 — 양사가 어휘 항목으로 드러난다', () => {
  ...
  it('名 — 사람 세는 양사 (오너 「名 양사 추가」 2026-09-02): 一名·二十名·几名이 갈리고 名+X 어휘는 그대로', () => {
    expect(textsOf('他是一名医生。')).toEqual(['他', '是', '一', '名', '医生', '。']);
    expect(textsOf('班里有三十名学生。')).toEqual(['班里', '有', '三十', '名', '学生', '。']);   // 班里는 jieba 한 토큰 — 표적은 三十/名
    expect(textsOf('劫匪劫持了几名人质。')).toContain('几');
    expect(posOf('他是一名医生。', '名')).toBe('양사');
    expect(textsOf('那名球员犯规了。').slice(0, 3)).toEqual(['那', '名', '球员']);
    // 名+X 어휘 — HSK 방벽(名字·名单·名片) + 「수량 자리 뒤」 조건(名牌·名画는 표 밖인데도 안 갈린다)
    for (const [line, w] of [['你叫什么名字？', '名字'], ['这是我的名片。', '名片'], ['推荐名单。', '名单'],
      ['她喜欢买名牌。', '名牌'], ['美术馆正在展览名画。', '名画'], ['他得了第一名。', '第一名'], ['这是有名的景点。', '有名']])
      expect(textsOf(line), w).toContain(w);
    // 이합사 회랑과의 이음새 — 갈린 名/q 뒤에 명사가 붙으면 양사 자리라 有…名≠有名 (양사 자리 가드)
    expect(tokenizeZhLine('班里有三十名学生。').find((t) => t.text === '有').base_form).toBe('有');
  });
