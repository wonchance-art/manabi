...
describe('억제 — 병합 항목이 실단어로 갈린다 (코퍼스 문장, 기대 조각은 실측값)', () => {
  ...
  it('갈리면 안 되는 것은 그대로 — HSK 밖 실단어(老虎·熊猫·胡同)·고유명사·성어·명사 접미 복합어(登机牌·安全带)', () => {
    for (const [line, w] of [['老虎很大。', '老虎'], ['熊猫很可爱。', '熊猫'], ['北京的胡同很有名。', '胡同'], ['我住在上海。', '上海'], ['请出示登机牌。', '登机牌'], ['这是一举两得的办法。', '一举两得'], ['请系好安全带。', '安全带']]) {
      expect(textsOf(line), w).toContain(w);
    }
    // 身体健康은 한때 이 목록에 있었다 — 라운드 8의 두-실단어 되가름(⑦)이 身体|健康으로 가른다(정답지 공백 3/3, l 태그 구).
    expect(textsOf('祝你身体健康。').join(' ')).toContain('身体 健康');
  });
