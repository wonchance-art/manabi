 ...
 * ja는 기존 정본(japaneseVocabRegistry.findWord)에 위임하고, fr/zh는 refLangs 레지스트리
...
/** 자료 언어 → 만남 조회 코드. en은 정본이 있으나 뷰어 기록은 지시 범위 밖(§4.7 보류). */
export function encounterLookupLang(materialLang) {
  return { Japanese: 'ja', French: 'fr', Chinese: 'zh' }[materialLang] || null;
}
 ...
 * 동일 문자열), 비교만 정규화 키로 한다. ja·en은 키가 원문 그대로라 기존 동작 불변.
 */
