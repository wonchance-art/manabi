...
export function boardCameraForBounds(bounds, camera, viewport, insets = {}, fit = false) {
  ...
  const zoom = Math.max(.1, Math.min(fit ? 1 : currentZoom, (right-left)/Math.max(1,x2-x1), (bottom-top)/Math.max(1,y2-y1)));
