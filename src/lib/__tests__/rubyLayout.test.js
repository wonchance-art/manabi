...
describe('rubyWidthStep — 폭 예약 단계', () => {
  it('1~3자는 한자 폭 안에 들어와 보정하지 않는다(대다수)', () => {
    for (const py of ['à', 'wǒ', 'nǐ', 'qùn', 'guǎ', 'xué', 'shū']) {
      expect(rubyWidthStep(py)).toBeUndefined();
    }
  });

  it('4·5·6자는 실측 폭에 맞춰 단계를 올린다', () => {
...
describe('병음 조판 배선 계약', () => {
  ...
  it('rt 절대배치와 폭 예약은 병음 전용이다 — 일본어 요미가나는 기존 동작 유지', () => {
    ...
    expect(css).toMatch(/\.word-token ruby\[data-syl="6"\] \{ min-width: 1\.\d+em; \}/);
