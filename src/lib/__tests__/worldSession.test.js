...
import { initialCityIdForSpawn } from '../../components/world/cities/manifest.js';
...
describe('isPersistablePosition — 위치 영속 판정(순수)', () => {
  ...
  it('출시된 APAC·EMEA 좌표를 확장 지역 영속 대상으로 삼는다', () => {
...
describe('cityRedirectScene — 초기 부팅 시 도시 직행 스폰 판별(순수, Codex P1-1)', () => {
  ...
  it('dev guest ?spawn=city:<id> 형상도 같은 선로드 ID로 정규화한다', () => {
    expect(initialCityIdForSpawn({ scene: 'city:geneva', x: 123, y: 45 })).toBe('geneva');
    expect(initialCityIdForSpawn({ scene: 'city:no-such-city', x: 1, y: 1 })).toBeNull();
    expect(initialCityIdForSpawn({ scene: 'overworld:emea', x: 271, y: 490 })).toBeNull();
  });
...
describe('defaultOverworldRegionSpawn — 신규 사용자 기본 지역 진입', () => {
  ...
  it('씬 복귀 데이터와 범위 밖 기본값은 닫고 출시된 EMEA 기본값은 허용한다', () => {
