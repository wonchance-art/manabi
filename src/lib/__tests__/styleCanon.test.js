...
describe('스타일 캐논 ① — 한글 음차의 장음은 하이픈', () => {
  ...
  it('내가 건드린 두 파일의 가나 장음이 줄지 않았다 — ①이 넘치지 않았다', () => {
    // 이게 없으면 "한글 뒤 ー 0"을 **ー 전량 삭제**로도 만족시킬 수 있다.
    // 처음엔 전체 합계 하한(2000)으로 뒀다가 돌연변이가 살아남았다 — 전체 2107에서
    // n5.js의 106이 통째로 사라져도 2001이라 하한을 넘었다. 그래서 **파일별**로 갔다.
    //
    // 처음엔 파일별 **등호**(toBe)로 뒀는데, 그건 요구를 넘어선 고정이었다 —
    // IP v2 되살리기로 `ファミリーマート` 같은 실존명이 들어오자 106→112가 되며
    // 깨졌다. 콘텐츠가 늘어 값이 커지는 건 정상이고, **내려가는 것**이 위반이다.
    // 그래서 하한은 캐논 라운드 시점 실측(ot 25 · n5 106)에 고정한다 — 이 파일들은
    // 순회 라운드마다 내용이 움직이므로, 등호로 두면 정상 변경마다 헛되이 깨진다.
    const kana = (f) => (read(f).match(/[ぁ-んァ-ヶ]ー/g) || []).length;
    expect(kana('src/content/japanese/grammar/ot.js'), 'ot.js 가나 장음').toBeGreaterThanOrEqual(25);
    expect(kana('src/content/japanese/grammar/n5.js'), 'n5.js 가나 장음').toBeGreaterThanOrEqual(106);
  });
