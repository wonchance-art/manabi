...
describe('도쿄 파일럿 controller', () => {
  ...
  it('철도 표지 anchor 관찰은 별도 정답 카피 없이 다음 질문으로 진행한다', () => {
    const controller = createTokyoPilotController();
    controller.enterAnchor('haneda-arrivals-exit');
    controller.enterAnchor('haneda-rail-sign');
    expect(controller.snapshot().phase).toBe(TOKYO_PILOT_PHASE.HANEDA_ASK_DESTINATION);
    expect(controller.snapshot().interactions['haneda.rail-sign']).toMatchObject({
      kind: 'observe', outcome: 'independent', assistLevel: 'none', attempts: 1,
    });
  });
