...
export async function readCloudBoard(root,day){const value=await requestNote(boardEndpoint(root,day));if(!value.board?.manifest)return {row:value.board||null,document:null};return {row:value.board,document:await unpackBoard(value.board.manifest,async entry=>{
  const {data,error}=await supabase.storage.from(BOARD_BUCKET).download(boardPagePath(value.board.owner_id,value.board.id,entry.hash));if(error)throw error;return data.text();
})};}
export const listCloudBoards=root=>requestNote(boardEndpoint(root));
