...
try {
 ...
 const pdfHref=new URL(await page.locator('.library-reading-row').filter({hasText:pdf.title}).getAttribute('href'),base);assert.equal(pdfHref.pathname,`/pdf/${pdf.id}`);assert.equal(pdfHref.searchParams.get('page'),'7');assert.equal(pdfHref.searchParams.get('returnTo'),'/materials');
 ...
 await page.getByRole('link',{name:records[0].title,exact:true}).click();await page.waitForURL(url=>url.pathname==='/viewer/91001');await page.getByRole('heading',{name:records[0].title,exact:true}).waitFor();await page.goBack();await page.getByRole('textbox',{name:'자료 제목 검색'}).waitFor();assert.equal(await page.getByRole('textbox',{name:'자료 제목 검색'}).inputValue(),'프랑스');await check('material reader and back retain library query');
 ...
 state='empty';await page.goto(base+'/materials?view=owned');await page.getByRole('alert').filter({hasText:'PDF 목록을 불러오지 못했어요.'}).waitFor();assert.equal(await page.getByText('아직 보관된 개인 자료가 없습니다.',{exact:true}).count(),0);await check('PDF failure does not masquerade as an empty owned shelf');
 await page.goto(base+'/materials?view=notes');await page.getByText('아직 작성한 노트가 없습니다.',{exact:true}).waitFor();assert.equal(await page.getByRole('alert').filter({hasText:'PDF 목록'}).count(),0);await check('notes do not inherit unrelated PDF query errors');state='populated';
 ...
 await page.goto(base+'/books/japanese-n5#u03-study1');await page.locator('#u03-study1').waitFor();await page.waitForFunction(()=>Object.keys(localStorage).some(key=>key.startsWith('manabi-book-progress:')&&JSON.parse(localStorage.getItem(key)||'{}').page==='u03-study1'));await page.goto(base+'/materials');const resumeLink=page.getByRole('link',{name:'교재 이어 읽기 ↗',exact:true});await resumeLink.waitFor();assert.match(await resumeLink.getAttribute('href'),/#u03-study1/);await check('real N5 book position resumes with edition and anchor');
