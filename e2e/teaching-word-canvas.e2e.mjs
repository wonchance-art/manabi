...
CREATE TABLE user_vocabulary(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),user_id uuid,word_text text,base_form text,meaning text,furigana text,pos text,language text,source_sentence text,source_material_id bigint,interval real default 0,ease_factor real default 0,repetitions integer default 0,next_review_at timestamptz,last_reviewed_at timestamptz,unique(user_id,word_text));
...
GRANT SELECT,INSERT,UPDATE,DELETE ON user_vocabulary TO authenticated;GRANT SELECT ON uploaded_pdfs TO authenticated;
GRANT ALL ON user_vocabulary TO service_role;
...
for(const f of ['20260905065205_textbook_material_contexts.sql','20260917023410_classroom_direct_vocabulary.sql','20260908234552_viewer_reliability_atomic_operations.sql','20260924114728_viewer_token_correction_atomic.sql','20260909165823_classroom_atomic_entries.sql','20260910024120_classroom_study_flow.sql','20260910070535_textbook_teaching_annotations.sql','20260911072945_classroom_source_anchors.sql'])await db.exec(fs.readFileSync(new URL('../supabase/migrations/'+f,import.meta.url),'utf8'));
...
try {
 ...
 // Finish analysis before the following cloud suite deliberately disconnects
