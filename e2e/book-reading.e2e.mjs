...
(async()=>{
 ...
 await page.goto(base+'/lessons');await page.getByRole('heading',{name:'나의 책장.'}).waitFor();await check('shelf-desktop');await page.screenshot({path:out+'/shelf-desktop.png',fullPage:true});
 await page.getByRole('link',{name:'책 둘러보기',exact:true}).click();await page.getByRole('button',{name:'전체 42과',exact:true}).waitFor();await page.getByRole('button',{name:'전체 42과',exact:true}).click();
