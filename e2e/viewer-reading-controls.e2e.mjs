...
try {
 ...
 if(process.env.QA_REAL_FONTS==='1'){
  ...
  const fontEvidence=async()=>{const {root}=await cdp.send('DOM.getDocument');const {nodeId}=await cdp.send('DOM.querySelector',{nodeId:root.nodeId,selector:'[data-tid="id_0_2_audit"] .surface'});return (await cdp.send('CSS.getPlatformFontsForNode',{nodeId})).fonts;};
  const sans=await fontEvidence();assert(sans.some(f=>/Noto ?Sans ?SC/i.test(f.familyName)),JSON.stringify(sans));
  await aa();await page.getByRole('button',{name:'명조',exact:true}).click();await page.waitForFunction(()=>[...document.fonts].some(f=>/Noto_Serif_SC/.test(f.family)&&f.status==='loaded'),null,{timeout:60000});await closeAa();await page.evaluate(()=>document.fonts.ready);
