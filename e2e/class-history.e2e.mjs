...
for(const f of ['20260908234552_viewer_reliability_atomic_operations.sql','20260909165823_classroom_atomic_entries.sql','20260910024120_classroom_study_flow.sql','20260911051522_classroom_source_anchors.sql'])await db.exec(fs.readFileSync(new URL('../supabase/migrations/'+f,import.meta.url),'utf8'));
...
const sourceOp=crypto.randomUUID(),sourceQuote='图书馆';
const sourceAnchor={type:'TextQuoteSelector',exact:sourceQuote,prefix:'',suffix:words.join('\n').slice(3,51),start:0,end:3};
const sourceSeed={meaning:'도서관',reading:'tú shū guǎn',source:{materialId:'10',quote:sourceQuote,tokenId:'id_0_word',anchor:sourceAnchor}};
note=(await db.query('select classroom_append_study(1,$1,$2,$3,$4) result',[day,sourceQuote,sourceOp,sourceSeed])).rows[0].result.material;
const sourceEntry=note.processed_json.metadata.classEntries.find(e=>e.id.startsWith(sourceOp)).id;
...
const admin={rpc:async(name,a)=>{try{
 ...
 }catch(error){return {error:{code:error.code,message:error.message}};}},from:table=>{const filters=[];const q={select:()=>q,eq:(key,value)=>{filters.push([key,value]);return q;},order:()=>q,range:async()=>({data:[]}),maybeSingle:async()=>{if(table!=='reading_materials')return {data:null};const rows=(await db.query('select * from reading_materials')).rows;return {data:rows.find(row=>filters.every(([key,value])=>String(row[key])===String(value)))||null};}};return q;}};
globalThis.__classQA={user,access:{admin,root:{id:1,owner_id:uid},team:{key:'fixture-class',pwGen:1,bookKey:'fixture-book'}}};
...
await build({configFile:false,resolve:{alias:{'@':path.resolve('src')}},build:{ssr:'src/app/api/class/[team]/source/route.js',minify:false,outDir:out,emptyOutDir:false,rollupOptions:{output:{entryFileNames:'source-route.mjs'}}},plugins:[{name:'fixture-unlock-only',transform(code,id){if(id.endsWith('/src/app/api/class/[team]/route.js'))return 'export async function authorizeTeamRequest(request){return request.headers.get("x-class-token")==="fixture-class-token"?globalThis.__classQA.access:{error:Response.json({error:"expired"},{status:401})}}';}}]});
const {GET:sourceGET}=await import(out+'/source-route.mjs');
await context.route('**/api/class/fixture-class/source?*',async r=>{const response=await sourceGET(new Request(r.request().url(),{headers:r.request().headers()}),{params:Promise.resolve({team:'fixture-class'})});await r.fulfill({status:response.status,json:await response.json()});});
...
const historyNotes=[{id:note.id,day:'2026-09-10',title:'9월 10일 함께 읽기',entries:[{id:'manual',text:'忙碌',meaning:'하루 종일 바쁘다',reading:'máng lù',source:{kind:'manual'}},{id:sourceEntry,text:'图书馆',meaning:'도서관',reading:'tú shū guǎn',source:{kind:'textbook',...note.processed_json.metadata.classSources[sourceEntry]}},{id:'legacy',text:'以前',meaning:'예전',reading:'yǐ qián',source:null}]}];
