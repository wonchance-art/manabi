...
try {
 ...
 await fresh();await tap(page.locator('[data-tid="id_4_1_audit"]'));await page.locator('.viewer-inspector').getByText('담다',{exact:true}).waitFor();
 await page.locator('.viewer-inspector details summary').first().waitFor();assert.equal(await page.locator('.viewer-inspector details').first().getAttribute('open'),null);assert((await page.locator('.viewer-inspector details summary').first().innerText()).includes('사전의 다른 뜻'));pass('contextual meaning remains primary; unrelated dictionary example is collapsed');await shotAt('context-meaning');
 await fresh();await tap(wordA());await page.locator('.viewer-inspector .save-grade button').first().waitFor();
 await page.getByRole('button',{name:'보조 패널 닫기',exact:true}).click();await page.keyboard.press('1');await delay(150);assert.equal(vocab.length,0);pass('closed sheet ignores grade hotkeys');
 await fresh();detailWait=1000;await tap(wordA());await page.locator('.viewer-inspector').getByRole('button',{name:'상세 설명 보기',exact:true}).click();await delay(100);await tap(wordB());await delay(1300);
 assert((await page.locator('.viewer-inspector .word-fit').evaluate(e=>{const copy=e.cloneNode(true);copy.querySelectorAll('rt,.rt-an,.rt-hun').forEach(n=>n.remove());return copy.textContent.replace(/\s/g,'');})).includes('周末'));assert(!(await page.locator('.viewer-inspector').innerText()).includes('상세설명-대상-爱惜'));pass('late detail A never appears on B');
 ...
 assert((await page.locator('.viewer-inspector').innerText()).includes('B문장'));assert(!(await page.locator('.viewer-inspector').innerText()).includes('A문장'));pass('late sentence A cannot overwrite B');
 await fresh();await tap(wordA());await page.locator('.viewer-inspector .save-grade button').last().click();await tap(wordB());await page.getByRole('button',{name:/저장 취소 ·/}).waitFor();await delay(1000);
 assert((await page.locator('.viewer-inspector .word-fit').evaluate(e=>{const copy=e.cloneNode(true);copy.querySelectorAll('rt,.rt-an,.rt-hun').forEach(n=>n.remove());return copy.textContent.replace(/\s/g,'');})).includes('周末'));assert.equal(vocab.length,1);assert.equal(vocab[0].meaning,'아끼다');assert.equal(vocab[0].furigana,'ài xī');
 ...
 await fresh(390,844);await tap(wordA());await page.locator('.reader-card-body').evaluate(e=>e.scrollTop=e.scrollHeight);await tap(wordB());await delay(200);
 assert.equal(await page.locator('.reader-card-body').evaluate(e=>e.scrollTop),0);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));pass('mobile new word resets real scroll container, no overflow');await shotAt('mobile-word');
 await fresh();analysisFail=true;await tap(page.locator('[data-tid="id_0_0_audit"] .line-pick'));await delay(1200);assert((await page.locator('.toast-container').innerText()).includes('단어 분석을 가져오지 못했어요'));assert(!(await page.locator('.viewer-inspector').innerText()).includes('단어 추출 중'));pass('failed sentence request ends loading visibly');
 ...
 async function menu(){await page.getByRole('button',{name:'자료 관리',exact:true}).click();}
 ...
 await fresh(1440,1000);await tap(wordA());await page.locator('.viewer-inspector .word-detail-card').waitFor();assert(await page.locator('.viewer-inspector .word-detail-card').isVisible());await page.waitForFunction(()=>document.activeElement?.closest('.word-detail-card'));await page.keyboard.press('4');await page.getByRole('button',{name:/저장 취소 ·/}).waitFor();assert.equal(vocab.length,1);assert.equal(vocab[0].word_text,'爱惜');assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));pass('wide desktop card keyboard save and overflow check');await shotAt('desktop-wide');await page.keyboard.press('Meta+z');await delay(250);assert.equal(vocab.length,0);pass('undo shortcut works after save moves focus back to the viewer body');
 await fresh();await tap(wordA());await page.locator('.viewer-inspector').getByRole('button',{name:'뜻·발음 수정',exact:true}).click();await page.locator('.viewer-inspector .token-edit__input').first().fill('직접 교정한 문맥 뜻');await page.locator('.viewer-inspector .token-edit').getByRole('button',{name:'저장',exact:true}).click();await page.locator('.viewer-inspector .word-detail-card__meaning').getByText('직접 교정한 문맥 뜻',{exact:true}).waitFor();await delay(500);assert.equal(records[0].processed_json.dictionary.id_0_2_audit.meaning,'직접 교정한 문맥 뜻');assert(await page.locator('.viewer-inspector .word-detail-card').isVisible());pass('manual correction refresh keeps the same card open with the saved meaning');await shotAt('manual-correction');
