...
try {
 ...
 await fresh(1440,1000);await tap(wordA());await page.locator('.viewer-side--right .word-detail-card').waitFor();assert(await page.locator('.viewer-side--right .word-detail-card').isVisible());await page.waitForFunction(()=>document.activeElement?.closest('.word-detail-card'));await page.keyboard.press('4');await page.getByRole('button',{name:/저장 취소 ·/}).waitFor();assert.equal(vocab.length,1);assert.equal(vocab[0].word_text,'爱惜');assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));pass('wide desktop card keyboard save and overflow check');await shotAt('desktop-wide');await page.keyboard.press('Meta+z');await delay(250);assert.equal(vocab.length,0);pass('undo shortcut works after save moves focus back to the viewer body');
 await fresh();await tap(wordA());await page.locator('.viewer-sheet').getByRole('button',{name:'뜻·발음 수정',exact:true}).click();await page.locator('.viewer-sheet .token-edit__input').first().fill('직접 교정한 문맥 뜻');await page.locator('.viewer-sheet .token-edit').getByRole('button',{name:'저장',exact:true}).click();await page.locator('.viewer-sheet .word-detail-card__meaning').getByText('직접 교정한 문맥 뜻',{exact:true}).waitFor();await delay(500);assert.equal(records[0].processed_json.dictionary.id_0_2_audit.meaning,'직접 교정한 문맥 뜻');assert(await page.locator('.viewer-sheet .word-detail-card').isVisible());pass('manual correction refresh keeps the same card open with the saved meaning');await shotAt('manual-correction');
