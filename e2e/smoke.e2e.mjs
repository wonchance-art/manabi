...
test('chapter story: 공개 N5 챕터에서 조립·빈칸 채점과 미디어를 제공한다', { timeout: config.timeout * 2 }, async () => {
  await runInFreshPage(async (page) => {
    ...
    await page.goto('/japanese/grammar/n5-04-desu-da', { waitUntil: 'domcontentloaded' });

    await assertVisible(
      page.getByRole('heading', { name: /이야기로 확인 — 처음 뵙겠습니다/ }),
      'N5 story heading'
    );
...
test('chapter story: 공개 입문 편의점 챕터에도 스토리 모듈을 렌더한다', { timeout: config.timeout }, async () => {
  await runInFreshPage(async (page) => {
    await page.goto('/japanese/grammar/ot-07-konbini', { waitUntil: 'domcontentloaded' });
    await assertVisible(
      page.getByRole('heading', { name: /이야기로 확인 — 민준, 첫 결제에 성공하다/ }),
      'konbini story heading'
    );
    await assertVisible(page.getByRole('button', { name: /いらっしゃいませ/ }).first(), 'konbini story dialogue');
    await assertVisible(page.getByRole('textbox', { name: '빈칸에 들어갈 말' }), 'konbini fill input');
    await assertVisible(page.getByRole('button', { name: '모범답 보기', exact: true }), 'konbini model answer toggle');
  });
});
