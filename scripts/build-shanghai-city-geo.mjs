...
const BBOX = Object.freeze([121.45, 31.19, 121.54, 31.26]);
const SNAPSHOT_URL = new URL('./data/shanghai-osm-v21.json', import.meta.url);
const SNAPSHOT_SHA256 = '24f8fcf9c14add0c2d771c7397fc7c587c871cdd5273479e1ec5a065dd01dbb2';
const CARDINAL = Object.freeze([[1, 0], [-1, 0], [0, 1], [0, -1]]);
const FERRY_LINK = Object.freeze({
  id: 'huangpu-river-ferry',
  mode: 'ferry',
  stopIds: Object.freeze(['the-bund', 'lujiazui']),
});
...
const POIS = Object.freeze([
  ...
  },
  {
    ...
    lon: 121.4735,
    kind: 'plaza',
    representationPolicy: 'plaza-and-museum-exterior-only-no-politics',
  },
  {
    id: 'xintiandi',
    nameZhHant: '新天地',
...
const STATIONS = Object.freeze([
  ...
  },
  {
    ...
    line: '2號線·10號線',
    routeId: 'line-2',
    routeIds: Object.freeze(['line-2', 'line-10']),
  },
  {
    id: 'lujiazui-station',
    nameZhHant: '陸家嘴站',
  ...
  },
  {
    ...
    line: '10號線',
    routeId: 'line-10',
    routeIds: Object.freeze(['line-10']),
  },
]);
...
function separateMarkerTiles(entries, meta, minDistance = 2) {
  ...
  for (const entry of entries) {
    ...
    if (!snapped) throw new Error(`Unable to separate Shanghai marker ${entry.id}`);
    entry.tile = snapped;
...
function ensureWaterfrontAccess(
  ...
  if (target < 0) throw new Error(`Unable to connect Shanghai waterfront marker ${entry.id}`);
...
function normalizeShanghaiBridgeTerrain(sourceTerrain, meta) {
  ...
  if (report.finalBridgeTileCount > 0) {
    throw new Error(`Shanghai bridge normalization left ${report.finalBridgeTileCount} bridge tiles`);
  }
    ...
    if (!connectToPrimaryLand(grid, component[0], primarySeen, meta)) {
      if (protectedComponent) {
        const blockedMarkers = protectedTiles
          ...
          .map(([x, y]) => `${x},${y}`)
          .join(';');
        throw new Error(`Unable to connect protected Shanghai marker without crossing water: ${blockedMarkers}`);
      }
      for (const tileIndex of component) grid[tileIndex] = CITY_TILE.BUILDING;
      continue;
    }
...
function assertFerryConnected(grid, mainTile, ferryTiles, meta) {
  ...
  for (let index = 0; index < grid.length; index += 1) {
    if (!isCityBlocked(grid[index]) && !seen[index]) {
      throw new Error(`Disconnected Shanghai walkable tile at ${index % meta.grid.w},${Math.floor(index / meta.grid.w)}`);
    }
  }
}
...
function normalizeShanghaiTerrain(
  terrain,
  protectedEntries,
  ferryEntries,
  entrance,
  ...
  for (const entry of entries) {
    for (const field of ['nameZhHant', 'nameZhHans']) {
      if (typeof entry[field] !== 'string' || entry[field].length === 0) {
        throw new Error(`Shanghai missing ${field} for ${entry.id}`);
      }
    }
  }
}

export function buildShanghaiCityGeo() {
  ...
  if (snapshotSha256 !== SNAPSHOT_SHA256) {
    throw new Error(`Shanghai snapshot SHA-256 mismatch: ${snapshotSha256}`);
  }
  ...
  if (JSON.stringify(snapshot.bbox) !== JSON.stringify(baseMeta.bbox)
    ...
    throw new Error('Shanghai OSM snapshot projection contract mismatch');
  }

  const sourceTerrain = buildTerrainFromSnapshot(snapshot);
  const initialBuildingStats = terrainStats(sourceTerrain);
  const bridges = normalizeShanghaiBridgeTerrain(sourceTerrain, baseMeta);
  ...
  return {
    ...
    pois,
    stations,
    transitPoints: [pois.find((entry) => entry.id === 'the-bund')],
    entrance,
    exitTiles,
    railways: {
      mask: railwayMask,
...
function generatedModule(geo) {
  ...
  return `// Generated by scripts/build-shanghai-city-geo.mjs. Do not edit by hand.
...
export function writeShanghaiCityGeo() {
  const geo = buildShanghaiCityGeo();
  const outputPath = path.resolve(
    path.dirname(fileURLToPath(import.meta.url)),
    '../src/components/world/cities/shanghai.geo.js',
  );
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  console.log(JSON.stringify(writeShanghaiCityGeo()));
}
