...
// ── 경기만(인천 일대) 전면 재작화 ───────────────────────────────────────────────
// GYEONGGI_BBOX 를 sea 로 클리어(한강 하구 RIVER 유지)한 뒤 손저작 폴리곤으로 다시 그린다:
//   ① 클리어: bbox 안 비-RIVER 타일 → SEA (조강·염하수로·서해가 통짜 땅으로 남지 않게)
//   ② 본토(김포·인천 해안)·강화도·영종도 LAND 배치 (SEA 타일만 → RIVER 보존)
//   ③ 강화대교·영종대교 BRIDGE (sea 타일만 → 두 섬을 본토에 도달 가능하게)
//   ④ 소도서 점 LAND (서해)
// 앵커 광선 제약(위 GYEONGGI_BBOX 주석)에 따라 육지·다리는 전부 광선 동측에 둔다. fence 해안 앵커는
// 이 뒤 단계에서 조강 sea 로 짧게 봉인된다. Math.random 미사용 → 재생성 byte-identical.
function redrawGyeonggiBay(grid) {
  const { x0, x1, y0, y1 } = GYEONGGI_BBOX;
  const setL = (x, y, v) => { if (x >= 0 && y >= 0 && x < MAP_W && y < MAP_H) grid[y * MAP_W + x] = v; };
  const get = (x, y) => (x >= 0 && y >= 0 && x < MAP_W && y < MAP_H) ? grid[y * MAP_W + x] : TERRAIN.SEA;
  // ① 클리어 — RIVER(한강 하구/조강)만 유지, 나머지(land·plain·lake·bridge·구 앵커 잔재)는 sea 로.
  let cleared = 0;
  for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
    const i = y * MAP_W + x;
    if (grid[i] !== TERRAIN.RIVER && grid[i] !== TERRAIN.SEA) { grid[i] = TERRAIN.SEA; cleared++; }
  }
  // ② 육지 배치 — SEA 타일만 LAND 로(RIVER 보존). 본토·강화도·영종도·소도서.
  let land = 0;
  for (const [x, y] of [...GYEONGGI_MAINLAND, ...GANGHWA_LAND, ...YEONGJONG_LAND, ...GYEONGGI_ISLETS]) {
    if (get(x, y) === TERRAIN.SEA) { setL(x, y, TERRAIN.LAND); land++; }
  }
  // ③ 교량 배치 — sea 타일만 BRIDGE 로(양끝 육지를 4-연결).
  let bridge = 0;
  for (const [x, y] of [...GANGHWA_BRIDGE, ...YEONGJONG_BRIDGE_REDRAW]) {
    if (get(x, y) === TERRAIN.SEA) { setL(x, y, TERRAIN.BRIDGE); bridge++; }
  }
  return { cleared, land, bridge };
}
...
// 하드코딩 교량(오너 #12·간몬) — 타일좌표 직접(근사). sea 타일만 BRIDGE 로 덮어 양끝 육지를
// 4-연결로 잇는다. 각 폴리라인 양끝은 육지 타일(가운데 sea→bridge)이라 두 육지 성분을 연결한다.
// (거가대교는 부산·거제·가덕도 재구성 블록으로 이관 — 아래 GADEOK_*/GEOGA_BRIDGE/GEOJE_BRIDGE 참고.)
const HARD_BRIDGES = [
...
// ── 부산·거제·가덕도 남해안 재구성 (오너 재지시 · 손저작 타일 · 전부 근사) ──────────────────────
// 실지리 체인: 부산 본토 —(가덕대교)— 가덕도 —(거가대교)— 거제도 —(거제대교)— 통영 — 본토.
//   거제는 두 경로(통영행 거제대교 · 가덕도 경유 거가대교)로 이어져 한쪽만 끊겨도 도달이 유지된다
//   (둘 다 끊길 때만 단절). 가덕도는 부산 앞바다에 분리된 1~2타일 섬(POI GADEOK · walkable) —
//   가덕대교만 끊겨도 거가대교→거제→거제대교→통영 우회로 도달이 남는다(가덕·거가 둘 다 끊길 때만 고립).
//   견내량(거제↔통영)은 실폭이 좁아 격자상 이미 1타일 sea 해협((97,275))이라 그 위에 bridge 1타일만 얹는다.
const GADEOK_ISLAND = [[104, 271], [104, 272]];             // 가덕도(≈35.02N,128.83E) — 부산 앞바다 분리 섬
const GADEOK_BRIDGE = [[104, 270]];                         // 가덕대교: 부산 본토(104,269)↔가덕도(104,271)
const GEOGA_BRIDGE = [[103, 272], [102, 272], [101, 272]];  // 거가대교(재배치): 가덕도(104,272)↔거제(101,273)
const GEOJE_BRIDGE = [[97, 275]];                           // 거제대교: 거제(98,275)↔통영(96,275) 견내량 1타일

// ── 부산 동남 해안 손저작 보정 (수영만 만입·이기대 곶·광안대교·부산항 힌트 · 근사) ──────────────
// 4.5km/타일 래스터화가 뭉갠 부산 동남 해안을 자연 굴곡으로 다듬는다(직각 회피). 낙동강 하구(서측
// river · x≤108)·부산 POI·부산국제여객터미널은 건드리지 않는다(이 편집은 x111~114 동측 한정).
//   · 수영만 만입: 광안리 안쪽 바다(코스트 인입) + 부산항측 얕은 만입 힌트. · 이기대 곶: 만 남측 육지 곶.
//   · 광안대교: 광안리(113,263)↔이기대(113,265) 만 어귀 1타일 bridge — 도보 통행(단절 요건 아님:
//     양안이 육로로도 이어져 제거해도 부산은 단절되지 않는다). 전부 열린 바다에 면해 갇힌 바다(호수)를 안 만든다.
const SUYEONG_SEA = [[112, 264], [111, 266]]; // 수영만 만입 + 부산항 힌트(육지→바다)
const IGIDAE_LAND = [[113, 265]];             // 이기대 곶(바다→육지)
const GWANGAN_BRIDGE = [[113, 264]];          // 광안대교(바다→bridge)
...
// ── 경기만(인천 일대) 전면 재작화 (오너 지시 · 손저작 폴리곤 · 전부 근사) ────────────────
// 근본 원인: ne_10m 래스터화 + 서측 DMZ 해안 앵커가 경기만 일대를 뭉개, 강화도가 철조망/북측 육지에
// 융합되고 영종도가 본토에 거의 붙어 있었다. 스무딩·오버라이드·질감·(영종)교량을 모두 반영한 뒤,
// fence 해안 앵커(anchorFenceToCoast) 직전에 이 bbox 를 일단 sea 로 클리어(한강 하구 RIVER 는 유지)
// 하고 손저작 타일로 다시 그린다. 결정적(하드코딩 좌표·Math.random 미사용) → 재생성 byte-identical.
//
// ⚠️ 핵심 제약(DMZ 서측 앵커): fence 서단(≈62,199)에서 남서(dir≈(-0.60,+0.80))로 뻗는 앵커 광선은
//    "마지막 비-sea 타일 + 4"까지 FENCE 로 봉인한다. 그 광선 위(경기만 구간)에 육지/강/다리 타일을
//    하나라도 두면 앵커가 그 타일까지 뻗어 강화도·영종도를 관통한다. 따라서 아래 손저작 육지·다리는
//    전부 앵커 광선 타일을 피해(광선 동측) 배치했다 — 앵커는 조강(한강 하구) sea 로 짧게 봉인되고,
//    강화도/영종도는 광선 동측(남측 생활권)에 남아 다리로 도달 가능해진다. 광선 타일(참고):
//    (61,200)(61,201)(60,201)(60,202)(59,202)(59,203)(58,203)(58,204)(58,205)(57,205)(57,206)
//    (56,206)(56,207)(55,207)(55,208)(54,208)(54,209)(54,210)(53,210)(53,211)(52,211)(52,212)…
const GYEONGGI_BBOX = { x0: 50, x1: 64, y0: 200, y1: 216 };
// 강화도(약 3×4 · 섬) — 앵화수로(x62 1타일 sea)로 본토와 분리, 북쪽은 조강 sea 로 북측 육지와 분리.
const GANGHWA_LAND = [
  [60, 204], [61, 204],
  [59, 205], [60, 205], [61, 205],
  [59, 206], [60, 206], [61, 206],
  [60, 207], [61, 207],
];
// 강화대교(초지대교 근사) — 강화도(61,206) ↔ 본토(63,206), 염하수로 1타일 bridge.
const GANGHWA_BRIDGE = [[62, 206]];
// 영종도(약 3×4 · 섬) — 인천국제공항(POI.INCHEON=57,211) walkable. 본토와 영종대교로만 연결.
const YEONGJONG_LAND = [
  [56, 210], [57, 210], [58, 210],
  [56, 211], [57, 211], [58, 211],
  [56, 212], [57, 212], [58, 212],
  [57, 213],
];
// 영종대교(재배치) — 영종도(58,211) ↔ 본토(62,211). 앵커 광선 동측 3타일 bridge(도달 가능).
const YEONGJONG_BRIDGE_REDRAW = [[59, 211], [60, 211], [61, 211]];
// 김포·인천 본토 해안(SK · 서울과 연결) — 앵커 광선 동측. 자연스러운 굴곡(직각 돌출 회피).
// 한강 하구 RIVER 는 클리어에서 보존되므로 그 위엔 land 를 덮지 않는다(SEA 타일만 LAND 로).
const GYEONGGI_MAINLAND = [
  // 김포 북동안(조강 남안) — 클리어가 남길 갇힌 sea 포켓을 land 로 메워 stray lake 를 막는다.
  // 한강 하구 RIVER 곁의 SK 육지(펜스 동측)라 국경 봉인·하구-바다 연결에 영향 없음.
  [62, 200], [63, 200], [64, 200], [62, 201], [64, 202],
  [63, 203], [64, 203],
  [63, 204], [64, 204],
  [63, 205], [64, 205],
  [63, 206], [64, 206],
  [63, 207], [64, 207],
  [62, 208], [63, 208], [64, 208],
  [62, 209], [63, 209], [64, 209],
  [62, 210], [63, 210], [64, 210],
  [62, 211], [63, 211], [64, 211],
  [61, 212], [62, 212], [63, 212], [64, 212],
  [61, 213], [62, 213], [63, 213], [64, 213],
  [61, 214], [62, 214], [63, 214], [64, 214],
  [60, 215], [61, 215], [62, 215], [63, 215], [64, 215],
  [60, 216], [61, 216], [62, 216], [63, 216], [64, 216],
];
// 소도서 점(석모도·무의도 등 · 목포 방식 1타일 점) — 앵커 광선 밖 서해 sea 에 흩뿌림.
const GYEONGGI_ISLETS = [[54, 205], [52, 209], [55, 214]];
...
const POI = {
  ...
  GEOJE: projR(128.63, 34.95),          // 거제 ≈(100,273) — 거제도 land 타일(거제대교·거가대교로 연결)
  DAISEN: projR(133.540, 35.370),       // 다이센 ≈(196,263) — PEAK(NAMED_PEAKS 로 승격)
  TOTTORI: projR(134.230, 35.540),      // 돗토리 ≈(209,258) — 사구 land(모래)
  TONGYEONG: projR(128.42, 34.85),      // 통영 ≈(96,275) — 거제대교로 거제와 연결(본토측)
  GADEOK: projR(128.83, 35.02),         // 가덕도 ≈(104,271) — 부산 앞바다 분리 섬(가덕·거가대교)
};
...
// 5b) 하드코딩 교량(시마나미해도·세토대교·간몬교 — 오너 #12·간몬) — sea 타일만 BRIDGE.
//     (거가대교는 부산·거제·가덕도 재구성으로 이관 — 아래 5d 참고.)
{
...
// 5d) 부산·거제·가덕도 남해안 재구성 + 부산 동남 해안 보정 (오너 재지시) — 질감 전 하드코딩 지오메트리.
//     ① 가덕도 섬(sea→land)을 놓고 ② 3다리(가덕대교·거가대교·거제대교, sea→bridge)로
//     본토—가덕도—거제—통영을 잇는다. 이어 ③ 수영만/부산항 만입(육지→바다) ④ 이기대 곶(바다→육지)
//     ⑤ 광안대교(바다→bridge)로 부산 동남 해안을 자연화한다. 전부 하드코딩 좌표(Math.random 미사용 · 결정적).
{
  let isl = 0, br = 0, sea = 0, lnd = 0;
  for (const [x, y] of GADEOK_ISLAND) if (getTile(x, y) === TERRAIN.SEA) { setTile(x, y, TERRAIN.LAND); isl++; }
  for (const [x, y] of [...GADEOK_BRIDGE, ...GEOGA_BRIDGE, ...GEOJE_BRIDGE]) if (getTile(x, y) === TERRAIN.SEA) { setTile(x, y, TERRAIN.BRIDGE); bridgeCount++; br++; }
  for (const [x, y] of SUYEONG_SEA) if (getTile(x, y) !== TERRAIN.SEA) { setTile(x, y, TERRAIN.SEA); sea++; }
  for (const [x, y] of IGIDAE_LAND) if (getTile(x, y) === TERRAIN.SEA) { setTile(x, y, TERRAIN.LAND); lnd++; }
  for (const [x, y] of GWANGAN_BRIDGE) if (getTile(x, y) === TERRAIN.SEA) { setTile(x, y, TERRAIN.BRIDGE); bridgeCount++; br++; }
  console.log(`  · 부산·거제 재구성: 가덕도 ${isl} 타일 · 교량 ${br}(가덕·거가·거제·광안) · 수영만/부산항 ${sea} · 이기대 ${lnd}`);
}
...
// 6b) 경기만(인천 일대) 전면 재작화 — 스무딩·오버라이드·질감·(영종)교량 반영 뒤, fence 해안 앵커 앞.
//     bbox 를 sea 로 클리어(한강 하구 RIVER 유지) 후 손저작 폴리곤으로 강화도(섬)·영종도(섬)·본토
//     해안·소도서·교량을 다시 그린다. 앵커 광선을 피해 육지를 두어 봉인이 조강 sea 로 짧게 끝난다.
{
  const gb = redrawGyeonggiBay(grid);
  console.log(`  · 경기만 재작화: 클리어 ${gb.cleared} 타일 → 육지 ${gb.land} · 교량 ${gb.bridge} (강화도·영종도·본토·소도서 · 결정적)`);
}
