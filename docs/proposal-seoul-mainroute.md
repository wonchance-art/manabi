...
- 입력 blob: 본 문서(부록 추가 전)
  `c52c9630c15fd2ef985f49cb1c7f98b22575a3de`, `seoul.js`
  `abedaad8910170c925bc8c944966dfe5d24c403e`,
  `cityDistricts.js` `2c3a8257fec829469fed70ac6ea1103bcf0d701a`
- 문서의 각 `stepsRle` 전문을 생략 없이 `{ direction, count }` 배열로 파싱했다.
  각 leg의 시작·끝은 현재 `SEOUL.nodes` 또는 `SEOUL.stations`에서 typed key를
  exact-1로 찾아 얻었고, 모든 count를 타일 단위로 전개했다. leg 접합점은 다음 leg의
  첫 타일을 한 번 제거했다.
- 전개한 2,610타일을 현재 `districts.open[*].tiles.rects`의 포함 경계
  (`x0 <= x <= x1`, `y0 <= y <= y1`)와 직접 대조한 결과는
  **open 1,610 / locked 1,000**이다. 따라서 위 기존 검증표의 위반 수는 재현되며,
...
|---|---|---:|---|---:|---|---|
| S1 | 1 / 26..110 | 26..110 | `[779,745]` → `[701,741]` | 85 | `[701,740,779,745]` | `historic-core` ↔ `west` |
| S2 | 3 / 60..464 | 618..1022 | `[701,963]` → `[1019,1035]` | 405 | `[701,963,1019,1038]` | `west` ↔ `southeast` |
| S3 | 5 / 72..194 | 1370..1492 | `[1231,999]` → `[1339,985]` | 123 | `[1231,985,1339,999]` | `southeast` 내부 |
| S4 | 6 / 47..433 | 1590..1976 | `[1373,949]` → `[1121,815]` | 387 | `[1121,815,1373,949]` | `southeast` ↔ `river-north` |

### 권장 회랑 rect

아래 rect는 승인 전 제안이다. 각 잠긴 지대의 연속 경로를 최대 48타일 조각으로 나누고
조각 bbox의 x·y 양쪽에 정확히 2타일을 더했다. 패딩 후 한 변 64타일·면적 768타일²를
상한으로 삼아 대형 bbox 하나로 잠금을 푸는 안은 배제했다. 서울 제안 23개 rect의
실측 최대치는 한 변 50타일·면적 748타일²다. 소속은 해당 잠긴 지대 바로 양쪽의 기존
지구로만 제한하고, 조각 경로에서 각 기존 rect까지의 맨해튼 거리 합이 작은 쪽을
제안했다. 각 목록의 rect 순서는 경로 진행 순서다.

- S1 → `historic-core`: `[735,738,781,747]`
- S1 → `west`: `[699,738,738,743]`
- S2 → `west`: `[699,961,736,979]`, `[733,975,766,996]`,
  `[762,993,796,1013]`, `[792,1010,830,1027]`
- S2 → `southeast`: `[826,1024,875,1030]`, `[872,1026,913,1039]`,
  `[910,1033,956,1040]`, `[952,1030,1001,1036]`, `[998,1030,1021,1037]`
- S3 → `southeast`: `[1229,988,1271,1001]`, `[1268,985,1316,992]`,
  `[1313,983,1341,989]`
- S4 → `southeast`: `[1338,933,1375,951]`, `[1320,904,1341,937]`,
  `[1317,859,1323,908]`, `[1282,846,1321,862]`
- S4 → `river-north`: `[1245,835,1286,849]`, `[1201,831,1248,839]`,
  `[1163,821,1204,835]`, `[1123,813,1166,825]`, `[1119,813,1126,817]`

이 rect 목록의 합집합은 잠긴 경로 타일 **1,000/1,000**을 포함한다. 실제
`seoul.js` 반영 시에는 rect 추가 후 `resolveCityDistricts`를
`(city, grid, resolvedMainRoute)` 인자로 실행해 경로 전수 open을 다시 확인해야 하며,
이 부록은 지구 승인이나 runtime 변경을 대신하지 않는다.
