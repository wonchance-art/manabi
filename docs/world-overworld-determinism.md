...
> 상태: **DRAFT — ①·② preview v1 투영 확정, 각 산출물은 출시 전용 게이트 적용**
...
지역 ①은 등장방형+screen-axis, 표준위도 27.5°N으로 고정했다. 지역 ② preview v1은 등장방형+screen-axis,
표준위도 50.25°N으로 고정했으며 해당 값은 체크인된 surface·terrain·playability·boundary·transport
manifest에 기록한다. legacy 노드 재투영과 DB 변경은 별도 승인 전까지 시작하지 않는다. 투영값 고정과
공개 출시는 별도 판정이다. APAC 생성 산출물은 `releaseEligible=false`, EMEA의 projection·surface·terrain·
playability·transport·node·boundary 산출물은 일반 공개 승인에 따라 `releaseEligible=true`다.
...
충돌·국가 채색·소유권 라벨과 분리된 희소 오버레이로 생성한다. EMEA 공개 의존 체인과 동일하게
`releaseEligible=true`다.
...
`scripts/world/build-overworld-transport-nodes.mjs`는 승인된 횡단철도·항공 게이트와 철도 허브를 256타일 청크별
...
- 문서 키·노드 키는 타입별 exact schema이며 노드는 ID code-point 순이다. 공통 필드는 ID·타입·표시명·
  `contentLocale`·타일이고, 횡단철도는 `corridorStopId`, 항공은 3자리 `airportCode`를 추가한다. 철도 허브는
  지형 스냅 근거인 정수 `arrivalOffset`을 입력·문서에 보존한다. 생성 시각·호스트명·절대 경로·DB ID는
  기록하지 않는다.
- 현재 인덱스는 기존 `vladivostok-transsib`, `moscow-transsib`와 공개 EMEA 진입점
  `paris-cdg-air`와 런던·파리·베를린·마드리드·로마·빈·이스탄불 철도 허브를 포함한다. 게이트와 허브는
  교통 지리 참조일 뿐 운임·시간표·운행 여부·학습 콘텐츠를 주장하지 않는다.
- 런타임은 content manifest에 등록된 청크만 요청하고, 지역 레지스트리의 게이트 ID·타입·표시명·
  언어 앵커·타입별 route key·타일이 모두 일치해야 씬을 연다. 불일치나 누락은 fail closed다.
- APAC 산출물은 `releaseEligible=false`, EMEA 산출물은 `releaseEligible=true`다. 기존 저장 좌표와 DB는
  바꾸지 않는다.
