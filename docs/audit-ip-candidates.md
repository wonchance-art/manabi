...
> ⚠ **2026-08-31 정책 갱신 — 본 감사는 v1 시점 기록이다.** 현행 정책은
> `docs/policy-brand-content.md` **v2**(브랜드 적극 사용, 허용목록 폐기, 금지는 시각
> 재현·본문 복제·비방 맥락·중화권 정치 넷뿐)다. 아래 A/B/C 분류와 "무재현" 전제는
> 그때의 판단이므로 **현행 판정 기준으로 쓰지 말 것**. 이 감사의 일반화 조치가 만든
> 가짜 요미는 별도 복원 라운드에서 되돌린다(#1077).
