...
시안에만 있던 문형 프레임과 본문 글꼴을 실제 일본어 웹 교재에 적용했다. 예문 묶음에는 독립된 사방 테두리, 밝은 모브 배경, `예문` 제목을 두었다. 기본 설명 섹션은 문형 → 설명 → 예문 순서로 읽는다. 제목은 고딕, 한국어 설명과 일본어 예문은 Noto Serif KR/JP로 구분하고, 모바일 여백은 교재 컨테이너 폭을 따른다.

- 최종 앱 커밋: `a1b1a2306adc15e7ae5fca33304ddcd3331cc570`
- 배포 ID: `dpl_CNUs7Kyzksp12RVbzSwH3uehWbdy`
- 검증 빌드: https://manabi-bavcrosb1-wonchance-arts-projects.vercel.app
- 운영 주소: https://teset-gilt.vercel.app/japanese/grammar/n5-04-desu-da
- 이전 운영 빌드: `dpl_6PngqDwRhxPY74BbkAkV3YQtnMVG`
- CI: https://github.com/wonchance-art/manabi/actions/runs/33956509386

변경 파일 ESLint와 관련 43개 테스트, GitHub 기본 전체 334파일·3,688개 테스트가 통과했다. 로컬 및 최종 배포 빌드의 실제 Chrome에서 밝음·어두움 × 320/390/768/1280px를 확인했고, 단원 예문 7개가 별도 박스 3개 안에 있음을 확인했다. 글꼴, 듣기 버튼, 복습 입력 펼치기, 비로그인 저장 안내, 원문 앵커를 검사했다. 화면 가로 넘침과 페이지·콘솔 오류는 없었다.

운영 주소 전환 뒤 `/api/version`의 `a1b1a23`을 확인했다. 운영의 4언어 × 밝음·어두움 × 390/1024px, 총 16개 화면에서도 넘침·페이지 오류·콘솔 오류가 없었다. 이미 열려 있던 앱 브라우저 탭을 새로고침하여 글꼴과 예문 박스 3개·예문 7개를 직접 확인했다. 최종 앱 커밋의 GitHub E2E는 조판 25, 뷰어 4, smoke 14, 학습 흐름 9개가 통과했다(기존 smoke skip 6개). 해당 배포의 최근 15분 오류 로그 조회는 결과 없음이었다.

원문 설명·예문 문구, 관리자 편집 경로, 원문 연결 ID와 공개 원고 지문을 보존했다. 개인 자료나 운영 원고의 DB 수정은 하지 않았다. N5 단원 하나의 PDF v6은 예문 묶음 박스 3개를 추가했고 7쪽·115개 학습 필드를 보존했다. PDF SHA-256은 `4b8d7de04a03fd8379c806c5cc206bcf7baeedbd39bb1ceb06b10ffc703aa246`이며 최종 렌더 7쪽 전체를 직접 검수했다.

이번 디자인 수정도 PR #1277의 draft 상태를 유지하며 merge하지 않는다. 운영 화면·보고서는 기존 증거 폴더의 `reader-design` 아래에 보관한다. PDF 자동 발행과 원고 교정은 이번 범위에 포함되지 않는다.
