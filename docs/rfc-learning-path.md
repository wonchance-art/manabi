...
## 追記 (2026-07-30) — 누적 복습 세트 v1

- 매 5번째 챕터(레벨 내 순번 기준)에서 직전 4챕터의 드릴을 **결정적 샘플로 1문항씩 재출제**
  (`buildCumulativeReview` — order 해시 선택, 출처 챕터 라벨 표시). 신규 저작 0 — 간격
  재노출은 재사용이 본질이며 저작 겹침 금지 게이트와 무관(명시적 재출제).
- 렌더: 변형 드릴 아래 "누적 복습 — 앞 챕터 다시 꺼내기" 섹션(ChapterDrills 재사용).
- 드릴 층 완주 현황: 51/51챕터 357문항(딕테 ~97) — 남은 검증 = R5 재판정.
