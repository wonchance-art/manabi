...
## 개방 범위 (2026-07-12, 오너 확정 — `20260712000400_world_open_all_users.sql`)
...
2. **한 계정 동시 접속 금지(계정 단위 단일 접속)** — 기존 `world_sessions` 임대.
   → ~~IP 단위 차단~~은 오너 확정으로 **해제**됐다(같은 IP 다중 접속 허용, `20260712000400`).
     `world_sessions.ip` 컬럼(`20260712000300`)은 기록용으로 남고, `claim_world_session_v2` 는 ip 를 계속
...
## 스키마 (마이그레이션 `20260712000300_world_multiplayer.sql`)
...
- `claim_world_session_v2(p_ip text DEFAULT NULL) RETURNS jsonb` (SECURITY DEFINER, `search_path=''`):
  ...
  - **`20260712000400` 이후: IP 차단 제거** — ip 는 계속 기록하되 duplicate-ip 는 발생하지 않는다.
...
- 인증: 모든 라우트 `requireUser()`(쿠키 세션 — 전체 로그인 유저 개방. `20260712000400` 이전엔 `requireAdmin()`).
...
- RPC 사유값 `duplicate-account` 는 `/api/world/session` 409 로만 노출(후속 배선 시 사용).
  `duplicate-ip` 는 IP 차단 해제(`20260712000400`)로 더는 발생하지 않으나 계약·WorldGate 문구는 재활성 여지로 유지.
