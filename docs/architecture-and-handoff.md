...
- **학습자 상태 = FSRS 기억(user_vocabulary 컬럼) × 숙련 rung(유도값, 컬럼 없음)**. rung은 `review_events`의 최근 이벤트에서 순수 함수로 계산(`skillRung.js`의 `computeRung`·`deriveVocabRungs`). 0~4단계(choice→typing→listening→produce), 2연속 승급/강등.
...
- **어휘 산출 문항 도입** ← rung 3+ 단어 ≥ 20 (화면에 "현재 N" 표시)
