...
### 1.1 두 학습 순환 지도 (2026-08-20 4축 전수 조사 · 2026-08-24 신규 정거장 5종 반영 — 시각화는 순환도 아티팩트)
...
[자료 읽기 순환]  콘텐츠 공급(직접·PDF·책·원문 수정=바뀐 줄만) → 자동 분석(줄 토큰·공유 사전)
                → 뷰어(조판·집중·문장 이동·단어 카드: 확대·글자 탐색·유의/반의
                       · 받아쓰기(추천/지정 문장 듣고 글자 diff) · '이미 앎' 표시)
                → 저장(단어장·문법 노트) → FSRS 복습 → **본문 귀환**(due 하이라이트·'내 단어' 칩)
                → 완독 → **재독 되부름**(14일 후 홈 카드 → 뷰어 재진입)
[일일 세션 순환]  서버 조립(/study — due 어휘4·문법2·이벤트400·연재 프리페치 48h)
                → 문항(rung/EWMA 적응) → 채점(review_events)
                → 산출(작문·회화에 **오늘 복습 단어 주입** → 연재 재주입)
[곁길 진입로]     빠른 분석(/quick — 붙여넣기 즉석 해부, 무저장) ─[자료로 저장]→ 콘텐츠 공급으로 합류
[연결점]          뷰어 '이 자료로 오늘 학습'(study_source_) · 공유 FSRS/review_events · 정본 챕터([자세히])
                · 서재 맞춤도(materialFit ← 담김 ∪ '이미 앎')
[받침]            동기·진단(스트릭·히트맵·예보·주간 리포트) · 학습 그룹(스냅샷 합계·같이 읽기 — 원장 비공개)
                · 3단 공유 캐시(형태소 사전·단어 상세·TTS CDN)
```
...
| 빠른 분석(/quick) → 만남 기록·SRS 무기록 | **의도적** | 무저장이 이 화면의 계약. 단어장 저장·[자료로 저장]만 순환에 합류 |
| 그룹 스냅샷 → 개인 학습 테이블 무접촉 | **의도적** | 원장 비공개(RFC §9) — 공유는 스스로 upsert하는 숫자뿐 |
| 받아쓰기 채점 → review_events 무기록 | **의도적** (오너 확정 2026-08-24) | 패널이 로컬 채점만 — 듣기 정확도는 진단·SRS에 쌓지 않는다. 기록하려면 `review_events.source` 신설이 선결인데, 그 값을 넣는 순간 weeklyReport·growthStats·적응 출제의 기존 집계가 전부 새 출처를 흡수한다(isGradedReviewEvent는 ui·dict만 제외). 받아쓰기는 스스로 채점해 스스로 끝나는 훈련이라 그 파급을 살 이유가 없다는 판단 |
| '이미 앎' → FSRS 무연동 | **의도적** | 학습 대상이 아니라 커버리지 재료(#1121 설계) |
