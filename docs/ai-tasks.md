...
### doing
### todo
- 🎧 받아쓰기 채점 엔진(#1077 제안 6, 발주 5386786944 — 오너 "작업 간다 ㄱㄱ"
  2026-08-23): `src/lib/dictation.js` 순수 채점(diffChars 재사용·언어별 정규화) +
  계약 테스트. UI·TTS 배선은 Claude 몫. target=`codex/dictation-engine`,
  WORKING 표식 30분 룰.
### done (최근)
...
### doing
### todo
- 📖 재독 후보 선정 엔진(#1077 제안 12, 발주 5386788169 — 오너 "작업 간다 ㄱㄱ"
  2026-08-23): `src/lib/rereadSchedule.js` 순수 선정(완독 14일 경과·KST 경계는
  growthStats 재사용) + 계약 테스트. 홈 카드 배선은 Claude 몫.
  target=`codex2/reread-engine`, WORKING 표식 30분 룰.
### done (최근)
...
- ✍️ 산출 연동 — 오늘 복습 단어 선정 엔진(#1077 제안 16+17, 발주 5386789708 —
  오너 "작업 간다 ㄱㄱ" 2026-08-23): `src/lib/outputWords.js` 순수 선정(오늘
  KST·채점 판정·오답 우선 — 기존 정본 재사용) + 계약 테스트. 작문·회화 배선과
  프롬프트 카피는 Claude 몫. target=`codex3/output-words-engine`, WORKING 30분 룰.
  ← 최우선(아래 월드 항목은 동결 유지)
...
### doing
### todo
- ⚡ 빠른 분석(클립보드 리더) 실측 조사(#1077 제안 11, 발주 5386791238 — 오너
  "작업 간다 ㄱㄱ" 2026-08-23, **RFC-first — 코드 변경 0**): analyze API 무저장
  경로·비용·rate limit·캐시 필요성·PWA share_target 요건 실측 → `[CODEX4_REPORT]`
  회신. WORKING 30분 룰. ← 최우선(아래 판별 캐시 RFC보다 앞)
- 🗃️ 중국어 판별 캐시 RFC(발주 5312202647 §C4, 오너 ㄱㄱ 2026-08-17) — **RFC만, 구현
  착수 금지**: 재호출 실측·스키마 무변경 캐시+무효화 규칙·벤치 추정 → #150 코멘트.
  user_verified 보호 계약 충돌 금지.
