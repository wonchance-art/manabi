...
## Codex-1 (codex/*)
### doing
- **뷰어 저장 보호·N5 전권 감사 후보 (2026-09-24, 오너 로드맵 승인)** — 일반 자료 교정을 소유권/RLS·서버 이전 토큰 비교/한 토큰 갱신으로 변경. 다른 토큰 동시 수정 보존, 같은 토큰 입력 유지/최신값 확인/명시 재저장, 응답 유실·수동 편집·키보드 복귀. 새 제품 빌드와 Chromium/WebKit 각11체크/화면 기준3개/오류0·정리 PASS, 관련47단위·lint·diff PASS. 로컬 전체 단위는 자원 경합 중단하여 최종 필수 전체/통합 CI는 정확한 head의 #150 인계로 확인. SQL20260924114728의 9/24 미적용 상태는 9/25 승인 후 적용·실계정 검수 완료로 갱신(아래 done 참조). 기존 개인 뜻/SRS·발췌 경로 보존. N5 판본7f572327dc67893e9453246c 42과/309문항 객체 구조 감사 오류0, 어휘625/문형125/한자103은 시험 전범위 인증이 아님; 42과 학습·복습 분리와 긴 과 경로·누적 회상 보완을 다음 개정 우선으로 선정. 기존 발행 원고/PDF/새 음성 불변. 격리 clone /private/tmp/manabi-classroom-release-20260924, codex/classroom-release-20260917; PR #1321 draft, DB·실계정·운영 전환과 구분. docs/verification/viewer-correction-concurrency-20260924.md 및 n5-edition-audit-20260924.md.

### todo
- **실계정 의미 검수 후속 (2026-09-18)** — 정상 일반 계정의 중국어 4표본/자료 저장·교정·재접속·원문 복귀 확인. 东道主 문맥의 ‘접대하는 주인’이 같은 명사 첫 뜻 ‘주최국’으로 표시되는 의미 선택 UI는 e8485f19에서 구현: 사전/AI 출처 분리·수동 후보 조회·자료 한정 저장·초점 유지. 실제 후보/저장 의미 검수는 위 doing으로 이관. 팀 암호 입장, 교사 설명판, 학생 원본 단어 저장/뜻 전달/복습 복귀, 일본어 나머지 표본·독립 감수·물리 iPad는 미검수. 개인 교정/SRS·공유 사전 자동 덮기 금지. docs/verification/authenticated-lexical-20260918.md.
- **검수 자동화 후속 (2026-09-17)** — 고정 QA 주소를 통합 후보b94300de에 연결하고 공개 사전 검사 PASS. 남은 것은 OAuth 허용 목록·정상 인증 교사/학생 검수, 실제 모델 의미 평가·발행 N5 42과 커버리지. 화면 품질 회귀·중일 의미 자료 준비는 #1323에서 구현하고 #1321에 통합 완료. 다음은 정상 계정/기기 확인과 독립 의미 검토·실제 모델 평가이며, N5 커버리지 감사는 별도 세션으로 분리 가능. PDF/신규 음성 보류. 병렬 세션은 아직 생성하지 않음. docs/qa-live-workflow.md.
- **통합 출시 후보 #1321 실사용 확인·운영 전환 (2026-09-17)** — #1316~#1320을 main 대상 draft 하나로 통합. 최신 #1321 Preview의 교사 가져오기/저장/재접속, 학생 암호 입장→원본→뜻/독음→직접 단어 저장→출처, 기존 개인 노트 누적 정리와 실제 iPad/Pencil은 로그인 후 확인. 합성 검수를 실계정으로 세지 않음. 고정 QA alias는 후속 승인으로 최신화 완료. 운영 병합/승격은 별도 승인 범위. 원본 PR은 운영 통합 확인 전 보존. 아래 개별 후속의 현재 검토 단위는 #1321.
- **설명판 병합·기기 후속 (2026-09-16)** — #1317 코드·DB 적용·실제 교사 저장/재접속/지난 판 검수 완료. #1316/#1317 운영 merge는 별도 승인. 물리 iPad/Pencil·손바닥 접촉·소프트 키보드·회전, 별도 학생 계정·미참조 객체 수거는 후속. 100개 초과 목록은 #1320에서 구현·합성 검수 완료. 페이지 전체 복사는 #1318, 요소 부분 복사는 #1319 구현·배포 검수 완료. #1318/#1319 최종 fq12zobov Preview 실계정 복사/저장/재접속과 #1316 개인 노트 누적 정리는 로그인 후 확인. Safari 자동 검수의 RSC prefetch 오류는 아래 후속으로 해결.
- **수업 학생 실계정 후속** — #1299~#1302는 #1303으로 main 통합, 교사 교재·뜻·설명판·수업 기록 경로는 후속 실계정 검수 완료. 별도 학생 계정의 팀 암호 입장→개인 사본→뜻/독음→어휘 저장→수업 복귀와 교실 네트워크/물리 기기 확인은 미완료. 당시 Production SHARE_LINK_SECRET 설정 대기 기록은 현재 학생 입장 검수 시 유효성 재확인(미설정이라고 단정하지 않음). QA_LOGIN/자동 fixture를 실제 학생 검수로 세지 않음.
- **뷰어 후속** — #1293/#1294 운영 통합 완료. #1296/#1297의 문맥 복귀·누락 분석 복구는 #1315로 최신 노트/설명판에 통합. 기존 독음·문법·일본어 대응 정확도, 오프라인 채점/undo·옛 설정 정리와 물리 Safari는 별도 검토. 선택 문장 외부 전송·PDF·신규 음성·원본 split view는 보류.
- **서재 후속** — #1288~#1292는 #1292로 운영 통합 완료. 추천 바로 읽기 #1305는 #1315 통합 대상으로 정리. 미참조 업로드 정리·URL 본문 가져오기·실제 iOS 원본 읽기 확인은 별도. 개인 노트 신규 사용성은 위 후속과 함께 검토.
...
### done
- **N5 읽기 근거·실사용 동선 점검 (2026-09-26)** — PR #1324 후속 후보54f70824da508cbcea7dd578. 41개 기본 읽기·39과 구조형 읽기·42과 독립3지문/4질문 대조 후 33과 지시어·42과 오답/신청 마감 근거 5필드 교정. 기존336문항/정답/후리가나/예문 상자/입력 키/출처1144/공개 포인터 보존. 관련54·전체437파일4642PASS/기존1skip·구조0·lint/diff PASS, 두 번 생성 동일. Chromium/WebKit 각4묶음9배치/오류0/후보쓰기0·답안 복원/이전 판본 분리·Codex1440/390 직접 확인. 고정 Preview 실제 일반 계정에서 QA 단어1개 저장→단어장/복습일→출처→원본→재접속 확인; 수업 팀 암호·물리 기기·독립 감수는 별도. 최종 exact head/원격CI/Preview/closeout/CODEX_DONE은 PR/#150 정본. 발행/merge/DB/env/PDF/새 음성 제외. 다음은 읽기 상황·선행어휘·오답 근거 균형과 반복 구간 선별. docs/verification/n5-reading-review-20260926.md.
- **N5 가타카나·필기 보강 (2026-09-26)** — PR #1324 후속 후보e146271b705f753e1971c163. 문자 준비 3쪽/9문항과 42과 문장 배열·글 속 문법 2쪽/6문항, 단어 준비/오답 이유/설명 왕복. 42과·기존321문항/출처1142/원고/입력 키 보존, 499페이지·출처1144. 새 선택지44px·책 홈/본문 Link7곳 자동prefetch 차단. 구현7948ea27 CI 전체436파일4638PASS/기존1skip·필수4잡 통과, 교재59/신규6·구조0·최종빌드/lint/diff PASS. 최종 Chromium/WebKit 각8흐름15레이아웃/오류0/자동요청0, Codex1440/390 직접 확인. 앞선 전체WebKit 실패와 답안복원 검사 타이밍 보완을 기록했고 합산 통과로 세지 않음. Preview dcgbmlxdm READY·실행SHA일치/원본200/후보401. 이후 검사 대기·기록만 변경; 최종 기록head/CI/closeout/CODEX_DONE은 PR/#150 정본. 발행/merge/DB/env/PDF/새음성 제외. 다음은 독립 읽기 상황·오답 근거 균형. docs/verification/n5-written-practice-20260926.md.
- **N5 시험 유형 대조·전권 1차 교정 (2026-09-25)** — PR #1324 후속 후보c04e4e451a9ddc602fce8260. 공식14유형 대조: 필기10대표 근거/가타카나 표기 부분,청해4보류. 42과 핵심 설명·표·대화·읽기·문항/해설 대조 후27문구 교정(14·17과 단일 정답 조건,가타카나 안내,19·39과 해설/조사,42과 선택형 안내,웹/PDF 범위). 정답·출처1142·문항/입력 키·42과494모델·이전5판본·발행 포인터/개인기록/SRS 보존. 전체435파일4632PASS/기존1skip·대상5·구조0·최종빌드/lint/diff PASS. Chromium/WebKit 각19흐름66레이아웃/오류0, Codex1440/390 직접 확인. 구현c936d040; 최종head/원격CI/Preview/closeout은 PR/#150 정본. 운영발행/DB/env/merge/PDF/새음성 제외. 다음 제안: 가타카나 표기·필기 취약 유형 독립 연습. docs/verification/n5-coverage-copy-20260925.md.
- **N5 한자 색인·본문 연결 정리 (2026-09-25)** — PR #1324 후속 후보6a1d083d5cf869c7ba0077c1. 색인103개 중38개 대표 표기/읽기 본문 연결,65개 카드/관련 과 유지; 中/前 쓰임·聞 활용·時間 수량형 명시. 한자/가나/한국어/문형 검색·본문→출발 카드 복귀/초점·상단 가림·쪽수 보완. 이전4판본·42과 본문/문항/정답·출처/입력 키·진도/SRS·발행 포인터 보존. 관련20·전체434파일4627PASS/기존1skip·구조오류0·최종빌드/lint/diff PASS. Chromium/WebKit 각17흐름51레이아웃/오류0·Codex 데스크톱/모바일 직접 검수. exact head/원격CI/Preview/closeout은 PR/#150 인계 정본. 운영발행/merge/DB/env/PDF/새음성 제외. 다음 제안은 시험영역별 근거·전권 문장 교정. docs/verification/n5-kanji-links-20260925.md.
- **N5 교재 연습 명칭·한자 노출 감사 (2026-09-25)** — PR #1324 후속 후보c6ed2c215bbf36a50fe555cb. 교재 연습/연결 연습/다음 날 다시 꺼내기, 상단 담은 표현·하단 복습 구분. 42과 노출 감사와 6곳 읽기 도움; 1–5과 가나 중심·本/休 읽기/표기 문항·의도된 体 오답 유지. 과내309/권말포함321문항 내용·ID/입력 키·원본/이전판본/진도·SRS 보존. 관련14·전체433파일4621PASS/기존1skip·구조오류0·빌드/lint/diff PASS. Chromium/WebKit 각13흐름42레이아웃·오류0, Codex 넓은/좁은 실제 화면 확인. 구현7f075da5, 최종 exact head/원격CI/별도 Preview/closeout은 이 커밋의 PR/#150 인계가 정본. 운영발행/merge/DB/env/PDF/새음성 불변. 색인103자와 본문103자는 공통56자로 구분; 다음 후보는 색인 학습 연결 정합화. docs/verification/n5-practice-naming-kanji-20260925.md.
- **N5 32·39과 누적 회상 개정 후보 (2026-09-25)** — PR #1324 후속. 32과2단계/마침점, 39과 기존 순서 유지·다음날/누적복습 안내 교체·선택연습 건너뛰기, 문항별 복귀18개와 지시문3개 보완. 후보486ee7a5c71a4ff19062d808, 원본/이전후보/모든 답안·출처 보존. 432파일4617 PASS/기존1skip·관련10·감사오류0·빌드/lint/diff PASS. 실제 앱 합성 Chromium/WebKit 각11흐름24레이아웃/오류0, Codex 모바일39과/넓은32과 직접 확인. 실계정·물리기기·독립감수와 분리. 운영발행/DB/env/merge/PDF/새음성 불변. 최종 head·서버CI·별도 Preview·closeout은 이 커밋의 PR/#150 인계가 정본. 다음은 한자 첫 노출·읽기 도움·자력 회상 연결 감사. docs/verification/n5-cumulative-recall-20260925.md.
- **N5 학습 경로·누적 복습 개정 후보 (2026-09-25)** — PR #1324 draft, 부모 #1321과 별도 검토. 새 웹 판본e44cd7230ef94ff7f6293e2d: 20·23·35·42과 10단계/마침점·15개 복귀 링크, 42과 연결어 문항 근거 명확화. 원본 489개 ID/답안 키/출처와 현재 후보 포인터 보존, 경로4개 추가. 전체 로컬432파일/4614PASS+기존1skip(후속 상속음성 집계 검사1추가), 관련37·구조 감사·제품 빌드·Chromium/WebKit 각9흐름/12레이아웃 통과. 원격 최종 CI/Preview·exact head/closeout 판정은 #150 CODEX_DONE 정본. 실행e713f80f·별도 Preview8xrjecasn; 미발행 관리자 전용 후보. PDF/새음성/DB발행/고정QA/운영/merge 변경0. 다음 우선:32·39과 누적 복습·정답 근거/선행 학습 감사. docs/verification/n5-study-routes-20260925.md.
- **저장 함수 활성화·승인 중복 요청 정정 (2026-09-25, 오너 명시 승인)** — SQL20260924114728 한 건 적용/workflow36018417851, 로컬·원격95이력/INVOKER/기존RLS 일치·새 advisor0·실제DB10검사 PASS/ROLLBACK/잔존0. 승인된 구체적 대상·범위의 재승인 요청 금지로 AGENTS/CLAUDE/quality-workflow 정정(포괄적 미래 권한은 아님). 실행 후보7e52b159의 CI36019496799/world36019496675 SUCCESS, 431파일/4607PASS(기존1skip)·통합14/재개14. Preview cywwgfeyp를 고정 QA에 연결/공개 버전 일치 PASS. 실제 로그인 두 창: 다른 토큰 보존·같은 토큰 충돌/입력 유지·확인만으로 무저장·키보드 명시 저장/재접속 PASS. 기존 원본·개인 뜻/FSRS 해시 동일, 자체 검수 행 정리/잔존0·브라우저 오류0, 390px 본문/단어창 넘침0. 처음 글꼴 빌드는 실패했으나 같은 코드 1회 재시도 성공(영구 해결 주장 안 함). 최종 기록/보드만 후속 커밋; exact head·재검수는 #150 인계 및 qa:closeout 참조. 통합PR #1321 draft/운영 미전환·교사/학생 전체 및 물리기기 조건 유지. 다음 독립 작업: N5 42과/긴 과 경로·누적 복습의 새 판본 초안. docs/verification/viewer-correction-activation-20260925.md.
- **실제 뷰어 전용 자동 검수 (2026-09-24)** — 구현 56692ea6. 기존 함수/합성 Auth·HTTP/PGlite를 재사용한 `qa:reader-app` 2엔진, Aa/집중/200%·자료 한정 뜻 저장/실패 재시도/재접속·원문 초점. 각각 8체크·기존 시각 기준3개·오류0/정리 통과(동일 앱 빌드의 로컬 점검). 최근 읽기/위치 기록만 계정·필드까지 검사해 허용하고 공유 사전/개인 뜻·SRS 변경 거부. reader8/classroom6/release14 구성으로 기존 범위 보존·중복 실행 제거. 3파일12단위/lint/diff PASS, 제품/DB/env 차이0. 첫 원격 새 빌드/4,584단위/통합13·14 통과 후 Linux 재생기호50픽셀만 검토해 독립 진입 기준1장 갱신(허용치20 유지); 최종 새 빌드/전체 CI·재검수→리뷰→다음 방향은 이 head의 PR #1321/#150/closeout 정본. 다음 제안은 legacy 자료의 동시 뜻 수정 보호. docs/verification/reader-app-qa-20260924.md.
- **뷰어 문맥 뜻·일본어 대응 후속 (2026-09-24 마감)** — 제품 후보9d71d214 최종 Preview 및 고정 QA 확인. 실제 수동 표본에서 ‘접대하는 사람’→ホスト, 별도 자형東道主·초점·1280/390 화면 보존. CI35402990583/world35402990631·통합12/재개12 PASS. 좁은 의미 보완은 reviewed, 통합 운영 출시/실계정 역할/iPad·독립 의미 감수는 아래 후속 유지. #150 댓글5811113753 및 closeout viewer-japanese-live-20260924 정본. 운영 병합/DB/env 변경 없음.
- **일본어 대응어/자형 요청 분리 구현 (2026-09-19)** — #1321 9ab60135 실제 문맥 후보 접대하는 사람/명시 저장/재로딩 통과 후 일본어 東道主+드문 표현 경고 모순을 확인. 3fa6daa9에서 자형을 모델 입력에서 제외하고 선택 뜻의 현대 일본어 번역만 요청, 자형 설명 생성 분리·캐시v4. UI 자형/사전 경고·수동 조회/취소/초점 보존. 관련82·전체429파일4583통과/기존1skip·lint/diff PASS. 새 Preview 실제 의미/브라우저/원격CI 및 최종판정은 #150/closeout 정본. 의미 정확성 보증·운영merge/DB/env 변경 없음.
- **뷰어 문맥 뜻 혼합 보완 구현 (2026-09-19)** — #1321 bce1a1b9. 실계정 저장/재접속은 통과했지만 후보가 문맥 밖 뜻을 함께 제안하는 문제를 발견해 기존 뜻/품사를 모델 입력에서 제거하고 단일 쓰임·보류 지침과 대비 예를 보강. API/저장/모델 티어 불변, 캐시 v2. 관련15·전체429파일/4,582PASS(기존1SKIP), lint/diff·reader6단계 통과. 로컬 첫 reader는 sandbox listen EPERM이며 같은 검사 권한 재실행 PASS. 의미 보증을 정규식으로 대신하지 않음. 최종 CI/배포/실제 재검수는 #150. docs/verification/viewer-context-meaning-20260918.md.
- **뷰어 문맥 뜻 선택 구현 (2026-09-18)** — #1321 e8485f19. 중국어 사전 후보/AI 문맥 후보의 명시 선택과 자료 토큰 한정 저장, 조회·저장 실패/늦은 응답/계정 분리·재조회 시 이전 AI 선택 해제, 일본어 조회 후 키보드 초점 유지. 하단 개인 단어 저장·SRS·공유 사전 보존, DB/env 변경 없음. 관련56·전체429파일/4,581PASS(기존1SKIP), JS/JSX lint·Chromium/WebKit reader6단계 통과; 최초 기존 콘텐츠 시간 초과는 동시2 재검수 PASS. 4개 OS/엔진의 의도된 단어창 화면 차이 직접 대조 후 해당 기준만 갱신, 허용 오차/검사 유지. 실계정/독립 의미 감수는 완료로 세지 않음. 최종 exact head·CI·배포는 #150 CODEX_DONE 정본. docs/verification/viewer-context-meaning-20260918.md.
- **작업 마무리 자동화 (2026-09-18)** — #1321에 최종 재검수→결과 리뷰→다음 방향 계약과 `qa:closeout` 구현. 오래된 후보·바뀐 증거·필수 미검수·빈 리뷰/다음 방향 거절. 관련25개·전체426파일/4,567개 PASS(기존1 SKIP). 마지막 커밋 이후의 실제 closeout·원격 결과는 #150 최종 head 인계. 다음 우선순위는 문맥 뜻 선택→조회 초점 복귀; 교사/학생·기기 검수는 접근 조건 분리. 작업 규약·QA만 변경, 제품/운영/DB/예약 설정 불변. docs/verification/qa-closeout-20260918.md.
- **실계정 읽기 검수·일본어 형식 오류 방어 (2026-09-18, 오너 “다음”)** — #1321 앱 수정50e37fd9. 자체 작성 비공개 원문4문장→학습 구간→교정 유지→원문 출처 복귀를 정상 계정에서 확인. 기존 사용자 자료·개인 단어/SRS 변경0. 실제 수동 일본어 조회에서 한국어 ‘주최자’가 AI 대응어로 표시된 오류 재현 후 문자 검증·캐시v3·용법 참고 문구 보완. 관련98·전체425파일/4,547PASS·기존1skip, JSX lint PASS. 문맥 설명/일본어 조회의 UI 실행과 사전4표본을 구분하고 번역 정확도 점수 미산출. 최종 exact head·원격CI·Preview 결과는 #150 CODEX_DONE 정본. 첫 Preview는 커밋태그 규칙으로 취소되어 승인된 [preview] 태그로 재배포. 운영merge·승격·DB·영구env 변경 없음. 나머지 의미/역할 검수는 위 todo. docs/verification/authenticated-lexical-20260918.md.
- **출시 후보 통합·중일 의미 준비 완료 (2026-09-17, 오너 “좋아 진행해”)** — 기존 #1321에 #1322/#1323을 fast-forward 통합. runtime b94300de Preview gdchpstum READY, 고정 QA alias 연결·공개 버전/인증 복귀 PASS, 운영 efa44c50/배포ID 전후 동일. 실제 홈→N5 첫 과·390/1280 가로 넘침0 확인. 의미40건 유지/위험6건 재검토·정답 없는 입력 내보내기·응답 반입/출처/미실행 분리, targeted42개·lint PASS. 이후 검수/기록만 변경하여 앱 diff0. 실계정·실제 AI0·독립 번역/물리 기기 미완료는 todo 유지. 최종 exact head/원격CI는 #150 CODEX_DONE. DB/env/main merge/운영 승격 없음. docs/verification/unified-preview-20260917.md.
- **화면·접근성 회귀 및 의미 검수 기준 완료 (2026-09-17, 오너 “진행해”)** — #1322 exact e43d48aa 기반 draft #1323. 실제 뷰어/뜻 창/설명판/노트 7화면×2엔진×2OS의 검토 기준 28개·axe·키보드/200% 확대·검출 canary·실패 수집 구현. 뜻 단어 줄바꿈·중첩 버튼·문장 선택 터치/초점 결함 수정. 코드 c0bfbe12: 전체 425파일/4,542통과·1skip, Mac/Linux 화면 차이0·자동 위반0, CI35223524186 release10/재개10·정리 종료 PASS, 월드35223524173 PASS. 중일 출처 기반 문맥32/모호성8 준비; 실제 AI 평가·이중언어 검토는 미완료. 최종 기록 head/CI는 #150 CODEX_DONE 정본. 운영 merge/배포/DB/env/alias 변경 없음. docs/verification/visual-quality-20260917.md.
- **고정 검수 주소 연결 완료 (2026-09-17, 오너 명시 승인)** — manabi-web-v2-preview.vercel.app → 2rgzutmdd / dpl_3AAYnSwhEZ3znijcy9k98yrjVFCV / 실행1a21318f. 변경 전후 qa:live로 후보/alias 버전·판본·캐시·안전한 인증 복귀·검사 중 불변 PASS, 동일 deploymentId 확인. 운영teset-gilt 배포efa44c50 전후 동일. 앱/DB/env/계정/merge 변경 없음. 기록만 갱신, 실제 로그인·OAuth·물리 iPad는 todo. docs/verification/qa-live-preflight-20260917.md.
- **규약 정정·공개 배포 사전 검사 완료 (2026-09-17, 오너 별도 승인)** — AGENTS/CLAUDE 웹Node24·월드Node22/전용 범위·직접 요청과 제한적 Cloud 발주 구분·force-push 예외 제거. 병합/DB/env 승인·시크릿·데이터·다른 변경 보호 유지. qa:live는 별도 실행SHA·두 주소·배포ID·판본·캐시·인증 복귀·검사 중 교체를 확인하며 민감 응답을 기록하지 않음. 신규14조건·전체423파일/4,534통과·기존1skip·ESLint PASS. 실제 최신 후보1a21318f PASS, 고정주소2932e889 불일치 감지(exit1). 앱/계정/DB/env/alias/운영 병합 변경 없음. 최종 PR #1322의 exact head/원격CI는 #150 CODEX_DONE 정본. docs/verification/qa-live-preflight-20260917.md.
- **검수 자동화 1차 구현·로컬 검수 완료 (2026-09-17, 오너 “좋아 적용해”)** — #1321 exact34a09fed 기반 codex/qa-workflow-20260917, 실행5fd40627·종료/증거수집 보완efbee56f. SQL/reader/notes/classroom/release 필수 묶음·그룹/정상 종료/정리 판정·지문/증거 해시 기반 재개·합성 보고서/실패 trace·고정 PGlite/브라우저·CI 연결·기록 전용 경량 판정 구현. 전체422파일/4,520통과·기존1skip·build480페이지·SQL7+26·Chromium/WebKit 수업각48/노트각11/일본어대응각4·표시10×8×2 PASS. WebKit 프레임 내 드래그 몰림은 공통 입력 도우미로 보완하고 조건 유지. 첫 Linux CI의 WebKit 노트11통과 후 종료 시간초과와 의존성 링크 동반 업로드를 확인해 문맥 우선 종료/자원별 계측/링크 없는 증거 수집/실패 원인 즉시 보고로 보완. 로컬 두 브라우저 노트11·정리 종료 PASS, 실제 CI 증거107개/4.7MB 보존. 최종 head의 원격 CI·전체10단계/재개 결과·인계 PR은 #150 CODEX_DONE과 생성 보고서가 정본. 앱/교재/DB/env/alias/운영 merge 변경 없음. docs/verification/qa-workflow-20260917.md.
- **수업 원본 단어 저장 DB 활성화 완료 (2026-09-17, 오너 “ㄱㄱ 자동화” 승인)** — #1321 SQL20260917023410 한 건만 미적용임을 대조한 뒤 workflow35178677108 SUCCESS로 운영 적용. 원격 migration 이력·service-only 함수·SECURITY INVOKER/빈 search_path·학생 문맥 소유권/RLS 확인. 실제 운영 DB 합성 데이터7시나리오(저장/중복/SRS/뜻 충돌/원문·암호 세대/교차 계정/기존3종 RPC/원본 삭제) PASS, 전체 예외 롤백·임시 행0·기존 개인 행 수정0. 재사용 검수 SQL은 supabase/verification/classroom_direct_vocabulary.sql. Preview2rgzutmdd 실행1a21318f 유지/버전 일치/비로그인 GET·POST401, 앱 재배포·merge/영구env/alias 변경 없음. 실제 사용자 로그인·운영 웹 전환은 위 todo. 보안 advisor의 이번 변경 관련 지적0, 기존 프로젝트 권고는 검수 문서에 별도 기록.
- **수업 원본 직접 학습·뜻 전달 구현/로컬 검수 완료 (2026-09-17)** — #1321 실행1a21318f. 원문 읽기→단어/문맥 원자적 저장→실제 복습→같은 표현 복귀, 개인 사본은 명시 보관. 교사 후속 뜻 수정의 dictionary/메타데이터 일치·원문 재열람·사본 변경 안내, 학생 개인 뜻/SRS 보존. 신규 SQL은 코드만 준비하고 운영 스키마/권한 읽기 전용 대조. 전체419파일/4,510개·SQL/RLS7시나리오·Chrome30/WebKit터치30/오류0·JS/JSX28개lint·build480페이지 PASS. 390px 화면 직접 확인·넘침0. Preview2rgzutmdd 원격 빌드와 실행CI35177280471의 최종 상태/최종head는 #150 CODEX_DONE에 기록. 신규 DB 활성화/실계정/운영 전환은 위 todo. 상세 docs/verification/classroom-direct-study-20260917.md.
- **수업·개인 노트 통합 출시 준비 완료 (2026-09-17)** — main 대상 draft #1321, 실행04fe73b9. #1316~#1320 기능 통합, 일본어 대응 품사별 요청/캐시/수동 조회·취소, 근거24의미 계약, 교사 확정 뜻·독음의 자동 분석 보존(개인 교정 우선). 전체417파일/4,497개·build480페이지 PASS, Chrome/WebKit 교사→학생 각46동선/오류0·개인 노트 각11·일본어 대응 각4 PASS. 실제 로컬 SQL/RLS에서 학생 사본 재사용·교사 원본/판/Storage 접근 차단·원본 불변 확인. Preview g2xjncacd READY/버전 일치/비로그인 노트·판 API401, 실행CI35171281494 두 job SUCCESS. 최종 문서/자기 보드 head와 CI는 #150 CODEX_DONE에 기록. 사용자 외출 중 로그인 요구 없음. DB/영구env/운영merge/alias/기존 데이터·SRS 변경 없음. 실계정/기기/운영 전환은 위 todo, 상세 docs/verification/classroom-release-20260917.md.
- **설명판 후속 A/B/C 구현·배포 검수 (2026-09-17)** — #1320, #1319 exact bcce8f12 기반, 실행3a640c70. 기간/30개씩 전체 조회·현재 수업 재사용·목록 위치/초점·권한 상실 정리, 검증 유지 최대2개 병렬 읽기, 공통 독음 대응 보완. 전체414파일/4,440개·선택 벤치·build480페이지 PASS. 배포 Chrome/WebKit 터치 각42흐름/오류0, 365날짜·실패/취소·복사/undo·저장/충돌/오프라인·Blob해제·권한 철회 확인. 언어60 구성 입력 및 실제 뷰어/설명판10×8×2 표시 검수. 합성20페이지 읽기 중앙값267→140ms. 실행 CI35131141103 두 job SUCCESS, Preview ajunlm9lm READY/버전 일치/비로그인API401. 최종 문서/자기 보드 head·CI는 PR/#150 완료 신호에 기록. 실계정 교사/학생·물리 iPad/Pencil·개인 노트 누적 정리는 위 todo 유지. DB/영구env/운영merge/원본교재/학생기록/SRS 변경 없음. docs/verification/classroom-refinements-20260917.md.
- **설명판 Safari 탐색 안정화·출시 자동 검수 (2026-09-16)** — #1319 후속 실행70f493b8. 홈/팀/학습 및 전환 중 복귀 링크의 불필요한 RSC prefetch 제거, 기존 오류 예외 삭제·요청0 계약·실제 클릭/뒤로가기/페이지 보존 검수 추가. 전체410파일/4,349개·최종 관련70개·build480페이지 PASS. 로컬/배포 Chrome·WebKit 각35조건·요청0·오류0, 개인 노트 WebKit11조건 PASS. Preview fq12zobov READY/버전 일치/비로그인API401, 실행CI35093095730 두 job SUCCESS. 실계정 교사 로그인·실제 저장 및 물리기기는 위 todo로 유지. 최종 문서/자기 보드 head·CI는 PR/#150에 기록. DB/영구env/운영merge/원본교재/학생기록 변경 없음. docs/verification/teaching-board-safari-20260916.md.
- **설명판 표현·필기 부분 재사용 구현·배포 검수 (2026-09-16)** — #1319, #1318 exact3a171115 기반, 실행7b5d8966. 영역/표현/목록 선택·현재 판/새 페이지 복사·단일 undo/redo·원본/출처/숨김 상태/기존 필기 보존·중복 안내·닫힌 요청 취소. 전체410파일/4,349개·build480페이지·Chrome/WebKit 각33흐름 PASS/오류0. 오너 공개 push/PR/Preview 승인 후 p4nl6jogi READY·버전 일치·비로그인 API401·배포본33 PASS/오류0/폰·태블릿 캡처 확인. 최초 CLI 버전 메타 누락은 기존 releaseDeployArgs로 해결. CI35089594244 두 job SUCCESS, 문서/자기 보드만 변경한 최종 head/CI는 PR/#150에 기록. 새 Preview 교사 로그인 대기·실계정/물리 기기는 todo, DB/영구 env/운영 merge/원본 교재/학생 기록 변경 없음. docs/verification/teaching-board-fragments-20260916.md.
- **지난 설명판 페이지 재사용 구현·로컬 검수 (2026-09-16)** — #1318, #1317 exact94e555a8 기반, 실행5c97b637. 현재 수업에서 날짜별 페이지 미리보기·선택 복사·전체 보기, ID/묶음/화살표 재매핑·원본/기존 필기/교재 출처 보존·학생 기록 명시 저장·중복/20개/6MiB 보호. 전체409파일/4,340개·핵심23·build480페이지·Chromium/WebKit터치 각각26흐름 PASS/오류0. 원격 배포/최종 head CI는 PR/#150 CODEX_DONE에 기록. DB/env/운영 merge 없음. 새 Preview 실계정 복사·물리 기기·요소 부분 선택은 후속. docs/verification/teaching-board-reuse-20260916.md.
- **설명판 계정 저장·지난 판 구현/DB·실계정 검수 완료 (2026-09-16)** — #1317, 선행 #1316 기반, 실행5eda8e64. 교사 비공개 페이지 업로드·원자적 revision·오프라인 초안·충돌 복구·지난 판·기존 로컬 이전. 전체408파일/4,333개·SQL26조건·build480페이지·Chrome/WebKit터치 각22·개인 노트11·Preview22 PASS. 오너 추가 승인으로 DB20260916030644 적용, 실제 교사 표현/획 저장→최신 판 내려받기→새로고침→지난 날짜 열기 PASS/콘솔 오류0/학생 오늘 표현0. 검수 요소는 되돌릴 수 있게 정리. Preview2x9ongcyd READY/실행버전 일치, 최종 head/CI는 PR/#150. 운영 웹 merge·환경 변수 변경 없음. 상세 docs/verification/teaching-board-cloud-20260916.md.
...
- **개인 학습 노트·단어 일괄 정리 1차 구현·로컬 검수 (2026-09-15)** — 실행3614b486, `codex/personal-study-notes-20260915`. 기존 private 자료/Excalidraw/어휘 문맥 RPC 재사용, 학생 개인 노트·서버 CAS/기기 초안·가나 후보·검토 후 일괄 담기·뜻 충돌/부분 재시도·원래 필기 복귀·모바일 초점 처리. 전체396파일/4,238테스트·JSX lint·6GiB 빌드 PASS. 최종 프로덕션 빌드에서 Chromium/WebKit 각각7흐름 PASS/실행 오류0, 실제 폐기용 PostgreSQL에서 기존 어휘 전체 행 불변 확인. 교사 설명판 기존13흐름 회귀 PASS. 새 DB/schema/env/외부 AI 호출/운영 병합 없음. 공개 handoff·미리보기 결과는 PR/#150에 기록하며, 실계정·물리 기기를 합성 검수로 대체했다고 주장하지 않는다. 상세 `docs/verification/personal-study-notes-20260915.md`.
- **여러 표현 설명판 출시 검수 완료 (2026-09-15, 오너 병합·배포 승인)** — #1312 실행cef2726. 네 글자 한 줄/긴 표현 균형 줄바꿈, 실제 교재 표현1건의 서버 저장·재조회·설명판 중복 거부·운영 수업 기록/원문 복귀 확인. CI34939392439 전체394파일/4,221테스트·smoke/learning-flow PASS, 로컬/배포 빌드 PASS, Preview aew9gob0x에서 Chromium/WebKit터치 각13항목·화면 오류0·좁은 네 글자 한 줄 캡처 확인. 뒤 변경은 문서/자기 보드뿐. 최종 head·merge·운영 버전은 #150 CODEX_DONE/PR 최종 결과에 기록한다. 물리 기기는 todo, 기록1건 외 개인 복습/주의점/기존 판 불변.
- **여러 표현 설명판 실계정 검수 완료 (2026-09-15)** — #1312 Preview ah6d78020/실행64a60be, 오너 교사 로그인으로 교재→뜻→판→팀 홈 복귀 확인. 교재 표현3개+실제 AI 표현1개, 실제 AI 조회2회, 연속 추가·네 표현/필기·undo/redo·일괄 테두리/독음/훈음/뜻·가로 정렬·전체 보기·발표 임시 가리기·새로고침 복원 PASS. 1366/1024/390 뷰포트 직접 확인, 마지막 실측CSS354px 가로 넘침0·주요 버튼44px·Tab 이동, 콘솔 오류0. 오늘 표현0 유지·검수 판 요소 정리·학생 기록/개인 복습 미변경. 실행 코드 변경·배포·merge 없음. 긴 표현 줄바꿈과 물리 기기는 todo. 상세 docs/verification/teaching-word-canvas-20260915.md.
- **여러 표현·필기 설명판 구현/미리보기 완료 (2026-09-15)** — #1312, `codex/teaching-word-canvas-20260914`, 실행64a60be. 글자만/카드, 독음 위·훈음 아래·뜻 오른쪽, 다중 토글/정렬/필기, 사전/AI 복수 뜻·연속 추가·일괄 수업 저장·오늘 표현. 기존 드래그 뜻 편집 보완. 전체394파일/4,220개·신규19개·lint 오류0·콘텐츠/빌드; 실행 CI34866544616 SUCCESS. 로컬/배포 Chromium·WebKit터치 각13항목, 화면 오류0, 취소된 사전 로딩/정상 SQL 충돌은 별도 기록. preview ah6d78020/dpl_6Xsdr5QQLAhsKZVJYzRp4ZBbztgw READY·version 일치·조회 비로그인401. 이후 검사/문서/자기 보드만 변경, 최종 head/CI는 #150 CODEX_DONE. DB/환경/개인 복습/기존 판 보존, 운영 merge·alias 변경 없음. 후속 실계정 검수 결과는 바로 위 done 항목 참조. 상세 docs/verification/teaching-word-canvas-20260915.md.
- **설명판 전체 화면·빠른 도구 구현 완료 (2026-09-14)** — 오너의 판 공간 최대 활용/아이콘 클릭 메뉴 및 후속 품질 개선 요청. 실행 138bd66386c3ccbef9d645510ce367ff66fbd8db, `codex/teaching-board-immersive-20260914`, PR #1311. 선택/펜/지우개 한 번 전환·재클릭 설정, 표현/교재, 분류한 전체 메뉴, 현재 색·빈 판 안내·초점 복귀. 기존 뜻 창·출처·초안·필기/복구·수업 기록 보존. 전체390파일/4,201개와 JSX/JS lint, Chromium20/WebKit터치20 흐름 오류0·354px부터 직접 캡처 검수. CI34831023820 두 job SUCCESS, Vercel 배포 빌드 완료. 물리 iPad/Pencil은 미검수. 최종 exact head/CODEX_DONE은 #150, 운영 반영 상태는 위 doing 및 #1311. 상세 docs/manabi-teaching-board-immersive.md.
- **설명판·뷰어 운영 마무리 검수 완료 (2026-09-14)** — 오너 병합·배포·검수 전부 승인. #1309→#1308→main c21859a4 운영 통합 및 tree 동일 확인. 추가 PR #1310 실행1574b266: Safari 발표 초점 복귀, 필기 중 자동 이동, 좁은 모바일 원문 가림 보완. CI34807902815 전체388파일/4,192개·빌드·기존 학습 흐름 SUCCESS. Preview7k39epz3n/dpl_4vZBYVrSeh1dgYKPKBXFRQfYFrer READY·버전 일치, Chrome·WebKit 각18+5흐름 오류0, 354px·390px·태블릿·데스크톱 직접 화면 검수. c21859a4 실제 교사 계정 교재→뜻→판→확대→복귀와 초안 유지 확인·검수 카드/입력 정리. 이후 문서/자기 보드만 갱신, 최종 head·CI·운영 merge·배포 결과는 #1310/#150 완료 코멘트. 개인 자료·DB·FSRS·원래 작업 공간 보존, 물리 iPad/Pencil은 별도. 근거 docs/verification/viewer-production-release-20260914.md.
- **뷰어 디자인 일관성·설명판 단어 창 (2026-09-13)** — `codex/viewer-manabi-design-20260912`, 구현 f7fa0db7. #1308의 9040b216 기반 후속 draft PR. 메인 종이/잉크·제목/도구 위계와 뜻 카드를 정리하고 설명판에서도 기존 단어 뜻·개인 저장·판 삽입/확대를 연결했다. 전체 388파일/4,191 테스트, lint/JSX 검사, 빌드, 일반 뷰어 33흐름, 배포용 설명판 18흐름/새 디자인 5검사 통과. 운영 원문·DB·학습 기록·권한 보존. 실계정/물리 iPad 검수와 운영 병합은 남음. 최종 head·CI·미리보기는 PR/#150 CODEX_DONE에 기록. 상세 `docs/verification/viewer-manabi-design-20260913.md`.
- **설명판 수업 동선 개선 구현·로컬 검수 (2026-09-12)** — `codex/teaching-workspace-20260912`, 실행 da663215, 운영 0a071374 기반. Excalidraw 재사용, 공통 도구/가변 분할, 빠른 표현·로컬 검색, 보여주기·뜻 공개·레이저·Esc 복귀, 과 이동/입력 초안 보존. 모바일 겹침/빈 판/undo·redo/키보드 입력 공간 보완. 실계정 중국어 교재 표현/발표/과 왕복·초안 유지 확인, 좁은 발표 화면 메뉴 및 원본 카메라 불변 보완. 전체388파일/4,191개, build475페이지, lint, Chromium 마우스·터치 각18흐름 PASS/오류0. 기존 판/출처/권한/기록 보존, DB/env/운영 변경 없음. 원격 CI·미리보기·exact head는 PR/#150 CODEX_DONE에 기록. 실제 iPad/Pencil·Safari 검수, 서버 동기화·필기 인식·재사용 묶음은 후속. 상세 `docs/verification/teaching-workspace-20260912.md`.
- **운영 설명판 실계정 검수·가림 보정 (2026-09-12)** — `codex/teaching-board-viewport-20260912`. #1306 운영 d5909ce8의 교사 진입·표현/필기·저장 복원·과 전환 확인. 확대 화면에서 모바일 도구막대가 카드 뜻을 가리는 문제를 실제 종이 영역 계산과 도구막대 교체 관찰로 보정. 전체387파일/4,186개·마우스/터치 각13흐름 PASS, 기존 내용/좌표/수업 기록/권한 보존. 최종 CI·배포·운영 재검수는 PR/#150에 기록. `docs/verification/teaching-board-viewport-20260912.md`.
- **교사 설명판 1차 구현·검수 (2026-09-12)** — `codex/teaching-canvas-20260912`, 실행891ffdf9. 교재 옆 네이티브 표현/필기·묶음·가나 후보·기기 저장·충돌 복구·백업. 기존 교사 권한/수업 저장 RPC/개인 뷰어 보존. 전체386파일/4,182개·모델/API12개·Chromium 마우스/터치 각12흐름 PASS, 로컬 빌드 PASS. 물리 펜·서버 동기화·필기 인식/AI는 후속. 최종 CI·미리보기 및 exact head는 #150 CODEX_DONE에 기록. 운영 merge·DB·env·고정 별칭 변경 없음. `docs/verification/teaching-board-20260912.md`. (최근)
...
- **뷰어 2차 구현·배포 검수 완료 (2026-09-09, 오너 “진행해”)** — `codex/viewer-reading-controls-20260909`, 부모 #1293 bb8fc05d, draft PR #1294. Aa 3탭/학습·관리 분리, 한 곳의 단어·문장 패널, 실제 SC 서체·12–16px 병음·3테마, 위치 복원·명시적 자동 진행·받아쓰기/회화 복구. 실행 `ed32d1efe44a1f3cb821a82a6b38c5cca8114707`: 전체363파일/3,931개, CI34300623868 SUCCESS, Vercel READY·버전 일치, 배포 신규/실제 글꼴17 + 레이아웃17 + 기존 신뢰성17 오류0. 미리보기 https://manabi-24lbnl1ap-wonchance-arts-projects.vercel.app/materials . 이후 검사/문서/자기 보드만 변경, exact head는 #1294/#150 CODEX_DONE. 실계정 HSK6 재검수는 Mac 잠금, 물리 iOS/마이크는 미검수. PDF/신규 음성/split view·DB 변경·merge/force-push/기존 alias 변경 없음. 상세 `docs/manabi-viewer-reading-controls.md`.
- **뷰어 신뢰성 1차·DB 적용 완료 (2026-09-09, 오너 설계 승인·“진행 가자”)** — `codex/viewer-reliability-20260909`, 부모 #1292 `7d7c6a6496243e014c17f9bd66161c34e9033212`. 문맥 뜻·표시/저장 일치, 요청 경합·캐시·undo·숨은 카드 입력, 중국어 읽기/인식 연결, 재분석 성공 후 조건부 교체. SPEC `docs/manabi-viewer-reliability.md`. 원본 공유 폴더/교재/FSRS/권한 보존, merge·force-push·월드·PDF 조판·새 음성 제외. PR #1293 draft: 전체 362파일/3,924테스트·브라우저 17흐름·로컬 SQL 13검사·기존 viewer 학습 흐름 통과. 오너 “승인 ㄱㄱ” 후 migration20260908234552 적용·로컬/원격88개 일치·실제 권한6검사·본문MD5 일치. 개인 자료 쓰기0. 취소 RPC의 의도한 authenticated DEFINER WARN1 검토 기록, 나머지 advisor 동일. 실행94cff1f1 CI/READY·배포17검사 통과, 후속은 migration 이력·SQL 검사 경로·문서·자기 보드만 변경. 웹 코드는 같아 기존 https://manabi-l55ad7efn-wonchance-arts-projects.vercel.app/materials 사용. 운영 웹 병합은 Claude 창구, exact head는 #1293/#150 CODEX_DONE.
- **기기 간 원본 읽기 위치 동기화 완료 (2026-09-08, 오너 후속 승인)** — draft PR #1292 / `codex/original-reading-sync-20260908`, 부모 #1291. 본문 텍스트/PDF 쪽/EPUB 장·위치, 마지막 첨부 복원, 오프라인 큐·서버 version 충돌·응답 유실/동시 탭 보호. 신규 migration1개 workflow34240343140 적용, 원격87개 일치·advisor 추가0. 전체358파일/3905개·실제 SQL/RLS·신규5/서재9/구간6/편집6 브라우저·473페이지 로컬/실제환경 빌드 통과. 320/390/768/1280px·200% 글자·키보드·충돌 안내 직접 검수. 실계정 서재→PDF2쪽/EPUB첨부2장 복원·위치2행 확인, 원본 해시/첨부2개/구간3개/진도0/기존 표현 복습일 보존. 실행97d6bb064d11abf23ed2215412d9550059ff0c7f CI34240326474 SUCCESS; 고정 https://manabi-web-v2-preview.vercel.app/materials / dpl_ABCo3J3UuZiMsNDXmc9NQvjbQd6h READY·버전 일치. 실제 물리 기기/iOS Safari는 미검수. 이후 검수 문서와 자기 보드만 별도 커밋, 최종 exact head는 #1292·#150 CODEX_DONE. 운영 웹 승격/merge/force-push 없음. 상세 `docs/manabi-original-reading-sync.md`.
- **원본 선택 구간 학습·DB 적용·실계정 검수 완료 (2026-09-08, 오너 승인)** — draft PR #1291 / `codex/source-passage-study-20260908`, 부모 #1290. 본문/PDF 한 쪽/EPUB 한 장 선택·모바일 블록 선택→최대1500자 비공개 학습→기존 표현/복습→원본 위치/인용 강조. 동일 구간 재사용·분석 임대·안전한 토큰 교정·보관 첨부 복귀. migration2개 원격84→86 적용(workflow34197245148/34198072983), advisor 추가0. 전체357파일/3888개·최종 집중22개·실제 SQL/RLS·신규6/서재9/편집6 브라우저·473페이지 빌드 통과. 실제 본문/PDF2쪽/EPUB2장 세 구간 분석·토큰 교정·표현1개 저장·각 원본 강조·동일 구간 재사용 확인, 원본 해시/첨부2개/진도0 보존. 실행f5a6a96a8e871b2de8363ac580de9f510144015f CI34198032249 SUCCESS, https://manabi-web-v2-preview.vercel.app/materials / dpl_89v5sN3oBvUk5WpR9Kwz85gvrEwZ READY·버전 일치. 실제 iOS Safari/복잡한 PDF는 미검수. 문서·보드 후속 커밋만 포함한 최종 exact head는 #1291·#150 CODEX_DONE. 운영 웹 승격/merge/force-push 없음. 상세 `docs/manabi-source-passage-study.md`.
- **통합 서재 v4·DB 적용·실계정 검수 완료 (2026-09-08)** — draft PR #1290 / `codex/library-v4-20260908`, 부모 #1289. 원본 단위 목록·검색/20개 페이지·개인 모음집·독립 글 북마크·실제 최근 열람. 원격83→84 migration20260908025511 적용(workflow34186288330), advisor 추가0. 전체356파일/3878개·집중31개·신규9/기존7 브라우저·최종 첨부 표지 회귀·473페이지 빌드 통과. 실제 새 검수 자료1개에서 글+PDF+EPUB 저장, PDF2쪽/EPUB2장 복원, 모바일 제목/본문/언어 수정, 모음집 이름/소속 변경 확인. 최초 raw/processed 불변·원본2개·학습 진도0, 비소유자0/anon 차단, 임시 모음집 삭제 후 원본 보존/ROLLBACK. 최종 실행b77017bec2518eacdbad4ab11f74b11fdfd44842 CI34187340418 SUCCESS. https://manabi-web-v2-preview.vercel.app/materials / dpl_DxBs2PJLi9NidBf9me2CSaWBrq1r READY, 화면/API 버전 일치. 운영main6e61b6e8 불변, merge/force-push/운영 승격 없음. 최종 exact head는 #1290·#150 CODEX_DONE, 상세 `docs/manabi-library-v4.md`.
...
- **manabi 웹 v2 · 인증 경로·안내 보완 완료 (2026-09-07)** — draft PR #1287, 실행 코드 `41e8c8bbc99693dcb0796480b843ae10290b8308`. Google·재설정 PKCE 콜백 및 내부 복귀 경로, 재설정 폼 필수 필드, 오류/성공 대비·ARIA 보완. 전체351파일/3,826개, 최종 실행 코드 웹·월드 CI 성공. Vercel Node24.x/READY, 인증7조건/6레이아웃·대비6.46/6.47, 버전4조건 오류0; 홈34레이아웃·실제4/fixture6흐름 통과. https://manabi-web-v2-preview.vercel.app/auth 갱신. 실제 DB anon 개인 자료/진도/PDF/표현/Storage 접근0 확인. 운영DB 수정·개인 세션 검수·운영 전환은 완료하지 않았으며 위 대기 항목으로 분리했다. 원본 작업 공간·운영 주소 유지, merge/force-push 없음. 상세 `docs/manabi-web-v2-release.md`, exact 최종 head 인계는 #150 CODEX_DONE.
- **manabi 웹 v2 · 승인된 Node 24 전환 완료 (2026-09-07)** — 오너 「승인. 다음 할 일 진행」에 따라 package/lock engines24·웹 CI24 적용, world Node22 유지. `codex/web-v2-release-20260907` / draft PR #1287. 실행 코드 `4f3c7cd05e2b6910a8b068c96b3d3b3bfadcfcb7`: npm11이 발견한 optional lock 4항목 보완(기존 의존성 변경0), 웹 CI 349파일/3,792개·474페이지·조판25·뷰어4·smoke14·학습9, world CI 130파일/1,102개 통과. 실제 Vercel nodeVersion24.x/READY, 교재54레이아웃·17흐름·42과 API, 홈34레이아웃·실제4/fixture6흐름, 버전4조건 오류0. https://manabi-web-v2-preview.vercel.app/home 갱신. Node 승인 대기 해소; 남은 운영 조건은 실제 계정 로그인 검수, Claude DB 번호 정합화·병합, 자동 도메인 할당 분리와 production 대기 빌드. 최종 exact head/CI는 #150 CODEX_DONE. 상세 `docs/manabi-web-v2-release.md`. 운영 주소·DB·개인 기록·원본 작업 공간 유지, merge/force-push 없음.
- **manabi 웹 v2 · 출시 후보 준비 완료, 운영 전환 대기 (2026-09-07, 오너 운영 반영 설계 승인)** — `codex/web-v2-release-20260907`, main 대상 통합 draft PR #1287. 누적 8개 웹 PR + CLI/Git 배포 식별/검수 도구. 실행 코드 `3c608476aed01844cb5dca6d883772616c2d4003`: 전체 349파일/3,792개, Node24 로컬 474페이지 빌드, Node20 현재 Vercel 빌드, CI와 실제 배포의 42과/홈/서재/복습/집중 읽기 검수 통과. 고정 검수 https://manabi-web-v2-preview.vercel.app/home . 상세 `docs/manabi-web-v2-release.md`, 최종 exact head/CI는 #150 CODEX_DONE. **RELEASE HOLD**: Node 배포20→24/웹 CI22→24 세 파일 변경은 자동 승인 검토 거부 후 오너 승인 대기. DB 파일 날짜 번호는 Claude 검토/정합화, 실제 계정 검수는 로그인 대기. production main 자동 도메인 할당 분리 후 Claude만 병합. 운영 주소/DB/판본/개인 기록/원본 작업 공간 보존; merge·force-push·운영 승격 없음.
- **manabi 웹 v2 · 연속 탐색·집중 뷰어 완료 (2026-09-07, 오너 다음 ㄱㄱ)** — `codex/web-v2-focused-reader-20260907`, base #1285. 로그인 직후 연속 탐색의 홈 prefetch 충돌 수리, 축소 내비·고정 도구막대·필요할 때만 읽기 도구 노출, native 설정 dialog·키보드 초점 복귀·글자 설정 후 가시 토큰 위치 유지. 실행 코드 `7d31cd666eeeed6efd0cbaa4a84d9550d82fae72`: 전체 348파일/3,778개 통과, 배포 브라우저 새 흐름 14 + 기존 왕복 20조건/오류 0, Vercel 474페이지 build 완료. 홈 prefetch 기대값 한 항목의 기존 계약 변경은 별도 테스트 커밋으로 기록. 최종 원격 CI는 PR checks, exact head는 #150 CODEX_DONE 인계. 미리보기 https://manabi-ra95lm8c8-wonchance-arts-projects.vercel.app/materials . 상세 `docs/manabi-web-v2-focused-reader.md`. 다음: 글 전체 북마크의 저장 모델·서재 진입, 별도 Node 20 배포 런타임 지원 종료 대응. 원고·조판·FSRS·DB·환경·음성·월드 및 원래 작업 공간 보존. 운영 병합/승격 없음.
...
- **🔤 언어 판별 — 「보여 줄 값」과 「저장해도 되는 값」을 갈랐다 (2026-09-01, 오너 「할 수 있는거 ㄱㄱ」)**:
  「`detectLang`이 2트랙이라 옛 행이 잘못 **보인다**」로 적어 뒀는데, 실측하니 **잘못 보이는
  게 아니라 DB에 굳고 있었다.** 같은 2트랙 판별이 **여섯 벌**이었고 그중 넷이 영속화 경로다:
  ⑴ `vocabIO.fetchVocabFromNetwork` — 조회할 때마다 빈 `language`를 추측해 **UPDATE**.
     옛 중국어 행은 단어장을 한 번 여는 것만으로 `Japanese`로 굳는다(프랑스어는 `English`).
  ⑵ `vocabIO.parseCSV` — CSV 반입이 ja/fr/en 3트랙으로 단정. **중국어 CSV가 통째로 일본어.**
  ⑶ `VocabPage` 수동 담기 — 언어 칸을 안 고르면 표기로 단정해 저장.
  ⑷ `api/admin/backfill-base-form` — 서버 백필이 `language`까지 되쓴다.
  ⑸ `HomePage` 집계 인라인 복제 ⑹ `constants.detectLang`(정본).
  · **규칙**: 표기가 결정적일 때만 답한다(`detectLangConfident`) — 가나 → 일본어,
    프랑스어 발음부호 → 프랑스어, **한자만·라틴만 → `null`**(`会社`도 `table`도 두 언어에 있다).
    화면용 `detectLang`은 그 위에 기본값을 얹어 **예전과 같은 모양**을 유지한다(바뀐 건
    프랑스어 단어가 프랑스어 목소리로 읽히는 것뿐). **영속화는 전부 확신 판별만 쓴다** —
    못 가르면 컬럼을 비워 두고, 다음에 사용자가 담을 때 진짜 언어가 실린다.
  · ⚠ **덤으로 나온 진짜 결함**: `ConversationPanel`이 `language === 'Japanese' ? 'Japanese'
    : 'English'`로 접어 **LLM 프롬프트**에 넣고 있었다(「ONLY in ${targetLang}」). 중국어·
    프랑스어 지문에서 **AI 튜터가 영어로 답했다** — 바로 아래 줄은 「중국어로 답해 보세요」라고
    적으면서. 아는 언어를 접을 이유가 없다.
  · ⚠ **EPUB 반입도 같은 병**: `importWholeBook`은 ja/en/zh 3갈래, `importPicked`는 ja/en
    2갈래라 프랑스어 EPUB은 양쪽에서, 중국어 EPUB은 한쪽에서 `null`로 떨어졌다.
    `langFromBcp47` 정본 매퍼로 모았다(표기 추측이 아니라 **파일이 선언한 값**이다).
  · 대소문자 판단(`Tシャツ`의 `T`)은 **표기 판별**이라 `hasCjkText`로 갈라 뒀다 — 언어를
    묻는 자리에서 쓰면 안 된다는 걸 주석과 이름으로 못 박았다.
  · 계약 14종 + **변이 5종 전부 검출**. M4(수동 담기 추측 저장)가 **1차에 살아남아** 계약
    구멍을 드러냈고, 그 경로 계약을 새로 넣었다.
  전량 vitest 3279 green · lint 0 error · 빌드 성공. 마이그레이션 0 · 새 쿼리 0.
- **🧹 죽은 CSS 제거 — **스타일시트의 20%가 화면에 닿지 못하고 있었다**
  (2026-09-01, 오너 「할 수 있는거 ㄱㄱ」)**:
  스크림 통합에서 `.overlay`·`.celebration-overlay`가 사용처 0인 걸 보고 「약 150줄쯤」이라
  적어 뒀다. 전수로 재니 **1,543클래스 중 354가 죽어 있었다** — 눈으로 세면 두 자리를 틀린다.
  **11,147 → 8,882줄(−2,265, 20%) · 299KB → 246KB · @keyframes 39 → 25.**
  · **판정 규칙이 이 라운드의 본체다**(계약으로 심었다 — `deadCss.test.js`). 죽음 =
    리포 어디에도 이름이 없고 **동적 조립도 불가**. 조립은 두 겹으로 막는다: ⑴ 백틱 `${`
    직전 토막(경계 `-`·`_`로 끝나고 3자 이상) ⑵ 가문 어간 생존.
  · ⚠ **초안 규칙이 살아있는 코드를 지울 뻔한 것이 세 번**이다 —
    ⑴ `is-easy`·`is-late`·`is-ontrack`은 `is-${VERDICT_CLASS[…]}`로 조립된다.
    ⑵ `markedContent`는 **pdf.js가 런타임에** 붙인다(서드파티 목록으로 명시).
    ⑶ 경계 요구를 안 걸었더니 **한 글자 접두사**(`h`·`m`·`p`)가 딸려 들어와 `home-*`·
       `mypage-*` 가문을 통째로 「살아 있다」고 오판했다(반대 방향 오류).
  · ⚠ **자기 참조를 두 번 밟았다.** 계약 주석·보드가 `.celebration-overlay`를 이름으로
    적고 있어 **죽었다고 적어 둔 문장이 그것을 살려 놓고** 있었다. ⇒ 문서(.md)는 스캔에서
    빼고 코드는 주석을 걷어낸 뒤 센다. 그 규칙을 검사하려고 쓴 단언조차 그 문자열을
    코드로 들어 같은 덫에 걸려, 이름을 **조립해서** 쓴다(`['celebration','overlay'].join('-')`).
  · **부분문자열 → 토큰 일치**로 바꿔 CI 시간을 **147초 → 2.5초**로 줄였고, 덤으로 판정이
    더 정확해졌다 — `.foo`는 `class="foo-bar"`에 안 걸리는데 부분문자열은 걸린다고 답한다.
    그 정밀화가 `stat-pill`·`guide-cta`·`mypage-stat-cell` 같은 **BEM 블록 잔재**를 드러냈다
    (요소 `__label`은 쓰는데 블록은 안 쓴다 — 컨테이너가 `bento-*`로 옮겨간 레디자인 자국).
  · 여러 줄 선택자 목록을 자르는 버그로 CSS가 한 번 깨졌다(`.a *,\n.a *::before,\n\n}`).
    빌드가 잡았고, 「마지막 줄만 지우기」를 「선행 공백만 남기기」로 고쳤다.
  · 딸림 정정 둘: `VIEWPORT_QUERY_CAP` **28 → 26**(@media 블록 둘이 통째로 비었다) ·
    스크림 흐림 목록 **2종 → 1종**(설계의 「흐림은 하나뿐」이 이제야 참이 됐다).
  · 계약 5종 + **변이 5종 전부 검출**(죽은 규칙 추가 · 고아 키프레임 · 접두사 경계 제거 ·
    서드파티 목록 도피 · 주석 제거 폐지).
  전량 vitest 3265 green · e2e:typography 25/25 · 빌드 성공 · lint 0 error.
- **📊 중국어 급수 커버리지 — **막고 있던 건 제품 결정이 아니라 하드코딩한 목록 하나**
  (2026-09-01, 오너 「우선순위대로」)**:
  「`LEVEL_MILESTONES`에 중국어 목표 단어 수를 정하는 건 제품 결정」이라 적어 두고 보류했는데,
  실측하니 **기존 세 언어의 값이 각 시험의 공표 수치**였다(JLPT N5 800…N1 10,000 / CEFR
  A1 500…C2 10,000 — 우리 콘텐츠 크기가 아니다. 영어 레지스트리는 1,382어인데 목표는 10,000).
  즉 취향이 아니라 **관례를 따른 것**이고, 중국어에도 같은 관례가 있다 — HSK 공식 누적 어휘 수.
  · **시점이 갈림길이었다.** 2021년 HSK 3.0 초안(500·1,272·2,245·3,245·4,316·5,456)은
    **2026년 7월 시행본으로 대체**됐다. 현행은 **300·500·1,000·2,000·3,600·5,400**이고
    오늘이 9월이므로 후자가 정본이다. 계약이 그 수치와 출처 문구를 함께 고정한다.
  · H6(5,400)이 우리 중국어 콘텐츠 누적(6,850어)보다 **작은 것은 오류가 아니다** — HSK 6은
    CEFR C2가 아니라 B2~C1 언저리라 사다리가 짧다.
  · **같이 드러난 무증상 결함**: `LEVEL_MILESTONES.French`가 6칸인데 `FR_LEVELS`는 7칸이라
    `A0 입문` 학습자는 폴백으로 **라벨은 A0, 수는 A1의 500**을 받고 있었다. `targetOf`가
    라벨과 수를 **함께** 첫 급수로 넘긴다. 입문 급수는 어휘 사다리에 칸을 만들지 않는다
    (수를 지어내면 옆 칸과 같은 퍼센트를 그리는 빈 막대가 하나 는다) — `LADDER_EXEMPT`에 사유 기재.
  · 하드코딩 셋 제거: 언어 배열(→ `Object.keys(LEVELS)`) · 언어 이름 두 벌(→ `langNameKo`) ·
    막대 색 삼항(→ 조회표. 그 파일 주석이 「삼항 체인은 언어가 늘 때마다 마지막 가지가 오답이
    된다」고 이미 경고하고 있었다).
  · 계약 10종(`vocabStatsLevels.test.js` — `langCheckCoverage`와 **같은 모양**의 커버리지 계약)
    + 변이 6종 전부 검출(사다리 제거 · 대체된 초안 값 · 목록 하드코딩 부활 · 입문 어긋남 부활 ·
    색 삼항 부활 · 예외 목록으로 도피).
  · ⚠ **적립**: `detectLang`이 아직 **2트랙**이다(`/[가나·한자]/ ? 'Japanese' : 'English'`).
    전 호출부가 `v.language || detectLang(...)` 폴백이라 이 라운드가 새 결함을 만들진 않지만,
    **`language` 없는 옛 행**은 중국어가 일본어로, 프랑스어가 영어로 읽힌다(TTS 목소리 포함).
    호출부 6파일에 걸쳐 있어 별도 라운드.
  전량 vitest 3261 green · lint 0 error · 빌드 성공. 마이그레이션 0 · 새 쿼리 0.
- **🔗 부채 ① 드릴↔단어장 R1 — **제가 권한 (b)가 절반만 성립했다** (2026-09-01, 오너 「우선순위대로」)**:
  드릴은 이미 SRS에 붙어 있었다 — 다만 **문법** 쪽(`review_events` source:'grammar' +
  `grammar_review`)이고 단어장에는 아무것도 안 갔다. 그 이음새를 **만남(encounter)으로만** 잇는다
  (FSRS 등급 없음 — 문장을 맞힌 것을 그 안의 단어를 인출한 것으로 세지 않는다).
  · **착수 실측이 권장안을 갈랐다.** 드릴 2,148개 전수: 목표어 문장을 든 것이 **21%**(454)뿐이고
    (나머지는 `prompt`가 한국어 메타언어 + `answer`가 조각 `束`·`りょう`), 공백 분할 + 정본 대조는
    **fr 229/230 · en 59/61**에서 오탐 0인데 **ja 36/92 · zh 0/71**이다.
  · ⚠ **최장일치 스캔은 수율이 오르지만(ja 95%·zh 100%) 뽑히는 게 틀린다**:
    `我有时间`→我·**有时**·**间** / `おにぎりを`→**り** / `きのうは`→きのう·**じゃ**.
    그리고 그 오답들은 **전부 정본에 실재하는 단어**라 `buildEncounterCandidates`의 유령 차단을
    통과해 **오늘 학습에 「り」가 출제된다** — 없는 것보다 나쁘다. ⇒ ja·zh는 기계 추출이 아니라
    **`refs` 저작**으로(R2). 원래 물었던 (a)/(b)는 양자택일이 아니라 **언어별로 답이 다르다**.
  · **누락된 호출 지점 하나를 같이 채웠다**: 드릴 화면에 `syncVocabEncounters`가 없어, 뷰어·어휘
    페이지를 안 여는 학습자의 만남이 기기에 갇힌다(/study는 서버 정본을 읽는다). 새 부품이 아니라
    **호출이 빠진 자리**였다.
  · 재사용만 했다 — `recordVocabEncounters`(뷰어·NPC와 같은 부품) · `encounterLookupLang` ·
    `loadRefVocabLookup`. **새 표 0 · 새 쿼리 0 · 스키마 0.**
  · 계약 15종 신설 + **변이 8종 전부 검출**(zh 추가 · 정본 대조 제거 · main 대신 조각 · 출처 태그 ·
    문맥 제거 · 반환 프로미스에 얹기 · 오답 제외 · 서버 동기화 제거).
  · ⚠ **리포의 계약 위생이 내 테스트를 잡았다** — raw `slice(…indexOf(…))`를 3쌍 썼다가
    `contractHygiene`에 걸려 `sliceBetween`으로 고쳤다(v2-L이 심은 그 규칙이 제 값을 했다).
  전량 vitest 3251 green · lint 0 error · 빌드 성공.
- **🌒 오버레이 스크림 값 통합 — **설계가 센 것보다 많았고, 그래서 「변화 없음」이 조건부였다**
  (2026-09-01, 오너 「우선순위대로」)**:
  설계는 「인라인 3벌 + 오버레이 클래스 **4종**」이라 적었다. 착수 실측은 **9종**이다 —
  0.3(`.overlay`) · 0.45(`.tile-modal__overlay`) · 0.5 셋(`.vocab-detail`·`.source-edit`·
  `.reading-test`) · 0.55(`.confirm`) · 0.6(`.modal`) · 0.65(`.onboarding`) · 0.75(`.celebration`).
  · **설계의 「흐림은 하나뿐」도 틀렸다** — `.celebration-overlay`도 `blur(8px)`을 쓴다.
  · **그리고 그 사실이 라운드의 전제를 흔들 뻔했다.** 목업은 **0.45~0.65**를 비교해
    「구분되지 않는다」고 판정했는데, 실측된 0.3과 0.75는 **그 범위 밖**이다. 둘이 살아
    있었다면 이 통합은 「겉모습 변화 없음」이 아니었다. 다행히 **둘 다 사용처 0**(죽은 CSS)
    이라 성립한다 — 그래서 **그 사실 자체를 계약으로 박았다**(되살리면 거기서 멈춘다).
  · 투명이 의도인 셋(`.rsheet-backdrop`·`.reanalyze-panel-overlay`·`.pdf-detail-overlay`)은
    스크림이 아니라 **클릭 받이**다. 배경을 아예 안 쓰므로 대상이 아니다.
  · 흐림은 **ⓐ 그대로**(오너 권장 채택) — 값 통합에 묻어가지 않는다. 우리가 의도적으로
    정한 적이 없어 그것대로 판단이 필요한 축이다. 쓰는 목록을 계약이 고정한다.
  · 계약 4종(`viewerChrome.test.js` ⑧) + **변이 5종 전부 검출**(리터럴 부활 · 토큰 값 변경 ·
    흐림 추가 · 흐림 제거 · 죽은 `.overlay` 되살림). 파서 공허 방지 단언 포함.
  · ⚠ **적립**: `.overlay`+`.bottom-sheet` 계열(약 100줄)과 `.celebration-*` 계열이 통째로
    죽어 있다(`celebOverlayIn` 키프레임만 `.vocab-detail-overlay`가 쓴다). 죽은 CSS 제거는
    이 라운드와 성격이 달라 **따로 뺐다** — 지우면 목차의 흐림 목록이 1종으로 줄어든다.
  전량 vitest 3236 green · lint 0 error · 빌드 성공. 마이그레이션 0 · JSX 0 · 겉모습 변화 0.
...
- 📚 R3 발주 중(**증량**) — N5 잔여 전체 order 13~30 **18챕터**(n5.js 16 +
  scene_emergency.js 2), **SPEC v2 = #150 코멘트 5480310604**(2026-09-01 00:0x KST 게시).
  구판 5429640272·5429565401은 **폐기**. v2에서 바뀐 것 셋: ⑴ 돈키호테 보류 해제 +
  **브랜드 일반화 금지** 신설(IP 정책 v2 — 방향이 뒤집혔다) ⑵ 스타일 캐논 2건 확정으로
  「손대지 말라」 지시 해제(이제 `styleCanon.test.js`가 CI에서 잡는다) ⑶ #1154가 한
  브랜드 일반화 재제출 금지 명시. 대상 18챕터 표·감사 항목 ⑴~⑺·게이트를 **한 문서에
  자립**시켰다(Codex Cloud는 #150을 못 읽어 오너가 인라인 전달해야 하므로).
  오너가 Codex Cloud에 인라인 전달 후 PR 대기.
### todo
...
- ~~⚠ 오너 결정 대기: ot-12-menzei의 돈키호테 상호~~ → **해소(2026-08-31 IP 정책 v2)**.
  상호를 그대로 쓴다 — R3 SPEC의 '돈키호테 보류' 특별 지시도 함께 **해제**(#150 고지).
  ⚠ **R3 재발주 시 확인할 것**(#1154는 2026-08-31 오너가 머지 없이 **닫음** — head
  `ad34f631`이 08-27 이후 정지·`dirty`): 그 PR이 구 정책에서 からあげクン→からあげ,
  ローソン 삭제 등 **브랜드 일반화**를 함께 했다. v2에서는 되살리는 방향이 맞으므로,
  다음 R3 SPEC에 **"브랜드 일반화 금지, 사실 정확성 사유만 허용"**을 명시하고,
  회신 PR에서 일반화가 IP 사유인지 오탐 예방인지 항목별로 갈라 판정한다.
  참고: `fukuoka.js`의 `ローソン`·`からあげクン`은 현행 main에 **그대로 살아 있다**.
...
- 🧊 **이 아래 전량 = 게임·월드 트랙 동결**(오너 "게임 월드는 ㄴㄴ" 2026-08-25).
  ...
  🧊 예외: `QuestReview.jsx` 한정 — W R3㉯ SRS 정합(오너 승인 2026-09-02, #1077 5504387198). 씬·연출·펫은 동결 유지.
...
- **🖥 뷰어 정돈(미니멀) A안 「끝은 끝에」 — 경로 줄·끝의 행동 본문 아래·PDF 출처 한 줄·안내문 첫 자료·비공개
  ...
  첫 글자가 화면 35~63% 지점(A 붙여넣기 298 · B 배지 3 374 · C 책 챕터 434 · D PDF 534px) — 본문 위에 뒤로가기 줄·
  시리즈 내비 줄·배지·**액션바 두 줄 89px**·**책 내비 바 50px**·**PDF 출처 카드 148px**이 쌓여 있었다. 「✓ 읽기
  완료 표시」는 누르면 퀴즈·완독 화면이 뜨는 **끝의 행동**이고 「오늘 학습 만들기」도 읽은 뒤 행동인데 본문 **위**에,
  다음 편·리딩 테스트·회화는 **아래**에 — 끝의 행동이 두 자리로 갈려 있었다. 형제 내비가 둘(시리즈 ◀ 3/20 ▶ 무배경
  vs 책 《…》 채워진 바 — 책 챕터 H1이 이미 「책 — 과」라 책 제목은 중복), 안내문 2벌 상시(본문 아래 영구 + 데스크톱
  빈 패널), **비공개 자료에도 토론 카드+댓글 조회**(비공개는 작성자만 여는데). **정돈**: 경로 줄 하나 [← 자료실 ·
  형제 내비 한 문법(이름은 툴팁) | ▷ 듣기 · Aa(· 분석 중단)] → 제목 → 배지(v2-Q 그대로) → (출처 한 줄) → 본문 →
  **「다 읽었다면」 [✓ 읽기 완료][오늘 학습 만들기][리딩 테스트][회화 연습]** 한 줄(같은 옷·44px, 완료만 색) →
  **다음 카드 하나**(시리즈 다음 편 → 책 다음 과 → 마지막 과 「+ 다음 과 적기」 → PDF 다음 범위(예전 카드 버튼) →
  완주) → 토론(공개만). 안내문은 단어를 아직 안 담은 로그인 사용자에게만 + 데스크톱은 CSS 감춤(빈 패널이 같은 말).
  이벤트·퀴즈·오늘 학습 핸드오프·이어 적기 딥링크·PDF 역방향 다리(`pdfViewerHref`)는 그대로 — **자리만 옮겼다**.
  **결과(같은 실측기)**: 390px 첫 글자 top A 298→204 · B 374→**280** · C 434→**280** · D 534→**310** · 시리즈
  410→280(px). v2-Q ⑥ 계약을 「행동은 끝·도구는 위」로 **개정**(오너 A안), ③은 시리즈·책 한 문법으로 확장, ④ 본문
  무접촉·⑤ 모션 가드 유지. 죽은 CSS 제거(`.viewer-actionbar`·`.book-nav`·`.pdf-origin__back`·완료 배지). 스키마 0 ·
  **쿼리 −1**(비공개 댓글). 계약 `viewerMinimal.test.js` 10종 + `viewerChrome.test.js` 개정 · **기하 e2e
  `e2e/viewer-chrome.e2e.mjs`**(정적 마크업 + index.css, 서버 불요 — 첫 글자 top 상한 B·C ≤300 · D ≤330, CI
  `e2e:chrome`) · 변이 11/11 검출. 전체 vitest 326/326 green · lint 0 오류.
- **🧹 「내 자료」 정돈(미니멀) R2 — 자료 추가 화면: 입구 칩 + 아코디언·본문 먼저·제목 자동 채움 (2026-09-05
  아침, 오너 「자료 추가하는 부분도 다뤄줘 복잡하더라」 → 목업 #1077 5547576227)**: 실측: 첫 화면에서 붙여넣기
  칸까지 카드 4장(PDF 책장은 항상 펼침 — 업로드·저작권 동의·목록, EPUB·문장 목록·링크는 제목+설명 2줄+
  버튼)을 지나야 했고, 폼 안에서는 주 행위(본문)가 다섯 번째였다. **정돈**: 입구 4장 → `📄 PDF · N권 / 📚 EPUB /
  📋 문장 목록 / 🔗 링크` 칩 한 줄 + 아코디언(한 번에 하나, 같은 칩 재클릭은 접힘) — 펼침 상태는 페이지 하나
  (`openEntry`), 입구는 `open`을 받고 딥링크(`?book=`·추천 영상 주소)·본문 폼 넘김 때만 `onOpenChange(true)`로
  열어 달라고 하며, 내용을 넘겨준 뒤엔 페이지가 접는다(시선을 폼으로). 접힌 입구는 마운트만(훅·책장 조회는
  산다 — 칩의 권수가 그걸로 나온다) · 설명문은 펼쳤을 때만 · 헤더 설명 삭제 · 폼은 **본문 → 제목 → [언어|난이도]
  → [공개|종류]** · 제목이 비면 본문 첫 줄 40자(`materialTitle.js` `titleFromBody` — 입력한 제목은 안 덮음,
  placeholder가 채워질 값을 미리 보여 줌; 책 나누기 제목도 같은 길). 소스 순서 계약(PDF→EPUB→문장 목록→링크→
  본문)·문장 목록 필드·비공개 고정·노트 흐름·`/quick` 초안 전량 유지. CSS `.add-page` 스코프(두 칸 줄은
  auto-fit — 새 브레이크포인트 0), 칩 44px + focus-visible, 색 토큰만. 스키마 0 · 쿼리 0. 계약
  `materialAddMinimal.test.js` 15종(+`titleFromBody` 단위) · 변이 9/9 검출. 전체 vitest 325/325 green.
- **🧹 「내 자료」 정돈(미니멀) R1 — 자료실 카드 위계·⋯ 메뉴 (2026-09-03 낮, 오너 「내 자료 부분 UI UX
  작업 흐름 살피면서 개선, 미니멀리즘적으로 깔끔하게」 → 설계 초안 ①진단 ②목업 #1077 5547520918)**:
  실측: 낱개 카드가 언어명 1.4rem 큰 글자 + 태그 최대 8 + 상시 버튼 3(받아두기·공개·삭제), 제목은 셋째 줄
  — 정보 위계가 뒤집혀 있었다. 묶음(책·PDF) 카드는 「분석 3/3」과 과마다 「완료」 태그(시스템 상태). 헤더에
  설명 문장, 빈 상태는 그라데이션 박스. **정돈**: 제목이 첫 줄 · 메타 한 줄(언어명 `langNameKo` 정본·급수·
  읽는 시간·날짜) · 상태는 오른쪽 **하나**(✓ 완독 / 진행 % / 분석 중 / 실패 / 일부 완료 / 분석 전 — 정상 완료·
  노트는 무표기, 「대기 중」→「분석 전」으로 묶음 카드와 같은 말) · 액션 3종은 `⋯` 메뉴 하나(네이티브
  `details`, JS 상태 0, 한 번에 하나만 열림, 메뉴 안 클릭은 카드 클릭 가로챔) · 있을 때만 붙는 작은 알약
  (복습 N·같이 읽기·받아둠·시리즈·테스트 점수·내 노트) · 묶음 카드는 「챕터 N · 읽음 a/N」에 「분석 전 k」는
  있을 때만, 과별 「완료」 태그 삭제 · 헤더 설명 삭제 · 여백 32/28→16/14 · 빈 상태 박스 없이 한 줄.
  CSS는 `.materials-page` 스코프(같은 클래스를 쓰는 단어장·교재 무영향), 색 토큰만, `⋯` 버튼 44px +
  focus-visible. 기존 핀 전량 유지(안 읽은 것만·받아둔 것만·받아두기 stopPropagation·배지 wrap·묶음
  컴포넌트 단일·언어명 정본·쿼리 다이어트·노트 배지). 스키마 0 · 쿼리 0. 계약 `materialsMinimal.test.js`
  8종 · 변이 7/7 검출. R2(자료 추가 화면 — 입구 4장 → 칩 한 줄 + 아코디언, 본문 먼저)는 목업 5547576227,
  오너 평가 뒤.
- **📚 교재 챕터 점진 완성 「이어 적기」 (2026-09-03 새벽, 오너 발안 「하루 한두 챕터씩 적으면서 완성」 →
  설계 초안 ①② 회신 → 「ㄱㄱㄱ」 — #1077 5520128974)**:
  실측: 책은 가상 컨테이너(`metadata.book {key,title,order,total}` 공유), `total`은 소비처 0(자료실·뷰어
  전부 실개수), 입구 3종(문장 목록·붙여넣기 분할·EPUB)이 전부 「한 번에 전부 → 등분 → 새 key」라 오늘
  배운 1과를 넣으면 **새 책이 하나 더** 생겼다. 모자란 건 입구와 순번뿐 — 순수 `bookAppend.js`
  (다음 순번 = 형제 order 최댓값+1 · `N과` 재번호 · 이어 적기면 책 key 그대로(새 key 금지) ·
  기존+새 총수 · 내 책 목록은 최근 등록순) + 문장 목록 입구에 「새 교재 / 기존 교재에 이어서」 갈래
  (책을 고르면 제목·언어·난이도를 물려받아 잠그고, 과당 문장 수는 마지막 챕터 한 행의 줄 수에서
  물려받되 편집 가능 — 교재마다 다르다). 등록은 같은 insert 흐름(`status:'pending'`, 비공개 고정,
  문단 감지 생략 불변) — 새 챕터는 `order: startOrder+i`. 진입 3곳: 딥링크 `/materials/add?book=<key>`
  · 자료실 책 카드 footer 「+ 다음 과 적기」(내 책만, PDF 카드 「이어 읽기」 자리) · 뷰어 마지막 과의
  「다음 →」 자리(내 책만). 완료 화면에 [바로 읽기](첫 새 과를 열면서 분석 — 등록 즉시 분석 부활 금지).
  내 책 목록 조회는 메타 경로만(통짜 금지, 쿼리 다이어트). 스키마 0. 계약 `bookAppend.test.js` 12종
  (순수 7 · 배선 5) · 변이 8/8 검출. 기존 핀(문장 목록 입구 7·책 묶음 3) 무변경 통과.
  선례: LingQ Course→Lesson 추가(채택) · Anki 덱(개념) · Readlang/Lute 통짜 업로드(배제 — 현행의 한계) ·
  Notion 편집기화(배제). 비범위: 책 전용 페이지 · 뷰어 안 입력 · 전체 과 수 진도 필드.
- **🈁 ja 표제어 읽기 매칭 — 표기가 달라 못 만나던 말을 읽기로 2차 조회 (2026-09-03 새벽, 오너
  「할 일 찾아서 진행」 — #1077 라운드 10 §남긴 것, 저장만이던 후보를 편입)**:
  **실측이 전제를 고쳤다** — 「카드 정본 조회」라 적어 뒀지만 ja의 정본 조회는 카드(사전 행)가 아니라
  **만남 기록·상태 점**(`refVocabLookup` → `japaneseVocabRegistry.findWord`, 표기 키)이었다.
  콘텐츠 7,299항 중 가나 표제어 1,284·한자 표제어 6,004, 읽기 키 6,348 — 어휘 정답지 대조의 표제어
  미생존 429 중 204건이 「어휘 いぬ·본문 犬」류 표기 차이(학습자가 先生을 せんせい로 읽어도 만남이
  안 남던 자리). 읽기 색인(`findWordByReading`, 콘텐츠 yomi로 지연 구축 — jaYomiIndex.json 재다운로드
  없음)을 **표면 키 미스 때만** 탄다. 규칙 셋을 계약으로: ① 〜표기 표제어(〜分·〜時) 제외
  ② 표기 차이는 **가나↔한자만** — 표면·후보가 둘 다 한자면(橋↔箸) 다른 말이라 제외 ③ 동음이의는
  최저 급수 우선, **동급이면 무개입**(はし → 橋·箸 N5 → null; 실측: 읽기 충돌 710 중 동급 399는 대부분
  같은 말의 가나·한자 이중 등재라 표면 히트가 먼저 잡는다) — 라운드 4 규칙 재사용. 가타카나는
  히라가나로 접어 비교(パン=ぱん). 뷰어는 후리가나(가나 표면은 표면 자체 — 토크나이저가 비운다)를
  넘기고, 표기 차이 히트의 main을 토큰 text로 기억해 **점 대조가 기록과 같은 열쇠**를 쓴다(안 그러면
  犬을 만나 いぬ로 기록해 놓고 犬의 점은 안 켜진다). 드릴 만남(문자열 토큰)은 읽기가 없어 불변.
  표면 우선·main=word.ja 불변(기록 → 재대조 왕복 안정). 스키마 0. 계약 `jaReadingMatch.test.js`
  10종(정본 payload 실측 핀) · 변이 7/7 검출.
...
- **🧠 U R3 — 내 노트(쓰기 자료): 자료에 「방향」 축 하나 (2026-09-02, 오너 「작업 개시
  ...
  선언한다. 오너 결정대로 `language='Korean'`을 **신설하지 않고**(4종 강제 상수가 전역에 퍼져
  파급이 크다) `reading_materials`에 **컬럼 하나** `direction text NOT NULL DEFAULT 'read'
  CHECK(read|write)` — 마이그레이션 SQL은 **코드로만**(`20260902130000_material_direction.sql`,
  적용은 오너 수동). 노트는 `reading_materials`의 행이다 — **별도 테이블을 만들지 않는다**
  (노션화 방어선). 추가 화면에 「읽기 자료 / 내 노트」 갈래: 노트는 `direction='write'`, 비공개
  고정(가장 사적인 데이터), `language`는 목표어를 그대로 선언(어느 언어로 옮길 노트인가), 본문은
  한국어 허용, **분석 큐에 넣지 않는다**(`status:'note'`로 저장하고 `runBackgroundAnalysis`
  미호출 — 토큰화·병음·FSRS 무접촉). 저장 흐름은 하나(같은 insert — `/quick` [자료로 저장]
  선례의 입구 형태). 자료실: 노트는 「내 자료」 안에 함께 보인다(탭 둘 그대로), `✍ 내 노트` 배지
  하나, 분석 상태 배지는 달리지 않는다(`STATUS_LABEL`에 note 없음). `direction` 컬럼 미적용
  환경 폴백 — PostgREST 400이면 컬럼 없이 같은 조회를 한 번 더(etym/hanja·writing_practice
  신규 컬럼 폴백과 같은 패턴, 그 환경엔 노트가 없으므로 결과 동일). 자동 번역 진입점 0(번역기화
  방어선). 정본 상수 `MATERIAL_DIRECTION`·`isWriteMaterial` 하나. 계약
  `materialDirection.test.js` 7종. 변이 검사 동반. 비범위(설계대로): 폴더·태그·백링크·리치
  에디터 · 노트 전용 화면 · 노트 공유 · 목표어 판(생기면 정상 자료로 분석).
...
- **🧠 U R1 — 커버리지를 추천 랭킹에 잇는다 (suggestionRank)
  ...
  고친 것 셋 — ⑴ 단어 **개수**로 레벨을 추측하던 것을 실제 커버리지로: 추천 카드 중
  `material_id`가 있는 것만 `processed_json`을 가져와(상한 12) `materialFit`을 그대로 태운다,
  저장어 집합은 이 화면이 이미 끌어온 단어 행에서 뷰어 관용구와 같은 `{surfaces, bases}`로
  (select에 `base_form` 추가). 클라이언트에 새 토큰화 0 ⑵ `scored[0]` 한 장 → 상위 N장(4)
  ⑶ 카드마다 사유 한 줄. **순수 랭커** `src/lib/suggestionRank.js`(DB 조회 0): ⑴ 선호 언어
  ⑵ 커버리지 산정 카드 우선 ⑶ i+1 대역(미지어 2%~15%, 중앙 근접순) ⑷ 나머지는 현행 레벨 점수,
  동점은 `created_at` 오름차순 → 원래 순서(안정). `FIT_MIN_TYPES`(20) 미만 표본은 밴드 없음 —
  기존 계약 재사용. 랭킹이지 필터가 아니다(카드 수 불변, 비로그인·어휘 0에서도 결정적,
  무작위·시간 의존 0). 사유는 **코드**(fit·fit_easy·fit_hard·level·level_near·lang·other)만
  돌려주고 문구는 뷰가 조립(「아는 단어 91% — 딱 맞는 난이도」). 사유 없는 카드는 그리지 않는다.
  SPEC 함정 준수: 뷰어의 `fetchUserVocabWords`를 옮기지 않았다(offlineCache 앵커). 계약
  `suggestionRank.test.js` 10종(순수 8 + 배선 2). 변이 검사 동반. 비범위(설계대로): 관심
  주제(R5)·소스 확장(R4)·스키마·크론·카드 디자인 개편.
- **🛡 X 품사 정본 게이트 — LLM 문자열이 품사 칸에 들어가는 구멍 넷을 한 정본으로 닫는다
  (2026-09-02, 오너 「작업 개시 우선순위대로」 — #1077 5504885559, 목차 5순위)**:
  오너 「품사에 단어가 통째로」 보고를 검사하다 나온 잠재 위험(보고 화면 자체는 R R2가
  닫았다). 토크나이저·수리 테이블은 깨끗(수제 43·HSK 수확 1,495 전부 정본 조합)이고 문제는
  **LLM 문자열을 받는 지점** — ① 프랑스어 줄 분석이 일본어 프롬프트로 폴백돼 `pos` 자유 문자열,
  복사 무검증 ② zh 문맥 판별 후보 `all` 무검증 → `pos_all`·사전 되쓰기 ③ 뜻별 `meanings[].pos`
  영어만 fail-closed → 칩 → 교정 → **`user_verified` 승격**(자가 치유에서도 안 나온다)
  ④ 설계에 없던 넷째: zh 상위 `entry.pos`(fetchMeanings)도 무검증. **정본 모듈** `posCanon.js`
  (순수·의존 0) — 초기 집합은 **현재 코드가 생산하는 값**에서 뽑았다(zh `POS_KO` 37 · ja
  `POS_MAP` 14 · en `ENGLISH_POS` 11 · fr = en + 기호), 계약이 원천 맵과 대조하므로 집합 갱신 =
  계약 갱신. `isCanonPos`는 `·`로 갈라 각 조각이 집합 안일 때만 참. **네 구멍에 같은 게이트**:
  ① 프랑스어 전용 프롬프트(영어와 같은 열거 문장) + 복사 직전 `canonizeTokenPos`(정본 밖이면
  토큰은 살리고 `pos: null` — 「미상」 관례) ② `all`을 정본으로 필터 후 pos∈all ③ 뜻별 pos
  언어 무관 게이트(정본 밖이면 그 pos만 떼고 뜻은 저장, 영어 행 단위 거부 불변) ④ zh 상위
  pos 정본 밖이면 토크나이저 pos 유지. **승격** `buildPromotedEntry(existing, corrections,
  language)` — 정본 밖 pos는 무시하고 뜻·발음만 승격(라우트가 language를 넘긴다). **2차 방어**
  `TokenPosLabel`이 합집합 정본으로 조각을 걸러 「동사·喝咖啡」 → 「동사」, 단독 단어 pos는 무표시.
  **정합 SQL** `20260902120000_pos_canon_cleanup.sql` — 정규식을 posCanon에서 생성(계약이
  재생성값과 대조), 비 user_verified만 NULL(다음 분석에서 자가 치유), user_verified는 **목록만**
  (오너 판정). DDL 없음 · **운영 적용은 오너 수동**. 계약 `posCanon.test.js` 11종 + 라벨 픽스처.
  변이 검사 동반. 잔여(설계대로): 사용자 직접 입력 칸(단어장 편집·수동 추가)은 의도된 자유도라
  게이트 밖 — 라벨 2차 방어가 걸러 보이지 않을 수 있다(정본 datalist 제안은 후속).
- **🃏 W 인라인·퀘스트 R3 — 4등급 정본 정렬 + 키 1~4 + Ctrl/⌘+Z undo (㉮ 인라인 · ㉯ 퀘스트 🧊 동결 예외)
  (2026-09-02, 오너 「작업 개시 우선순위대로」 — #1077 5504387198 · 예외 승인 5504406191, 목차 4순위)**:
  두 지점이 **정본과 다른 척도**를 쓰고 있었다 — 인라인 `모름/애매/알아`=1/2/3(Easy 없음, D 2.12에
  갇힘), 퀘스트 `틀/맞`=1/3. undo보다 그 손실이 커서 **정렬이 선행**: 둘 다 `SAVE_GRADES`(R1의
  정본, 복습 화면 ScoreSection과 같은 라벨·순서·값)로. 인라인은 `.review-score-btn--*` 재사용,
  퀘스트는 **모양은 GBC·값은 정본**(auto-fit 그리드라 좁은 임베드에선 2단 2열, 값·순서 불변).
  `correct = rating > 1`·`qtype:'flash'` 규약 불변(rung 비대칭 신뢰 승계). **undo = R2 모델
  그대로**(SRS 5필드 복원 + `source:'ui'` 보상 이벤트, `undo_of.reviewed_at` = 원 채점 시각) —
  두 지점은 오프라인 큐가 없어 outbox 분기 없음. ㉮ `useInlineReview`가 결과에 `prev`(5필드)·
  `reviewedAt`을 실어 **스냅샷은 호출부(카드)가 만든다**(훅은 채점만); 채점 시각을 한 번 찍어
  SRS·이벤트 `created_at`이 같은 시계. `fetchUserVocabWords` select에 `last_reviewed_at` 추가
  (읽기 컬럼 1, 스키마 무변경 — 없으면 undo 뒤 isNewWord·「복습 시점이에요」 판정이 어긋난다).
  세션이 없으니 되감을 것은 카드 상태뿐 — vocab-words 무효화로 「복습 시점이에요」가 저절로
  돌아온다. 키는 R1 리스너에 합류: 저장 그리드(!isWordSaved)와 인라인 그리드(isWordSaved ∧ due)는
  **상태상 배타**라 한 리스너가 보낸다, ⌘Z는 인라인 → 저장 순. ㉯ `QuestReview.jsx` 한 파일:
  스냅샷(idx·right·prev) · undo(idx·flipped·right·phase 되감기, **버스 신호는 재방출·취소 없음** —
  소비처가 연출·펫 재조회(읽기)라 되돌릴 쓰기가 없다) · 다이얼로그 마운트 동안만 사는 키
  리스너(처리한 키는 `stopPropagation`으로 Phaser window 리스너와 경쟁 차단). 씬·연출·펫·버스
  무접촉(계약이 `undo_of|SAVE_GRADES` 부재를 잡는다). 보드 🧊 예외 줄 동봉. 계약
  `inlineReviewUndo.test.js` 5종 + `questReviewUndo.test.js` 5종 · 옛 핀 1본(`useInlineReview`
  이벤트에 created_at). 변이 검사 동반. 잔여(설계대로 비범위): 두 지점 오프라인 큐 도입 · 펫
  count +1 잔류.
...
- **🃏 W 저장 등급 R1 — Anki식 4등급 저장 + 키 1~4 + Ctrl/⌘+Z undo, 「이미 알아요」 쓰기 일몰
  ...
  첫 등급이 D를 영구히 가른다(1 다시 6.41 · 4 쉬움 1.00, 2회차부터 20배). ① `buildVocabRow`에
  선택적 `grade` — 변환은 `calculateFSRS` 재사용(손계산 금지), 「다시」 특례만 `next_review_at`
  오늘(최소 간격 1일 반올림 때문에 1·2가 둘 다 「내일」이라). **`last_reviewed_at`은 세우지
  않는다** — 세우면 복습 카드가 되어 한도 없는 큐로 직행(「쉬움」 500개면 8일 뒤 500개).
  무등급(인라인 목록·10경로)은 **한 바이트도 안 바뀐다**(JSON 문자열 핀) ② 카드 4버튼은
  `SAVE_GRADES`를 돌며 복습 화면 `.review-score-btn--*` 재사용 — 계약이 `ScoreSection` 소스와
  라벨·순서·접미사를 대조한다(같은 이름이 같은 값이면 충돌이 존재하지 않는다). 숫자 배지 +
  부제(오늘 또·내일·2일 뒤·8일 뒤) ③ 키 `1`~`4`: 문서 리스너 하나(VocabPage 관용구), 가드 =
  입력 요소·조합키·카드 닫힘·이미 저장 ④ undo: upsert에 `.select('id')` — `ignoreDuplicates`라
  **새로 넣었을 때만** `[{id}]`, 그 한 행만 delete(단일 레벨). 입력 요소 안 ⌘Z는 브라우저
  기본 undo 무간섭. 단어가 바뀌면 소멸(저장 직후 자동 닫힘은 저장 동작의 일부라 유지). 토스트
  `"咖啡" 저장됨 · ⌘Z 취소` ⑤ 「이미 알아요」 쓰기 일몰 — 뷰어 `markKnown` 호출 0, 이미 known인
  단어의 「취소」만. 읽기(커버리지·wordState·「모르는 단어만」) 불변. 계약 `saveGrade.test.js`
  10종 · 옛 핀 3본 갱신(`knownWords` 토글 → 취소만 · `wordCardOrder` 줄 수 · e2e features-r2
  토글 왕복 → 4버튼 존재, CI 밖 스위트). 변이 8/8 검출. 알려진 잔여: undo가 `saveCountRef`·
  활동 기록은 되돌리지 않는다(카운트 토스트용) · 토스트 액션 버튼은 API 부재라 비범위.
- **🃏 단어 카드 R2 — 이합사 표제어를 기본형으로, 순서 뜻→日→예문→유의어→한자, 액션 2단
  (2026-09-02, 오너 「작업 개시 우선순위대로」 — #1077 5504878570, 목차 2순위)**:
  오너 보고 「카드 머리에 동사 · 道歉」 — 품사 오염이 아니라 **「품사 · 기본형」 연결이 겸류
  구분자(` · `)와 같은 모양**이었다. 검토에서 더 큰 결함이 나왔다: 이합사 조각을 탭하면
  **표제어(道·「길 도」)와 뜻(사과하다)이 다른 단어** — 뜻·유의어·예문·日 대응은 `道歉`의
  것인데 표제어·훈음만 표면형이었다. ① 표제어 = `selectedLexKey`(sep_link ?? base_form)로,
  탭한 구간만 강조(`headwordPick.pickedRangeOf` — 포함이면 그 자리·아니면 공통 접두·아니면
  없음, 억지 매칭 금지) · 기본형 읽기는 사전 `reading`(token-dict 조회를 표면≠기본형에도
  켠다, 조회 완료 후 없으면 루비 없이 + 메타 「기본형 …」 라벨 폴백) · 훈음 루비·`--fit-n`도
  기본형 기준 ② 메타 줄 `· base_form` 제거(중의성 소멸, 품사·급수 자리 고정) ③ 순서 오너
  확정: 뜻 → 日(≒ 대신 「≠ 다른 단어」 부제, jaRef.js 불변) → 예문(복습 카드 정본 헬퍼
  `splitSentenceAroundWord`로 `<mark>`, 삽입형은 term null이라 강조 0) → 유의어(라벨을 칩
  컨테이너의 형제 캡션으로 — 인라인 라벨의 둘째 줄 들여쓰기 어긋남 해소) → 한자 노트
  ④ 액션 전폭 4단 → 2단([이 문장에서는? | 상세 설명] · [저장 | 이미 알아요], W R1이
  둘째 줄을 4등급 그리드로 교체). 토큰 데이터·저장 행 무변경(R4a·저장 계약 무회귀 —
  계약이 저장 경로에 headText 부재를 잡는다). 계약 `wordCardOrder.test.js` 10종 +
  `headwordPick.test.js` 5종 · 옛 핀 2본 갱신(`synAntWiring` 위치·`wordCardFit` 분모).
  allowlist 밖 신설 1: `src/lib/headwordPick.js`(순수 헬퍼 — 컴포넌트 안에 두면 단위
  검증 불가). 변이 8/8 검출. ✏️ 행 전체 탭 타깃(⑤ 작은 것)은 미착수.
- **🎨 지정 칠 문법 통일 — 안 A: 알약 문법을 공통으로, 연속 구간은 안쪽 모서리만
  (2026-09-02, 오너 「작업 개시 우선순위대로」 — #1077 5503687165, 목차 1순위)**:
  오너 보고 「지정 전과 배경색 칠해지는 모양새가 다르다」의 실체는 셋이었다 — 모서리
  5px→1px · 좌우 확장 0→±1px · **인접 칠 사이 4px 틈이 완전 연결**(낱개 넷이 한 덩어리).
  08-30 「수용된 트레이드오프」 주석은 앞의 둘만 적었고, 눈에 제일 걸리는 셋째가 빠져 있었다.
  원인은 순서 — 밴드(08-29, 지정용)가 먼저고 알약(08-30, 상태용)이 나중이라 두 결정의
  문법이 달랐다. **단순 통일은 둘 다 배제**(상태→밴드: 인접 하이라이트가 이어져 보이는 회귀
  실측 / 지정→낱개 알약 = 안 B: 「여기까지 골랐다」가 안 읽힘). 안 A = **같은 문법으로**:
  ① 기반 `::before`를 알약(확장 0·5px)으로 올림 ② `--picked:has(+ --picked)`의 오른쪽,
  `--picked + --picked:has(.surface)`의 왼쪽 모서리만 0(버튼 그룹 관용구 — 뒤 규칙의
  `:has(.surface)`는 항상 참, 미지원 엔진에서 둘과 이음매가 함께 빠지게 하는 장치)
  ③ 이음매를 `[W-1px, W+gap+1px]`로 넓혀(칠 확장 0의 보상) 실금 0 ④ `--hl` 블록의
  `left/right 0·5px` 오버라이드는 죽은 규칙이라 삭제. 결과: **낱개 단어 지정은 색만
  바뀐다**(08-29 원래 계약 「형태·범위 불변」이 비로소 성립), 구간 지정은 하나의 긴 알약,
  비지정 4px 틈은 그대로. 대가: 킨들 톤 각형(08-29 레퍼런스)을 뒤집음 — 실기에서 거슬리면
  3px 절충(계약은 「비지정과 같은 값」). 계약 `pickedBandGrammar.test.js` 6종 + 옛 핀 2본
  갱신(`hlBand` ±1px/알약 오버라이드 · `pickedEffect` 이음매 폭 — 설계 allowlist 밖이나
  옛 문법을 핀하던 계약이라 같이 옮겼다). 변이 5/5 검출(±1px 복원 · 이음매 물림 제거 ·
  왼쪽 모서리 규칙 삭제 · --hl 오버라이드 부활 · 지정 규칙이 모서리를 다시 씀).
  vitest green · lint 0. CSS만 — JS·데이터 무변경.
- **🈁 분석기 라운드 10 — ja 복합명사 재병합 (2026-09-02, 결정 항목 ④ · 오너 「추천대로 ㄱㄱ」)**:
  kuromoji(IPADIC)는 복합명사를 형태소로 낸다 — 映画館 → 映画|館, 誕生日 → 誕生|日, お皿 → お|皿, 天気予報 → 天気|予報.
  어휘 정답지 대조(표제어 7,299의 자기 예문 토큰화)에서 미생존 528(7.2%) 중 복합명사 부류 ≈95종, 읽기도 틀린다(誕生|日의
  日 ヒ·リア|充 タカシ — 결합 읽기 ≠ 정본 106/289).
  · **정본 읽기 색인의 역색인으로 합친다**(jaCompoundMerge): 라운드 4가 가나 분절에 쓴 jaYomiIndex를 거꾸로(표면 → yomi)
    걸어, 인접 명사류 창(2~3 토큰)의 표면 결합이 표제어이고 yomi가 하나면 한 토큰으로 — 읽기는 색인 값. 예문 전수 병합
    자리 289: 역색인 유일 287 · 다독 0 · 없음 2(〜時間目·〜丁目).
  · **정밀도** — 수사 창은 제외(三時|十|分의 十分은 じゅっぷん인데 표제어 十分은 じゅうぶん(충분히) — 실측이 잡은 유일한
    함정; 수사+조수사는 라운드 3 몫), 조사 낀 구(男|の|人)는 안 합침, 표제어 아니면 무개입(今日中), 다독이면 무개입.
  · 결과: 표제어 생존율 **0.9277 → 0.9412**(+99), 품사 일치 0.986, 요미 완전 일치율 0.9726 → **0.9740**(회귀 0). 남은 미생존
    429는 표제어 표기 차이(어휘 いぬ·예문 犬, 204)와 〜표기(〜分·〜時, 100)가 주류 — 분절이 아니라 조회 쪽 과제.
  · 계약: jaCompoundMerge 5종(합침·읽기·3토큰·수사 창·조사·함수 형상) + **jaCorpusVocab**(생존율 ≥0.935·품사 ≥0.97·핀 2) 신설,
    jaCorpusYomi 주석 갱신. 변이 4/4 검출(호출 제거·수사 제외 제거·읽기 결합값·조사 허용) · vitest 전량 green · lint 0.
- **🈁 오너 결정 3건 이행 — 경성 등재 8항 · 콘텐츠 경성 45줄 교정 + 게이트 · 품사 라벨 통일 (2026-09-02, 오너 「추천대로 ㄱㄱ」)**:
  라운드 7·9 보고의 결정 표 세 개를 권고안대로 닫았다. 한 PR, 커밋 셋.
  · **① 경성 사전 예외 등재 8항**(zhNeutralTone HAND): ② 예외 妻子(정답지 4:0 — 타의미 주형 妻子儿女는 라운드 8이 妻子|儿女로
    가르고 갈린 妻子는 「아내」라 경성이 맞다) + ③ 예외 융합 N+上 7항(墙上·地上·山上·桌上·身上·脸上·天上 — 정답지 36:0,
    융합 토큰 한정: 단독 上은 경성률 31%라 규칙 아님). 地方·多少·起来·外边은 배제 유지. 기준 주석에 예외 근거를 박았다.
  · **② 콘텐츠 경성 교정 45줄 + 게이트**(scripts/zh-neutral.mjs, 성조 변조 게이트와 같은 구조): 필독 경성 어휘 10(任务 ×9·
    队伍·答应·聪明·尾巴·衣服·眼睛·码头·耽误·下巴) · 가능보어 가운데 不/得 3(看得见·放不下·看不见) · 경험상 过(앞이 닫힌
    동사 27자·뒤가 过来/过去/过年류 아닐 때, 13줄). 예행 45건 전수 눈검사 오탐 0 → 적용 → 재실행 0(수렴). 린트가 부르고
    위반은 오류. 표는 오너 승인 항목으로 닫혔다(크기 핀).
  · **③ 품사 라벨 통일 43건**: 개사→전치사 31·대사→대명사 12(뷰어 라벨 POS_KO와 같은 이름 — 4개 언어 공통 어휘). 린트 규칙
    「zh pos에 개사·대사 금지」 + 계약(콘텐츠 0건). 어휘 정답지 품사 일치율 0.8550 → **0.8602**.
  · 병음 완전 일치율 0.9412 → **0.9464**(문턱 0.93 → 0.94 상향). 변이 4/4(콘텐츠 한 줄 되돌리기·라벨 하나 개사·등재 한 줄·
    게이트 过 규칙 — 전부 검출) · 콘텐츠 린트 0 errors · vitest 전량 green · lint 0.
- **🈁 분석기 라운드 9b — 실단어 방벽 불변식 (2026-09-02, 라운드 9 정답지 대조의 부산물)**:
  어휘 표제어 6,985개가 자기 예문에서 어떻게 토큰화되는지 전수 분류하니(단일 토큰 6,034 · 흡수 767 · 정확 분할 157 ·
  걸침 19) 우리 규칙이 **HSK 실단어를 부수는** 자리가 둘 있었다 — jieba 사전에 없는 HSK 어휘를 HMM이 x로 달면 x-블록
  되가름이 微信을 微|信으로(HSK 3.0 등재인데 허용목록만 봤다), 정도부사 규칙 ⑤가 真诚/a를 真|诚으로(머리말은 「진짜
  표제어를 부수지 않는 쪽」이라고 써 놓고 방벽을 안 걸었다). 코퍼스 10건/4종(不太/x ×7·别看·微信·真诚).
  · 두 규칙에 `isRealWord` 방벽을 걸었다 — 不太(교재 H2 표제어)도 이제 한 토큰이고 줄 병음이 변조(bú tài)를 이미 처리한다.
  · **불변식 계약**: 「jieba가 한 토큰으로 낸 HSK 실단어(≥2자)를 후처리가 가르는 경우 = 0」(zhCorpusSegment). 규칙이
    하나 늘 때마다 이 줄이 방벽을 대신 확인한다. x 폴백 게이지는 후처리가 **만든** x 조각만 세도록 좁혔다(방벽이 지킨
    raw x 실단어는 제외).
  · 실측 분류 중 걸침 19건(关机 → 机关|机·大海边·止咳药|水·出|门前·播种|子)은 jieba DAG의 탐욕 경계 오독 — 일반 규칙
    없이 두고 기록. 우리 규칙이 가른 교재 표제어 14 중 11은 ⑦(踢足球·打篮球·车牌号码류 — 정답지 공백 17:0으로 두
    단어, 드래그 범위 지정으로 카드 복구)이라 보호하지 않는다.
  · 일본어 같은 대조(표제어 4,000 표본): 품사 일치 **0.984**(불일치 56은 연체사/명사·부사가능 명사 등 분류 차이),
    표제어 미생존 360(9%) — お皿→お|皿·映画館→映画|館·誕生日→誕生|日·朝御飯 같은 kuromoji 복합명사 과분할이 주류.
    다음 라운드 후보(정본 읽기 색인으로 재병합) — 계약 없이 기록만.
  · 계약 2종 신설(단위·불변식) · 변이 2/2 검출 · vitest 전량 green · lint 0.
- **🈁 분석기 라운드 9 — 품사: 어휘 정답지 대조 (2026-09-02, 라운드 7·8에 이어 「바로 진행」)**:
  병음·분절엔 정답지가 있었는데 품사엔 없다고 여겼다 — 있었다. 어휘 표제어 6,985개마다 사람이 적은 `pos`와 자기
  예문이 있으니, 예문을 토큰화해 그 표제어 토큰의 품사를 대조하면 된다. 단일 토큰으로 살아남는 표제어 6,034(86%),
  그중 일치 5,042(**0.8356**, 겸류 후보 경유 232 포함) · 불일치 981.
  · **불일치의 절반은 분류 체계 차이**라 손대지 않았다: 콘텐츠 명사 ⊃ 시간사(今天 104)·방위사(前面 60)·처소사(家里 45),
    개사=전치사(21)·대사=대명사(10)는 같은 것의 두 이름(콘텐츠 자체가 개사 31·전치사 12로 갈린다 — 오너 결정 항목으로
    보고), 구별사(常用·公共)·상태사·어기조사는 jieba가 더 잘게 가른 것.
  · **실오류 두 부류만 고쳤다** ① jieba 사전이 V-O 이합사를 n으로 싣는다(走路·跑步·唱歌·上学·生病 — 코퍼스 220건,
    정답지 65건 중 콘텐츠 동사 55). 이합사 표(HSK ∥ 마커 532 + 수제)가 이미 이들을 알고 있다 — **표 가입 + n → 동사
    기본·명사 후보**(结果·讲话는 명사로도 쓰여 판별기가 짚는다). gain 55 · loss 0. ② 사전 태그 자체가 틀린 단어(문맥
    태그 = 고립 태그 — 一起/m·红绿灯/nr·回国/ns·高兴/b·过年/t): 정답지가 있는 51개만 수제 POS_FIX에. 성씨 겸용 1자
    (金·熊·钟·云·富·蓝·帅·咸)는 金先生의 金이 nr이 맞아 정밀도 원칙상 뺐다.
  · **버린 가설 하나**: 「문맥 태그가 고유명사면 고립 태그로 되돌린다」 — 재보니 고립 태그가 문맥 태그보다 나쁘다
    (n→nr 62 loss·v→n 97 loss). 측정이 없었으면 실었을 규칙이다.
  · 결과 **0.8356 → 0.8550**(+117). 계약: zhPosVocabGold 6종 + zhCorpusPos에 정답지 일치율 문턱(0.84)·표제어 단일 토큰
    생존율(>0.8 — 분절 규칙이 표제어를 부수면 잡힌다)·핀 4종. POS_FIX 개수 핀 42 → 93. 변이 6/6 검출 · vitest 전량 green · lint 0.
  · 남긴 것(오너): 품사 라벨 어휘 통일 — 콘텐츠 개사/전치사·대사/대명사 혼용(POS_KO는 전치사·대명사인데 중국어 교육
    관례는 개사·대사) → 어느 쪽으로 맞출지. 표제어가 예문에서 여러 토큰에 걸치는 943건(1자 517·2자 291)은 대부분 정상
    흡수(喝茶의 茶·今年의 年)이고, 우리 규칙이 가른 표제어(踢足球·打篮球·不太·请进)는 라운드 8의 의도.
- **🈁 분석기 라운드 8 — 분절: 비실단어 융합 토큰의 두-실단어 되가름 (2026-09-02, 라운드 7 보고의 「분절 라운드 감」 이행)**:
  jieba 사전은 구(句)를 단어처럼 싣는다 — 看电视·天气预报·十分钟·回家吧가 한 토큰으로 와서 학습자에겐 看·电视가
  숨고 공유 사전엔 「看电视」 표제어가 쌓인다(5b가 청소한 두 글자 쓰레기의 세 글자판). 코퍼스 45,612토큰 실측:
  **비실단어 ≥3자 1,577건(3.5%)**.
  · **정답지를 분절의 정답지로 썼다** — 예문 병음은 단어 단위로 띄어 쓰므로(RFC §1) 가른 자리에 정답지도 공백을
    뒀는지가 곧 대조다. 두 HSK 단어로 **유일하게** 분해되는 토큰만, 모양 셋 — (a) 둘 다 ≥2자(432건, 불일치 **0**)
    (b) 1자 접두+단어(看|电视·好|消息) (c) 단어+조사·방위 1자(回家|吧·操场|上). 단어+명사 접미 1자(垃圾桶·科学家·
    咖啡店)는 정답지가 붙여 쓰는 파생 복합어라 안 받는다. 유일성은 **모든** 분해에 대해 — 火车站은 火|车站·火车|站
    둘 다 성립해 모호 → 유지(느슨한 유일성 21 불일치 → 엄격 12). 수사 머리(二~十·两·几·百千万)는 우선(十|分钟 —
    十分|钟도 성립하지만 수량구; 一·半은 一路上·半路上이 반대라 제외). 3자 가운데 不·得(行不行·吃不完)는 한 구조라
    제외 — 계약이 잡았다(行|不行으로 갈리자 라운드 7의 不 경성이 죽었다).
  · 결과: **가름 678 · 정답지 공백 동의 672 : 3**(99.6% — 불일치 셋은 着를 붙여 쓴 정답지) · 잔여 융합 1,577 → 920.
    병음 완전 일치율 0.9386 → **0.9412**(好消息 → 消息 xiāo xi처럼 경성 사전이 조각에 다시 걸린다). 남는 오분리
    7종 9건(小孩子·照相机·班主任·日用品·初学者·发动机·好容易)은 ZH_KEEP_MERGED에 실측과 함께 실었다 — 방향의
    근거는 그 파일 머리말의 복구 비대칭(오분리는 드래그로 되붙이고 오병합은 분리 액션이 없다). 경성 사전 표제어
    보호는 넣었다가 뺐다 — 변이가 살아남아 재보니 그 가드가 붙잡는 7건 중 4건(民间故事·门把手·指甲剪·坏东西)을
    정답지가 가르고 조각이 경성을 지킨다(故事 gù shi). CEDICT는 구도 싣는 병음 사전이지 어휘 단위의 권위가 아니다.
  · 조각 태그는 jieba에게 다시 묻는다(pushClassifierPiece 선례) — 分钟은 q(양사)로 온다. jieba가 조각을 홀로 다시
    가르면 x 폴백(胡说·恩人 2건, 품사 미상 상한 5 → 8).
  · 계약: zhCompoundSplit 10종(가르는 쪽 4·두는 쪽 6) + **zhCorpusSegment**(정답지 공백 동의율 ≥0.97·잔여 ≤1,050·
    x 폴백 ≤10) 신설, zhSuppress(身体健康 → 身体|健康)·zhPinyinContext(各个)·zhCorpusPos(미상 상한 8) 개정. 드릴 refs
    산출물은 신선도 계약이 잡아 재생성(분절이 바뀌면 낡는 것이 설계). 변이 10/10 검출(2종은 처음 살아남아 규칙을 고쳤다) ·
    vitest 전량 green · lint 0.
- **🈁 분석기 라운드 7 — zh 경성 정합 (2026-09-02, 오너 「작업 리스트 확인하고 바로 진행」)**:
  경성 사전이 내는 경성과 정답지(예문 병음 8,388문장 정렬)의 표기를 전수 대조하니 어긋남이 두 방향이었다 —
  분석기가 정답지보다 **더** 경성으로 내는 쪽(这里 99:1·知道 33:0·别人 25:9·打算 12:0·太阳 9:1 … ≈250줄)과
  **덜** 내는 쪽(每个 32·墙上 10·地方 10·多少 7 … ≈110줄). 里 오독 116 중 99가 这里 하나였다.
  · **판정 규칙을 세우고 그대로 적용**: 정답지 다수가 원조(n≥3)이고 리포 안 근거가 하나뿐(수제 단독·CEDICT 단독)이면
    원조로 돌린다. pinyin-pro와 CEDICT가 함께 경성인 항목(任务·队伍)은 지킨다 — 그쪽은 콘텐츠 후보. 수제에서
    这里·哪里·知道·打算 4종 제거(라운드 1의 这里 추가는 那里(CEDICT)에 맞춘 것인데 정답지·pinyin-pro·CEDICT 셋이 전부
    원조라 반대로 맞췄다), CEDICT 층 억제 16종(那里·别人·太阳·真是·照顾·道理·大人·值得·记住·后面·力量·欺负 + 사전 근거
    马马虎虎·好久不见·刚刚·学生会). 억제는 JSON을 안 건드리고 병합 때 뺀다(생성 결정성) — 되돌리기는 한 줄.
  · **결정적 규칙 4종을 문맥 층에** — 사전이 아니라 규칙인 것들: ① 个 양사 자리 → ge(단독 个 정답지 357:0, 융합
    每个·第一个·上个月·打了个 54:0; 整个·各个 22:0 원조) = 오너 결정 「个 양사 자리는 경성」의 분석기 쪽 ② 동사 중첩
    VV 둘째 음절 경성(想想·试试 29:0; 부사 중첩 常常·等等/u는 태그로 제외) ③ 3자 V不C·A不A 가운데 不 → bu(吃不完·
    行不行; 실단어·4자 성어·부사 융합 绝不会/l 제외) ④ 단독 不의 변조를 이웃의 **최종** 첫 음절로 재계산(不应该
    bú→bù ×6, 不为…所 ×2 — 라운드 1 为 규칙이 만든 이음새). 토큰 정확 일치 층에 一切·一等奖(단어 내부 一 변조 누락)·
    马马虎虎(hū)·谁(shéi, 정답지 18:0) 4항.
  · **계약의 가면을 벗겼다**: 코퍼스 계약이 一·不·个를 성조 지우고 비교하던 「관례 중립화」가 실은 base가 yi/bu/ge인
    **모든** 음절(意·以·部·各…)을 가리고 있었다. 해제하니 완전 일치율 0.8664(같은 기준의 전) → **0.9386**(중립화
    기준으로는 0.9186 → 0.9423). 글자별 상한 9종 추가(个·一·不·里·道·算·阳·人·谁) — 규칙이 죽거나 这里가 돌아오면
    그 글자가 수십~백 단위로 튄다.
  · 남긴 것(#1077 보고에 표): ⑴ 기준 ②·③ 오너 결정 — 정답지 100% 경성인데 사전 기준이 막는 地方 10·多少 7·妻子 4(다의)
    ·墙上류 N+上 36·起来류 방향보어 15·外边 3 ⑵ 콘텐츠 후보 — 사전·라이브러리가 맞고 정답지가 원조인 任务 ×9·경험상
    过 guò ×15·队伍·放不下 등 ≈40줄 ⑶ 교차 토큰 V不C(听/不/清楚 — 정답지 자체가 看不清楚를 bu·bù 양쪽으로 씀)·2자
    不C 융합(买/不到) ≈20줄 보류 ⑷ 융합 토큰 분절(好消息·优秀学生·讲故事 — 사전 단어를 품은 비실단어 토큰 ≈25줄)은
    분절 라운드 감. 里/上 단독은 정답지 경성률 5%/31%로 규칙이 아니다.
  · 계약 5종 신설·3종 개정 · 변이 12/12 검출 · vitest 전량 green · lint 0.
- **🈁 名 양사 추가 (2026-09-02, 오너 「名 양사 추가 ㄱㄱ」)**:
  라운드 5b가 `百名`을 못 가른 이유 — 사람 세는 양사 名이 닫힌 양사 집합에 없었다. 코퍼스에서 一名 18·
  二十名 2·几名 2·三十名·一百多名이 통째로 남아 양사가 어휘 항목으로 안 보였고, 이합사 회랑 `有三十名学生→
  有名` 오탐의 뿌리이기도 했다(가드는 앞 PR에서 먼저 넣었다).
  · **파급면을 전수 diff로 쟀다.** 집합은 지시사+양사·양사+명사·수사+양사·이합사 회랑 네 규칙이 같이 쓴다.
    넣기 전후 코퍼스 8,533문장 토큰을 통째로 대조하니 **변경 23줄 전부 수사+名 갈림**(一名·二十名·几名·
    三十名)이고 名+X 어휘는 한 줄도 안 건드렸다 — 名字·名单·名片은 HSK 방벽, 名牌·名画는 표 밖인데도
    「수량 자리 뒤」 조건이 지켰다. 那名球员은 이미 那/名/球员로 갈려 있었고 名이 양사로 찍힌다.
  · 남긴 것: `多名官员`은 多가 수사 집합 밖이라 그대로(1건). 5b 계약의 HSK 사정거리 목록(半场·零件·一头)은
    名을 넣어도 불변 — 수사+名인 HSK 표제어가 없다.
  · 집합 파일 주석이 없는 파일(`zhQuantifier.test.js`)을 가리키고 있어 실제 계약(zhTokenBoundary·
    zhNumClassifier)으로 정정 — 낡은 포인터는 게이트를 두는 이유다.
  · 계약 1종(一名·二十名·那名 갈림 + 名+X 7종 불변 + 회랑 이음새) · 변이 1/1 · vitest 3,400 green · 서버 계약 291 · lint 0.
- **🈁 이합사 O 연결 + 회랑 게이트 교체 (2026-09-02, 오너 보고 「到了歉에서 到歉 서로 연결되어야」→「권장대로 ㄱㄱ」)**:
  `道了歉`(사과했다)이 두 층에서 어긋나 있었다. ① `道`가 회수되지 않았다 — jieba가 이 자리의 道를
  양사(q)로 달았는데 회랑 게이트 `SEP_V_TAGS`(v·vn·vd·x·f·n·m)에 q가 없어 회랑이 안 열렸다. 같은
  단어가 삽입 성분에 따라 됐다(道个歉/x) 안 됐다(道了歉/q) 했다. ② 회수돼도 `歉`은 base_form `歉`인
  독립 토큰이라 뷰어가 낱글자로 사전을 조회했다 — 운영 DB에 `歉` 행이 쌓여 있던 게 그 증거.
  · **게이트를 표 가입으로 바꿨다.** 표에 있는 V 글자 + 회랑 화이트리스트가 실질 게이트이고 태그는
    一幅/d 선례처럼 오태그가 잦아 근거가 못 된다. 단 **조사 태그(u*)는 V가 아니다** — 중립 스위프가
    바로 잡았다: `他得了到北京的机会`의 得/ud…到가 표의 `得到`(HSK 수확층)에 걸렸다. 옛 태그 게이트는
    이걸 우연히 막고 있었던 것. 명시 가드로 바꿨다.
  · **O 토큰에 `sep_link`(VO) 표식.** base_form은 자기 글자 그대로 둬서 만남·FSRS는 V가 한 번만
    남기고(이중 계상 방지 유지), 뷰어는 표식이 있으면 카드·조회·저장·「이미 앎」·호(arc)를 VO로 보낸다
    (`selectedLexKey = sep_link ?? base_form`). 歉을 눌러도 道歉 카드가 뜨고 道로 호가 그어진다.
    사전 수확기(`collectMissingBaseForms`)는 표식 토큰을 건너뛰어 낱글자가 공유 사전에 더 안 쌓인다.
    라우트는 O에 VO의 사전 항목을 붙여 보낸다. 수량구 캐리어(一架/m)는 그 자체가 학습 단위라 표식 없음.
  · **덤 — 원래 있던 오탐 4건.** 변경 전후 코퍼스 회수 목록을 대조하니 차이 0(道了歉은 코퍼스에
    없어 단위 계약으로만 검증). 그 목록에서 `班里有三十名学生→有名`(유명하다)이 보였다 — 名이 「30명」의
    양사인데 有…名이 표의 有名에 걸린 것, 코퍼스 4문장. **양사 자리의 O 후보 뒤에 명사가 붙으면**
    (三十名|学生) 목적어가 아니다 — 이합사의 O는 문장을 닫는다(吵了一架|。). 가드 후 대조: 사라진 회수
    = 그 4건뿐, 새로 생긴 회수 0.
  · 계약 6종 신설(태그 불문 회랑·sep_link·캐리어/연속형 무표식·조사 가드·양사 자리 가드·코퍼스 불변식
    「짝 없는 표식 0」) + 수확기 O 제외. 변이 6/6 검출(서버 5 + 뷰어 배선 계약 1) · 서버 계약 290 · vitest 3,399 green · lint 0.
  · 남긴 것: 대명사·형용사 삽입(帮我一个忙·救了他的命·做个好梦 — 코퍼스 6건)은 회랑 화이트리스트를
    넓혀야 해 오탐 면이 같이 넓어진다 — 정밀도 원칙상 보류. `天上掉下几滴雨→下雨`(방향보어 下)도 1건 보류.
- **🈁 zh 성조 변조 표기 통일 — 一·不 변조 + 个 경성 (2026-09-02, 오너 「1,2,3 다 추천대로 ㄱㄱ」)**:
  분석기 리뷰가 남긴 오너 결정 3건의 이행. ① 一·不 변조 **표기로 통일** ② 个 양사 자리는 **경성 ge**
  ③ 예문 병음 음절 띄어쓰기 2,329건은 **현행 유지**(RFC-zh-pinyin-orthography §3이 예문을 범위 밖으로
  명시 — 규범화하려면 단어 경계가 필요한데 그건 분석기 산출이라 정답지가 분석기를 베끼는 순환).
  · **정답지가 이미 대세를 정하고 있었다.** 실측: 一 변조 표기 1,038 대 미표기 231, 不 228 대 76,
    个 경성 742 대 gè 218. 소수파를 다수파로 맞추는 작업이지 새 관례 도입이 아니다.
  · **전수 정렬로 고쳤다** — 한자↔음절 위치 정렬 후 **다음 음절의 성조를 정답지 자신에게서 읽어**
    규범 표기를 계산한다. 457건 치환(个 경성 134종·一 152종·不 69종). 재실행하면 0건 = 수렴 확인.
  · **예외는 추측이 아니라 전수 실측에서 뽑았다.** 一가 끝음절인 어휘는 콘텐츠 표제어를 전수로 훑어
    9종(万一·之一·初一·单一·同一·唯一·如一·第一·统一) + 周一·十一. 个가 양사가 아닌 어휘는 160종
    문맥에서 7종(个子·个体·个性·个别·整个·各个·个人).
  · **규칙이 네 번 틀렸고 네 번 다 실측이 잡았다.** ⑴ A不A를 글자 일치로 보니 `热得不得了`(de/dé)가
    걸림 → 음절 성조까지 일치할 때만 ⑵ 앞 글자 예외가 다른 단어에 걸림(`四周|一片`·`总之|一切`)
    → 검증한 문맥만 명시 배제 ⑶ 문장 첫머리 대문자(`Bùyào`)가 규칙에 안 걸리고, 고치니 이번엔
    대문자를 소문자로 낮춤(삼항 반환에 보존 함수 누락) ⑷ `个子` 예외가 `两个子女`의 양사 자리를 덮음
    → 양사 자리 우선. 관용구(一路顺风·一如既往·一模一样)는 표준 변조가 곧 사전 표기라 예외가 아니다.
  · **게이트로 심었다.** `scripts/zh-sandhi.mjs`(규칙 정본) + `lint-curriculum (h)` 배선 + 계약
    `zh-sandhi.test.js`(규칙 단위 + 콘텐츠 전수 위반 0). 한 번 고치고 끝내면 다음 저작에서 다시
    갈리므로 — 라운드 6의 「트리 ⊆ 정규식」과 같은 자리.
  · 변이 5/5 검출(변조 방향·A不A 조건·양사 우선·끝음절 예외·콘텐츠 되돌림) · vitest 3,391 green ·
    prebuild 게이트 0오류 · lint 0.
- **🈁 분석기 라운드 5b + 공유 사전 위생 (2026-09-02, 오너 「지금 해줘」)**:
  운영 DB 감사(오너 수동 적용)가 두 가지를 드러냈다. `morpheme_dictionary.language` CHECK는
  **이미 수동 ALTER가 돼 있었고** 중국어 154행이 실제로 쌓여 있었다 — v2-T 설계의 「적재된 적 없다」는
  전제가 틀렸다. 그 154행을 현재 분석기로 전수 판정하니 사문서 3 + 기호 4, 나머지 147은 정상 캐시.
  · **캐시 도달성 판정.** 뷰어는 선택 토큰의 `base_form`으로만 조회하므로, 분석기가 더는 만들지 않는
    표제어의 행은 영원히 안 읽히는 사문서다. 판별 중 `报到`가 사문서로 잡혔는데 **오탐**이었다 —
    분리된 `报` 토큰이 `base_form=报到`를 달고 있어 그 키로 조회된다. 이합사 회수 경로까지 보도록
    판별기를 고쳐 오탐 1건을 걷어냈다(패턴 삭제가 아니라 전수 판정으로 간 이유).
  · **라운드 5b — 수사 머리 확장.** 라운드 5 규칙이 두 글자만 봐서 `三十个`·`五十六个`·`一万块`가
    통째로 남았다(코퍼스 20건/14종). 大수사(百千万亿)와 零을 수사 집합에 더하고 머리를 **한 토큰으로**
    담는다(`三十`이 수사 한 단어이지 `三`+`十`이 아니다). 이 패턴에 걸리는 HSK 표제어는
    半场·零件·一头 셋뿐이고 전부 방벽이 지킨다 — 계약이 그 셋을 전수로 못 박아 표가 자라면 걸린다.
    라운드 5 주석의 「一块는 방벽이 지킨다」는 **틀린 예시**였다(표에 없어 실제로 갈린다) — 정정.
  · **기호가 공유 사전에 새고 있었다.** 분석기의 모든 토큰이 `base_form`을 달아 문장부호도 예외가
    아닌데 `/api/analyze`가 거르지 않았다 — `。`·`，`의 뜻을 Gemini에 묻고 그 답을 전 사용자 공유
    사전에 적재한다(코퍼스에 base_form 가진 기호 13종·`。`만 8,191회). 지우기만 하면 다시 쌓이므로
    원천을 막았다. 판정은 뷰어가 쓰는 정본 `isWordToken` 재사용 — 같은 질문에 술어를 두 벌 두지 않는다.
  · **이음매를 계약 가능하게 떼어냈다.** 필터를 라우트 루프 안에 두니 「술어가 기호를 거른다」는
    계약할 수 있어도 「경로가 그 술어를 쓴다」를 못 박을 수 없었다(변이가 살아남아 드러남).
    수확을 `dictLookup.js`로 빼자 변이 3종(판정 제거·캐시 무시·중복 무시)이 전부 잡힌다.
  · 변이 4/4(5b) + 3/3(수확기) 검출 · 서버 계약 282 · vitest 3,376 green · lint 0.
- **🈁 분석기 리뷰 라운드 6 — 콘텐츠 게이트 (2026-09-02, 오너 「순위대로 ㄱㄱ」)**:
  리뷰 6순위. 실측하니 전제가 둘 다 달랐다.
  · **zh 병음 게이트는 이미 있었다** — `lint-curriculum (h)`가 zh·pinyin 쌍의 문자·음절 수(儿화 보정)를
    전수 검사한다. 대신 **사각 406쌍**: 소스를 정규식으로 읽는데 챕터 예문 파일 7본(`h1~h6·ot _examples.js`)이
    JSON식 `"zh":` 표기라 못 봤다. 정규식을 `zh-pinyin-pairs.mjs`로 빼 따옴표 유무를 받고, 「모듈 트리
    16,113쌍 ⊆ 정규식」을 계약(`zh-pinyin-pairs.test.js`)으로 심었다. 새로 보게 된 406쌍에 결함 0.
  · **ja 요미 게이트는 복제본이 낡아 있었다** — `check-content.mjs`가 스토리 대사·노래 한 줄을 자체 복제
    정렬기로 검사했는데, 그 복제본은 한글 병기 요미 「かな (한글)」을 KO_MIXED로 통째 면제했다(원본
    `check-furigana.mjs`는 병기를 떼고 가나를 정렬한다). 스토리 대사 2건이 숨어 있었다: 免税 `5,000えん`
    (요미에 숫자 그대로 → ごせんえん) · 新幹線 `よかったかな…?`(요미에 … 누락). 복제본 82줄을 지우고
    `checkFuriganaData`가 examples+story.body+media.line을 한 정렬기로 순회한다(n5 315·n4 186 예문, 병기 104건).
  · **음절 띄어쓰기 2,329건은 게이트가 아니다** — RFC-zh-pinyin-orthography §3이 예문 병음을 범위 밖으로
    명시(「예문 병음 형식은 현행 유지」). 리뷰에서 「위반」이라 쓴 건 오독. 규범화하려면 단어 경계가 필요한데
    그건 분석기 산출이라 정답지가 분석기를 베끼는 순환 — 오너 결정 항목(个 gè/ge 216:352 혼용·一/不 변조와 한 묶음).
  · 변이 3/3 검출(정규식 따옴표 관용 제거·스토리 순회 제거·콘텐츠 수정 되돌림). prebuild 게이트 0오류 · lint 0 · vitest 3,369 green.
- **🈁 분석기 리뷰 라운드 5 — zh 수사+양사 되가름·很多 (2026-09-02, 오너 「순위대로 ㄱㄱ」)**:
  리뷰 5순위. jieba가 数量词를 한 토큰(m/mq)으로 낸다 — `一个`·`一件`·`两张` 융합 토큰 **636건/72종**(코퍼스
  8,534문장), 정답지는 449건을 갈라 적었다. 학습자에겐 양사(件·张·条)가 어휘 항목이라 숨으면 안 된다.
  `很多`도 154건 한 토큰(정답지 112건 분리) — R3의 「수사+양사는 안 가른다」 방침을 실측으로 뒤집었다.
  · **⑥-c 수사+양사 되가름.** 실단어 방벽(isRealWord) 뒤에서 닫힌 수사 집합(一~十·两·几·半) + 닫힌 양사
    집합 두 글자면 m/q로 가른다. 一下·一点·一起·一样은 둘째 글자가 양사가 아니라 저절로 빠지고, 一块·一半
    같은 HSK 표제어는 방벽이 지킨다. 태그 조건은 안 둔다 — jieba는 一幅를 d(부사)로 단다(코퍼스 5건).
  · **很多는 억제가 아니라 후처리.** 라운드 2에서 억제(add_word 0)하니 很多人×7·多书×5 연쇄가 나 뺐던
    자리 — 후처리 되가름(⑤ 정도부사+형용사)은 DAG를 안 건드려 연쇄가 없다. 多·少가 jieba엔 m이라
    형용사 게이트에 안 걸렸던 것을 두 글자만 명시 허용.
  · **이합사 창 3→4.** 가르면서 O가 +4에 온다(开/了/一/个/会) — 사이 토큰은 여전히 조사·수량구
    화이트리스트여야 하므로 오탐 면은 안 넓어진다(我吵他架·穿过马路 불변).
  · 드릴 refs 신선도 계약이 또 잡았다 — 재생성 408→416(h104-d5 我有两个哥哥에 两·个 표기 등), ja 불변.
  · 회귀 계약 `zhNumClassifier.test.js` — 코퍼스 전수에서 HSK 밖 수사+양사 융합 토큰 0·很多 토큰 0.
    정책 계약 3곳(zhTokenBoundary·zhSuppress) 갱신. 변이 4/4 검출 · 서버 계약 275 · vitest 3,366 green · lint 0.
- **🈁 분석기 리뷰 라운드 4 — ja 가나 문절 분절 (2026-09-02, 오너 「순위대로 ㄱㄱ」)**:
  리뷰 4순위. N5 첫 텍스트가 가나 전용인데 kuromoji의 최악 조건 — `えいが`→え[간투사]+いが, `がくせい`→がく+せい,
  `にほんご`→に+ほん+ご, `くるま`→동사 くるむ, `かれ`→동사 かれる. 명사가 동사로 둔갑해 뷰어에 떴다.
  · **정본 읽기 색인.** 정본 조회는 표제어 키만 알아 「がくせい」가 学生과 못 만났다. 항목마다 있는 `yomi`로
    색인(6,151키·동음이의 664)을 만드니 「문절 = 정본 단어 + 닫힌 꼬리」로 덮이는 문절이 43% → **69.5%**.
    서버가 콘텐츠 트리를 지지 않게 `drillRefs.json` 선례대로 **생성 JSON**(252KB) + 신선도 계약.
  · **부서진 문절만 덮어쓴다.** 덮인 1,308문절 중 kuromoji가 맞는 758은 두고, 조각낸 420 + 동사로 오독한
    130만(코퍼스 실측). 동음이의는 최저 급수가 정답(先生/専制·駅/液·今日/強 — 100건 실측), 동급(雨/飴·橋/箸)은
    기본형을 표면으로 둔다 — 틀린 카드에 잇지 않는다. 접미 2단어(にほん+じん)는 머리가 N5·N4일 때만(勧告+じん 배제).
    띄어쓰기 없는 가나 텍스트는 문절 경계가 없어 손대지 않는다(알려진 한계·계약으로 명시).
  · 드릴 R2의 「정본 고정 문절 매칭」 부품을 뷰어 분석기로 올린 것 — 새 발명이 아니라 교재의 분かち書き를 읽는다.
  · 회귀 계약 `jaCorpusKana.test.js` — 가나 전용 621문장: 조각 지문(문중 1자 간투사·1자 동사 파생)
    **40 → 6**, 내용어 정본 적중 **82.2% → 89.2%**. 문턱 0.87·조각 ≤12.
  · 변이 6종 중 5종 검출. 살아남은 1(1자 머리 허용)은 색인 생성기가 1자 키를 안 실어 루프 가드가 잉여인 경우 —
    색인 불변식(1자 키 0)을 계약으로 못 박아 의존을 드러냈다. 서버 계약 265 · vitest 3,356 green · lint 0.
- **🈁 분석기 리뷰 라운드 3 — ja 요미 수리층 (2026-09-02, 오너 「순위대로 ㄱㄱ」)**:
  리뷰 3순위. 한자 문장 6.5%에 오독 — 범인 1위 `日本`→**にっぽん** ×66, `二人`→ににん ×40, `一人`→いちにん ×19,
  `何です`→なに ×21, `間`→ま ×21, `九時`·`七時`·`十分`. 독해 게이트(`check-reading.mjs`)의 `PLACE_YOMI`·
  `BANNED_YOMI`가 이걸 이미 막고 있었는데 **스크립트 층**이라 뷰어·카드는 몰랐다 — 분석기 층으로 올렸다.
  · **세 모양.** ① 토큰 정확 일치(日本·夜中·大勢·山道·間·物·皆·味·道·来ら·仮名 — 정답 일치 100%인 것만)
    ② 이웃 조건(何+です→なん 12/12 · 명사+不足→ぶそく 7/7 · が/は/で+開く→あく · 今日+中→じゅう · 十分은
    뒤가 な·に·ある면 じゅうぶん 아니면 じゅっぷん 11/11 · 명사+後→ご) ③ **수사+조수사 병합표** — kuromoji가
    二/に+人/にん으로 쪼개는 걸 닫힌 표(음편+불규칙)로 **한 토큰**으로 합친다(二人 ふたり·七時 しちじ·三百
    さんびゃく·二十歳 はたち). 아라비아 숫자(３時 さんじ)도 표가 받는다 — kuromoji는 독음을 안 준다.
  · **실측이 규칙을 세 번 좁혔다.** 者→もの를 토큰 일치로 넣었더니 코퍼스 者 오독이 3→**40**(研究者의 しゃ가
    깨짐) — 동사·たる 뒤로 좁힘. 一日→ついたち를 표에 넣었더니 0→**16**(いちにち '하루') — 月 뒤로 좁힘.
    今→いま가 今学期(こん)를, 後→あと가 十年後(ご)를 깼다 — 이웃 조건으로. 市場→いちば는 4 얻고 8 잃어
    **뺐다**(양쪽 다 맞는 이독은 손대지 않는다는 파일 규칙). 独해 게이트 지명 14개 중 kuromoji가 실제로
    틀리는 건 日本·山手線뿐이라 그 둘만.
  · 회귀 계약 `jaCorpusYomi.test.js` — 한자 문장 9,207개를 정답지와 전수 대조해 완전 일치율(≥0.965)과
    범인 상한(日本·二·一·何·間·者·一日·今·後)을 못 박는다. **수리 전 93.4% → 후 97.3%**(문장 357개 추가).
    남은 상위(人 にん/じん 접미·家 いえ/うち·形)는 이독이거나 규칙으로 못 가른다.
  · 변이 8/8 검출 · 서버 계약 254 · vitest 3,343 green · lint 0.
- **🈁 분석기 리뷰 라운드 2 — zh 품사·사전 억제 (2026-09-02, 오너 「순위대로 ㄱㄱ」)**:
  리뷰 2순위. 뷰어에 `喝`(HSK1)가 **품사 미상**으로, `喝咖啡`가 **인명**으로, `坐地铁`가 **성어**로 뜨던 자리.
  · **품사 미상 179건 → 2건.** jieba 어소·복합 태그(vg 喝·vq 去过·df 不要·mq 这点·tg 晚)를 `POS_KO`에
    옮기고, 태그 하나가 여러 품사를 덮는 zg·nrt(您/啊/往·令人/保姆)는 단어별 `POS_FIX`로. 고유명사
    태그로 오던 보통명사(老虎·熊猫·胡同·安全带 — HSK 밖이라 R5 수확층 사각) 19항도 단어별.
  · **설계가 반 틀렸다 — 되가름이 아니라 억제.** 「nr/i/l 병합을 R1 되가름으로 편다」는 실측에서
    무너졌다: 喝咖啡·坐地铁·今天天气는 HMM 산물이 아니라 **jieba 사전 항목**이라 HMM off에도 안 갈리고,
    HSK 방벽은 老虎·熊猫·胡同을 못 지킨다. `add_word(词, 0)` 빈도 억제는 성립한다(빈도 1은 불성립).
    R1 주석의 「add_word 배제」는 조각을 올리던 방향이고 여기는 병합을 내리는 반대 방향.
  · **억제는 연쇄를 낳는다.** 억제하면 jieba가 다음 사전 항목으로 도로 붙인다 — 很多를 억제하니
    很多人×7·多书×5가 생겼다(제외). 항목별로 「새로 생기는 쓰레기」를 재서 부작용 0인 42항만 실었다
    (坐地铁는 先坐를 낳아 함께). 山上은 10문장 중 1개만 갈려 제외, 吃了饭은 이합사 R4a가 이미 받아 제외.
    결과: HSK 밖 쓰레기 토큰 156건 소멸, 새로 생긴 18건은 전부 실단어(一封·周一·每天).
  · **파생물이 따라왔다.** 억제로 `我不喝咖啡`·`看不清楚`의 토큰이 갈리자 드릴 refs 신선도 계약
    (부채 ① R2)이 잡았다 — 재생성 402→408 표기(`喝`·`咖啡`·`看`·`清楚`), ja 불변. 계약이 일한 자리.
  · 회귀 계약 `zhCorpusPos.test.js`(품사 미상 ≤5·억제 항목 토큰 0) + `zhSuppress.test.js`(항목별 기대
    조각·유지 목록·연쇄 배제). 변이 7/7 검출(하나는 픽스처 보강 没喝). vitest 3,328 green · lint 0.
- **🈁 분석기 리뷰 라운드 1 — zh 병음 문맥 수리 (2026-09-02, 오너 「순위대로 ㄱㄱ」)**:
  리뷰(#1077 5501779373)가 정답지 전수 대조로 찾은 zh 최대 부류 — **구조조사를 원조로 읽는 것**
  (得 142·地 66·过 43)과 다음자(为·只·种·长·重·倒·待·教·还) — 를 닫았다.
  · **세 번째 층을 놓았다.** 경성 사전·경계 수리는 토큰 정확 일치라 1자 조사의 문맥 오독을 못
    잡는다. `zhPinyinContext.js`가 jieba 태그(ud·uv·ul·ug)와 **이웃 토큰**으로 가른다 —
    `afterCountSlot`·이합사 회랑과 같은 이웃 판독 선례. 적용 순서: 경성 > 경계 > 문맥 > 줄 병음.
  · **실측이 규칙을 바꿨다.** 설계는 「得/ud → de」였는데 jieba는 我得走了(děi)·得了第一名(dé)에도
    ud를 단다. 코퍼스 분포로 가르니 「뒤가 了면 dé, 앞이 대명사면 děi, 나머지 de」가 정확했고,
    앞 품사를 열거하면 오태그된 술어(高兴/b·图画/n)를 놓친다(13건). 种은 동사 자리에서도 m으로
    오고, 为…所의 为는 p, 只는 양사 자리에서도 d — 전부 이웃으로 갈랐다.
  · **뺀 것 둘.** V得了 가능보어(做得了吗 liǎo)를 「앞이 得」로 잡으려다 得了第一名까지 liǎo가
    됐다 — 1건 얻고 19건 잃어 뺐다. 断线重连은 되가름 뒤 连이 성씨(nr)로 태깅돼 규칙 밖 — 억지로
    안 넓혔다. 逮捕는 정답지가 dài로 맞고 pinyin-pro가 틀려 등재, 值得·去过 4건은 정답지 쪽 오류.
  · 다글자 등재 21항은 **pinyin-pro가 실제로 틀리는 것만**(被选为·举行·长满·局长·种植·重组·
    心脏·背着·逮捕…). 成为·校长·重建·头发처럼 맞게 내는 건 싣지 않았다(목록이 지키는 것 없이
    커지지 않게 — 계약으로 못 박음). 경성 공백 5항(早上·晚上·这里·哪里·故事) — 那里만 있던 비일관.
  · **회귀 계약으로 심었다.** `zhCorpusPinyin.test.js`가 콘텐츠 8,534문장을 정답지와 전수 대조해
    완전 일치율(≥0.91)과 글자별 오독 상한을 못 박는다. 一·不 변조와 个 경성은 관례로 중립화.
    실측: **수리 전 88.8% → 후 91.8%**(문장 246개 추가 완전 일치). 남은 상위(里·上 방위사 접미
    경성, 知道·打算 可轻读, 谁 shéi/shuí)는 v1 배제 조항·오너 결정 항목인 관례 부류.
  · 변이 13종 전부 검출(둘은 픽스처가 갈라 주지 못해 보강: 老师得了奖·举行会议). 전체 스위트가
    성어 迫不得已를 잡아 V得C 규칙을 둘째 글자로 좁혔다. vitest 3,299 green · lint 0.
- **🈁 드릴 → 단어장 만남 ja·zh — 저작이 아니라 파생이었다 (2026-09-01, 부채 ① R2)**:
  R1은 「ja·zh는 기계 추출 불가 → refs 저작」으로 닫았다. 그 결론이 **절반 틀렸다** —
  틀린 건 '기계'가 아니라 **경계를 어디서 얻느냐**였다.
  · **zh는 이미 고쳐져 있었다.** R1 실측(`我有时间`→我·有时·间)은 v2-T 후처리 **전**이다.
    지금 71문장 전수 추적하니 분할·정본 대조가 **전부** 맞는다(我·有·时间) — 4.5 refs/문장.
  · **ja는 분석기를 쓸 수 없다.** 92문장 중 **89개가 가나 전용**(초급 장이 한자 앞에 가나를
    가르치니 당연)이고, 그게 kuromoji의 최악 조건이다: `まいあさ`(毎朝) → まく(씨뿌리다)+ある,
    `あには`(兄は) → あ+に+は, `えきの` → え+きの. 이 유령들이 정본에 부딪히면 **학습자가
    만난 적 없는 말이 FSRS 대기열에 박힌다** — R1이 세운 계약을 정확히 어긴다.
  · **대신 문장이 이미 든 신호를 썼다.** ja 드릴은 배열 문제라 사람이 문절마다 띄어 뒀다
    (`えきの ちかくに ぎんこうが あります`). 그 경계 안에서만 앞에서부터 맞추고, 꼬리는
    **격조사·계사만** 뗀다. 동사 어미(ます)를 빼 둔 게 핵심 — 떼면 어간이 남고 어간이 정본의
    **다른 표제어**와 부딪친다(`行きます`→`行き` ゆき'가는 길' N3명사 = 다른 말).
  · **92문장 전수 수동 감사.** 남은 오류 5건은 전부 동형이의(ところが≠ところ, 〜うちに≠うち)라
    규칙으로 못 가른다 ⇒ 감사 제외로 명시하고, 「죽은 제외」를 계약이 잡는다.
  · 산출 `src/lib/data/drillRefs.json` — 드릴 133개·표기 402개(ja 62/86 · zh 71/316), 8KB,
    2회 재생성 동일. 실행 시점 분석기 호출 0(클라이언트가 jieba를 안 진다).
  · **생성기가 테스트에 산다.** `scripts/build-*.mjs` 선례는 전부 외부 파일을 받는 순수
    변환기라 이 문제를 안 겪었다 — 우리만 런타임 분석기 체인을 문다. node로 돌리려면
    production 파일의 확장자 없는 import 7건·JSON import 5건을 고쳐야 하는데, 스크립트
    하나 편하자고 서버 핫패스를 건드리는 건 배포 위험만 사는 거래다. vitest는 그 체인을
    이미 해석하고 **머지 게이트가 바로 vitest**다 ⇒ 생성과 신선도 검사를 한 파일에 뒀다.
  · 변이 7종 중 6종 즉시 검출. 살아남은 1종(zh 기호 필터)은 **정본에 기호 표제어가 없어서**
    관측이 안 되는 것으로 실측 판명 — 필터는 두고 산출물 성질을 계약으로 못 박았다.
  · 계약 21종(신규 5 + R1 계약 갱신) · 전체 vitest green(3,286) · lint 0 · `npm run build` 통과.
- **🈁 요미 자리 分散配置 — **우리가 정할 문제가 아니었다** (2026-09-01, 오너 「권장대로 ㄱㄱ」)**:
  오너 질문 「일본에선 보통 어떻게 처리?」로 조사가 시작됐고, **정본이 있었다**
  (JLReq / JIS X 4051). ⑴ 요미 크기 = 본체의 절반 ⑵ 삐침은 한쪽 요미 1글자까지, **줄머리·
  줄끝으로는 금지** ⑶ 그보다 길면 **본체 글자를 벌린다**(分散配置).
  우리 카드는 ⑵⑶을 어기고 있었다 — 실측 `志望者`(요미 1.50자 초과)가 카드 왼쪽으로 47px,
  요미가 줄 위로 17.8px(메타 줄과 겹침). ⇒ 삐짐 **전부 0**.
  · **설계와 배선이 어긋난 자리였다.** `fitDivisor`는 `max(글자수, 요미÷2)`로 **요미가
    패널을 꽉 채우도록** 크기를 정해 놓고(志望者: 요미 폭 정확히 100%), 위치만 본체
    기준으로 잡아 삐졌다. 分散配置는 그 전제를 위치 쪽에도 세우는 일이다.
  · **A/B로 물었던 것 자체가 조사 전이라 좁았다.** A(세로만)는 줄머리 금지 조항 위반이
    남아 규범상 선택지가 아니었고, B(가운데 정렬)는 JLReq에 없는 처리다. C(分散配置)만
    표준이고, 덤으로 **왼쪽 정렬이 그대로 유지**된다(B의 부작용이 없다).
  · **v2-S의 우회를 걷어냈다.** S는 요미 칸 상자가 어긋나 있어 훈음을 `top: 100%`로
    우회시켰는데, 그 우회는 훈음을 **줄 아래로 3.3px 밀어내고** 있었다. 상자를 줄상자로
    맞추니 병음과 같은 식이 서고 훈음이 줄 안에 든다 — **원인을 고치니 우회가 사라졌다.**
  · 폭은 `--yomi-n` × 0.5em을 CSS가 계산한다(`--fit-n`과 같은 패턴 — 개수만 넘긴다).
    0.5em/자는 **가나 전제**라 라틴 읽기에는 안 넘긴다: 실측상 `壮/zhuàng`이 필요 198.5px
    자리에 **384px를 예약**해 루비가 카드(340px)보다 넓어진다.
  · 계약 6종(배선·동조) + 브라우저 기하 5종. 변이 6종 전부 검출. **계약 자신의 파싱 약점
    둘도 드러났다** — 주석에 적은 `.rt-hun {`을 선택자로 오인(주석 제거로 수리), import
    줄을 통째로 박아 이름 하나 추가에 깨짐(요구만 고정으로 수리).
  · **CI가 계약의 폰트 의존을 잡았다(수리 완료).** 기하 계약 하나가 「본체가 요미 폭 안에
    든다」를 단언했는데, 그건 **렌더된 가나 폭**에 기댄 말이었다 — CJK 폰트가 없는 러너에서는
    가나가 0.5em보다 좁게 떨어져 `志望者`조차 본체가 이겼다(실측 본체 225px vs 요미 216px).
    동작은 정상이고 **단언이 틀렸다.** 이 파일 머리가 「절대 px가 아니라 등식·단일값만」이라고
    이미 적어 둔 그 규칙을 내가 어긴 것이다. 등식 `칸 폭 = max(본체 폭, --yomi-n × 0.5em)`으로
    바꾸고, 벌림/안 벌림 두 갈래는 **실측한 본체 폭 위에서 예약값을 만들어** 폰트와 무관하게
    양쪽을 다 밟게 했다.
  · **변이가 죽은 줄을 찾아냈다.** `width: max-content` 제거가 안 잡히길래 재보니 **무동작**
    이었다 — 항목이 전부 글자 한 칸이라 이 flex 상자는 min-content == max-content이고 줄어들
    여지가 없다(300px 칸에 1024px 상자를 넣어도 auto와 동일). 계약을 늘리는 대신 그 줄을 지웠다.
    남은 세 줄은 변이 3종(min-width·space-evenly·inline-flex)으로 전부 검출.
- **🧮 부채 ② 「기억 통계 이중 진실」 — **대부분 이미 해소**돼 있었고, 남은 하나를 닫았다
  (2026-09-01)**:
  목록에 「미설계·미정」으로 오래 앉아 있던 부채를 코드로 확인했다. `growthStats.js`가
  스스로 「단일 소스」를 선언하고 **17개 파일**이 소비하며, 주간 기억 수치는 세션 화면도
  주간 리포트도 **둘 다 `review_events`**에서 센다 — 이중 진실이라 부를 것이 없다.
  · 남아 있던 하나: `VocabDetailCard`의 단계 라벨이 경계 `7`을 **`KNOWN_WORD_MIN_INTERVAL`을
    import하지 않고 리터럴로** 쓰고 있었다. 값이 같아 증상이 없었을 뿐, 상수를 바꾸면
    **카드는 「학습 중」인데 카운터는 「아는 단어」가 아닌** 상태가 생긴다.
  · 판정을 `growthStats.wordStage`로 올리고(모듈의 선언 그대로 — 「무엇을 세는가」의
    판정·문구가 여기를 지난다) 색은 CSS로 내렸다(`.badge--stage-*`). 판정과 표현이 같은
    삼항에 얽혀 있으면 한쪽만 낡는다.
  · 계약 4종 — 그 중 하나가 **「학습 중 이상 == 아는 단어」 동치**를 interval 8종으로
    전수 확인한다(부채의 본체가 그 선이 갈리는 것이므로). 변이 3종 전부 검출.
  · **부채 ①(드릴↔단어장 SRS)은 조사만 하고 남겼다** — 「연결이 끊긴」 게 아니라 **붙일
    고리가 없다**(드릴 항목이 문자열뿐, 단어 참조 없음. 반면 `/study` 세션 문항은
    `word_id`를 실어 이미 붙는다). 재배선이 아니라 신설이고 제품 판단이 선행한다.
    조사 전문 = #1077 코멘트.
- **🎨 v2-K 잔여 — 여러 줄 인라인 색 **8 → 0** (2026-09-01)**:
  v2-Q가 「성격이 갈려 한 라운드로 안 묶인다」며 라쳇으로 잠가 둔 8건을 넷 다 처리했다.
  값이 **사라진 게 아니라 한 곳에 모였다** — 그걸 계약이 확인한다.
  · **모달 스크림 3건** → `--scrim` 토큰 + `.scrim` 유틸. 셋 다 `rgba(0,0,0,0.45)`라
    **겉모습 불변**이다. 실측하니 스크림 값이 다섯 갈래였다(인라인 0.45 + 오버레이
    클래스 4종의 0.5·0.55·0.6·0.65) — **클래스 4종의 통합은 뺐다**: 모달 넷의 겉모습이
    바뀌는 일이라 목업 우선. 규약 문서에 그 사실을 적어 다음 세션이 모르고 손대지 않게 했다.
  · **자료실 복습 배지 1건** → `.tag--due`가 뷰어 `.viewer-badge--due`와 **같은
    `color-mix` 식**을 쓴다(2트랙 병의 처방은 값 복사가 아니라 식 공유다). 테두리는
    `.tag` 기본을 그대로 둬 현행 렌더를 보존했다.
  · **레터박스 `#000` · 게임 바닥 `#0b0d08`** → **예외 목록을 늘리지 않고** CSS로 옮겼다.
    규약이 금하는 것은 *인라인* 색 리터럴이고 CSS 본문은 값이 사는 자리다 — 예외로
    도망가면 그 자리가 다음 예외의 선례가 된다.
  · **VocabReview 정오답 2건** → `quizOptClass` + `.quiz-opt--right/--wrong`. 삼항
    **안**이라 한 줄 계약도 여러 줄 라쳇도 못 잡던 자리다. 같은 블록이 화면 둘(데스크톱·
    모바일)에 **통째로 복제**돼 있어서, 인라인으로 두면 고칠 때마다 두 벌을 맞춰야 했다.
  · 라쳇을 **0 계약**으로 바꾸고, 「어디로 옮겨 갔는가」와 「예외가 늘지 않았다」를 함께
    고정했다. 변이 6종 전부 검출(예외 목록에 `#000`을 더해 도망치는 변이 포함).
- **🐞 v2-T R3 양사 계열 — 설계 표적이 실물에 없었고, 청소할 것도 없었다
  (2026-09-01, 오너 "우선순위대로 ㄱㄱ")**:
  설계 §6이 `本书`·**`这本`**을 표적으로 적었는데 재측하니 **`这本`은 애초에 토큰으로
  나오지 않는다**. `这本书`는 `这`+**`本书`** 로 갈린다 — 양사가 앞 지시사가 아니라
  **뒤 명사에 붙는다**(`这首歌`→`这`+`首歌`도 같다). 갈래를 둘로 나눠 잡았다:
  ⑥-a 지시사+양사(`这件`·`那条`·`那位`·`那本书`) · ⑥-b 양사+명사(`本书`·`首歌`).
  실측 적중 11건 · **오분리 0건**.
  · 정밀도의 근거 둘 — ⑴ **실단어 방벽 = HSK 표제어 10,935종**(후보 규칙을 표제어
    전량에 돌려 발화 **0건**) ⑵ **앞자리 조건**(양사는 수사·지시사 뒤에서만 양사다).
    둘은 겹치지 않는다: 앞자리만으론 `那家人`을 못 막고(방벽 빼면 家人·本子·台灯·
    瓶子·盒子·部门·双手 **7종이 부서진다** — 실측), 방벽만으론 `块头很大`를 못 막는다.
  · **수사+양사(`三个`·`两杯`·`一部`)는 안 건드린다** — 규칙이 양사 목록으로만 발화하니
    수사로 시작하는 갈래는 **구조적으로** 닿지 않는다. 방침을 코드가 아니라 규칙의
    모양이 지킨다(계약이 「수사 글자가 양사 목록에 없다」를 고정).
  · `这个`·`那个`·`哪个`는 **실단어라 지킨다**(R2가 `最`·`更`를 뺀 것과 같은 판단).
    HSK 표가 `这`와 `个`를 따로만 실어 방벽에 구멍이 있어 `ZH_KEEP_MERGED`로 막았다.
- **🔌 그 과정에 드러난 것 — 중국어 공유 사전이 **처음부터 닫혀 있었다**
  (2026-09-01)**:
  R3 후반 작업은 「`morpheme_dictionary` 가짜 표제어 청소」였는데, **청소할 것이 없다.**
  `morpheme_dictionary.language`에 `CHECK (language IN ('Japanese','English'))`가
  2026-04-15부터 걸려 있고 **완화된 적이 없다** — 중국어 행은 upsert마다 거부됐고,
  실패는 `console.warn`으로 삼켜져 아무도 몰랐다(사용자에게 오류를 안 새게 한 설계).
  ⇒ 설계 §2의 「전 사용자 공유 사전에 영구 적재」는 **코드 기준으로 일어난 적이 없다.**
  대신 두 가지가 조용히 죽어 있었다: **공유 사전 캐시가 중국어·프랑스어에서 영구히
  차갑고**(같은 단어를 매번 다시 묻는다), 그 행을 읽어야 뜨는 **한자 대조의 日 자형
  줄·⚠ 경고가 뜰 수 없다**(기능이 없는 게 아니라 닿지 못했다).
  · 마이그레이션은 코드로만 냈다(`20260901000100` — **적용은 오너 수동**). 적용 전
    감사 질의 3종을 파일에 동봉했다 — 운영 DB에 수동 ALTER가 있었다면 얘기가 다르므로
    **실제 상태를 먼저 보게** 했다. 그 경우에 쓸 청소 DELETE도 같이 적었다
    (`user_verified`는 절대 건드리지 않는다).
  · **같은 누락이 세 번째다**(`user_ref_progress` → `content_sources` → 여기).
    세 번 반복된 실수는 문서가 아니라 계약으로 막는다 — `langCheckCoverage.test.js`가
    마이그레이션 전량에서 표별 최종 `language` CHECK를 재구성해 정본 4언어를 덮는지
    본다. 다음 표는 CI가 잡는다.
  · 변이 10종 전부 검출. 그 중 **둘(M4·M10)이 무증상 통과**해 계약의 구멍을 드러냈고,
    실측으로 두 가드가 **정말 일하고 있다**는 것까지 확인해 사례를 심었다.
- **🗂 v2-P 자료실 통합 — 같은 책이 **두 탭에 흩어져** 있었다
  (2026-09-01, 오너 발안 「내 자료랑 PDF 통합」·「내 자료가 먼저」)**:
  PDF 원본은 `PDF` 탭, 그 PDF에서 뽑은 자료는 `내 자료` 탭. PDF 카드는 제목·쪽수·날짜뿐이라
  자료 카드가 주는 진도·복습·커버리지가 **전무**했고, 언어·레벨 필터는 `tab !== 'pdf'`
  가드로 PDF 탭에서만 숨었다. ⇒ **PDF 탭 폐지**, `내 자료` 안의 묶음 카드로 흡수.
  탭 순서도 뒤집었다(내 자료 → 공용, 로그인 기본 `private`·비로그인 `public`).
  · **스키마가 이 통합을 이미 강제하고 있었다**(실측) — `CHECK (source_pdf_id IS NULL OR
    visibility = 'private')`. PDF 파생 자료는 구조적으로 **항상 비공개**라 `내 자료` 밖에
    있을 수 없다. 공용 탭에 PDF 묶음이 뜰 경우의 수 자체가 없다.
  · 설계 계약 ③ 「**같은 컴포넌트**」를 지키려면 먼저 컴포넌트가 있어야 했다 — 책 카드는
    자료실 렌더 안 **인라인 JSX 50줄**이었다. `MaterialGroupCard`로 뽑고 CSS 클래스로
    옮겼다(인라인 style 0). 흡수는 그 다음에야 복제 없이 됐다.
  · 계약 ②(자료 0개 PDF 노출)와 ④(필터 동일 적용)가 **부딪히는 자리**를 찾아 규칙을
    박았다 — 언어·레벨은 자료에 붙는 조건이라 자료 0개 PDF엔 대응물이 없다. 조건이
    걸리면 자료가 남은 PDF만, 조건이 없을 때만 전부.
  · **묶음 카드가 `filtered.length > 0` 분기 안에 있었다** — 자료 0개 + PDF 1개인 사용자는
    빈 화면으로 떨어져 계약 ②가 무증상으로 깨진다. 분기 조건을 넓혔다.
  · 계약 20종(순수 함수 8 + 배선 12), 변이 10종 전부 검출. 그 중 **하나는 실제 결함**을
    잡았다 — `Number(null)`은 NaN이 아니라 **0**이라 `page_start` 없는 옛 자료가 0쪽으로
    읽혀 맨 앞에 끼어들고 있었다.
  · 기존 계약 3종 재조준(형태 → 요구): `queryDiet`의 select 문자열 통짜 핀(그 파일이
    ProfileStats에서 이미 내린 판단을 자기 자신에게 적용), `V-11` 순서 단언의 사라진
    앵커(공허 통과 방지로 존재부터 단언), `materialFit`의 커버리지 판정 위치.
- **🈶 v2-S 훈음 하단 루비 — 헤더에 있는 글자를 **아래서 또 그리고** 있었다
  (2026-09-01, 오너 "우선순위대로 ㄱㄱ")**:
  단어 카드 표제어가 `杯子`인데 그 아래 훈음 나열 줄이 `杯 잔 배 / 子 아들 자`처럼 **같은
  글자를 다시 그렸다**. 줄 하나를 그 중복에 쓰는 동안, `.word-fit .surface`의 행간 1.9가
  글자 **위아래로 0.45em씩** 만들어 둔 여백은 **위쪽만 병음이 쓰고 아래는 놀고 있었다**.
  ⇒ 훈음을 그 빈 자리로 옮겼다 — 「후리가나는 위, 훈음은 아래」(오너 발안). 유도식은
  병음과 **같고 `bottom`↔`top`만 뒤집는다**(분모 = 행간과 동조). 실측: 블록 세로 증가
  **0**, 병음 아래끝이 위에서 64.0 · 훈음 위끝이 아래에서 64.0(대칭), 나열 줄 폐지로
  **줄 하나 추가 회수**.
  · 실측이 설계에 없던 구멍을 하나 팠다 — **혼종 토큰**(`T恤`·`QQ号`: 라틴이 섞여 병음
    격자가 성립하지 않는다. 한자 토큰 45개 중 2개)은 요미 칸으로 흐르는데, 폐지한 나열
    줄이 그 글자들의 **유일한 훈음 공급처**였다. 요미 칸도 받게 했다. 단 이 칸은 상자가
    달라(글자 상자 143px vs 줄상자 243px) 같은 유도식이면 훈음이 글자를 37.6px 덮는다 —
    이 칸만 글자 상자 바로 아래에 건다.
  · 글자별 신자체(`jaFormOf(ch)`)는 사라진 게 아니라 **글자 카드**로 옮겨 앉아 있다
    (日 칩 + 繁·简·正, 탭하면 그 자형 카드로). 계약을 그 자리로 다시 겨눴다.
  · 계약 4종(범위·동조·부활 금지) + 재조준 1종 + 브라우저 기하 5종. 변이 8개 전부 검출 — 그 중 **둘이
    무증상 통과**해 계약이 헛돌던 것을 드러냈다(`^\.`로 시작을 묶어 `:is(…)` 선택자를
    건너뛰던 것, 규칙이 둘인데 아무 규칙이나 집던 것). 둘 다 고쳤다.
- **🎨 v2-R 시트·카드 정돈 — 같은 것을 여는 방법이 **셋**이었다
  (2026-09-01, 오너 "우선순위대로 ㄱㄱ")**:
  하단 바 버튼 · 시트 안 섹션 헤더 · 그 헤더의 셰브런. **라벨까지 같아** 화면에
  「번역·맥락」이 **두 번** 보였다. 게다가 **접힌 섹션도 헤더 줄을 차지해** 안 보는 내용이
  세로를 먹었고, 시트 상한 60svh에서 그만큼이 카드 하단 예문(한자/병음/번역 3줄)을 잘랐다.
  ⇒ 하단 바가 **유일한 전환 수단**, 시트는 **선택된 하나만** 그린다.
  ⚠ **동작 변경 하나 — 모바일 「둘 다 펼치기」 폐기**: 예전에는 문장 드래그가 번역+단어
  목록을 둘 다 펼쳤다(#992 의도). 60svh를 둘이 나누면 각자 ~30svh인데 **단어 카드 하나만
  으로도 예문이 잘리는** 상황이라 동시 오픈은 실질적으로 **둘 다 못 보게** 만들었다.
  대체 규칙 = 문장 드래그 → **번역·맥락**(주 목적이 문장 이해고, 단어 목록은 배지로 존재를
  알릴 수 있지만 번역은 열지 않으면 알 길이 없다). **되돌리려면 `resolveSignalTransition`
  하나 + 시트 렌더 복원이 전부**이고 그 사실을 계약으로 잡았다. 데스크톱(≥1180px)은 좌우
  칸이라 **무영향**.
  **카드**: `word-detail-card__actions`가 **왼쪽이 통째로 비고** 우측에만 `▷ ✕`가 떠 소속이
  모호했다 — 단어와 뜻 **사이에 끼어** 둘의 연결을 끊던 메타(품사·급수)를 그 자리로 옮겨
  **줄 하나를 회수**하고 단어→뜻을 직결시켰다(오너 배치안). **위계도 뒤집혀** 있었다:
  단어 1.5rem/800 → 메타 0.78rem → **뜻 1rem/보통**으로 가장 중요한 것이 가장 약했다.
  한 단계 키우고, `✏️`는 뜻 상자가 `flex:1`로 남는 폭을 다 먹어 멀어지던 것을 글자 옆에
  붙였다. 유의어 칩은 테두리·여백으로 커서 **뜻을 시각 무게로 누르고** 있었다 — 걷어냈다.
  ⚠ **설계 한 항목은 실측으로 무효**: 「하단 바 두 버튼 폭 불균등 → `flex:1`로 균등하게」는
  **이미 균등**이다. 브라우저 실측 배지 없음/짧음/아주 김 **세 경우 모두 폭 차이 0px**이고
  `flex:1`·`overflow:hidden`·배지 `max-width:80px`가 전부 있다. 없는 문제라 손대지 않고,
  **그 셋이 균등 폭의 실제 근거임**을 계약으로 잡아 사라지면 잡히게 했다.
  계약 18종 · **돌연변이 18종 전량 검출**(처음 쓴 M7이 no-op이라 다시 설계해 재측정 —
  변형이 실제로 적용됐는지 확인하는 규율의 실물).
  green(3183) · `e2e:typography` 14 pass · lint 0 errors · **마이그레이션 0**. PR #1231.
- **🐞 v2-T R1·R2 중국어 토큰 경계 — 탐지기를 **거꾸로** 쓰고 있었다
  (2026-09-01, 오너 "우선순위대로 ㄱㄱ")**:
  jieba의 HMM은 사전에 없는 한자 조합을 문맥으로 붙여 놓고 `x`를 단다. **x+한자는 조작물의
  지문**인데, 이 코드는 그 신호를 정확히 식별해 놓고 **실단어로 승격**시키고 있었다. 당시
  목적(기호 오분류로 병음이 통째 사라지는 사고 차단)은 옳았고 **신호 소비 방향만 반대**였다.
  화면에서 끝나지 않는다 — 중국어는 `base_form = 표면형`이라 **토큰 경계가 곧 표제어**이고,
  뜻 조회를 거쳐 **전 사용자 공유 사전(`morpheme_dictionary`)에 가짜 표제어가 적재**된다.
  `人要`의 뜻이 「사람은 마땅히」로 나온 것이 증거다 — **조사 '은'이 들어간 순간 그건 단어가
  아니라 절(節)**이고, LLM도 절인 줄 알면서 표제어로 뜻을 지어냈다.
  **R1** = x+한자 다글자 토큰을 HMM 없이 되가름. 근거는 **복구 비대칭**이다 — 오분리는
  드래그 범위 지정으로 되붙일 수 있지만 **오병합은 분리 액션 자체가 없다**. 병음 손실 0
  (줄 단위 계산 후 글자별 재분배). **R2** = 정도부사+단음절 형용사(사전 등재 병합이라 상보).
  **실측 셋이 설계를 고쳤다**:
  ⑴ **R2에서 `最`·`更`를 뺐다** — 설계는 「TP 7/7 · FP 0/14」였는데 넓은 집합으로 재보니
     **오분리 3건**(`最近`(HSK1)·`最好`·`更新`이 전부 「부사+형용사」 꼴). 빼면 **오분리
     0/26**, 잃는 건 `最大`·`更高`·`更好`뿐인데 붙어 있어도 뜻이 투명하다. x 토큰(정의상
     사전 밖)과 달리 **사전에 있는 단어를 가르는** 규칙이라 근거가 다르다.
  ⑵ **잔여 조각이 되가름을 건너뛰고 있었다** — `我学过/x`는 말미 상조사 규칙이 먼저 잡아
     `我学/x`+`过/ug`를 만드는데 그 **`我学`가 그대로 나갔다**. 잔여도 같은 헬퍼를 통과시켰다.
  ⑶ **막혀 있던 이합사 인지가 살아났다** — `他去理了发。`는 HMM이 去+理를 붙여 V 클러스터가
     성립하지 않아 **회랑 탐색이 시작조차 못 했다**. 되가름이 원인을 없애자 `理[base=理发]`로
     잡힌다. 옛 계약은 그 미스를 「v1 수용 미스」로 **박제**해 두고 있었다.
  착수 전 실측으로 위험 하나를 지웠다: 기존 주석이 경고한 `穿过`·`睡着`·`接着`는 x가 아니라
  `v`/`c`로 제대로 태깅돼 **사정권 밖**이다(가정이 아니라 확인).
  허용목록(`zhKeepMerged`)은 교재 x-토큰 **158종 전수**에서 건진 실단어만 — 신조어 扫码·网红,
  실단어 好用·点餐, 2자 인명 小明·小王·李明(3자 인명은 jieba가 `nr`로 제대로 잡는다).
  **계약 개정 3건** — 셋 다 요구가 아니라 **당시 한계를 값으로** 얼려 두고 있었다. 부정 단언
  (`not.toBe('기호')`)은 토큰이 사라지면 **공허 통과**하므로 전부 긍정 단언으로 바꿨다.
  교재 실측: 2332문장 · 토큰 14703 → 15025 · **사라진 가짜 표제어 166종**.
  계약 15종. **돌연변이 13종 중 12 검출, 1종 등가**. 처음 생존한 셋 중 둘이 진짜 결함 —
  허용목록 검사 **중복**, 그리고 「긴 것 먼저」라는 **내 주석이 사실이 아니었던 것**(일곱
  항목이 서로 접두가 아니라 순서가 무의미하다) → 접두 비관계를 계약으로 심어 해소.
  green(3169) · lint 0 errors · **DB 변경 0 · 마이그레이션 0**. R3(양사 계열 + 사전 청소)는
  닫힌 양사 목록과 **오너 수동 DB 정합**이 필요해 별건. PR #1230.
- **🎨 v2-Q 뷰어 크롬 정돈 — 배지 폭이 제각각이던 건 취향이 아니라 **래퍼 부재**였다
  (2026-09-01, 오너 "Q ㄱㄱ")**:
  `.page-header`는 flex가 아닌 **블록**인데 배지 3종이 그 **직계 자식**이었다.
  `inline-block`인 수집 배지만 내용 폭이고 나머지 둘(`div`)은 **전체 폭까지 늘어났다** —
  같은 층위 정보인데 폭만 달라, 정보 위계가 아니라 **우연한 마크업 차이**가 시선을 끌었다.
  방증도 있었다: 제목에 `style={{ flex: 1 }}`이 붙어 있었는데 **부모가 flex가 아니라 죽은
  값**이다. 누군가 이미 이 헤더를 flex로 착각하고 있었다는 자국이다.
  래퍼 하나(`.viewer-badges`)가 원인을 없앤다. 인라인 style 3벌은 공용 클래스
  `.viewer-badge`(+`--due`)로 수렴 — **K R1 토큰화가 남긴 나머지**다. 헤더 자체를 flex로
  만들지 않은 이유는 뒤로가기·내비·제목까지 한 줄로 눕기 때문이다.
  **액션 줄은 세 언어로 말하고 있었다**(채워진 버튼·밑줄 링크·아이콘 버튼) + 오른쪽
  정렬이라 왼쪽 정렬인 제목·배지와 축이 어긋났다. 성격도 섞여 있었다 — 읽기 완료·오늘
  학습은 **행동**, 듣기·Aa는 **도구**. 축을 갈라 행동은 왼쪽·같은 버튼 모양, 도구는
  오른쪽에 모으고 header 밖 독립 줄에 떠 있던 듣기를 도구 그룹으로 들였다.
  `편집`은 `h1` **안** 인라인이라 상첨자처럼 떠 제목의 일부로 읽혔다 — h1이 그라디언트
  텍스트라 버튼 글자까지 그 클립에 얹혀 있었고 hover 색을 JS로 매만지고 있었다(`:hover`가
  할 일). 밖으로 빼고 baseline 정렬 + CSS hover로.
  크롬 강조색은 **복습 하나만** 남겼다. 수집 배지는 무채색으로 내리되 **팝은 남긴다** —
  단어를 담은 순간의 되먹임은 색이 아니라 **움직임**이 나른다. 옛 배지에 없던
  `prefers-reduced-motion` 가드도 함께 넣었다.
  ⚠ **실측이 설계 한 항목을 무효화했다.** 설계는 「시리즈 내비가 **큰 채워진 바** 안에
  `《HSK 5》 1/20`을 띄워 제목을 반복한다」고 했는데, `.viewer-series-nav`는 `inline-flex`에
  **배경도 테두리도 없고** 시리즈명은 `title` 툴팁일 뿐 **렌더되지 않는다**. 설계가 본
  채워진 바는 `.book-nav`(**책 챕터 내비 — 다른 기능**)이고 그 `《…》`는 **책 제목**이라
  H1(이 챕터 제목)과 중복이 아니다. **없는 문제를 고치면 있는 정보가 사라지므로** 둘 다
  손대지 않고, 현 상태를 회귀 방지 계약으로 못 박았다.
  ⚠ **부산물 — K R1 색 계약에 여러 줄 구멍**: 검사가 `style={{ … 색 }}`을 **한 줄 안에서만**
  찾아, 여러 줄로 펼친 inline style이 통째로 빠져나간다. 이 라운드가 없앤 복습 배지가
  정확히 그 꼴이었고 **계약이 있는 채로 살아남아 있었다.** 전수 잔여 8건인데 성격이 갈려
  (스크림 토큰 부재 3 · 같은 배지의 두 번째 하드코딩 1 · 예외 판단 2 · 삼항 안 상태색 2)
  한 라운드로 안 묶인다 ⇒ **고치지 않고 잠갔다**(8을 넘기면 실패하는 라쳇). 별도 라운드 적립.
  ⚠ **크롬 기하는 소스 계약으로 못 잡는다** — 이 결함은 CSS 규칙 어디에도 문제가 없는
  종류였다(원인이 각 배지가 아니라 **부모**였다). `typography.e2e.mjs`의 존재 이유
  (「규칙이 있어도 상호작용으로 결과가 깨진다」)가 그대로 적용돼 **브라우저 좌표**로 못 박고
  **옛 구조 대조군**도 남겼다(옛 구조 렌더 실측 = 폭 `[900,176,900,900]`).
  그 기하 실측이 **셋을 더 고쳤다**:
  ⑴ **flaky의 씨앗** — 팝이 `scale(0.9)`에서 시작해 그냥 재면 **애니메이션 중간 좌표**가
     잡힌다(첫 배지 left 0 → 7.02). `getAnimations().finished`로 제거.
  ⑵ **중복 선언** — `justify-content: space-between`과 `margin-left: auto`가 서로 중복이라
     **어느 쪽을 지워도 결과가 같았다**(돌연변이 양쪽 생존 = 등가). 게다가 행동 그룹이 비면
     (비로그인) space-between은 도구를 **왼쪽으로 보낸다**. auto 마진만 남기고 그 상태를
     e2e 케이스로 추가.
  ⑶ **계약이 겨눈 곳이 틀렸다** — `flex-wrap`이 지키는 것은 **넘침이 아니라 모양**이다.
     wrap을 빼도 넘침은 0인데(flex 항목이 기본으로 줄어든다) 배지가 눌려 **알약 안에서
     글자가 접힌다**(2줄·높이 30 → 1줄·높이 **50**). 줄 나눔 + 폭·높이 보존으로 교체.
  계약 **소스 20종 + 기하 5종**. **돌연변이 30종 중 29 검출, 1종 등가**(flex 컨테이너
  안에서 자식의 `display`는 크기를 바꾸지 않는다). **처음에 생존한 넷이 전부 진짜 결함을
  가리켰다** — 순서 단언의 공허 통과(없는 요소의 `indexOf`가 `-1`) + 위 ⑴⑵⑶.
  green(3154) · `e2e:typography` 14 pass · lint 0 errors ·
  **본문 무접촉 · 마이그레이션 0 · 로직 변경 0**. PR #1229.
- **🙈 후리가나 셀프 테스트 v1-4 R1 — 「가려진 것만 한 번 더」
  (2026-09-01, 오너 "우선순위대로 ㄱㄱ")**:
  착수 실측이 규모를 절반으로 깎았다. v1-4는 **08-20** 제안인데 읽기 설정 리뉴얼이
  **08-27**이라, 그 사이에 **가리는 축이 이미 생겨 있었다** — `pronDisplay` 3단
  (all·unknown·none, 주석에 「구 후리가나 토글의 후신」)과 프리셋 🙈 「암기 확인」.
  남은 결핍은 「가림」이 아니라 **「확인」 하나**였다: 가려진 발음을 다시 보는 길이
  **카드 시트를 여는 것뿐**이라, testing effect의 핵심인 「떠올린 뒤 **즉시** 확인」이
  무거웠다. **규모 중 → 소중**으로 재산정.
  공개가 싼 것도 실측이다 — 가려진 읽기는 **이미 DOM에 있고** `.surface--furi-off`가
  `visibility`만 꺼 둔다(ruby는 토글과 무관하게 항상 만든다는 기존 성질). 공개 =
  **클래스 한 겹 벗기기** → 렌더 경로 신설 0, **폭·행간 불변**(글이 안 밀린다).
  탭 규칙은 한 줄이다 — **「탭은 그 자리에서 가장 덜 아는 것을 연다」**:
  ① 집중 모드 + 지정 문장 밖 = 이동(무변경) → ② 발음이 가려져 있으면 공개(신설) →
  ③ 그 외 카드 시트(무변경). **가려지지 않은 단어는 지금과 완전히 같다.**
  길게 누르기는 쓸 수 없었다(`onPointerDown`을 드래그 범위 지정이 이미 점유) — 그래서
  **순서**로 푼다. 옵트인 스위치 `pronReveal`은 `pronDisplay`의 **종속** 항목으로 붙여
  축을 늘리지 않았고, 「전체」에서는 흐림+`disabled`(가릴 게 없다). 프리셋 🙈 암기 확인이
  이 스위치를 켠다 — **이미 있던 프리셋이 비로소 이름값을 한다**(새 프리셋 0).
  **넣지 않은 것이 이 라운드의 본체다**: SRS 연동 배제(「탭해서 봤다 = 모른다」는 신호가
  약하다 — 궁금해서·확인차·오탭. 약한 신호를 FSRS에 흘리면 복습 전체가 흔들린다),
  공개 상태 저장 배제(세션 로컬 `Set` 하나. 자료를 나가면 리셋 — 다음에 또 인출 연습이
  되는 게 맞다). **localStorage 0 · DB 0 · 마이그레이션 0.**
  설계에 없던 구멍 하나를 실측이 잡았다: `furigana`가 있어도 한자가 없으면 `splitRuby`가
  plain 한 조각만 내어 **벗길 rt가 없다** — 그런 토큰까지 「가려짐」으로 두면 탭이 아무
  일도 없이 먹힌다. `hasReading` 게이트로 막고 계약으로 심었다.
  **계약이 또 모양을 얼리고 있었다**(이 축에서 네 번째): `readingSheet.test.js`가 프리셋
  표를 `toEqual`로 통째 고정하고 키 목록까지 못 박아, 표시 축을 하나 늘리는 **정당한
  변경에서 깨졌다**. 요구(⑴3장 ⑵오너 확정 표시값 ⑶**조판 불가침**)로 고쳐 썼고,
  `presetActive` 단언도 프리셋 자신을 기준으로 삼아 **전 키를 하나씩 틀어** 보게 넓혔다.
  실측: 신설 계약 22종 + 기존 갱신, **돌연변이 19종 전량 검출**, 전량 green(3134).
  PR #1228.
- **🗃️ 단어 저장 페이로드 수렴 — 적립 때 센 「6곳」이 실제로는 11곳이었다
  (2026-09-01, 오너 "ㄱ")**:
  H R3에서 적립할 때 `user_vocabulary` upsert를 **6곳**으로 적었는데, 전수하니 **쓰기 경로가
  11개**였다(참조 덱 2 · 문형 색인 · CSV 가져오기 · 수동 추가가 빠져 있었다). 손으로 센
  숫자를 그대로 믿을 뻔했다.
  **진짜 버그는 하나였다**: `PdfViewerPage`·`QuickPage`가 `word_text`에 **surface(활용형)**
  를 넣고 `ViewerPage`는 **base(기본형)** 를 넣는다. `onConflict: 'user_id,word_text'`는
  키가 다르면 못 막으므로 **같은 단어를 두 문으로 담으면 행이 둘로 갈리고 복습이 두 번
  온다**. 화면은 멀쩡해 보인다 — 뷰어의 저장 판정이 surface·base 두 집합을 다 보기 때문.
  이게 `architecture §4.4`가 「규약 이원화」로 적어 둔 것의 실체인데, 문서는 「viewer 3개
  저장 경로」라 했고 실측은 달랐다(ViewerPage 2곳은 이미 base로 고쳐져 있었다).
  **`normalizeWordText` 규약은 이미 있었고 7개 토큰 경로 중 2곳만** 쓰고 있었다 —
  없던 건 규약이 아니라 **그걸 강제하는 조립 지점**이다. `vocabIO.js`(규약이 사는 곳)에
  `buildVocabRow`·`VOCAB_UPSERT`·`SOURCE_SENTENCE_MAX`를 놓고 7경로를 수렴시켰다.
  **판단 셋**: ⑴ `next_review_at`을 **항상** 싣는다 — `ignoreDuplicates`라 삽입 때만 쓰여
  컬럼 기본값이 무엇이든 결과가 같다(**확인할 수 없는 것에 의존하지 않게** 됐다. 적립 때
  「첫 단계는 기본값 확인」이라 적었는데, 설계로 그 질문 자체를 없앴다). ⑵ 출처는 **있을
  때만 키를 만든다** — null로 실으면 먼저 담긴 출처를 지운다. ⑶ `ignoreDuplicates`를 정본
  으로 — 빼면 나중에 담은 얄팍한 페이로드가 먼저 담긴 풍부한 것을 밀어내고 **결과가 담은
  순서에 좌우된다**(8경로 중 5곳은 이미 이 옵션이었다).
  ⚠ **고치다가 나온 두 번째 버그**: `progressStore`의 스키마 폴백이 주석은 「base_form 없이
  폴백」인데 구조분해가 **`word_text`까지 벗겨** 단어 없는 행을 넣으려 했다 — 늘 조용히
  실패하고 있었다. 벗길 건 하나다. `insert`도 `upsert`로 바꿨다(중복이 조용한 실패였다).
  **범위를 그었다**: 참조 덱·문형 색인·CSV·수동 추가는 `word_text`가 애초에 사전 표제어라
  surface/base가 갈릴 여지가 없고 페이로드도 다르다(어원·한자·덱 라벨). 같이 뭉치면 그
  정보가 정본에서 새어 나간다 — **밖으로 두고 계약에 그 사실을 적었다**.
  계약 15종 신설 + **돌연변이 11종 전량 검출**. 전량 green(3111). 마이그레이션 0.
- **🎚️ 학습 수준 4언어화 — F R6을 착수하려다 **쓰는 쪽이 이미 고장나** 있었다
  (2026-09-01, 오너 "다음 할 수 있는 일 ㄱ")**:
  다음 라운드로 F R6(난이도 점수)을 잡고 「이 점수가 어디 쓰이나」부터 실측했다.
  쓰는 쪽이 추천 카드 레벨 필터인데, 거기가 이랬다:
  `const userLevel = s.language === 'Japanese' ? 일본어수준 : 영어수준;`
  ⇒ **프랑스어 카드가 사용자의 영어 수준으로 걸러진다.** 영어가 C1이면 프랑스어 B1
  카드는 diff 2로 조용히 숨는다. **F R2가 프랑스어 공급을 연 순간 생긴 실사용 버그**다.
  컬럼은 이미 넷이 다 있었다(`20260810120000_profile_levels_fr_zh`) — 없던 건 정본이다.
  **같은 병이 한 파일에 넷**이었다: ⑴ 프로필 컬럼 삼항 ⑵ `LEVEL_ORDER` 지역 순서표
  (ja/en만) ⑶ `LANG_FILTERS` 손목록(**프랑스어 없음** — 자료실에서 프랑스어만 골라 볼
  방법이 없었다) ⑷ `levelOptions` 삼항(프랑스어를 고르면 전체 목록이 떴다).
  `VocabStats`에도 같은 삼항이 하나 더 있었다.
  **정본 신설**: `constants.js`에 `PROFILE_LEVEL_COLUMN`·`profileLevel`·`levelRank`.
  순서는 `LEVELS` 배열 자체에서 나온다 — 지역 순서표를 또 만들면 언어가 늘 때마다 갈린다.
  **같은 상수가 자리에 따라 갈렸다**: 모르는 레벨의 `99`가 **필터에선 틀리고**(카드가
  사라진다) **정렬에선 맞다**(뒤로 밀릴 뿐). 필터는 「모르면 보여 준다」로, 정렬은 99 유지.
  **전체 목록 중복 제거**: 영어·프랑스어가 CEFR 급수를 공유해 안 지우면 'B1 중급'이 두 번
  뜨고 React key도 겹친다.
  ⚠ **F R6 자체는 보류했다 — 그리고 그 판단이 실측에서 나왔다.** 난이도 신호를 우리
  교재로 재 보니 한자비율이 **단조증가**로 잘 갈렸다(N5 0.030 → N4 0.174 → N3 0.222 →
  N2 0.297 → N1 0.309). 그런데 **N5의 0.03은 「초보용으로 그렇게 쓴 것」**이지 자연스러운
  일본어가 아니다 — 그 잣대를 실제 자막에 대면 전부 N2/N1으로 찍힌다. 어휘 커버리지
  방식은 레벨 인덱스가 **일본어에만** 있고, 무엇보다 egress 차단이라 **실자막으로 검증할
  방법이 없다**. 순진한 잣대가 체계적으로 틀린다는 걸 방금 확인한 직후에 검증 못 하는
  점수를 배포하지 않는다. 재개 조건 = 실자막 표본 확보.
  ⚠ **적립**: `VocabStats`는 3트랙(ja/en/fr)이고 **중국어가 `activeLangs`에서 빠져 있다** —
  크래시는 아니고(그래서 못 들어온다) 급수 커버리지를 못 보는 것이다. `LEVEL_MILESTONES`에
  중국어 목표 단어 수를 정하는 건 제품 결정이라 임의로 넣지 않았다.
  ⚠ **계약이 내 파손을 잡았다**: `LEVEL_ORDER` 정의만 지웠는데 **소비처가 하나 더**
  있어(레벨 정렬) 런타임 ReferenceError였다. 파일 전수 검사가 아니었으면 놓쳤다.
  계약 14종 신설 + **돌연변이 11종 중 11종 검출**(M6은 1차에 생존 — 블록에 `return true;`가
  여럿이라 하나만 뒤집혀도 통과했다. 「언어 게이트 뒤엔 `return false`가 없다」로 교체).
  전량 green(3096). 마이그레이션 0 · 새 컬럼 0(있는 컬럼을 쓰기 시작했을 뿐).
- **🅾️ v2-F R5 공유 가능한 영상 — 오픈소스 조사가 라운드를 만들었다
  (2026-09-01, 오너 "A ㄱㄱ")**:
  오너 지시로 오픈소스를 더 팠고, 거기서 나온 것이 이 라운드다.
  **조사 결론 ①: 이 분야 OSS는 저작권을 구조로 피한다.** asbplayer(MIT)는 **클라이언트
  사이드**(확장이 사용자 브라우저에서 자막을 읽고 서버가 없다), Voracious(MIT)는 **로컬
  파일**, Lute v3(MIT, OSS LingQ)는 **셀프호스팅 단일 사용자**라 배포가 아예 없다.
  ⇒ **호스팅 웹앱이면서 남의 자막을 다루는 OSS는 없다.** F R4의 자리가 그 빈자리에서
  가장 가깝다는 확인이고, 이식할 코드는 없다(구조가 달라서 — 배제, 근거 기록).
  **조사 결론 ②: 재사용할 건 코드가 아니라 아이디어였다.** Linglass가 「every video is
  scored **from its own subtitle track**」을 한다 — 자막을 일시적으로 읽어 **숫자만** 남기면
  복제가 아니라 사실이다. 우리엔 `materialFit.js`가 이미 있어 꽂을 자리가 있다(R6 후보).
  **조사 결론 ③ = 이 라운드의 근거.** `youtubei.js`의 `SearchFilters.features`가
  **`'creative_commons'`를 받는다**(실측: `types/Misc.d.ts:9` → `Innertube.js:157`
  `featuresCreativeCommons`). 키 불필요. 규모 증거도 있다 — **YouTube-Commons**가 CC-BY
  영상 **316만 편/채널 72만**에서 전사 2,270만 건을 모았고, Common Pile은 공개 라이선스
  채널 2,000+를 큐레이션했다. CC-BY는 희귀하지 않다.
  ⇒ **재배포가 허용된 자료는 본문을 담아 공개 자료로 둘 수 있다**(분석 1회를 전원 공유).
  **2티어로 얹었다 — R4를 갈아엎지 않는다**: A(공개) = CC BY 검색 + VOA(퍼블릭 도메인),
  B(비공개) = 큐레이션 채널(R4 그대로). 역할 분담이지 경쟁이 아니다.
  **판정은 전량 fail-closed** — 잘못 공개하는 쪽이 놓치는 쪽보다 훨씬 비싸다.
  검색 필터를 **믿지 않고** 영상마다 라이선스 행을 재확인하고, 확인 못 하면 버린다.
  퍼블릭 도메인은 **설정이 근거**다(VOA는 미국법에서 오지 YouTube 표식에서 오지 않는다 —
  표식만 보면 이 자료를 통째로 놓친다).
  ⚠ **실측이 두 번 고쳐 줬다**: ① `VideoInfo.license`를 찾았다고 보고까지 했는데 실측하니
  그건 **`music_tracks` 안**(배경음악)이었다. 영상 라이선스는 `secondary_info.metadata.rows`의
  **지역화된 문자열**이라 판정이 그만큼 약하고, 그래서 세션 언어를 `lang:'en'`으로 고정하고
  **License 행만** 읽는다(전체 텍스트를 훑으면 설명에 그 말이 든 영상이 통째로 공개된다).
  ② **`metadata.source`가 F R1부터 저장만 되고 어디에도 표시되지 않았다.** CC BY에서 표기는
  선택이 아니라 **조건**이다 — 저장은 표기가 아니다. 뷰어에 출처 한 줄을 신설했다.
  **본문 취득은 부품 조립이다(신규 파서 0)**: `selectCaptionTrack`→`withCaptionFormat`→
  `parseCaptionData`(media.js)→`stripCueNoise`→`paragraphize`(linkImport.js). 뒤 둘은
  **F R1 붙여넣기와 같은 정본**이라 같은 영상이 어느 문으로 와도 본문이 안 갈린다.
  **크론·읽기 라우트 무변경** — tier A는 transcript가 있어 기존 글 소스 경로를 그대로 탄다.
  ⚠ **미검증**: YouTube egress 차단이라 CC 검색·라이선스 행·자막 취득을 **실피드로 못 쟀다**.
  자막 경로는 문서 §4.3이 경고한 데이터센터 IP 차단이 걸릴 수 있다 — 실패하면 그 영상은
  안 담긴다(조용한 빈 자료 없음).
  ⚠ **돌연변이 4종이 처음에 살아남았다**: 자막 언어 요건은 `selectCaptionTrack`이 어차피
  막아 `isListable`의 고유 역할이 **임베드 판정 하나**뿐이었고(픽스처 교체), `{ok:false}`만
  준 픽스처는 `res.text()` 부재로 **예외로 죽어** 가드 없이도 통과했고(본문 정상 픽스처로
  교체), 뷰어 계약이 **파일 grep**이라 로직만 죽여도 통과했다(표현식으로 고정).
  나머지 하나(`if (!s) return false` 제거)는 **등가 변이**다 — `''.includes()`가 false라 동작 동일.
  계약 24종 신설 + **돌연변이 17종 중 16종 검출**(1종 등가). 전량 green(3082).
  마이그레이션 0 · 새 테이블 0 · 새 컬럼 0(`source` 값으로 티어를 가른다).
- **📺 v2-F R4 영상 리스트업 — 「리스트업은 우리가, 복제는 각자 개인이」
  (2026-09-01, 오너 "목업 ㄱㄱ + 채널 조사해")**:
  오너 제안은 「영상 리스트를 받아와 클릭 하나로 가져오게」였다. 실측이 셋을 뒤집었다.
  **① 그 리스트가 이미 있었다.** `daily_suggestions`는 애초에 **유튜브용 스키마**
  (`source DEFAULT 'youtube'`·`video_id`·`thumbnail_url`·`channel_name`)이고, 추천 카드도
  이미 영상 카드다(썸네일·채널·배지·클릭→폼 자동 채움). 비어 있던 이유 하나 —
  **크론이 글만 긷는다**. 새로 만들 게 아니라 채우는 일이었다.
  **② 부품도 있고 API 키도 필요 없었다.** `media.js`(계약 66종)에
  `normalizeVideoList`·`sortByUsability`·`extractCaptionLangs`·`extractEmbeddable`이
  살아 있고 `youtubei.js@17.2.0`은 이미 의존성이다. 그중 **한 함수만 쓰이고 나머지는 죽어
  있었다**. 반면 `youtube.js`의 `searchVideos`는 YouTube Data API 키를 요구한다(안 씀).
  **③ 내 첫 초안이 선을 넘었다 — 오너 질문이 잡았다**("남의 자막이라는 건? 사용하면
  안되나?"). 선례를 조사하니 모델이 셋이었다: 확장(Language Reactor·Migaku)은 **서버가
  콘텐츠를 안 만지고**, 카탈로그(Lingopie·Yabla)는 **계약**하고, **LingQ는 사용자가 넣고
  기본 비공개**다. LingQ 문구가 F R1의 결정과 문장 단위로 같았다 —
  "all content you import is **private unless you explicitly share it**".
  **아무도 안 하는 것이 하나 있었는데 그게 내 초안이었다** — 서비스가 서버에서 자막을
  미리 긁어 저장해 카탈로그로 제공하기. 셋 중 어디에도 없다. 확인된 사실 둘: YouTube ToS
  §5.B(API 외 자동 접근 금지)·**공식 Data API는 영상 소유자가 아니면 자막 본문을 안 준다**
  (합법적 대량 취득 경로 자체가 없다).
  ⇒ 오너가 정리한 표현이 곧 설계가 됐다 — **크론은 목록만 담고(transcript NULL), 자막
  복제는 누르는 사람이 자기 비공개 자료로**. `extractCaptionLangs`가 **본문을 안 받고도**
  자막 유무를 알기에 「자막 있는 것만 올린다」 필터가 그대로 성립한다(fail-closed).
  **클릭 경로는 신설 0** — F R1의 `/api/import/link`를 그대로 타고, 422면 F R1이 만들어 둔
  붙여넣기 창이 **그 자리에서** 열린다. 공개범위는 `handleLinkReady`가 이미 private 고정.
  **실측이 찾은 유일한 차단 지점**: 읽기 라우트의 `.not('transcript','is',null)` —
  영상 행을 통째로 걸러 카드가 0건이 된다. `.or(...)` 한 줄이 이 라운드의 급소였다.
  **곁다리 수리 2건**: 언어명 하드코딩(`'English' ? '영어' : '일본어'`)이 MaterialsPage·
  HomePage 두 곳에 있어 **F R2가 연 프랑스어 카드가 「일본어」로 뜬다** → 정본
  `constants.langNameKo`로 수렴(HomePage의 지역 복본도 삭제). `transcript: a.transcript`가
  undefined를 흘려 컬럼을 건너뛰던 것 → `?? null`.
  **채널 조사**: ⭐ **VOA Learning English는 퍼블릭 도메인**(미국 정부 저작물 — 교육·상업
  재사용 허용, 출처 표기). 학습자 전용 설계라 우리 용도에 정확히 맞는다. 나머지는
  Comprehensible Japanese·Onomappu·InnerFrench. 중국어는 하드리밋 ⑷로 제외.
  ⚠ **미검증**: YouTube egress 차단이라 **채널 핸들을 확인하지 못했다**. 그래서 설정에
  ID가 아니라 **핸들**을 넣고 크론이 런타임에 해석하며, 실패는 로그에 소리 나게 남긴다.
  ⚠ **운영 교훈**: 돌연변이 백업을 `basename`으로 떴다가 **`route.js`가 두 개라 서로
  덮어썼고**, 복원이 크론 내용을 읽기 라우트에 써 넣었다(전체 스위트가 잡음). 백업 이름은
  **경로 기반**으로. 또 계약 하나가 **내 주석의 리터럴을 잡았다** — 검사 범위를 JSX로 좁혔다.
  **F R1 계약 2건이 구현 모양을 고정하고 있어 완화**했다(`url: url.trim()` 리터럴 ·
  프롭 닫힌 목록). 요구는 그대로 지켜지고 돌연변이 4종으로 재확인했다.
  계약 23종 신설 + **돌연변이 19종 전량 검출**. 전량 green(3058). 마이그레이션 0 · 새 테이블 0.
  **남은 것**: R2(영상 길이·자막 언어 배지 — 컬럼 필요) · R3(채널 관리). VOA는 퍼블릭
  도메인이라 **공개 자료로 둘 수 있으나**(분석 1회를 전원이 공유) R1은 이음매를 늘리지
  않으려 전부 비공개로 통일했다 — R2 후보.
- **🏪 IP v2 이행 — 구 정책이 **교재 본문을 깎아 놓은 것**을 되살렸다 + R3 SPEC v2 게시
  (2026-09-01, 오너 "나머지 차근차근 ㄱㄱ")**:
  R3 SPEC을 쓰다가 실측에서 걸린 것 — **v1이 살아 있던 5주 동안 교재가 실제로 깎였다.**
  순회 R1이 `ローソン`·`からあげクン`을 지웠고(#1150), R2가 `Suica`·`PASMO`를
  「교통계 IC 카드」로 일반화했다(#1151). 둘 다 사유가 "IP 규약 이행"이라 v2에서는 **오답**이다.
  **왜 지금 아니면 안 되나**: 두 챕터는 order **05·10**이라 **순회가 이미 지나갔다**.
  R3(13~30)도 그 뒤 라운드도 돌아오지 않는다 — 계약이 없으면 아무도 다시 안 본다.
  **되살린 4곳**: ⑴ 편의점 본문에 핫스낵 실명(ローソン=からあげクン·ファミリーマート=
  ファミチキ·セブン-イレブン=ななチキ) — 간판에 からあげ라고 안 적혀 있다는 실전 정보가
  같이 산다 ⑵ からあげクン 예문 신설 ⑶ 전철 본문에 Suica·PASMO·ICOCA + 전국 상호 이용
  ⑷ `IC カード` 항목 note가 실명을 든다(총칭 자체는 실제 표지에 쓰이므로 유지 — 문제는
  **총칭만 남는 것**이었다).
  **계약 범위를 두 번 고쳐 잡았다**: 파일 전체 검사는 예문 note 한 곳만 남아도 통과했고
  (돌연변이 M13·M14 생존), 반대로 설명 본문만으로 좁히면 이름이 예문으로 **옮겨간 정상
  변경**에도 깨진다. 지킬 것은 「이 **챕터**가 실명을 쓴다」라 `slug` 앵커로 챕터를 자른다.
  ⇒ 이름이 엉뚱한 챕터에만 남는 변이(M19)까지 잡힌다.
  **덤으로 잡은 등호 계약 2건** — 둘 다 요구를 넘어서 고정돼 있었다:
  ① `styleCanon` 가나 장음 `toBe(106)` → 요구는 "**줄지 않았다**"인데 등호라 실존명이
     들어와 112가 되며 깨졌다. 하한(캐논 시점 실측)으로 교정.
  ② `check-furigana` 한글 병기 `toBe(61)` → 계약 **이름 자체가** 「0건으로 축소하지
     않는다」인데 단언이 등호였다. 품질은 위 `fail` 단언이 이미 책임진다. 하한으로 교정.
  같은 병(요구가 아니라 구현 모양을 고정)이 이번 축에서 반복 관찰된 그것이다.
  **R3 SPEC v2 게시**(#150 코멘트 5480310604) — 위 Codex-5 열 참조. SPEC에 「브랜드
  일반화 금지, 사실 정확성 사유만 허용」을 명시했고, order 05·10 되살리기는 **범위 밖**으로
  못박았다(이 항목이 그걸 처리한다).
  계약 3종 신설 + 등호 2건 교정. **돌연변이 4종 전량 검출**(M17 R1 재현 · M18 R2 재현 ·
  M19 타 챕터 잔존 · M20 코퍼스 축소). 전량 green(3035). 마이그레이션 0.
- **🚦 v2-F R3 중국어 개통의 선행 조건 — 막고 있던 건 스위치가 아니라 필터였다
  (2026-08-31, 오너 "나머지 차근차근 ㄱㄱ")**:
  F R2가 「중국어는 DB 행 하나면 켜진다」고 적은 것을 #1221에서 정정했고, 이번 라운드가
  그 정정이 가리킨 두 구멍을 실제로 메운다.
  **구멍 ① CHECK 제약** — `content_sources.language`가 `IN ('Japanese','English')`라
  Chinese INSERT 자체가 거부된다. 마이그레이션으로 French·Chinese를 허용
  (`20260831120000_content_sources_langs.sql`, 선례 = `user_ref_progress` 20260619000100).
  **덤으로 잡은 것**: 이 제약 때문에 **F R2로 연 프랑스어를 DB로 끌 수도 없었다**(French 행을
  못 넣으니 코드 기본값을 덮을 방법이 없다). 즉 off 스위치가 없는 상태였다 — 같은 한 줄로 해소.
  **구멍 ② 정치 기사 필터 부재** — 하드리밋 「중화권 정치 서술 완전 배제」인데 뉴스 피드는
  정치 기사를 자동으로 추천 카드에 올린다. 이게 진짜 선행 조건이었다.
  **판정을 뒤집은 것이 설계의 핵심**: 키워드 denylist는 "걸린 것만" 막아 **미분류·미매칭
  기사가 그대로 샌다** — 「완전」이 성립하지 않는다. 그래서 **allowlist(fail-closed)**로
  갔다: 안전 분류가 **확인된** 기사만 통과, 미분류는 거부. 판정 재료도 내 키워드 추측이
  아니라 **위키 자신의 카테고리 체계**다(편집자 합의가 판정을 대신한다).
  **지역 처리**: 대만·홍콩·마카오·티베트·신장은 주제 무관 거부(하드리밋 「지리·외관만」).
  반면 `中国` 자체는 막지 않는다 — 막으면 게이트가 아니라 차단기가 되어 개통 포기가 된다.
  통과하는 것은 「중국 + 과학/체육/문화」류다.
  **왕복 0 증가**: 본문과 카테고리를 한 요청(`prop=extracts|categories`)으로 받는다.
  따로 부르면 게이트가 대부분을 거부하는 만큼 왕복이 곱해진다. allowlist라 수확률이 낮으므로
  게이트가 붙은 언어판만 후보 폭을 넓힌다(rclimit ×12, en/fr은 ×4 그대로).
  **en/fr 무영향**: 게이트는 `WIKINEWS_EDITIONS.zh.gate`만 건다 — 같은 필터를 en/fr에
  씌우면 F R2로 연 프랑스어가 첫 피해자가 된다(계약으로 고정).
  ⚠ **미검증이라 켜지 않았다**: 이 세션은 `*.wikinews.org` 전 호스트가 egress 차단
  (403 CONNECT)이라 **실피드에 대고 게이트를 돌려보지 못했다**. 하드리밋은 절대 조건이므로
  검증 못 한 필터를 배포로 켜지 않는다 — `DEFAULT_SOURCES`에 Chinese는 여전히 없다.
  ⇒ **개통은 오너의 2단계**: ① 위 마이그레이션 적용 ② `content_sources`에 Chinese 행 1개.
  이제 그 2단계가 **실제로 전부**다(F R2가 「1단계」라 적었던 것이 틀렸던 지점).
  계약 21종 신설 + **돌연변이 12종 전량 검출**(순수 6 · 배선 6). 전량 green(3028).
- **📄 v2-H R3 유지 확정 — 「중복 기능」은 중복이 아니었다 (2026-08-31, 오너 "ㄱㄱ")**:
  H 설계가 오너 결정으로 남긴 마지막 항목(PDF 뷰어의 자체 단어 팝업·자체 vocab 저장을
  없앨 것인가)을 **유지**로 확정. Claude 권고와 같은 방향이다.
  **유지 근거**: R1이 놓은 다리(`PdfReadBridge`)는 PDF 범위를 **자료로 만들어** 일반
  뷰어로 보내는데, 그때 읽는 건 **추출된 텍스트**지 원본 지면이 아니다. 도표·레이아웃이
  있는 교재 지면을 **원본 그대로 보면서 단어만 훑는** 용도는 그 다리로 대체되지 않는다.
  없애면 그 용도가 사라지고 얻는 건 440줄 화면에서 코드 몇십 줄뿐이다.
  **착수 실측이 「중복」의 실체를 다시 정의했다**: 설계는 이걸 「PDF 뷰어의 중복 기능」
  이라 불렀는데, 실측하면 중복은 기능이 아니라 **저장 페이로드**다 — `user_vocabulary`
  upsert가 **6곳**에 흩어져 있고(ViewerPage · PdfViewerPage · QuickPage ·
  StudySessionPage · NpcDialog · progressStore) PDF 뷰어는 그중 하나일 뿐이다.
  팝업을 지워도 나머지 5곳은 그대로 남는다. ⇒ 팝업 제거는 중복 해소가 아니다.
  **그 6중복에서 실제 갈래 둘을 발견**(별건으로 #1077 적립, 이 라운드 밖):
  ⑴ `next_review_at` — PdfViewer·Quick은 **안 보내고** StudySession·NpcDialog는 보낸다.
     `user_vocabulary`는 마이그레이션 폴더보다 앞선 테이블이라 **컬럼 기본값을 확인할 수
     없었다**(DB 조회는 하드리밋). 다만 `isTokenDue`가 NULL을 방어하고 있어 가능성은 열려
     있다 — 기본값이 없다면 그 두 경로로 담은 단어가 영영 due가 안 된다.
  ⑵ `ignoreDuplicates` — 앞 둘은 없고(덮어씀) 뒤 둘은 있다(보존). 재저장 시 기존
     meaning이 덮이는지가 갈린다.
  ⇒ 고치려면 둘을 함께 정해야 하고(그냥 `next_review_at`만 더하면 재저장이 FSRS 일정을
  **오늘로 되돌린다**) 그건 H R3 범위 밖이라 제안으로 올렸다.
  **계약 4종**(팝업·자체 저장·R1 다리 생존 + "팝업 제거는 중복 해소가 아니다") +
  **돌연변이 3종 전량 검출**. green(3011). 코드 변경 0 — 결정을 계약으로만 박았다.
  ⚠ 계약을 처음엔 카피 문자열로 걸었다가 헛짚었다 — 다리는 문자열이 아니라
  `<PdfReadBridge>` 컴포넌트로 마운트돼 있다. **계약은 실물에 건다**(이번 세션 반복 교훈).
  ⇒ **H 축 R1·R2·R3 완결. #1077 「남은 할 일」 소진 — 남은 것은 오너 액션뿐.**
- **🧹 정리 3종 — 스펙이 낡아 절반이 이미 반영돼 있었다 (2026-08-31, 오너 "정리 ㄱㄱ")**:
  #1077 「남은 할 일」의 문서·콘텐츠 묶음 셋(교재 R3 건지기 · 스타일 캐논 · #152 보드 정리)을
  한 PR로 처리. **착수 실측이 또 목록을 줄였다** — 건지기 목록은 닫힌 PR #1154의 디프에서
  뽑은 것이라 그 뒤 main이 움직였다.
  **① 교재 R3 건지기 — 스펙 8건 중 5건만 유효**:
  ⑴ `が` 조사 오표기 — 스펙 3곳 중 **실측 1곳**(`scene_emergency.js`). 나머지 둘은 현재
     main에 문구 자체가 없다. ⑵ 오탈자 — 「받해요」만 남아 있었고 「였해요」는 **이미
     `~이었어요`로 고쳐져** 있었다. ⑶ 동사 그룹 완화 — かえる·はいる 예외 문장이 **이미
     들어가 있었고**, 「불규칙은 이게 전부」는 3그룹=する·くる라는 **사실이 맞아** 기각.
     ⑷ 명사 부정 콜아웃 — 스펙 문구 그대로 **이미 존재**(n5.js pitfall). ⑸ 조수사 — 규칙이
     **이미 표에** 있고(`三人(さんにん) — 단, 一人(ひとり)·二人(ふたり)`) 퀴즈도 가른다 → 기각.
     ⇒ 실제로 한 것: が 1곳 · 받해요 1곳 · 라멘집 절대 표현 완화(「한국인 최대 당황 포인트」
     제거 + 「모든 라멘집이 식권식은 아님」 단서) · 콜아웃 2건 신설(신사 박수는 절에서 안 침 +
     참배 방식은 신사마다 다름 / 허가 も vs 금지 は). pitfall 29→31.
  **② 스타일 캐논 — 스펙 34건, 실측 63건**: 스펙은 `yomi` 필드만 셌는데 규칙("한글 음차의
  장음은 하이픈")은 body·note·tip에도 걸린다. `ot.js` 47 + `n5.js` 16 = **63건**을 `-`로.
  **가타카나 장음은 한 글자도 안 건드렸다** — 규칙을 「ー **앞이 한글일 때만**」으로 좁혔고,
  전후 대조로 가나+ー가 ot.js 25·n5.js 106 불변임을 확인했다(`n4.js`의 박자 설명용
  `ニュ-ー-ス`도 무접촉). SCHEMA의 「yomi = 전체 히라가나 독음」은 **규칙이 틀린 것**이라
  정정(가타카나 yomi 781건을 히라가나로 바꾸면 일본어로 오답이 된다).
  **③ #152** — owner-gate 줄 삭제 + `docs/ai-relay.md` 머리에 조건부 보류·재개 조건 2개.
  마이그레이션은 그대로 둔다(테이블 신설이라 스키마 표류 위험 0).
  ⚠ **측정 함정 둘을 스스로 밟았다**: ⑴ `grep -P '[가-힣]ー'`가 **바이트 단위로 걸려**
  2168건 오탐(`カーペット`에 한글이 없는데 잡혔다) — UTF-8을 제대로 다루는 Python으로
  재측정해 63건 확정. ⑵ 첫 가드를 전체 합계 하한(2000)으로 뒀더니 **돌연변이가 살아남았다**
  — 전체 2107에서 n5.js의 106이 통째로 사라져도 2001이라 하한을 넘었다. **파일별 고정**으로
  바꿔 재검출. 세션 내내 반복된 교훈이 또 나왔다.
  계약 13종 + **돌연변이 5종 전량 검출**(1종은 가드를 조인 뒤 재검출).
  green(3007) · 콘텐츠 게이트 오류 0 · lint 0 errors. 코드 변경 0(콘텐츠·문서만).
  ※ 첫 전체 실행이 2파일 실패·18 skipped였으나 **연속 2회 재실행이 272/272 · skipped 0**으로
  전량 green — 일시적 수집 실패로 판정하고 기록만 남긴다.
  ※ 같은 지시에서 **사진 반입(OCR) 배제** 결정(오너 「사진은 ㄴㄴ」)을 #1077 「보류」에 기록.
- **📥 v2-N R3 받아두기 — 자동 캐시는 "최근 연 것"이라 챙겨갈 수가 없었다
  (2026-08-31, 오너 "ㄱㄱ")**:
  **실측한 빈 자리**: 자동 캐시는 뷰어에서 열 때만 걸리고 상한 3의 LRU다 — 4번째를 열면
  첫 자료가 조용히 밀려난다. 비행기 타기 전에 "이거랑 이거 챙겨가자"가 성립하지 않았다.
  **함정 셋을 실측이 미리 잡았다**:
  ⑴ 자료실 목록 조회에 뷰어가 읽는 `raw_text`·`source_pdf_id`·`page_start`·`page_end`·
     `status`가 **없다**(목록은 6컬럼, 뷰어는 10필드). 목록 행을 그대로 담으면 오프라인
     뷰어에 빈 칸이 생긴다 ⇒ 받아두기는 `select('*')` 1회를 한다.
  ⑵ 받아둔 자료를 뷰어에서 열면 자동 캐시가 같은 키를 통째로 덮어써 **핀이 풀린다**
     ⇒ `cacheMaterial`이 이전 `pinned`를 읽어 보존한다.
  ⑶ `getCachedMaterial`이 TTL을 적용해, 받아둔 지 8일 된 자료가 **저장은 살아 있는데
     안 열린다** ⇒ 핀은 TTL 조회에서도 뺀다. 저장돼 있는데 못 여는 건 같은 실패다.
  **두 상한은 별개**(자동 3 · 받아두기 10) — 핀이 자동 상한을 먹으면 3개 받아두는 순간
  R1의 "최근 연 자료" 캐시가 죽는다. 핀은 TTL도 면제한다(R2에서 세운 구분: 파생 사본은
  낡으면 버리고, **사용자가 고른 것**은 안 버린다).
  **화면**: 자료 카드 배지 무리에 「⬇ 받아두기 / ✓ 받아둠」 토글(태그 줄은 320px에서
  이미 3줄까지 간다는 F R3 실측이 있어 그쪽을 피했다) + 「받아둔 것만」 칩(F R3 칩 패턴
  재사용, 받아둔 게 0이면 칩 자체가 없다). 카드 전체가 클릭 대상이라 stopPropagation.
  **렌더 실측에서 내가 만든 회귀 1건 자적발**: 버튼을 넣자 320·360px에서 문서 폭이
  386px로 **넘쳤다**. 버튼만 뺀 대조군으로 원인이 이 버튼임을 확인하고 배지 무리에
  wrap을 줘 넘침 0으로 복구. (태그 줄이 세로로 쪼개지는 답답함은 대조군에도 있어
  **원래 있던 것** — 이 라운드에서 넓히지 않았다.)
  **하네스 함정도 하나**: `newPage({viewportSize})`가 이 playwright-core에선 안 먹어
  `innerWidth`가 1280 고정이었다 — 첫 측정이 통째로 무의미했다. `setViewportSize`로
  고쳐 재측정(K R3 하네스와 같은 방식). 뷰포트를 재는 하네스는 **innerWidth를 함께
  찍어 검증**한다.
  **낡은 계약 3건 정리**: 내 변경이 F R3 계약 둘을 깼는데, 둘 다 `const filtered =` 라는
  **좌변 이름을 얼린** 계약이었다(요구가 아니라 구현 형태를 고정). 요구 기반으로 재작성.
  그리고 내가 심으려던 새 계약에도 같은 병이 있어 함께 고쳤다. 또 내가 금지된 raw
  `slice(…indexOf(`를 써서 `contractHygiene`에 걸렸다 — sliceBetween 정본으로 수리.
  계약 15종 + **돌연변이 9종 전량 검출**. green(2994). 마이그레이션 0.
  ⇒ **N 축 R1·R2·R3 완결.** (직후 정정: 같은 항목에 「v2 축 전량 종료」라고 썼는데
  **틀렸다** — H 축은 R1·R2만 끝났고 **R3가 오너 결정 대기**로 남아 있다. 보드가 단일
  진실원인데 여기가 틀리면 다음 세션이 "v2엔 남은 게 없다"로 읽는다. 이번 세션에만
  낡은 값 2건(규약 문서 쿼리 총량 29 vs 실측 28, 설계 §0 전제 4건)에 물렸던 것과
  같은 종류라 즉시 고친다. ⇒ **내가 착수할 수 있는 v2 축은 전부 끝났고, H R3만
  오너 결정 대기다.**)
- **📴 v2-N R2 오프라인 복습 — 큐가 시각을 안 실으면 기록이 조용히 왜곡된다
  (2026-08-31, 오너 "N ㄱㄱ")**:
  착수 실측이 범위를 크게 좁혔다. R1 이후 오프라인에서 **이미 되던 것**: 단어장 조회
  (스냅샷 폴백)·복습 세션 시작(`startSession`의 rung 조회에 이미 실패 폴백이 있었다).
  깨지는 건 **쓰기 하나**뿐이었다 — 채점하면 `recordReviewCompleted`가 `{ok:false}`를
  돌려주고 「복습 저장 실패 … 다음에 다시 나와요」가 뜬 뒤 **채점이 유실**됐다.
  `/study` 일일 세션은 조립이 `studyMaterials.js`(서버 전용, 콘텐츠 레지스트리 import)라
  오프라인이 **구조적으로 불가** — 이 라운드에서 자연히 빠졌다.
  **순진한 재시도로는 안 되는 이유(이 라운드의 핵심)**: `logReviewEvents`가 `created_at`을
  안 보내고 서버 `DEFAULT now()`에 기댄다. 그대로 재시도하면 비행기에서 한 복습이
  **착륙 시각**으로 찍혀 14일 창(헷갈린 말·유형 큐·약점 프로파일)·주 경계(주간 리포트)·
  연속 학습일이 전부 밀린다. 그래서 큐가 `reviewedAt`을 담고 flush가 그걸 `created_at`으로
  보낸다. 컬럼이 이미 있어 **마이그레이션 0**. 정본은 **가산적으로만** 고쳤다(주면 존중,
  안 주면 종전) — 기존 14개 호출처 무영향.
  **덮어쓰기 방지**: 어휘 FSRS는 append가 아니라 UPDATE라 재생이 다른 기기의 복습을
  되돌릴 수 있다. flush가 서버 `last_reviewed_at`을 읽어 **내 복습이 더 최신일 때만**
  적용한다(`shouldApplyGrade`). 이벤트는 append-only라 이 조건과 무관하게 항상 실린다 —
  기록과 상태는 층이 다르다.
  **중복 방지**: 유니크 제약이 없다(스키마 변경은 하드리밋). flush가 담긴 시각 구간의
  기존 이벤트를 1회 읽어 (출처·항목·시각) 완전 일치를 건너뛴다. 온라인 경로도 같은
  `reviewedAt`을 싣게 한 것이 이 대조가 성립하는 근거다.
  **TTL을 두지 않았다** — R1 캐시는 파생 사본이라 7일 TTL이 맞지만, 큐가 담은 건 **원본
  이력**이라 버리면 그 복습이 세상에서 사라진다. 개수 상한(500)만 둔다.
  **선례 재사용**: R1의 IndexedDB를 v2로 확장(별도 DB 신설 금지 — 두 DB는 버전이 갈려
  서로를 blocked로 세운다), `syncVocabEncounters`의 DI 스타일, `OfflineNotice` 문구 정본.
  **화면(오너 선택 '안 A' — 조용한 큐)**: 채점 중엔 침묵(리듬을 끊지 않고, 전역 배너가
  이미 오프라인을 말하고 있어 중복), 단어장에 「복습 n개가 기기에 저장돼 있어요」 한 줄,
  동기화되면 토스트 1회. 폴링 없음 — 카드 전환과 flush 신호 두 시점에만 다시 센다.
  **자체 검수에서 결함 1건 자적발**: 오프라인 분기가 `enqueueReview` 결과를 안 보고
  무조건 `{ok:true}`를 돌려줬다 — 사생활 모드처럼 IndexedDB를 못 쓰는 환경에서 유실이
  무증상이 됐을 것이다. 큐 실패는 `ok:false`로 표면화하도록 수리하고 계약으로 고정.
  **돌연변이 운영 개선**: 3번 변이가 처음엔 "살아남은" 것처럼 보였는데, 확인해 보니
  bash가 `${}`·백틱을 먹어 **치환이 아예 안 됐던** 것이다. 이제 변이는 스크립트 파일로
  돌리고 **적용 여부를 먼저 확인**한다(K R3의 `git checkout` 사고에 이은 두 번째 운영 교훈).
  계약 24종 + **돌연변이 9종 전량 검출**. green(2979) · world(1102) 동시 확인
  (정본 2본을 건드려 world 스위트까지 돌렸다). 마이그레이션 0.
  ⇒ N R3(선택 다운로드)만 남는다.
- **📐 v2-K R3 2차 — 잔여 11건을 실물 브라우저로 전수 실측, 하나만 진짜 결함이었다
  (2026-08-31, 오너 "K R3 ㄱㄱ")**:
  1차가 남긴 「미착수」 두 줄(`560` 7곳 · `min-900`·`min-760`)은 **추측이었다**. 2차는
  재고 시작했다 — 실물 Chromium에 실제 DOM과 실제 `index.css`를 물려 뷰포트 17폭
  (360~1920)에서 각 요소의 **실폭**을 재고, 뷰포트를 1:1로 따라가는지 갈라지는지 대조.
  **판정을 가른 구조 사실**: 챕터·어휘 페이지가 `maxWidth: 760`으로 캡돼 본문 실폭이
  **712px를 못 넘는다**(뷰포트가 2560이어도 712). 그리고 `--sidebar-width: 240px`는
  **정의만 되고 아무 데서도 안 쓴다** — 학습 웹에 상주 사이드바는 없다.
  **결함 1건(수리)**: 요음 획순 패널은 글리프가 둘이라 306px를 먹는데 본문 상한이 712px다.
  뷰포트 **759→760에서 요음 표가 683→322px(−53%)** 로 무너졌다(기본 세트는 683→478, −30%).
  1px 차이로 콘텐츠가 반토막 나는 건 어떤 설계 의도도 아니다. 패널의 획순 줄을 감싸
  글리프를 세로로 쌓아 패널 폭을 기본 세트와 같은 한 글리프로 맞췄다 → 요음도 478px로
  회복되고 낙폭이 기본과 같아진다. **기본 세트는 17폭 전부 수치 불변**(회귀 0, 전후 대조).
  글리프 한 변은 `--kana-glyph` 하나로 모았다(패널 폭이 그 값에 걸려 있어 두 곳에 두면
  조용히 갈린다) — 이관이 무해함은 ≤560의 120px가 전후 동일함을 재서 확인.
  **1차 추측 2건 기각**: `review-dash`(min-900)·`jpmap-layout`(min-760)을 「컨테이너
  후보」로 적었으나 실측은 둘 다 `page-container` 직속이라 **1:1 추종**(상수 오프셋 48).
  옮기면 같은 답을 다른 숫자로 쓰는 churn이고, 옮기는 과정의 시각 변화만 새로 생긴다.
  **역전 2건은 실재하나 무해라 유지**: `gojuon-cell`(1920px에서 셀 87.6px인데 데스크톱
  크기 25.6px, 560px에서 95.2px인데 모바일 크기 21.6px) · `fr-pager`(560에서 폭 500 1열,
  561에서 485 2열). 넘침이 없어 고치면 얻는 것보다 잃는 게 크다 — 다음 세션이 다시 재지
  않도록 수치를 남겼다.
  **덤으로 잡은 낡은 값**: 문서가 쿼리 총량을 「현재 29」로 적고 있었는데 실측 28이다
  (1차가 767을 없앨 때 이 줄이 안 따라왔다). 이제 문서·계약이 갈리면 계약이 잡는다.
  **뷰포트 쿼리 신설 0**(기존 `min-760` 블록에 규칙을 더했을 뿐) — 동결 목록 불변.
  ⚠ **운영 교훈**: 돌연변이 되돌리기에 `git checkout -- <파일>`을 썼다가 **아직 커밋
  안 한 그 라운드의 수정까지 날렸다**(재작성으로 복구). 돌연변이 복구는 파일 백업
  (`cp`)으로 하거나, 계약을 먼저 커밋한 뒤에 돌린다.
  계약 5종 신설 + **돌연변이 8종 전량 검출**. green(2955). 마이그레이션 0.
  ⇒ **K 축 R1·R2·R3 완결.**
- **🔗 v2-F R2 공급 개통 — 막고 있던 건 소스가 아니라 크론 한 줄이었다
  (2026-08-31, 오너 "F ㄱㄱ")**:
  설계 §0 갭 2는 「추천이 JA/EN 전용 — `fetchSuggestions`·`content-sources` 모두 zh/fr 0」.
  소스가 0인 건 **맞았다**(F R1 때 §0이 틀렸던 것과 달리 이번 전제는 실측과 일치).
  그런데 소스만 추가해선 **절대 개통되지 않는 구조**였다.
  **진짜 병목**: 크론이 `byLang = { Japanese: [], English: [] }`로 고정돼 있어, DB
  `content_sources`에 French 행을 넣어도 `if (byLang[s.language])`에서 **조용히 버려진다**.
  **두 번째 병목**: `dbSources.length > 0 ? dbSources : DEFAULTS` — DB에 ja/en 행이 하나라도
  있으면 기본값 전체가 죽어, 새 언어를 **코드로 열 방법이 없었다**.
  **이미 준비돼 있던 것**(실측): 읽기 API(`/api/suggestions/today`)는 언어 무관,
  표시 층도 4개 언어 완비(`LANG_NAME_KO`·`LEVELS`에 French·Chinese). DB `language`는
  text 컬럼 — **프랑스어에 한해 마이그레이션 0**. ⇒ 손댈 곳이 크론 하나로 좁혀졌다.
  ⚠ **정정(2026-08-31, 오너 "5건 자세히" 답변 중 자적발)**: 「text 컬럼」은 절반만 맞다 —
  `content_sources.language`에는 **CHECK 제약**이 걸려 있다(`IN ('Japanese','English')`,
  `20260328000100_content_sources.sql`). 완화된 적이 없다(중국어용 CHECK 해제 선례는
  `user_ref_progress`뿐). 프랑스어가 마이그레이션 0인 건 **DB를 안 거치고 코드 기본값으로**
  열리기 때문이지, 컬럼이 자유로워서가 아니다. 부작용: 프랑스어를 DB로 **비활성화하는
  것도 불가**하다(French 행 자체를 못 넣는다).
  **수리**: 편성 결정 둘을 순수 함수로 분리(`suggestionSources.js`) — `resolveActiveSources`는
  기본값을 **언어별로** 보충하고(DB가 아는 언어는 활성 행만 = 의도적 비활성 존중, DB가
  모르는 언어는 기본값으로 개통), `groupByLanguage`는 언어 목록을 소스에서 유도한다.
  ⇒ **배포만으로 프랑스어 공급 개통**(오너 DB 수작업 0 — 하드리밋 회피).
  **Wikinews 언어판 매개변수화**: 같은 MediaWiki API라 서브도메인만 갈면 된다 —
  신규 파서 0. `videoId`에 언어판을 넣어 upsert 키(date,video_id) 언어 간 충돌도 막았다.
  **중국어는 일부러 켜지 않았다** — 하드리밋 「중화권 정치 서술 완전 배제」. 뉴스 피드는
  정치 기사를 자동으로 추천 카드에 올린다. `wikinews_zh` 디스패처는 두어, 오너가 켜기로
  하면 DB 행 하나면 되게 했다. **오너 결정 대기 항목**.
  ⚠ **정정(2026-08-31)**: 「행 하나면 된다」는 **틀렸다**. 위 CHECK가 `language='Chinese'`
  삽입을 거부한다. 실제로는 **① CHECK 완화 마이그레이션 ② 행 삽입** 2단계이고 둘 다
  오너 수동이다. 게다가 하드리밋(중화권 정치 서술 배제)은 그대로라, 켜려면 **정치 기사
  필터가 먼저** 필요하다 — 별도 라운드 사안이지 스위치 하나가 아니다.
  ⚠ **미검증**: 이 세션은 Wikinews 전 호스트가 egress 차단(403 CONNECT)이라 **실제 피드
  산출량을 못 쟀다**. fr 일일 기사 수는 배포 후 확인 필요.
  계약 12종 신설 + **돌연변이 3종 전량 검출**. 전량 green(2915).
- **🎯 v2-A R3 유형 큐 — 없던 건 기능이 아니라 다리였다 (2026-08-31, 오너 "A R3 ㄱㄱ")**:
  설계는 R3을 「신규 화면」으로 뒀는데 **화면이 필요 없었다**. 실측 셋:
  ⑴ `confusedQueue`도 화면이 아니라 VocabPage의 **한 줄 진입 버튼**이다(「⚔ 헷갈린 말」).
  ⑵ **`reviewMode`가 이미 `auto|flash|typing|context|listening`으로 존재**한다 —
     "듣기로만 복습"은 원래 가능했고, 사용자가 **스스로 알아내 드롭다운을 바꿔야** 했다.
  ⑶ 함께 넣자던 **v1-7(성조 듣기)은 구현체가 없다**(성조는 표시·채점 폴딩·TTS 음색뿐) —
     합류할 대상이 없어 뺐다.
  ⇒ R1이 진단하고(ProfileStats 한 줄) R2가 편성에 반영했는데, 사용자가 **한 번에 그
  연습으로 들어갈 길**이 없었다. 그 다리 한 줄이 이 라운드의 전부다.
  **처방은 1:1로 떨어지는 둘만**(typing·listening). `choice`·`cloze`를 '문맥'에 억지로
  붙이면 진단과 처방이 어긋나고(고른 문제에 약한데 빈칸을 준다) `order`·`match`·`produce`는
  단어 복습에 대응물이 없다 — 허용 목록이라 모르는 축은 줄이 안 뜰 뿐이다. 듣기는
  TTS 없으면 건너뛰고 **다음 순위로 넘어간다**(하나에 막혀 침묵하지 않는다).
  **세는 일은 `confusedVocabWords` 정본에 맡긴다** — 방식으로 거르고 넘길 뿐이라 14일 창·
  가중·하한이 헷갈린 말 큐와 같다(두 큐가 다른 '헷갈림'을 말하면 안 된다).
  **추가 조회 0** — 기존 `confused-events` 쿼리에 `detail` 컬럼 하나만 더했다.
  **돌연변이가 또 공허한 테스트를 잡았다**(A R2에 이어 두 번째): 하한 검사 픽스처가 이미
  빈 배열이라 `>= 2`와 `> 0`을 못 갈랐다. **정확히 1개** 담기는 픽스처로 고쳐 재검출.
  계약 12종 + **돌연변이 5종 전량 검출**(1종은 재작업 후). green(2950). 마이그레이션 0.
  ⇒ **A 축 R1·R2·R3 완결.**
- **🎯 v2-A R2 약점 처방 — 응답이 채점 지점에서 버려지고 있었다 (2026-08-31, 오너 "A R2 ㄱㄱ")**:
  설계 §0 ③은 「`buildExerciseResult`가 `{response, answer, qtype}`를 보유」라 했는데
  **그 함수는 존재하지 않는다**(F R1·F R3에 이어 세 번째 낡은 §0). 실측한 진짜 경로는
  `settle(ok, pickedValue, result)` → `recordSettle`인데, **`pickedValue`(=응답)가
  `recordSettle`로 넘어가지 않고 버려지고 있었다** — 설계의 취지는 맞았고 이름만 틀렸다.
  R1이 유형까지만 진단할 수 있었던 게 이 한 칸 때문이다.
  **① `resp`**: 그 칸을 잇고 `detailBase`에 **오답일 때만** 40자 상한으로 싣는다. jsonb
  안이라 마이그레이션 0, `source`·`isGradedReviewEvent` 무변경(weeklyReport·growthStats 무영향).
  **② `glyph` 축**: R1이 「R2 이후」로 비워 둔 자리를 채웠다. 조건을 좁게 뒀다 — 오답 ·
  어휘 · `typing|listening`(item_key가 곧 정답 표기인 qtype) · `resp` 존재. `ins`(놓친
  글자)만 세고 `del`은 버린다(오타·다른 단어라 근거가 얇다). 옛 이벤트는 resp가 없어
  이 축만 조용히 빠진다 — P1 소급 안전.
  **③ 편성 편향**: **항목 단위 편향은 이미 있었다**(`buildWeaknessMaterials` —
  `computeWeakness`로 이 단어·이 슬러그, cap 3). 없던 건 R1이 만든 **유형 단위**(pattern)
  축이다. `promoteWeakFirst`로 자주 틀린 챕터를 앞세우되 **만들지도 버리지도 않고 순서만**,
  상한 3(EWMA 다이얼과 핸들 다툼 방지).
  **돌연변이가 내 공허한 테스트 둘을 잡았다**: ⑴ glyph 정답 가드 — 오답 픽스처의 resp가
  정답보다 *길기만* 해 `ins`가 0이라 가드를 빼도 통과했다 ⑵ 편성 상한 — 후보 **전부**를
  약점으로 두면 상한 유무가 순서를 안 바꾼다. 둘 다 실제로 가르는 픽스처로 고쳐 재검출.
  계약 14종 + **돌연변이 6종 전량 검출**(2종은 재작업 후). green(2938). 마이그레이션 0.
- **🔗 v2-F R3 공개 자료 고르기 — 표제는 이미 출하돼 있었다 (2026-08-31, 오너 "F R3 ㄱㄱ"
  → 범위 선택 "남은 두 조각")**:
  설계 R3은 「공개 탭 = 최신순 + 제목 검색뿐」이라 했는데 **실측이 다르다**. 정렬은 이미
  4종(최신·**내 수준 맞춤**·쉬운순·제목순)이고 `sortByFit`·맞춤도 줄·`✓ 완독`·진도 %가
  전부 붙어 있다 — R3 표제(「최신」→「나에게 맞는 순」)는 **#1108(서재 i+1 R2)에서 이미
  끝나 있었다**. F R1에 이어 이 설계 §0이 두 번째로 낡았던 셈이다.
  **설계가 든 재료 3개 중 하나는 하드리밋에 막혀 있다**: `reading_progress`는
  `.eq('user_id', …)`로 잠겨 **타인 읽힌 횟수·완독률 집계가 불가**하다(집계 뷰/카운터 =
  DB 작업). 그래서 오너에게 범위를 물었고 「남은 두 조각」을 택했다.
  **① 「안 읽은 것만」 필터**: 이미 로드된 완독 인덱스를 재사용해 **추가 조회 0**.
  완독분만 걷어내고 **읽는 중은 남긴다** — 숨기면 이어읽기가 목록에서 사라진다.
  정렬 뒤에 적용해 '좁히기'지 '정렬 변경'이 아니다. 게스트는 칩 자체가 없다(진도 0).
  **② 「같이 읽기」 태그**: 내 그룹이 이번 주 읽는 자료 표시. 뷰어용
  `fetchGroupsReadingMaterial`은 자료 1건당 1조회라 목록(12+)엔 N+1이므로 목록형
  `fetchGroupReads`를 쓰고, **쿼리 키를 홈(`useGroupEntryItem`)과 일부러 같게** 해
  캐시를 그대로 타 왕복이 0이다. 순수 `groupReadIdSet`으로 이음새를 빼 실제 유닛 검증.
  **렌더 A/B 실측(320·390·768px)**: 가로 넘침 0, 새 칩은 정렬과 같은 줄로 내려가 짝을
  이룬다. 320px에서 복습+같이읽기+완독 **동시**일 때만 태그 2→3줄 — 드문 조합이고 라벨을
  줄이면 뜻이 흐려져 **재고 끝에 남겼다**(측정 사실을 코드 주석에 박제).
  **내 계약이 내 주석을 잡았다** — 「자료별 조회 금지」 부정 단언이 훅 JSDoc의 설명 문구에
  걸렸다. `codeOf`로 주석을 걷어 코드만 보게 고쳤다. 계약 9종 + **돌연변이 4종 전량 검출**.
  전량 green(2924). 마이그레이션 0.
- **🏷 브랜드 복원 R2·R3 — 전수 스캔 결과 잔여 0, 그래서 스캔을 상시화했다
  (2026-08-31, 오너 "ㄱㄱ")**:
  설계 R2는 「나머지 22도시 미검사」였다. R1의 동어반복 스캐너로는 부족하다 — **가짜
  요미형(에비스·나카노)은 앞 설명과 글자가 달라 그 스캐너에 안 걸린다**. 그래서 그 계열의
  진짜 서명을 먼저 찾았다: **「순한글 이름」에 (외국어 읽기)가 붙은 꼴**. R1이 고친 요미
  4건이 전부 이 구조였다 — **한국어 구절에는 일본어 독음이 있을 수 없다**. 집 관례는 정반대
  한 방향뿐이다(「실명(한글 읽기)」).
  **전수 스캔(도시 파일 32종) 실측 = 잔여 0건.** 일반화 피해는 R1 범위에 갇혀 있었다.
  **오탐 3건은 근거와 함께 계약 주석에 박제**했다(다음 세션이 재확인 반복하지 않게):
  bordeaux 「물의 거울」(노드 실명 「Place de la Bourse」는 살아 있는 부차 언급) ·
  lyon 「정석 한 바퀴」(코드 주석의 동선 표현) · lyon 「트라불」(같은 파일이 이미
  `트라불(traboule)`로 실명 제공). ②차 스캔의 name 66건도 전부 도시명·NPC 역할명
  ('시장 상인'·'약사')으로, 설계가 권장한 「이름 짓지 말고 서술로」에 **부합**한다.
  **0건 결과를 주장이 아니라 계약으로** 굳혔다 — 스캔을 상시 판으로 심었고, kana뿐 아니라
  **모든 외국 문자**를 읽기 자리에서 봐 zh·fr 도시까지 덮는다. 돌연변이로 검증: 교토에
  가짜 이름을 심으니 **새 탐지기만 잡았다**(R1 값 고정도, 동어반복 스캐너도 못 잡는 자리).
  **R3 정책도 함께 넣었다**(R2가 0건으로 끝나 얇았고, 정책 없는 스캐너는 테스트일 뿐):
  정본 §1.1 「실재하지 않는 고유명을 만들지 않는다 — 일반화가 필요하면 이름을 짓지 말고
  서술로 풀어라」 + 신규 SPEC 요미 검수 1줄. 계약 1종 신설, 돌연변이 2종 검출.
  world 1102 green · 본 스위트 2903 green. 코드 0 — 문서·계약만.
- **🏷 브랜드 복원 R1 — 일반화가 만든 것은 안전한 카피가 아니라 가짜 일본어였다
  (2026-08-31, 오너 "브랜드 복원 R1 ㄱㄱ")** 🧊 동결 예외:
  2026-07-24 IP 판정이 A그룹을 일반화하며 실명을 지우고 **없는 고유명 + 그 직역 요미**를
  넣었는데, 그 요미가 틀렸다. `refsLang: 'ja'` 노드의 학습 보조 표기라 **학습자에게 그대로
  노출**된다 — 명분은 브랜드가 아니라 **학습 정확성**이다.
  **출처 대조**: 설계가 「목표값은 제안, 착수 시 출처 대조 필수」라 못 박아 전건 대조했다.
  git 이력은 **shallow clone(152커밋)이라 일반화 커밋이 범위 밖** — 확인 불가를 확인하고
  외부 사전으로 갔다. 大濠公園=おおほりこうえん(kotobank·공식 사이트) · 一風堂=いっぷうどう
  (1985 **大名** 창업, 白丸元味·赤丸新味) · ベイサイドプレイス博多(공식, 志賀島 항로) ·
  中野ブロードウェイ(1966 개업) · 恵比寿ガーデンプレイス · スターバックス 福岡大濠公園店
  (池のほとり·湖畔 테라스) — 스타벅스는 **검증 후에** 복원했다.
  **리포 자체가 증거였다**: 주석엔 `一風堂 大名本店`·`一蘭 본사`가 그대로 있고, 지구 라벨은
  `大濠公園／福岡城跡`, 역 id는 `ohori-koen`, 노드 id는 `fukuoka-ippudo` — **`name`/`desc`만**
  일반화돼 파일이 자기모순 상태였다.
  **설계보다 나빴던 3건**: QAGOMA·MCG·DDP는 '미확인'이 아니라 **동어반복**이었다
  (`'강변의 현대미술관 「강변 현대미술관」'`). 형제 노드가 전부 「설명 「고유명(읽기)」」
  관례를 지키는데, 고유명 자리에 **설명을 복사**해 넣은 자국이다. 실명으로 채웠다.
  **확인만 하고 무변경**: `kyoto/nishiki-market`(錦市場)·`sydney/qvb`(Queen Victoria
  Building)는 멀쩡했다 — 설계 지시대로 확인만.
  **계약**: 동어반복 **전 도시 스캔**을 심었다(조사·공백 정규화 후 앞 설명과 대조) — 이번
  3건을 실제로 잡아냈고 **R2 전수 스캔의 상시 판**이 된다. + 가짜 요미 부활 금지 · 복원 값
  고정 · 스탬프 우주 불변. 선행 계약 2건(`대형 잡화점`·`하카타 라멘 골목` 고정, `総本店`
  금지)은 구 정책 산물이라 **파사드·챕터·id와 이름의 정합**이라는 진짜 요구로 재작성.
  **기존 계약이 이음새를 잡았다**: `refs: ['ラーメン']`가 **가짜 요미 안에만** 있어
  (`はかた ラーメン こうじ`) 요미를 고치니 표기가 사라졌다 — `cityNodeRefs` 계약 3이 FAIL.
  본문에 심어 해소. **`npm test`는 world를 제외**하므로 `npm run test:world`로 직접 게이트.
  계약 6종 신설 + **돌연변이 6종 전량 검출**. world 1101 green · 본 스위트 2903 green.
- **📌 IP 정책 정합 — 문서가 결정을 5주간 덮고 있었다 (2026-08-31, 오너 "IP ㄱㄱ")**:
  브랜드 정책 v1(#642, 2026-07-25 **"최대한 활용"**)은 문서로 멀쩡히 살아 있었는데,
  CLAUDE.md 하드리밋에는 조건도 예외도 없는 `IP: 상호·인물·작품·브랜드·국기·엠블럼
  무재현` 한 줄만 있었다. **세션은 CLAUDE.md를 자동 로드하고 정책 문서는 찾아 읽어야
  한다** — 그래서 단정형 한 줄이 오너의 실제 결정을 덮었고, `ot-12-menzei` 돈키호테가
  그 틈에서 멈춰 섰다. 오너 확정("아주 적극적으로 사용해야함")으로 정합을 맞춘다.
  **교체**: 하드리밋을 「실존 상호·브랜드·시설·작품명은 텍스트로 적극 사용」 +
  **금지 넷**(⑴시각 재현 — 로고·엠블럼을 도트/이미지로 그리기 ⑵저작물 본문 복제
  ⑶허위·비방 맥락 ⑷중화권 정치)으로. 위험이 실제로 사는 자리는 **텍스트가 아니라
  시각 재현**이라 금지를 거기로 좁혔다.
  **정책 정본 v2**(`docs/policy-brand-content.md`): 허용목록(등재제) 폐기 — 허용목록은
  마찰이라 '적극 사용'과 정면으로 싸운다. v1 등재부 10행은 **이력**으로 보존하되
  게이트에서 내렸다. 감사표(`audit-ip-candidates.md`)에는 스테일 경고를 달았다
  (A/B/C 분류가 "무재현" 전제 위에 쓰였으므로 현행 기준으로 오독되면 같은 사고다).
  **발주 §0의 사실 오류를 바로잡았다**: 「등재부가 실물로 존재하지 않는다」는 **틀렸다** —
  등재부는 v1 §2에 10행으로 실재했다. 실패 원인은 등재부 부재가 아니라 **CLAUDE.md 한
  줄의 우선순위**였다. 원인을 잘못 짚으면 수리도 빗나가므로 문서에 함께 남겼다.
  **정책만 고치면 반복된다** — 사고의 구조적 원인은 내용이 아니라 **포인터 부재**였다.
  그래서 CLAUDE.md에 정책 정본 포인터를 박고 둘의 정합을 `ipPolicy.test.js`로 고정했다
  (선례: `uiConventions`·`designBankCommand`의 이중 진실 방지). 계약은 **문구가 아니라
  요구**를 잡는다 — 옛 단정형 부활 금지 · 금지 넷 존재 · 포인터 존재 · 등재제 부활 금지.
  **owner-gate 해소**: 돈키호테 항목 삭제, Codex-5 R3 SPEC '보류 특별 지시' 해제(#150 고지).
  ⚠ 그 과정에서 **R3(#1154)가 구 정책으로 브랜드를 일반화한 것을 발견**했다
  (からあげクン→からあげ, ローソン 삭제) — 재검수 시 IP 사유인지 오탐 예방인지 갈라 판정.
  계약 4종 신설 + **돌연변이 6종 전량 검출**. 전량 green(2903). 코드 0 — 문서·계약만.
- **🐛 비로그인 옛 버전 — 캐시가 아니라 라우팅 비대칭이었다 (2026-08-31, 오너 "미로그인 시
  아직도 다르게 나오나?? 해결 우선" → 갈래 선택 "둘 다 /home으로 통일")**:
  오래 미처리로 남아 있던 보고를 **로컬 프로덕션 실측**으로 종결했다. 앞선 두 차례 조사가
  1순위로 지목했던 후보 둘이 모두 **반증**됐다.
  **배제 ① 서비스워커 캐시**: `public/sw.js`의 내비게이션은 network-first이고 `activate`가
  현행 아닌 캐시를 전부 지운다 — 옛 HTML을 붙들 경로가 없다.
  **배제 ② `/lessons` CDN 캐시**: 빌드 산출이 `ƒ /lessons`(Dynamic)이고 실제 응답 헤더는
  `Cache-Control: private, no-cache, no-store, max-age=0, must-revalidate`다. 즉 **항상 최신**.
  거기 있던 `export const revalidate = 60`은 `searchParams`를 await하는 동적 라우트라
  **런타임 효과가 0**이었다 — 효과 없는 그 한 줄이 조사를 CDN 오진으로 끌고 다녔기에 지웠다.
  **확정: 라우팅 비대칭**. `/`가 쿠키로 갈려 **비로그인 → `/lessons`(교재 목록 + 언어별
  피치 카드), 로그인 → `/home`(대시보드)** 로 *아예 다른 화면*을 보여 줬다. "옛 버전"은
  옛 배포가 아니라 **다른 페이지**였다. 뒤집힌 사실 하나 더 — 캐시되는 쪽은 오히려
  로그인 쪽이다(`/home`은 Static·`s-maxage=31536000`).
  **수리**: `/`의 분기를 걷어내고 상태 무관 `/home`으로 통일(라우트가 `○ Static`이 됐다).
  다만 로그인 벽만 세우면 **볼 것 있던 화면이 볼 것 없는 화면으로 나빠지므로**, `/home`
  게스트 분기를 「로그인이 필요합니다」 한 줄에서 **들어갈 수 있는 문 3개**(교재·자료실·
  로그인)를 가진 첫 화면으로 채웠다.
  **선행 계약 2건이 요구가 아니라 구현 모양을 얼려 뒀다** — `a11yAuditR1`이 목적지 삼항을,
  `performanceRepairs` P3가 `revalidate = 60` 리터럴을 통째로 고정하고 있었다. 각각
  진짜 요구(**랜딩 부활 금지** / **동적 구조 유지**)로 재작성하고 돌연변이로 재확인했다.
  실측: 쿠키 유무 양쪽 `/` → 307 → `/home`, 실서버 렌더 390·320px 문 3개·가로넘침 0.
  계약 6종 신설 + **돌연변이 8종 전량 검출**. 전량 green(2899).
- **📐 v2-K R3 1차 — 브레이크포인트 이관, 측정이 절반을 기각했다 (2026-08-31, 오너 "K R3 ㄱㄱ")**:
  설계가 R3을 「값 병합은 공짜가 아니다 · **한 건씩, 해당 화면 실기 확인 후**」로 못
  박았으므로, 이관 후보 4군을 **하나씩 렌더로 재고** 결정했다. 절반이 기각됐다.
  **이관 ① `767 → 768`(뷰어 풀블리드)**: `768`은 모바일 크롬(`--gnb-height` 56px·하단
  네비)이 쓰는 경계다. 뷰어만 767이던 탓에 **폭이 정확히 768px일 때 크롬은 모바일인데
  뷰어만 데스크톱**이었다. 실측으로 확인: 변경은 768px 한 점에만 걸리고 700·767·769·800은
  전부 동일. 두 판단이 다시 갈리지 않게 **"뷰어 경계 = 크롬 경계"를 계약으로** 걸었다.
  **이관 ② `880 → @container`(myplan 격자)**: R2가 세운 정책(「신규 접힘은 @container」)의
  첫 실물 전환. 실측이 뷰포트 쿼리의 오답을 잡아냈다 — 260px 사이드바 옆 화면 1000px에서
  격자 실폭은 **712px**인데 뷰포트 쿼리는 "1000 > 880"이라 **2열**을 답했다. 컨테이너
  쿼리는 1열로 옳게 답한다. 선례 `.word-fit-wrap` 그대로.
  **기각 ① `400 → 480`(stats 격자)**: 먼저 "이 규칙은 `auto-fill minmax(220px,1fr)`이라
  중복 아닌가" 가설을 세웠는데 **렌더가 반증**했다 — 규칙을 빼면 400px 미만이 **136px
  2열**로 쪼그라든다(base→768 `1fr 1fr`→400 `1fr` 3단 사다리의 마지막 칸). 480으로 옮기면
  401~480의 2열(196px — 충분)이 1열이 돼 흔한 폰 폭이 나빠진다.
  **기각 ② `420 → 480`(한자 다리)**: 421~480에서 표가 **이미 스크롤 없이** 들어간다
  (실측 0px 넘침). 옮기면 좁은 여백·작은 글자(16.8→16px)를 필요 없는 구간에 확대 적용한다.
  **미착수**: `560`(7곳 — 서로 다른 컴포넌트라 각각 실기 필요) · `min-900`
  (`admin-edit__pencil`은 본문 컬럼 밖 여백을 재는 것이라 **뷰포트가 맞는 축**) · `min-760`.
  동결 목록 11종 → **9종**, 뷰포트 쿼리 29 → **28**. 기각 2건도 근거와 함께 문서에 남겼다
  (다음 세션이 같은 시도를 반복하지 않게) — 그 근거 문구까지 계약으로 고정.
  **v2-L 메타 계약이 나를 잡았다**: 새로 쓴 계약이 raw `slice(css.indexOf(`를 써서
  `contractHygiene`가 FAIL. 정규식 + `toBeTruthy()`로 재작성하고 돌연변이 재확인.
  계약 3종 신설 + **돌연변이 5종 전량 검출**. 전량 green(2893). CSS·문서·계약만 — 코드 0.
- **🔗 v2-F R1 링크 반입 — 사용자가 URL을 넣는 첫 문 (2026-08-31, 오너 "F ㄱㄱ")**:
  반입 입구가 여섯인데(붙여넣기·클립보드·PDF·EPUB·문장 목록·책 묶음) **사용자가 URL을
  직접 넣는 문은 없었다** — 자료는 크론이 고른 추천 카드로만 들어왔다. 오너 방향
  「유튜브 같이 개인이 만든 퀄리티 높은 것들 가져오는 식」의 입구를 연다.
  **설계 §0의 핵심 주장이 어긋났다**: 「크론 수집 → 자막 추출(`youtube.js
  extractTranscript`) → 추천 카드」라는 **검증된 유튜브 파이프라인이 실재하지 않는다**.
  실측: 크론(`content-sources.js`)은 Qiita·NHK RSS·Dev.to·Wikinews **글**만 가져오고,
  `youtube.js`는 **소비처 0인 죽은 코드**다. 그 안이 쓰는 `youtube-transcript`는 문서
  §4.3이 이미 데이터센터 IP 차단을 경고한 물건이라 서버리스 성공률을 장담할 수 없다.
  ⇒ 설계가 「실패 화면을 곁다리가 아니라 주력 경로로」라 한 판단이 **더 옳았던 것**이라,
  422를 정상 분기로 두고 붙여넣기를 1급으로 만들었다.
  **설계에서 고친 것 2건**: ⑴ 취득 3단(transcript→Supadata→설명문)에서 **설명문 단을
  뺐다** — `extractTranscript`가 내부에 이미 갖고 있고, 사용자가 붙인 URL에는 설명문을
  줄 YouTube Data API 키가 없다. ⑵ 대신 **oEmbed**(공개·키 불필요)로 제목·채널을 받는다 —
  자막이 실패해도 폼이 비지 않아야 붙여넣기 경로가 이어진다.
  신설: `linkImport.js`(순수 — `detectLinkKind`·`stripCueNoise`·`paragraphize`·
  `cleanVideoTitle`) · `api/import/link/route.js` · `MaterialAddLinkSection.jsx`.
  **두 취득 경로가 같은 순수 함수를 탄다** — 자동분과 붙여넣기분의 본문 모양이 갈리면
  같은 영상이 두 자료가 된다. 문단 묶음 8줄은 `extractTranscript` 정본과 일치시켰다.
  정책: 링크는 **기본 비공개이되 강제는 아니다**(PDF·EPUB와 다른 점 — 재배포 판단은
  사용자 몫이라 토글을 남긴다). 출처는 `metadata.source`(book 선례) — 스키마 변경 0.
  **판별은 호스트로만** 한다 — `?u=youtube.com`이 섞인 주소가 통과하면 남의 서버를
  대신 두드리게 된다(돌연변이로 확인).
  **돌연변이가 계약의 공허를 잡았다**: 인증 계약이 401 *문자열*만 봐서 관문(`if (authErr
  || !user)`)을 통째로 없애도 통과했다 — 조건과 return을 함께 보도록 재작성.
  렌더 실측에서 `[가져오기]`가 320·380px 모두 혼자 아래로 떨어졌다(flex 기저 220px) →
  140px으로 줄여 한 줄에 세웠다.
  계약 30종 + **돌연변이 16종 전량 검출**. 전량 green(2890).
  **새 테이블 0 · 마이그레이션 0.** 오너 실기 대기: 링크 → 자료 생성 → 뷰어 진입.
- **🇯🇵 v2-G R3 일본어 확장 — 규칙을 실측이 정했다 (2026-08-31, 오너 "G R3 ㄱㄱ")**:
  R1이 중국어만 연 이유가 여기 있었다. 중국어 표기는 슬롯이 한글·로마자라 **한자만
  남기면** 표지가 떨어지는데, 일본어는 고정 형태소가 대부분 **가나**다(〜てから·
  〜なければならない) — 같은 규칙을 쓰면 커널이 **하나도 안 나온다**. 게다가 스캔
  게이트(`isScannable`)가 한자를 요구해 일본어 표지가 통째로 걸렸다.
  **규칙은 손이 아니라 실측이 정했다**(N5 독해 526문장 · kuromoji 토큰 7,334):
  ① 제한 없음 42.4%(は·の·ました·です가 글을 덮는다) → ② 가나 3자↑ 15.2%(중국어와
  같은 수준이나 최다 적중이 ました·あります) → ③ ②+슬롯 있는 문형만 13.4% →
  ④ ③+정중형 활용 제외 **4.6%, 최다 적중이 전부 실제 표지**. ④ 채택.
  난이도에 따라 4.6%(N5 본문)→10.1%(N4)→15.8%(N3)→22.9%(N2 예문)로 자연히 오른다 —
  어려운 글일수록 볼 표지가 많다는 뜻이라 옳은 방향이다.
  **임의 목록 대신 구조 규칙 하나를 찾았다**: とても·たくさん·すこし가 밑줄을 받길래
  출처를 보니 **슬롯이 없는 표기**(852 중 85)였다 — 문형이 아니라 어휘 항목이다.
  `hasSlotMarker`로 구조에서 걸러 손 목록을 그만큼 줄였다.
  `JA_LOW_SIGNAL_KERNELS`(정중형 활용 13종)은 일본어의 '的·了·是'다 — 〜ました는 정본
  문형이 맞지만 과거 정중형마다 밑줄이 붙으면 표지가 아니라 배경이 된다(제외 전 177회 1위).
  **`isScannable` 확장이 중국어에 무해함을 증명하고 넣었다**: 가나까지 받도록 넓힌 뒤
  중국어 예문 651문장·표시 토큰 1,757에 대해 구/신 산출을 대조해 **차이 0**을 확인했다
  (문자 종류가 이미 갈라 준다 — 중국어 커널은 한자뿐이라 가나가 낀 결합이 맞을 수 없다).
  **계약이 실버그를 잡았다**: `hasSlotMarker`가 슬롯 문자 뒤에 가나가 바로 붙으면
  (`Nのために`) 못 잡았다 — 경계를 라틴 문자로만 보도록 고쳤다(커널 690→699).
  배선: `PATTERN_LANGS`/`supportsPatterns`로 언어 게이트를 한 곳에 모으고, **자료 언어가
  바뀌면 인덱스를 갈도록** `patternIndexLang`을 뒀다(안 갈면 중국어 커널로 일본어를 훑고,
  가는 동안 옛 인덱스로 스캔하면 남의 언어 밑줄이 깜빡인다 — 둘 다 계약으로 박제).
  실측: 일본어 커널 699개(852 문형 중 커버 다수), 챕터 링크 보유 576/852.
  계약 26종 신설 + **돌연변이 12종 전량 검출**. 기존 R1·R2 계약 4건은 의존성 배열·
  언어 게이트가 바뀌어 앵커만 갱신(요구는 그대로, 각각 돌연변이로 재확인).
  **새 조회 0 · 새 테이블 0 · 마이그레이션 0.** 전량 green(2860), 렌더 실측(380px).
- **🔗 v2-G '약한 것' 필터 — A↔G 결합점 (2026-08-31, 오너 "G 약한 것 ㄱㄱ")**:
  G R2가 비워 둔 세 번째 자리를 v2-A R1의 산출로 채웠다(G 설계 §4 3분할 완성).
  **경계를 그대로 지킨 채 합류했다** — 내가 G R2에 박아 둔 계약("스캔 층이 약점을
  모른다")을 무르지 않고 **집합만 넘기는 형태**로 만족시켰다: 약점 정본(오답 이벤트·
  점수식·집계 창)은 여전히 v2-A 소유이고, `patternIndex`는 챕터 slug 집합 하나를 받아
  거를 뿐이다. 조회도 v2-A의 `fetchWeaknessRows`를 그대로 재사용 — 같은 재료에 조회를
  두 벌 파지 않았다(신설 조회 0).
  신설: `weakChapterSet`(weaknessProfile — `pattern:` 축만 걸러 Set으로).
  일반화: `hitIsDue`→`hitIsMarked`, `orderPatternsByDue`→`orderPatternsByMark`,
  `dueFilterNote`→`filterNote`(모드별 사유). due든 weak든 판정 모양이 같아 한 벌로 쓴다 —
  축마다 따로 짜면 한쪽만 고쳐지는 자리가 생긴다.
  **상한을 안 걸었다**(cap 0): 본문에 어느 챕터가 나올지 모르는데 "상위 8개"로 자르면
  9번째로 약한 문법이 눈앞에 있어도 밑줄이 안 붙는다.
  **드릴 약점은 못 잡는다**(문서화된 한계): 문법 이벤트의 item_key가 챕터 slug일 수도
  드릴 id일 수도 있는데 역매핑에 챕터 레지스트리(6MB)가 필요하다. 뷰어는 그걸 안 든다 —
  챕터 단위 복습분만 잡고 드릴분은 조용히 빠진다(없는 밑줄 < 엉뚱한 밑줄).
  빈 사유를 모드별로 갈랐다: "약점이 없다"가 아니라 **"아직 가릴 만큼 기록이 없어요"** —
  오답이 쌓여야 약점이 서므로 "없다"고 말하면 거짓말이다.
  표식은 색을 새로 만들지 않고 **테두리 알약**으로 갈랐다(채운 알약 둘 — 레벨 회색·복습
  예정 노랑 — 옆에서 형태만으로 구분).
  **돌연변이가 내 버그를 잡았다**: `filterScan`이 `weak` 모드에서 집합이 아직 없으면
  `!marked → return scan`으로 **전체를 통과**시켰다(fail-open). 필터가 조용히 열리는
  자리다 — 아는 모드는 반드시 좁히도록 고쳤다. 계약을 먼저 써서 잡힌 실결함.
  계약 41종(신규 7 + 갱신) + **돌연변이 10종 전량 검출**. 기존 계약 1건은 통짜 태그
  매칭이 과해(weeklyTile 자기함정과 같은 클래스) 요구만 보도록 재작성.
  **새 조회 0 · 새 테이블 0 · 마이그레이션 0.** 전량 green(2834), 렌더 실측(320/380px).
- **🩺 v2-A R1 약점 진단 — 유형 단위 (2026-08-31, 오너 "A ㄱㄱ")**:
  단어 단위 약점은 이미 있었다(`confusedQueue` — 14일·가중·최소 표본 2). 없던 건
  **유형 단위**다: "이 단어를 자꾸 틀린다"는 아는데 "듣고 쓰기에서만 무너진다"는
  아무 데서도 말해 주지 않았다. 앞은 단어를 더 외우게 하고 뒤는 **연습 방식**을
  바꾸게 한다 — 처방이 다르다.
  **설계(#1077 issuecomment-5446944816) §0 실측 대조에서 2건이 어긋났다**:
  ⑴ `pattern:{slug}` 축의 근거 「`item_key='slug#문항id'`」는 **코드에 없다** — 실제로는
  챕터 복습이 `item_key=slug`, `buildDrillReviewEvent`가 `item_key=drill.id`로 **두 종류가
  섞여** 있고 드릴→챕터 역매핑은 콘텐츠 레지스트리 의존이라 순수 함수 밖이다 →
  **해석기 주입**(`chapterOf`)으로 순수성을 지켰다(`deriveVocabRungs` 선례).
  ⑵ §0③의 `buildExerciseResult`는 **존재하지 않는 함수**다 — R2 재료(`resp` 필드)라
  R1엔 무영향이지만 R2 착수 전 재실측이 필요하다. ①②④는 실측과 일치.
  **설계에서 좁힌 것 1건**: `pos:{품사}` 축은 R1에 **쓸 화면이 없어** 뺐다. 한 줄이 담는
  건 하나고 "명사가 약하다"보다 "듣고 쓰기가 약하다"가 훨씬 행동 가능하다 — 만들어
  두고 안 쓰면 계약만 통과하는 죽은 축이 된다. 소비처가 생기는 라운드에서 넣는다.
  신설: `errorTags.js`(이벤트 1건 → 태그[], LLM 0) · `weaknessProfile.js`(태그 축 집계) ·
  `weaknessRows.js`(조회). 배선: 이번 주 모달에 `약한 곳 — 듣고 쓰기 9번 중 6번 틀림`.
  **점수를 새로 만들지 않았다** — 이벤트를 태그 축으로 투영해 `computeWeakness` 정본에
  넘긴다. 점수식·최소 표본이 그대로라 "헷갈린 단어"와 "약한 유형"이 같은 헷갈림을 말한다.
  **회상 방식은 허용 목록**(제외 목록 아님) — qtype이 20종 가까이 적재되는데 대부분
  회상이 아니다(도움 요청·푸시 열람·완독 계측·자기채점·콘텐츠 고장). 제외 목록이면
  새 qtype마다 정체불명의 축이 화면에 튀어나온다(G R1 허용 목록 뒤집기와 같은 이유).
  실측으로 잡은 오염원: `assist`·`explain`이 **`correct:false`로 적재**된다 — 막혀서
  [도움]을 누른 걸 약점으로 세면 "도와달라 했다"가 "그 유형에 약하다"로 둔갑한다.
  `isGradedReviewEvent`(정답률 정본)는 이들을 통과시키지만 **그쪽은 안 건드리고**
  (설계 §4 무변경 단언) 약점 모듈 안에서만 좁혔다.
  문구는 설계 초안 "6/9 실패"에서 고쳤다 — 이 카드는 **거울이지 성적표가 아니라서**
  (WeeklyReportCard 규약) 분수와 '실패'가 채점표로 읽힌다. 색도 회색 하나.
  계약 34종 신설 + **돌연변이 11종 전량 검출**(허용→제외 목록 뒤집기·도움 요청을 오답으로·
  최소 표본 우회·안 틀린 축 잔류·라벨 없는 축 노출·드릴 id 미역매핑·정답률 정본 축소·
  창 무시·상시 조회·경고색·모달 머리 중복). 기존 계약 1건은 **통짜 태그 매칭이 과해**
  무관한 prop에 깨져 요구(`header={false}`)만 보도록 재작성.
  **새 테이블·새 source·마이그레이션 0.** 전량 green(2826), 렌더 실측(320/390px) 통과.
- **🎯 v2-G R2 문법 표시 필터 — 무엇부터 볼지 (2026-08-31, 오너 "G R2 ㄱㄱ")**:
  R1은 정본 484문형을 전부 후보로 잡는다. "본문에서 문법을 만난다"는 됐지만 **무엇부터
  볼지**는 여전히 독자 몫이었다. 복습이 다가온 문법을 본문에서 다시 만나면 그 자체가
  복습이 된다 — 이미 쌓이고 있는 `grammar_review` 큐(챕터 slug 단위)를 **읽기만** 한다.
  큐의 slug가 문형의 `ch`와 그대로 맞물려서 변환층이 필요 없었다.
  **설계 §4의 '약한 것' 필터는 이번 라운드에서 뺐다** — v2-A(약점 진단)가 미착수
  (#1077 "승인 대기")라 `errorTags`·`weaknessProfile`이 존재하지 않는다. 여기서 만들면
  **중복 신설**이다(제품 나침반 ②). v2-A 착수 뒤 그 산출을 읽어 한 줄로 합류한다 —
  이 판단을 계약으로 박제해 두었다(patternIndex·patternRows가 약점 부품을 모른다).
  신설: `patternRows.js`(due 조회) · `PATTERN_FILTERS`·`dueChapterSet`·`hitIsDue`·
  `filterScan`·`orderPatternsByDue`·`dueFilterNote`(순수 6종). 배선: 시트에 `[전체 |
  복습할 것]` 세그먼트(문법 표시를 켠 중국어 자료에서만), 카드에 **복습 예정** 알약.
  **거른 것은 인덱스가 아니라 산출** — 인덱스는 자료 사이에서 공유·캐시되는 물건이다.
  **잘리는 자리가 3개뿐이라 순서가 곧 노출**이라, 복습 예정 문형을 카드 앞으로 당긴다
  (把는 14문형 — 7번째가 오늘이면 정렬 없이는 영영 안 보인다).
  빈 상태를 4갈래로 갈랐다(비로그인·읽는 중·큐가 빔·이 자료에 없음) — 밑줄 하나 없는
  화면은 "필터가 걸렸다"가 아니라 **"고장"으로 읽힌다**(v2-K). 색 어휘는 단어 복습
  알약(`.vocab-row__due`)과 같은 것을 쓴다 — 같은 뜻에 다른 색이면 두 번 배운다.
  계약 33종 신설(R1 배선 계약 2종은 산출 이름 변경분 갱신) + **돌연변이 9종**
  (due 판정 뒤집기·빈 큐를 전체 통과로·byToken 미축소·정렬이 원본 반환·빈 상태 문구
  통합·밑줄이 필터 우회·미착수 '약한 것' 끼워넣기·전체 모드에서도 조회·표식 삭제)
  전량 검출. **④는 1차에 생존**했다 — `[...patterns]` 복사가 필터 경로에서는 무의미하고
  **조기 반환 경로**에서만 의미가 있었다(인덱스의 통을 그대로 돌려주면 호출자가 오염시킨다).
  총량 대신 그 불변식을 직접 겨눠 재작성하니 검출.
  **새 테이블·새 이벤트·마이그레이션 0.** 전량 green(2792), 렌더 실측(320/380px) 통과.
- **🔦 v2-G R1 문법 패턴 본문 하이라이트 — 문서에 적힌 부채 상환(2026-08-31, 오너 "G")**:
  아키 문서가 이음새 부채로 적어 둔 것: **챕터 → 자료(뷰어) 역방향 없음**. [자세히]는
  뷰어→챕터 단방향이라 챕터에서 배운 문법이 본문에서 다시 만나지지 않았다. 정본 문형은
  중국어만 484개(415개가 챕터 slug 보유)인데 읽는 글과 한 번도 마주치지 않았다.
  **핵심 난점**: 문형 데이터가 `"A 是 B"`·`"주어 + 很 + 형용사"`처럼 슬롯 섞인 표기라
  정규식이 아니다 → 슬롯을 버리고 **고정 형태소(kernel)** 만 남긴다(1단 표지어 스캔,
  LLM 0회). `patternIndex.js`(순수)가 `buildKernelIndex`·`scanTokens`를 맡고, 뷰어는
  옅은 밑줄 + 단어 카드 안 문형 블록(챕터 링크)만 그린다.
  **오탐을 막는 두 겹**: ① 한 글자 커널은 기본 폐기, 把·被·越처럼 구문을 특정하는 것만
  허용 목록으로 되살린다 — **'제외 목록'이 아니라 '허용 목록'으로 뒤집은** 이유는 새 커널이
  조용한 쪽으로 떨어지게 하려는 것(모르는 것이 소음이 되면 토글을 끈다). ② 토큰 경계로만
  매칭 — 문자 부분일치면 觉得의 得가 걸린다.
  실측: 커널 333개가 정본 484패턴 중 345개를 가리키고, 본문 밑줄 밀도는 **토큰의 15%**
  (문법 교재 산문 기준 문자 부분일치 상한 18%). 기본 꺼짐 + '후보' 톤 + 지연 로드(304KB).
  계약 24종 신설 + **돌연변이 4종**(한 글자 커널 전면 허용·최장 일치를 최단으로·겹침 방지
  제거·토글 무시 로드 → 각각 FAIL 확인 후 역치환 복원).
  **돌연변이가 계약의 약점을 먼저 잡았다**: 첫 시도에서 '최장 일치' 돌연변이가 **살아남아**,
  테스트 인덱스에 짧은 커널(越)과 긴 커널(越来越)이 같은 자리에서 겨루지 않는다는 걸
  알았다 — 인덱스를 보강하니 그때부터 잡힌다. 다섯째 돌연변이(문장부호 가드 제거)는
  **행동이 같음을 분석으로 확인**하고(커널은 한자만이라 부호를 낀 결합은 어차피 불일치)
  계약을 억지로 만들지 않았다 — 그 가드는 조기 종료지 계약이 아니다.
  **기존 계약 1건이 배치를 잡았다**: focusMode 계약이 "발음 표기~집중 모드는 언어 무관
  구획"을 지키고 있었는데 그 사이에 중국어 전용 토글을 끼웠다 → 성조 색상·한자 대조가
  있는 중국어 구획으로 옮겼다.
  렌더 실측에서 **오탐 1건 발견·수정**: 시간 명사 今天이 밑줄을 받았다(문형 표기에 등장하지만
  가리키는 건 '언제'지 구조가 아니다) → 시간 명사·인사 15항을 저변별 목록에 추가하고 계약으로 박제.
  밑줄 색도 초록(--accent-text)에서 중립(--text-muted 파생)으로 바꿨다 — 초록은 이 앱에서
  '아는·통과'라 문법 표지에 얹으면 다른 뜻으로 읽힌다.
  **마이그레이션 0 · 새 테이블 0 · 새 이벤트 0 · LLM 호출 0.** → PR #TBD
- **📚 v2-D R3 어휘 축 합류 — D 축 3라운드 완결(2026-08-31, 오너 "D R3 ㄱㄱ")**:
  계획표에는 처음부터 "어휘는 제외"라고 적혀 있었다(설계 §0 문제 ③). 그런데 HSK5든 B2든
  문법 챕터만 끝낸다고 닿는 목표가 아니고, 사용자는 **이미** 단어를 담고 '이미 앎'을 찍고
  있었는데 그 둘이 목표와 만나지 않았다. `goalVocab.js`(순수)가 정본 어휘 인덱스와
  단어장·'이미 앎'을 대조해 "정본 4,645개 중 597 확보 (13%)"를 레벨별로 낸다.
  **확보 = 담음(=FSRS 큐) ∪ 이미 앎** — 둘 다 다시 배우지 않아도 되는 말이라 한 셈이다.
  궤도 카드가 탭 가능해지고(타일→모달 정본 재사용), 상세 모달에 **진행·속도·어휘 3줄**이
  선다(설계 §2의 MyPage 상세 목업 그대로).
  **핵심 판단 — 무거운 정본은 상세를 열 때만**: 중국어 어휘 정본은 소스만 1.9MB라 홈 첫
  화면에 얹으면 한 줄짜리 진단이 페이지 값을 치른다. 인덱스 로드와 '이미 앎' 조회 둘 다
  `open` 게이트 뒤에 뒀고 계약으로 고정했다.
  **설계 대비 정정 2건**: ① 설계 목업은 "H5 필수 1,300 중 812"(해당 레벨만)였으나
  **목표까지 누적**으로 냈다 — 목표는 H5 한 레벨이 아니라 거기까지 가는 길 전체이고,
  레벨별 내역은 그 아래 줄에 그대로 있다. ② "FSRS 보유량"은 단어장과 별개 축이 아니었다 —
  담은 단어가 곧 FSRS 큐라 실제 재료는 셋이 아니라 **둘**이다(익힘 tier(repetitions≥2)로
  더 쪼갤 여지는 남겨 뒀다).
  어휘 정본이 있는 언어에서만 줄이 선다(현재 중국어 — `refVocabIndex` 승인 범위).
  단어장 조회에 `language` 한 칸 추가(새 쿼리 0) — 없으면 남의 언어 단어가 확보로 샌다.
  계약 16종 신설 + **돌연변이 3종**(목표 밖 레벨도 계산·language 빈 옛 행 폐기·정본을 홈에서
  즉시 로드 → 각각 FAIL 확인 후 역치환 복원).
  **내가 처음 쓴 계약 1건을 스스로 물렀다**: "R3 이후 마이그레이션이 하나도 없다"로 세면
  다음 축의 정당한 스키마 변경에 깨진다 — 지키려는 건 "R3은 읽기만 한다"이므로
  **읽는 대상을 고정**하는 형태로 바꿨다(weeklyTile 총량 계약과 같은 교훈).
  **마이그레이션 0 · 새 테이블 0 · 새 이벤트 0.** → PR #TBD
- **📈 v2-D R2 목표와 궤도 — 역산을 붙였다(2026-08-31, 오너 "D R2 ㄱㄱ")**:
  R1로 계획표가 곧 진도표가 됐지만 표는 "몇 개 했다"까지만 말했다. 목표일이 정해져 있는데
  **역산이 없어** 그 진도가 빠른지 늦은지 알 수 없었다. `goalPace.js`(순수)가 남은 일수·
  하루 필요량·최근 2주 실제 속도·예상 완료일을 내고, 판정은 세 단계(여유·적정·이탈)까지만
  한다 — 벌칙·독촉 없음(Beeminder 배제 근거). 홈 벤토 머리에 궤도 한 줄(진도 막대 포함).
  **저장은 `dday_date` 재사용 + `goal_lang`·`goal_level` 2컬럼**(마이그레이션 1본,
  `20260831010000_profile_goal.sql`). **운영 DB 적용 완료**(오너 "1 완료", 2026-08-31) —
  이하 폴백 서술은 적용 전 동작 기록이다. **운영 DB 적용은 오너 수동** — 컬럼 부재 폴백
  (`profile_levels_fr_zh` 선례)이 걸려 있어 적용 전에도 D-Day 저장은 그대로 되고, 과정을
  고른 경우엔 "날짜는 저장했어요 — 목표 과정은 아직 준비 중이에요"로 알린다(조용히 삼키면
  "골랐는데 궤도가 안 뜬다"가 된다).
  **설계 대비 정정 3건**: ① 설계는 편집기를 MyPage에 뒀으나 **날짜 편집기가 실제로는 홈
  D-Day 타일에 있다** — 목표를 두 화면으로 쪼개지 않으려고 그 모달 하나에 모았다(제목
  "목표 · D-Day"). ② 설계 §4-5의 "목표 미설정 시 침묵(weeklyReport hasAny 결)"은 그대로
  따르되, 2026-08-30 "상시 표시" 확정과 충돌하지 않음을 확인했다 — 사라지는 게 아니라 아직
  생긴 적이 없고, **세우는 자리(D-Day 타일)는 늘 자리를 지킨다**. ③ 홈은 정본 manifest를
  번들에 안 끌어오므로 이미 받아 둔 투영(`continueManifest`)으로 `buildPlan`을 돌린다 —
  두 투영이 같은 계획을 내는지 계약으로 고정(챕터 수·레벨 순서·slug 순서 4언어 전수).
  계약 25종 신설 + **돌연변이 4종**(침묵 깨기·0 나눗셈 방어 제거·미완 챕터를 속도로 계산·
  컬럼 부재 폴백 제거 → 각각 FAIL 확인 후 역치환 복원).
  **기존 계약 1건이 내 변경을 잡았고, 그 계약이 과했다**: weeklyTile이 `bento--4x1 card`
  **총량 1개**를 세고 있어 무관한 새 카드에 깨졌다. 지키려는 건 "주간 거울이 와이드로
  부활하지 않는다"이므로 주간 컴포넌트 안쪽만 보도록 고쳤다(통짜 문자열 배선 계약과 같은 결).
  렌더 실측에서 **실결함 1건**: 좁은 화면(390px)에서 본문이 두 줄이 되면 6px 진도 막대가
  flex-shrink로 **0까지 눌려 사라졌다** → `flex: 0 0 6px`. 테스트로는 잡을 수 없는 종류.
  → PR #TBD
- **🎯 v2-D R1 목표를 데이터로 — 삼중 진실을 하나로(2026-08-31, 오너 "D ㄱㄱ")**:
  진도표가 세 곳으로 갈려 있었다. 계획은 `StudyPlanPanel`의 PLAN 상수(zh 119·fr 104 항목
  하드코딩), 진도는 서버 `user_ref_progress`, 체크는 손으로 누르는 localStorage. 항목이
  `[1, '한어병음']` 꼴이라 **slug가 없어 정본과 대조조차 안 됐고**, 실측하면 정본은
  zh 79·fr 87챕터로 **이미 어긋나 있었다**. 손 체크는 실제로 읽었는지와 무관해서 표가
  학습을 반영하지도 못했다.
  이제 계획은 정본 manifest에서 유도하고(`buildPlan`), 진도는 서버 한 곳에서만 읽는다
  (`markProgress`, `passed ?? read`). 계획표가 곧 진도표라 누를 것이 없고, 정본 챕터가
  개편되면 표가 저절로 따라온다. 칩은 죽은 토글이 아니라 그 챕터로 가는 문이 됐고,
  레벨 머리에 문형·어휘 화면으로 가는 길을 놓았다(어휘 진도 합산은 R3).
  **설계 대비 정정 2건**: ① 설계는 "체크가 localStorage"라 했으나 실제로는
  `study_plan_progress` 테이블 서버 동기화까지 붙어 있었다(그래도 실제 학습과 별개인 수동
  체크라 요지는 그대로). ② 진도 대조에 `normalizeSlug`(storageSchema.slugAliases)를 태웠다 —
  설계엔 없지만 fr rename 10건이 실재해 정규화 없이는 옛 slug 진도가 통째로 사라진다.
  목표 레벨(HSK5·B2)은 화면에 **레벨 키 하나씩**만 남겼고(챕터·제목·순서·개수는 전부
  정본), 계약이 그 키가 정본에 실재하는지까지 본다. R2에서 `profiles.goal_lang`·`goal_level`로 옮긴다.
  계약 21종 신설 + **돌연변이 3종**(`passed ?? read` → `||`·slug 정규화 제거·localStorage
  부활 → 각각 FAIL 확인 후 역치환 복원). 기존 V-13 계약은 대상이 사라졌으므로 같은 요구를
  새 조회에 다시 걸었다(실패를 삼키면 "0/72 완주"라는 거짓 진도가 남는다).
  **마이그레이션 0 · 새 테이블 0 · 새 이벤트 0** — 이미 쌓이는 것을 읽기만 한다.
  미사용이 된 `study_plan_progress` 테이블은 **손대지 않았다**(DB 스키마 변경 금지 하드리밋).
  렌더 실측: 1200/430px 가로 넘침 0, 문형·어휘 링크에 점선 밑줄을 줘 개수 라벨과 구별.
  → PR #TBD
- **↩️ v2-H R2 역방향 다리 — 자료에서 원본 PDF 그 쪽으로(2026-08-31, 오너 "H R2 ㄱㄱ")**:
  다리를 한 방향으로만 놓으면 "이 대목 원문이 어떻게 생겼더라"에서 다시 막힌다. 재료는
  이미 완비돼 있었다 — 자료 행의 `source_pdf_id`·`page_start`가 돌아갈 자리를 안다.
  자료 뷰어의 기존 'PDF 출처' 줄에 [원본 PDF 보기 →]를 얹고, `/pdf/<id>?page=N`으로 그 쪽에
  착지시킨다. **두 렌더 경로 모두** 지원: pdf.js는 `initialPage`로 시작 쪽을 잡고, 기본
  `<embed>` 경로는 우리가 렌더를 쥐고 있지 않으므로 PDF Open Parameters(`#page=N`)로
  브라우저 내장 뷰어에 부탁한다. 낡은 자료·교체된 PDF로 쪽이 문서 밖이면 끝 쪽으로 당겨
  빈 화면을 막는다. 주소 조립은 순수 함수 한 곳(`pdfViewerHref`) — 두 곳에 적으면 파라미터
  이름이 갈리고, 계약이 뷰어의 손수 조립을 금지한다.
  계약 5종 추가(총 20) + **돌연변이 5종**(쪽 미탑재·embed 기존 조각 상실·문서 밖 당김 제거·
  주소 손수 조립·기본 경로에 쪽 미전달 → 각각 FAIL 확인 후 역치환 복원).
  **같은 교훈을 두 번 만났다**: R1에서 남의 계약(PdfJsViewer)이 통짜 문자열이라 무해한 prop
  추가에 깨졌는데, R2에서 **내가 R1에 쓴 계약도 똑같이** 깨졌다. 둘 다 "게이트/배선 구조"만
  지키도록 좁혔다 — 통짜 문자열 계약은 의도를 못 말하고 변경마다 잡비용만 만든다.
  전체 vitest 255파일 2,673 green. 스키마 변경 0. **H 축 R1·R2 완료, R3만 오너 결정 대기.**
- **🌉 v2-H R1 PDF 읽기 절벽 — 자료 뷰어로 건너갈 다리(2026-08-30, 오너 "H ㄱㄱ")**: 같은
  앱인데 PDF로 읽으면 뷰어 기능이 0이었다(집중 모드·단어 카드·상태 하이라이트·받아쓰기·
  재분석 전부). **그런데 기능 부재가 진짜 문제가 아니다** — PDF를 자료로 만들면 그것들이
  전부 따라온다. 온전한 경험이 이미 있는데 PDF 뷰어에 **건너갈 길이 없었을 뿐**이다
  (툴바 링크가 `← 자료실` 하나). 그래서 12기능을 이식하지 않는다(이중 구현 부채가 깊어진다) —
  역할을 나누고 다리만 놓았다: PDF 뷰어는 원본 열람·범위 고르기, 읽기·학습은 자료 뷰어.
  **실측이 설계보다 더 좋은 답을 줬다**: `useNextRangeMutation`이 이미 추출→OCR 폴백→자료
  생성(강제 private·source_pdf_id·page 범위)→`last_page_read` 동기화→백그라운드 분석→
  뷰어 이동을 통째로 하고 있었다. 자료 뷰어의 '다음 범위' 전용이던 것을 **시작 쪽을 받게
  일반화**하고(`usePdfRangeMutation`으로 개명) 입구를 하나 더 열었다 — 신규 추출·생성 코드 0.
  두 입구가 같은 경로를 쓰는 게 요점이다: PDF 뷰어에 따로 짰다면 강제 private 같은 규칙이
  두 곳으로 갈려 한쪽만 낡는다(계약이 PDF 뷰어의 `reading_materials` 직접 접근을 금지).
  시작 쪽은 pdf.js 경로에선 보고 있는 쪽(`onPageChange` 신설), 기본 `<embed>` 경로는 쪽을
  모르므로 `last_page_read`로 떨어진다 — 자료 추가 화면이 이미 쓰던 기본값이라 관례가 하나다.
  쪽수 상한 10(설계 §2, 없으면 분석 요청이 쪽수만큼 늘어 분당 제한에 걸린다)을 **누르기 전에**
  화면에 밝힌다. 게스트·쪽수 미상이면 버튼 자체가 안 뜬다(눌러도 못 만드는 버튼은 없느니만 못하다).
  계약 15종 + **돌연변이 6종**(상한 제거·문서 밖 시작 허용·낡은 저장값 당김 제거·PDF 뷰어가
  자료 직접 생성·게스트 노출·material 없으면 차단 → 각각 FAIL 확인 후 역치환 복원).
  기존 PdfJsViewer 계약이 prop 추가를 잡아, 통짜 문자열 대신 **게이트 자체**를 지키도록 좁혔다.
  렌더 실측에서 결함 1건: 끝 쪽 입력의 number 스피너가 폭을 먹어 세 자리 쪽번호가 잘렸다
  ("146"→"4") → 스피너 제거 + 폭 보정. 전체 vitest 255파일 2,668 green. 스키마 변경 0.
- **🪜 v2-I R1b R3 점진 상승 + 이해도 가드(2026-08-30, 오너 "I-b R3 ㄱㄱ")**: 속도만 올라가고
  이해가 무너지면 훈련이 아니라 **훑기**다. Nation의 이해 70% 유지선을 기구현
  `ReadingTest`(내용 이해 문항)로 검사해, 받쳐 주면 회당 +5%·미달이면 한 칸 되돌린다.
  설계 §9 그대로이되 판단이 셋 들어갔다:
  ⑴ **사다리는 목표에 더하지 않고 배수(1.05^step)로 따로 둔다** — R2의 자동 제안이 이미
  내 실측을 따라 움직이는데 거기에 5%를 더해 저장하면 두 상승이 섞여 무엇이 실력이고
  무엇이 훈련 강도인지 알 수 없게 된다. 바탕값과 사다리를 분리해 "실력이 올라 바탕이
  올랐나, 훈련을 밀어 단계가 올랐나"가 구분된다.
  ⑵ **증거가 없으면 오르지 않는다** — 이해도 확인 없이 매 회 5%씩 올리면 가드가 있으나
  마나다(재보지 않으면 영영 미달을 모른다). 제자리가 기본값이다.
  ⑶ **완독이 아니라 채점 시점에 판정한다** — 완독 순간엔 이번 회차를 이해했는지 알 길이
  없어서 그때 올리면 가드가 사후 통보가 된다. `ReadingTest`에 `onGraded` 콜백만 얹었고
  컴포넌트는 페이서를 모른다(채점 결과만 알리고 판정은 호출자 몫 — 계약이 강제).
  페이서로 읽은 회차에만 적용한다(자기 힘으로 읽은 이해도는 훈련 강도와 무관). 상한 10단계
  (1.63배) — 무한히 오르면 어떤 검사도 못 따라잡는 속도가 된다. 바닥에서 미달이면
  '되돌림'이 아니라 제자리로 판정한다("낮췄어요"가 거짓말이 되지 않게).
  직접 조절하면 사다리를 접고 보이는 값을 바탕값으로 굳힌다 — "내가 90으로 맞췄는데 왜
  104로 도나"를 원천 차단. 설정 화면은 "내 속도 77.4자/분 기준 +10% **훈련 +16%**"로 두 축을
  갈라 보여 준다. 알림은 오르내릴 때만(제자리·상한은 침묵 — 잔소리가 된다).
  계약 19종 + **돌연변이 6종**(증거 없이 상승·미달 무시·상한 제거·비페이서 회차 적용·
  직접 조절이 사다리 유지·판정 로직 컴포넌트 유출 → 각각 FAIL 확인 후 역치환 복원).
  R2 계약이 우선순위 표현식 변경을 잡아 바탕값 쪽으로 갱신했다.
  전체 vitest 254파일 2,653 green. 스키마 변경 0. **v2-I 유창성 축 5라운드 전량 완결.**
- **📈 v2-I R1a R2 회차 비교 + 재독 배지(2026-08-30, 오너 "I-a R2 ㄱㄱ")**: 유창성의 표준
  측정은 **같은 텍스트를 다시 읽은 속도**다(Nation timed reading). 그래서 비교 대상은
  언제나 **같은 자료의 직전 회차** — 자료가 다르면 난이도가 달라 "쉬운 글을 빨리 읽었다"가
  실력 향상으로 보인다(설계 §1). 재료는 append-only `review_events`뿐이라 재독 회차가
  이미 쌓여 있었다 — **새 테이블 0**, `item_key='material:<id>'` 조회만 붙였다(설계 §2).
  급소 셋: ⑴ **기록 전에 조회** — 순서가 뒤집히면 방금 넣은 행이 '직전 회차'로 잡혀 언제나
  0%가 된다 ⑵ **페이서 회차는 양쪽 다 제외**(설계 §8, 현재분은 `compareRound`가·과거분은
  `readingCpmSamples`가) ⑶ ±5% 안은 "비슷한 속도" — 읽기 속도는 원래 그 정도 흔들리는데
  "+2%"를 띄우면 노이즈를 성취로 읽게 된다.
  표시는 완독 모달 한 줄("지난번 61자/분 → +25%"). **느려진 회차에 경고색·↓ 화살표를 쓰지
  않는다** — 재독이 느려지는 건 대개 더 꼼꼼히 읽었다는 뜻이지 실패가 아니다(설계 §11
  "경쟁 요소 0"). 빨라졌을 때만 강조색.
  재독 카드는 kicker에 "지난번 76자/분"을 얹어 "두 번째는 훨씬 빨라요"라는 약속을 실물
  숫자로 바꿨다. **meta에 넣으려다 렌더 실측에서 회귀를 잡았다**: `.deck-card__meta`가
  `nowrap; flex-shrink:0`이라 제목을 밀어 2줄 클램프에서 잘렸다("— 열두 번째 이…").
  kicker 자리는 레이아웃이 기준선과 동일하다.
  계약 22종 + **돌연변이 5종**(페이서 회차 비교 허용·"비슷" 구간 소거·자료 특정 없이 조회·
  기록 뒤 조회·재독 조회를 조건부 반환 뒤로 → 각각 FAIL 확인 후 역치환 복원).
  전체 vitest 253파일 2,634 green. 스키마 변경 0. **v2-I 유창성 축 4라운드 완결.**
- **🎯 v2-I R1b R2 목표 속도 자동 제안(2026-08-30, 오너 "I-b R2 ㄱㄱ")**: 페이서 목표가
  언어별 고정값이면 두 방향으로 어긋난다 — 잘 읽는 사람에겐 답답하고 이제 시작한
  사람에겐 못 따라갈 속도다. **내가 낸 속도에서 +10%**로 잡는다(설계 §4, timed reading
  정석). 재료는 I-a가 완독 detail에 남긴 `{cpm, v, paced}`뿐이라 **새 테이블·새 이벤트
  여전히 0** — `review_events`를 읽기만 한다.
  표본 자격 두 가지가 급소다: ⑴ **페이서 회차 제외**(설계 §8) — 안 빼면 '내가 설정한
  속도'가 '내 속도'로 되먹임돼 지표가 제자리에서 맴돈다 ⑵ **정의 버전(`detail.v`) 일치** —
  시간·글자수의 뜻이 달라진 기록을 섞으면 안 된다. 대푯값은 최근 10회의 **중앙값**:
  평균이나 최신 한 건은 자료 열어 두고 자리 비운 회차 하나에 통째로 끌려간다. 표본
  3회 미만이면 제안 없음(한두 번으로 내 속도를 단정할 수 없다).
  조회는 **언어로 좁힌다**(같은 사람도 중국어와 영어의 자/분이 몇 배 다르다) + 페이서를
  켠 사람에게만. 목표 우선순위 = 직접 설정 > 자동 제안 > 언어 기본값. 설정 시트가
  **숫자의 출처를 밝히고**("내 속도 77.4자/분 기준 +10%"), 직접 조절한 뒤엔 [자동으로]로
  되돌아갈 길을 남긴다 — 없으면 한 번 누르고 못 돌아오는 막다른 길이 된다.
  제안값이 수동 조절과 **같은 눈금**(`PACE_STEP_UNIT`)에 앉도록 상수를 공유해, 두 곳에
  5를 적어 두고 언젠가 갈리는 일을 막았다.
  계약 16종 + **돌연변이 5종**(페이서 회차 포함·버전 무시·평균 대체·항상 조회·복귀
  경로 제거 → 각각 FAIL 확인 후 역치환 복원). **계약이 실결함 1건을 잡았다**: 아주 느린
  독자(1자/분)의 제안이 눈금 반올림에서 0이 되어 통째로 사라졌다 — 하한이 필요한 바로
  그 사람에게 하한이 안 걸렸다. 범위 클램프를 눈금보다 앞으로 옮겨 수리.
  전체 vitest 252파일 2,612 green. 스키마 변경 0.
- **▶ v2-I R1b 자동 진행(페이서)(2026-08-30, 오너 "I-b ㄱㄱ")**: 지정 문장에 머물다 다음
  문장으로. 체류는 **고정 초가 아니라 글자수 ÷ 목표 속도**(하한 1.2초·상한 20초) —
  고정 초는 짧은 문장에서 지루하고 긴 문장에서 못 따라가 어느 쪽으로 맞춰도 나머지가
  망가진다. 목표 속도는 언어별 보수적 기본값(CJK 낮게·라틴 높게)에 ±10% **곱셈** 조절
  (90과 400을 같은 증분으로 움직이면 한쪽이 무의미해진다). 설정엔 자/분과 **초를 병기** —
  오너가 처음 말한 "몇 초 후"를 그대로 쓸 수 있게(설계 §7②).
  **새 버튼 0개**(오너 수정 반영): 발동 조건이 곧 정지 조건이라 상태가 하나도 안 는다 —
  설정 켬 + 집중 모드 + 문장 지정이 `paceArmed`이고, 빈 공간 탭으로 지정이 풀리면 셋 중
  하나가 깨져 진행도 끝난다. 진행은 `moveSentence(1)` 하나만 불러 집중 모드의 '순수 이동'
  계약(분석·시트·Gemini 0)을 통째로 상속한다 — 페이서가 매 문장 번역을 부르는 사고가
  원천 차단된다. 일시정지(카드·시트)는 I-a와 같은 신호를 쓰고 **재개는 이어서** —
  처음으로 돌아가면 사전을 찾을수록 제자리걸음이 된다. 페이서로 읽은 완독엔 `paced:true`가
  실려 유창성 지표가 자기 설정값을 되비추지 않는다(설계 §8).
  체류 표시는 지정 문장 글자 아래 옅은 선이 **오른쪽부터 물러나는** 방식 — 숫자
  카운트다운은 배제(시선이 글에서 숫자로 옮겨간다). 각 토큰이 글자수 몫의 구간만
  담당하고 자간까지 덮어 한 줄로 이어지며, 시간은 CSS 애니메이션이 재서 **JS 프레임
  루프가 0**이다. 모션 축소는 같은 선을 `steps(3)`으로 — 설계의 ●●○를 새 DOM 없이 대체
  (이 한 곳만 설계와 다르게 갔고 근거는 PR 본문에 명시).
  렌더 실측이 결함 2건을 잡았다: 선이 단어마다 토막나 렌더 오류처럼 보인 것(→ 구간
  분배 + 자간 잇기)과 인라인 상자 바닥에 붙어 루비 여백만큼 떠 보인 것(→ 글자 잉크
  좌표계 `--hl-band-top`). 계약 26종 + **돌연변이 5종**(분석 직접 호출·집중 모드 전제
  제거·재개 처음부터·선 번짐·하한 제거 → 각각 FAIL 확인 후 역치환 복원).
  전체 vitest 251파일 2,596 green. 스키마 변경 0.
- **⏱️ v2-I R1a 유창성 측정 — 순수 읽기 시간(2026-08-30, 오너 "I ㄱㄱ")**: 설계 §9
  단계표대로 **측정(I-a)만** 먼저, 페이서(I-b)는 다음 라운드로 쪼갰다. "읽기 속도"는
  무엇을 시간으로 치느냐로 숫자가 통째로 달라져서 정의부터 못박았다 — 분자는 자료
  글자수(공백·개행 제외, 스크롤 %는 훑어도 100%라 배제), 분모는 **일시정지 3종을 뺀
  시간**: ①탭 숨김 ②**단어 카드·번역 시트 열림** ③30초 무동작. ②가 핵심 — 사전 찾는
  시간을 빼지 않으면 "많이 찾아볼수록 느린 독자"가 되어 지표가 학습을 되레 왜곡한다.
  `readingTimer.js`(순수 계산)·`useReadingTimer.js`(신호 수집) 2층 분리, 훅은
  **setInterval 없이 구간 차이 누적** — 배경 탭이 얼어도 시간이 새지 않는다.
  기록은 **기존 완독 이벤트의 detail만 넓혀서**(`{ms,chars,cpm,v,paced}`) — 새 이벤트도
  새 테이블도 0이라 이벤트 개수·기존 집계가 그대로다(계약이 `logReviewEvents` 호출
  1회를 강제). 200자 미만·시간 0은 metric이 null → detail이 예전 모양 그대로인 조용한
  무기록. 완독 모달엔 "320자 · 4분 12초 · **76.2자/분**" 한 줄만, 랭킹·경쟁 요소 0.
  계약 14종 + **돌연변이 4종**(MIN_CHARS 무력화·이벤트 2회 호출·paused 신호 제거·
  setInterval 부활 → 각각 FAIL 확인 후 역치환 복원). 렌더 실측에서 한 줄이 요약 상자에
  달라붙어 '요약 머리글'로 보이던 것을 잡아 제목 쪽으로 붙였다(`:has(+)` 선례 재사용).
  전체 vitest 250파일 2,570 green.
- **📓 /design-bank 절차 명령(2026-08-30, 오너 "design-bank ㄱㄱ")**: 시중 스킬 조사
  (obra/superpowers `brainstorming`) 산출을 **개념만** 이식 — 플러그인 통째 설치는
  배제(worktree·subagent·TDD 강제가 머지 단일 창구·게이트와 충돌, 스펙 파일은 #1077
  박제와 이중 진실). 훔친 것 셋: ⑴ 규모 3분류(탐침/국소/구조)를 **먼저 선언** —
  J(~120줄)를 A와 같은 무게로 다룬 과설계의 교정 ⑵ 섹션별 승인(I 페이서 되돌림
  비용) ⑶ 박제 직전 자기 검토(한 세션에서 자기 진단 4회 정정). 여기에 v2-L 약속을
  이행해 §5 신설: 새 계약은 **깨뜨려 FAIL 1회 확인**·앵커는 sliceBetween·변조 복원은
  역치환(미커밋 파일에 git checkout 쓰다 작업분 날린 사고를 절차로 차단).
  **설계 원칙은 CLAUDE.md 정본, 이 파일은 절차만** — 계약이 원칙 본문 복사를 금지해
  이중 진실을 막는다. 계약 6종 + 돌연변이 2종(원칙 복사·돌연변이 절 삭제 → 각각
  FAIL 확인). 명령 7번째로 등록 확인. 전체 vitest 249파일 2,556 green.
- **🧹 v2-J R2 캐시 비우기 — 스테일 버그 자가 수리(2026-08-30, 오너 "J R2 ㄱㄱ")**:
  R1 배지가 구버전을 알려주면(SHA 비교) 여기서 앱 껍데기 캐시를 비우고 새로 받는다.
  `swCache.js` 신설 — CacheStorage 전량 삭제 + SW 등록 해제(등록을 남기면 옛 워커가
  다시 채운다). 패널에 SW캐시명 행(콘텐츠 해시라 축약)과 [캐시 비우고 새로고침] +
  "학습 데이터는 지우지 않아요" 문구. **층 분리가 이 라운드의 생명**(v2-N 설계 §6):
  CacheStorage=앱 껍데기 / IndexedDB=학습 데이터 — 오프라인 자료·단어장 스냅샷은
  건드리지 않는다(지하철에서 캐시 비웠다고 단어장이 사라지면 진단 도구가 아니라
  사고다). 캐시명 조회는 패널을 펼 때만. 계약 6종(층 분리 금지어·정리 범위·조용한
  실패·축약·배선·지연 조회) + 돌연변이 2종(IndexedDB 삭제 추가·SW 등록 해제 누락 →
  각각 FAIL 확인). 렌더 검증에서 '외 2' 꼬리표가 쪼개지는 것을 잡아 수리.
  전체 vitest 248파일 2,550 green. J 트랙 완결(R3는 오너 결정 대기 항목).
- **📐 v2-K R2 브레이크포인트 동결(2026-08-30, 오너 "K R2 ㄱㄱ")**: 실측 — 뷰포트
  쿼리 11종·총 29개(index.css 28 + CourseMapPage.module.css 1), JS 폭 쿼리 0.
  진단은 '난립'보다 **컴포넌트마다 자기 접힘 폭을 뷰포트로 대신 재는 것**이 정확
  하다(400 stats-grid 1열·420 hanja-bridge 여백·880 myplan 1열). 조치: ⑴ 정본 티어
  선언 480/600/768/1180 ⑵ 동결 목록 계약 — 목록 밖 값 금지 + **총량 상한 29**(값을
  바꿔치기해도 새 쿼리를 못 늘리므로 신규 접힘은 @container로 갈 수밖에 없다)
  ⑶ container-type 선례 생존 계약 + 문서에 정책·지원범위(iOS 16+, 리포 사용 중) 명시.
  값 병합(560→600 등)은 40px 구간의 실제 시각 변화라 R3에서 한 건씩 실기 확인 후.
  계약 4종 추가(총 13) + 돌연변이 2종(목록 밖 값·총량 초과 → 각각 FAIL 확인).
  코드 변경 0 — 전체 vitest 248파일 2,544 green.
- **🎨 v2-K R1 UI 일관성 — 색 토큰화·OS 테마·규약 문서(2026-08-30, 오너 "K ㄱㄱ")**:
  ⑴ 인라인 하드코딩 색 전량 수리 — 계약이 설계 실측(39)보다 넓은 우회를 찾아냈다:
  `--success`·`--bg-subtle`·`--surface-2`·`--brand`·`--accent2`·`--warning-bg/text/
  border`·`--border-color`가 **정의된 적 없어 폴백이 실제로 렌더되던** 진짜 우회
  8건 + 죽은 폴백 7건. 토큰 11종 승격(warning-bright·admin-accent·track 4·notice 3·
  speaker 2)은 기존 값 그대로 담아 브라우저 계산색 대조로 동일 확인, 미정의 폴백의
  실재 토큰 매핑 5건은 같은 계열 근사 대체(미세 이동 — 문서·주석에 명시, 조정은 R3).
  ⑵ OS 테마 존중 — 저장값이 있으면 그것, **없을 때만** prefers-color-scheme. 부착
  effect의 자동 저장을 걷어냈다(그대로 두면 사용자가 고른 적 없는 값이 첫 렌더에
  저장돼 OS 추종 상태가 사라진다). ⑶ `docs/ui-conventions.md` 신설 + CLAUDE.md 상호
  포인터. 계약 9종(색 0·폴백 금지·승격값·죽은 토큰 소거·문서·a11y 하한 29/12/10·
  테마 2종) + 돌연변이 3종(색 재도입·자동 저장 부활·포인터 삭제 → 각각 FAIL 확인).
  전체 vitest 248파일 2,540 green. R2(브레이크포인트 동결·@container)는 다음 라운드.
- **📴 v2-N R1 오프라인 읽기 — 학습 데이터층 신설(2026-08-30, #1077 오너 "N R1 ㄱㄱ")**:
  SW가 캐시하던 것은 앱 껍데기뿐이라 비행기·지하철·로밍 없는 해외에서 "앱은 열리는데
  단어장이 비고 본문이 없다" — 어학연수 앱 맥락과 정면으로 어긋나던 구멍을 IndexedDB
  학습 데이터층으로 메웠다(SW=껍데기 · IDB=학습 데이터, 층 분리). `offlineCache.js`는
  pdfCache의 검증된 패턴(TTL 7일·LRU·조용한 실패) 복제 — 다른 점은 processed_json이
  커서 자료 상한을 3으로 조인 것. 축출 판정을 순수 `pickEvictions`로 뽑아 상한·TTL을
  실동작 검증. 단어장 스냅샷은 next_review_at을 품은 전체 행이라 '오늘 due'가 여기서
  파생된다(별도 due 캐시 미신설). 읽기는 **네트워크 실패 시에만** — 온라인이면 언제나
  네트워크가 정본이라 스테일이 구조적으로 성립하지 않고, NOT_FOUND는 폴백 금지(삭제
  자료 부활 차단). 구현 중 계약이 실결함 1건 검출: 캐시 쓰기가 동기 throw하면 네트워크
  성공분이 폴백으로 새어 계약 4·6을 함께 깨뜨림 → fire-and-forget 완전 격리로 수리.
  UI: 전역 배너 문구를 실제 동작에 맞춰 갱신 + 캐시 사본 화면에 `OfflineNotice`(뷰어·
  단어장 공용 문구 정본). 렌더 검증에서 조사 오류('자료을') 발견 → 받침 판정으로 수리.
  계약 14종(R1 해당 4계약+배선+조사) + 돌연변이 3종(캐시 우선화·LRU 무력화·NOT_FOUND
  폴백 → 각각 FAIL 확인). 전체 vitest 247파일 2,531 green. R2(동기화 큐)는 다음 라운드.
...
- **🧪 v2-L 계약 위생 — 공허 통과 구조 차단(2026-08-30, #1077 오너 확정 1순위 ㄱㄱ)**:
  ...
  부재 즉시 throw, __tests__ 배치라 번들 제외) ⑵ focusMode(슬라이스 6+순서 단언
  가드 1)·easierText(1) 치환 ⑶ `contractHygiene.test.js` 메타 계약 — 전 테스트
  파일 스캔, raw `slice(…ndexOf(` 0 강제(줄 넘김 회피 불가·스캐너 공동 방지 100+
  파일 핀) + 헬퍼 throw 계약. 수용 검증(돌연변이): M2 앵커 진개명 → **FAIL 전환
  확인**(수리 전 PASS), M1 동작 삭제·easierText 앵커 개명·M3 CSS 값·M4 CLAUDE.md
  삭제 전부 여전히 FAIL. 잔여 한계 명시: 접두 포함 개명(X 붙이기)은 indexOf 특성상
  통과하나 슬라이스 영역이 동일해 공허 아님. /design-bank 돌연변이 확인 절은 그
  항목 착수 시 동봉(설계에 기포함). 전체 vitest 244파일 2,493 green.
- **🪞 이번 주 타일 상시 표시(2026-08-30, 오너 확정 "상시 표시")**: 오너 보고 "홈에
  위젯 하나 사라짐" 진단 — 현황 스택의 코드 변경 소거로 #1148(8-26, 오늘 활동 폐지
  — 오너 확정)과 이번 주 타일의 hasAny 숨김(빈 주·조회 실패 시 조용히 생략)이 겹쳐
  스트릭 옆 1×1 자리가 통째로 비는 경우 특정. 수리: hasAny 게이트 제거(로드 대기
  `weekly &&`만 유지), 빈 주 값은 스트릭 타일 '–' 선례(0 무표기 원칙과의 절충),
  빈 주 모달에 "이번 주 기록이 아직 없어요." 한 줄. weeklyTile 계약 갱신(게이트
  부활 금지 핀). 전체 vitest 2,490 green.
