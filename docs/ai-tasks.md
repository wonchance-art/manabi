...
### doing
### todo
- 런던 EMEA 게이트 후보 타일 사전 검증: -0.1258/51.5316 세인트판크라스
  투영·충돌·체크인 검사(니스 선례 테스트 문법) — 노드 커밋은 도시 merge 후.
...
- 런던 콘텐츠 배선(london.js — desc 24종 검증 완, 타워브리지 오탐 보정으로 전 게이트 통과)
- 상하이 콘텐츠 배선(shanghai.js — desc 10종 사전 저작 완, 2·10호선+황푸강 도선 스타페리 문법)
### todo
- 중국어 도어 zh-01~06 저작(트랙 챕터 정합 감사 선행)
- 영어 도어 en-01~06 tile 배선(런던 배선 후)
