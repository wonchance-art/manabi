...
- **아이디어 보드 전건 승인** — 1차 구현: ①+③ studiesRef 딥링크(NPC·도어·desc → 여행 폰), 이후 점수순
- **인기 누락 POI 보강 라운드**(오너 상시 지시): 츠키지 장외시장(도쿄)·니시키시장(교토)·
  광장시장(서울)·흰여울문화마을(부산)·우메다 공중정원(오사카) — geo 재생성 문법(도쿄 v2.3 선례)
- 지역학 근현대사 ②편(승인 — 사실·구조 층위 수위로 한일 양국 교차 저작) / 프랑스학 개시
- #152 AI Relay 검수·merge 라운드(승인 — 단 DB 적용·Vercel env는 오너 수동 단계, 코드만 merge)
