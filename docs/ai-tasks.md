...
## Codex-1 (codex/*)
### doing
- **manabi 두 실제 계정 권한 검수·홈 집계 안내 (2026-09-07)** — 오너가 두 번째 계정 로그인 완료. 비공개 자료 직접 접근/서재 검색 차단, 표현 2개 검색 제외와 계정별 이어 읽기 확인 중. 홈 전체 대기 수와 복습 하루 한도 수의 의미 차이를 안내 문구로 명확히 한다. 앱 집계/FSRS/DB 정책은 유지하며 운영 전환·migration 정합화는 기존 담당 유지.
### todo
- **manabi 운영 전 잔여 검수·전환** — 실제 소유자 Google 로그인·비공개 자료 저장·표현 복습·원문 복귀 및 비로그인 접근 차단은 완료했다. 두 번째 실제 계정의 비공개 접근 분리는 검수 계정 확보 후 진행한다. 기존 교재 migration 번호 정합화·병합은 Claude, 자동 도메인 할당 분리·production 대기 빌드 검수는 운영 담당으로 유지한다. 상세 `docs/manabi-web-v2-release.md`의 최신 실제 계정 검수 절.
