...
- (없음)
### todo
- 타이베이 비콘텐츠 선행 수집(그랑파리 선례): bbox [121.49,25.02,121.58,25.11] Overpass 분할
  수집·스냅샷 파이프라인 준비 — POI 선택·콘텐츠 금지(스펙은 Claude가 브뤼셀 마감 무렵 게시).
  중국어권 스냅샷 스크립트 신설 시 한국 파이프라인 미러(소유권: Codex-1).
