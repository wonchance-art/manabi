...
- **인기 누락 POI 보강 라운드**(오너 상시 지시): 니시키시장(교토)·광장시장(서울)·
  흰여울문화마을(부산)·우메다 공중정원(오사카) — geo 재생성 문법(도쿄 v2.3/2.4 선례,
  츠키지 장외는 v2.4로 반영 완료). 아이디어 보드 2차 구현은 점수순(④스탬프 카드·⑤진입 브리핑)
- 지역학 근현대사 ②편(승인 — 사실·구조 층위 수위로 한일 양국 교차 저작) / 프랑스학 개시
- 레만호 본생성 검수 라운드: verifier 프로필 게시 완료(#307 — main 87a0b75), PR #304는
  라보 POI 좌표 교정(dezaley→epesses 6.7375/46.4925, 회신 5012615779) 재생성 대기 —
  green CODEX_DONE 수신 시 merge → 배선 → 게이트
