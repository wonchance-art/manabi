...
## Codex-1 (codex/*)
### doing
- **실제 뷰어 전용 자동 검수 (2026-09-24)** — 승인된 다음 작업. 기존 `verifyPersonalReaderDesign`·Playwright·PGlite를 재사용해 단어창/Aa/집중/200% 확대를 긴 수업 동선과 분리하고 선택 뜻의 자료 한정 저장·실패 재시도·재접속을 검증. 담당: e2e의 기존 하니스/뷰어 검사, scripts/qa 프로필·계약 검사, package 실행 항목, 품질 문서/자기 보드. 완료 조건: Chromium/WebKit 독립 실행·기존 전체 게이트 보존·최종 재검수/리뷰/다음 방향. 제품 코드·운영 DB/env/병합 제외.
- **뷰어 문맥 뜻 의미 검수 후속 (2026-09-19)** — #1321 a1949db9 정상 계정에서 명시 후보 선택→자료 저장→새로고침 유지·원문 복귀·사전 후보 보존·조회 초점 확인. 실제 AI가 접대 문맥에도 ‘접대하는 사람 또는 주최 측’을 함께 내고 일본어 主催者로 이어지는 의미 혼합 재현. bce1a1b9에서 기존 뜻 유도 제거·한 의미 선택 지침·캐시 판본 보완. 새 Preview의 실제 응답 결과와 남은 의미/역할 검수는 최종 head #150/closeout 정본. 합성 통과를 실제 의미 감수로 세지 않음. DB/env/공유 사전/개인 SRS·운영 병합 제외.
