...
### doing
### todo
- **manabi 운영 후속 대기** — DB 프로필 초기화·권한 수정안은 실제 migration 파일 기준 로컬 PostgreSQL12조건 통과. 원격 정의가 제안 당시와 같음을 재확인했다. 적용 호출은 자동 승인 검토가 명시적 Codex 적용 권한/Claude 인계 승인 부족으로 거절했다. 운영 DB 변경0, 준비 파일은 오너 비공개 검토 폴더로 이동했고 적용·검수 세션 사용의 명시적 승인을 요청했다. 실제 검수용 인앱 브라우저 세션 사용 승인은 자동 승인 검토 거절 후 요청한 상태. 실제 계정 쓰기/두 계정 권한 검수, Claude migration 번호 정합화·병합, 자동 도메인 할당 분리와 production 대기 빌드가 남았다.
