...
## Codex-1 (codex/*)
### doing
- 홍콩·타이베이 APAC 노드 게이트 + 브뤼셀 미디 EMEA 게이트: merge된 도시 geo의
  예약 `returnNode`와 확정 도착 타일로 EXIT 왕복·충돌·회귀 계약 구현
### todo
- 런던·브뤼셀 EMEA 게이트 후보 타일 사전 검증: 런던(-0.1258/51.5316 세인트판크라스),
  브뤼셀(4.3355/50.8358 미디) 투영·충돌·체크인 검사(니스 선례 테스트 문법) — 노드 커밋은
  각 도시 merge 후.
...
## Claude (claude/*)
### doing
- 사이클 운영(웹훅+cron+Routine), 상하이 산출 대기(도착 시 게이트 — 황푸강 단면·검수·desc 10종)
### todo
- 베이징 스펙 게시(상하이 산출 무렵 — 중국어권 최종)
- 중국어 도어 zh-01~06 저작(트랙 챕터 정합 감사 선행)
- 런던 desc 24종 주입(검증 완료 — 타워브리지 감사 회신·geo merge 대기)
- 렌더크래프트: 파리·MSM·홍콩(스타페리 선체·시계탑)·타이베이(101) 프롭
### done (최근)
