...
## Codex-1 (codex/*)
### doing
- **수업용 뷰어 태블릿 사용성 — PR/DB/Preview 완료, 실계정 로그인 대기 (2026-09-11)** — `codex/classroom-tablet-20260911`, draft #1302(부모 #1301), 배포10851b15. 기기 초안/IME/늦은 조회·두 탭 보존, 패널 주동작 고정, 키보드/범위 노출 및 같은 단어 재선택 전환 완료. 전체382파일/4,145개·실제 IndexedDB 각7·최종 배포 교사 Chrome/WebKit 각12·학생 각7 PASS, CI 단위/콘텐츠/lint/기존 학습흐름 SUCCESS. 출처 SQL 적용 이력20260911072945, 권한/Unicode/기존 교재 지문/원격 롤백 검수 PASS. Preview `manabi-2sfn6fnox-wonchance-arts-projects.vercel.app` READY·SHA 일치. 실제 로그인 요청 후 대기, 물리 iPad 미검수. 로컬 WebKit 기록 스크롤 최초 시간초과 후 단독 전체 재검수 통과 사실 유지. 상세 `docs/verification/classroom-tablet-20260911.md`. 운영 병합·음성/PDF/시험/새 split view 제외.
- **교재 출처 복귀 1차 — PR #1302에 포함·DB 적용 완료, 실계정 검수 대기 (2026-09-11)** — `codex/classroom-source-return-20260911`, 부모 #1301/cdf9b76c, 실행3d7f4cfd. 단어·드래그·문장 위치/문맥 보존, 재분석/중복 안전 복귀, 기록 검색·필터·스크롤 복원 및 터치/마우스 혼합 입력 충돌 수정. 전체381파일/4,135개·PGlite33개·교사 Chrome22/WebKit23·학생 각각7 PASS/실행 오류0, 운영 형식 빌드/prebuild/lint PASS. 오너 승인 후 SQL 적용(이력20260911072945)·공개 PR #1302·최종 Preview10851b15 완료. 실계정 로그인 검수만 별도 대기. 기존 주의점/개인 단어/FSRS/학생 사본 권한 유지. 상세 `docs/verification/classroom-source-return-20260911.md`. PDF/음성/시험/새 split view/운영 병합 제외.


### todo
- **수업 후속** — #1299 → #1300 → #1301 검토 및 운영 통합은 Claude 창구. 실제 계정 노트 입력/뷰어/판 쓰기 왕복 완료. 정식 교재 선택 표현의 출처를 포함한 실제 저장 왕복, 물리 iOS/실교실 지연/실제 인증 제공자 왕복은 별도 확인. 학생 대표 사본·명시적 갱신·로그인 표현 저장은 기존 구현 유지. 상세 `docs/manabi-classroom-study-flow.md`.
