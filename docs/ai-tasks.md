...
## Codex-1 (codex/*)
### doing
- 현재 착수 중인 검수 없음. #1312 실계정 결과는 done, 남은 개선은 todo 참조.

### todo
- **여러 표현 설명판 후속 (2026-09-15)** — #1312 실계정 기본 동선·실제 AI 조회·복수 표현/필기·복원 검수 완료. 긴 표현 끝 한 글자 줄바꿈(판 6+1/모바일 3+1)을 균형 있게 개선할 여지가 있다. 물리 iPad/Pencil·소프트 키보드 검수와 운영 병합·반영은 별도. 실제 학생 기록 발행은 이번 검수에서 하지 않았고, 쓰기 검증은 격리 SQL/브라우저 결과로 구분한다.
