...
## Codex-1 (codex/*)
### doing
- **학습 웹 운영 통합 (2026-09-16)** — 오너가 잔여 검토안의 작업 개시 승인. #1314→#1313의 개인 노트·선택 필기 인식을 운영에 통합하고, #1296/#1297의 문맥 복귀·누락 분석 복구 및 #1305 추천 바로 읽기를 최신 뷰어/설명판과 대조·검수한다. 격리 브랜치 `codex/learning-release-20260916`. 기존 교재·단어/FSRS·권한·교사 판/개인 노트 기능 보존, DB/env/world/PDF/음성 변경 제외. UI 정리와 판 서버 동기화는 별도 후속.

### todo
- **개인 노트 기기·정리 보강 후속 (2026-09-16)** — #1313 개인 노트의 실제 계정 저장·Gemini 가나/한자 후보·개인 어휘 저장·원문 복귀는 #1314에서 검수 완료. 물리 iPad/Pencil·실제 필체 표본·실기기 간 이어 쓰기, 여러 선택 영역 누적 정리, 외부 PDF/이미지 반입은 후속. 수업 설명판 직접 인식은 #1314 완료. 운영 병합은 이번 승인 범위 밖이다.
- **여러 표현 설명판 기기 후속 (2026-09-15)** — 물리 iPad/Pencil·손바닥 접촉·실제 소프트 키보드와 별도 학생 계정 검수는 미완료. 줄바꿈/실제 수업 저장/운영 반영은 위 출시 마무리 범위. 기기 간 판 동기화는 별도 후속이며, 선택 필기 인식은 #1314에서 완료했다.
- **수업 후속** — #1299~#1302를 포함한 #1303을 b5f87bcd로 main 병합했고 운영 배포J3TetFrJ READY. ChatGPT/Codex 직접 병합 규약도 반영됐다. 후속 이력 파일명 정합과 최종 CI/DB 검사 상태는 PR/#150 참조. Production SHARE_LINK_SECRET 설정과 실제 계정·기기 검수는 후속이다. 실제 교사 계정 노트 입력/뷰어/판 및 정식 교재 출처 저장·왕복 완료. 별도 실제 학생 계정 로그인/팀 암호 입장 후 사본·뜻/독음·수업 복귀 확인과 물리 iOS/중국어 입력기/회전/실교실 지연은 미완료. Codex에 최종1g6gftegw 수업 입장 화면을 준비했다. 앞선 실제 교사 검수는 이전 배포 기준이며 #1303의 실계정 검수는 남아 있다. 저장소 QA_LOGIN은 합성 계정이므로 실제 계정으로 세지 않음. 상세 `docs/verification/classroom-student-auth-20260912.md`.
