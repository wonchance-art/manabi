...
### doing
### todo
- **manabi 운영 전 잔여 전환** — 실제 소유자 로그인·저장·복습·원문 복귀, 비로그인 차단 및 두 번째 실제 계정의 비공개 자료/표현/진도 화면 분리 검수 완료. 기존 교재 migration 번호 정합화·병합은 Claude(적용용 패치는 PR #1287 코멘트5571511285), 자동 도메인 할당 분리·production 대기 빌드 검수는 운영 담당으로 유지한다. 상세 `docs/manabi-web-v2-release.md` 최신 절.
