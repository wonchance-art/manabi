...
- **가와구치코/후지 라운드(오너 승인)**: geo #276 검수·merge(2성분 98,201+33 독립 실측) +
  콘텐츠 배선 #278(desc 11종·ja 도어·첫 등산 버스 TRANSIT·유람선 8호·후지 등산 게이트 개통·
  verifier 호수 단면 2,560m) — **22도시 등록**. 후지 액트 씬 완성(골격 #275 + 텍스트·훅 #277).
  잔여 = 오버월드 게이트만(SPEC 5011235474 — 기존 fuji 랜드마크 무변경·신규 노드 문법)
- **지역학(스터디즈) 트랙**: 🏁 일본학 1기 6도메인 완간(#256·#261·#265·#270·#272) +
  한국학 개시(#274 — 개관). 다음 = 한국학 각론·크로스링크 검토
