...
## Codex-5 (codex5/*) — 교재 순회 검수·개보수 (Codex Cloud)
### doing
- 📚 일본어 순회 R1 — N5 챕터 01~06 감사·개보수 (발주 #150 코멘트 5427543001).
  SPEC은 오너가 Codex Cloud 세션에 직접 전달(해당 환경이 #150을 못 읽음 — 실측).
  브랜치 `codex5/ja-audit-n5-r1`(환경이 codex/ 접두 강제 시 `codex/ja-audit-n5-r1` 수용).
  draft PR 대기 중.
### todo
- 순회 지도(라운드당 6챕터): N5 R1~R5 → N4 R6~R9 → N3 R10~R12(확장 신규 저작 병행 검토)
  → N2·N1 R13~R17. 매 라운드 SPEC은 동일 템플릿에 창만 교체 — 오너 "다음 라운드" 한마디로 발주.
