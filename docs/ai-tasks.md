...
### doing
### todo
- **📱 [최우선·신규 발주 2026-08-07] 모바일 반응형 실측 + 수리** — 이슈 코멘트가 아니라 이 보드로 발주한다(자족 지시).
  **배경**: 학습자 상당수가 모바일로 보는데 뷰포트별 검증이 한 번도 없었다. PWA manifest·viewport 메타는 있으나 실제 깨짐은 미확인.
  **과제**: 360×740 · 390×844 · 768 세 뷰포트에서 학습 핵심 경로를 **실렌더 측정**하고 수리한다.
  1. 랜딩 — 마케팅 섹션이 전부 삭제되고 `.landing--minimal`·`.landing-minimal`·`.track-grid` 구조다(#863). 새 구조로 측정.
  2. `/lessons` — **이어서 학습 카드가 필터 위 최상단으로 이동**(#862). 언어·레벨 칩 줄바꿈, 세계지도 SVG 가로 넘침.
  3. **챕터** — 드릴 선택지 버튼(긴 보기 줄바꿈·터치 타겟 44px), order 토큰 배열, 써 보기 textarea.
  4. `/vocab`(복습) — 게스트 안내 화면 포함.
  **중점**: `overflow-x` 발생 지점(모바일 최악 결함), 고정폭 px, 44px 미만 탭 타겟, 잘리는 텍스트.
  **Claude 선행 실측(중복 금지)**: CSS 고정폭 grep은 **거의 전부 오탐**이었다 — `min-width: 760px` 등은 미디어 쿼리이고
  챕터 표는 `.fr-table-wrap { overflow-x: auto }`로 이미 격리됨. **CSS grep 대신 실렌더 측정**(scrollWidth vs clientWidth,
  getBoundingClientRect)으로 갈 것.
  **산출**: `docs/review-mobile.md`(뷰포트별 재현 가능한 측정치 표) + 수리 PR(CSS 중심).
  **공통 규격**: base = 최신 `origin/main`(작업 전 `git rev-parse HEAD` 보고) · 자기 네임스페이스 브랜치 ·
  **게이트 한 문장 결합**(`npm test`까지 green이어야 커밋) · 보드는 **별도 커밋** · draft PR + 완료 보고(head 해시·수치).
  **하드리밋**: DB 스키마·시크릿·실화폐 금지 / **world·studies·`/home` 수정 금지**(리뷰는 하되 제안만) / 챕터 증식 금지.
  **사설 도구 원칙(2026-08-06 전환)**: 홍보·전환 유도 카피 신설 금지, 로그인은 벽이 아니라 권유.
  **라우트 스모크는 실재 slug로만**: `a1-01-pronouns-etre`·`h1-01-shi`·`n5-04-desu-da`·`a1-01-be-verb`(없는 slug는 404).
  **인증 e2e는 반드시 `npm run e2e:full`**(빌드·런타임 `NEXT_PUBLIC_*` 불일치 시 클라 인증이 조용히 죽는다).
...
### doing
### todo
- **⚡ [최우선·신규 발주 2026-08-07] 성능 2차 — 남은 상위 라우트** — 이 보드로 발주한다(자족 지시).
  **전제 변경**: #863으로 랜딩의 마케팅 섹션·데모가 전부 삭제됐다(300→74줄). `/` 번들이 줄었을 테니 **최신 main 재측정부터**.
  **1차 성과(#852, 유지 확인 대상)**: CSS gzip 274→217KB(-20.9%), `/lessons` First Load 183→137KB(-25%).
  **과제**:
  1. 라우트별 First Load JS 표 갱신(cold build) — #852 이후 회귀 없는지 확인.
  2. 남은 상위 라우트 원인 분해·수리:
     - `{french,japanese,english,chinese}/grammar/[slug]` **156kB — 학습자가 가장 오래 머무는 화면이라 최우선**.
       첫 화면에 필요 없는 상호작용 컴포넌트(써 보기·누적 복습·패턴 체크는 스크롤 아래)의 지연 로드 여지 검토.
     - `/viewer/[id]` 162kB — 선택 기능(문법 모달·PDF·대화 패널) dynamic import.
     - `/vocab` 149kB.
  3. **주의**: 챕터 **본문 SSR은 SEO·첫 페인트의 핵심이라 클라이언트로 미루지 마라.** 지연 대상은 상호작용 컴포넌트에 한정.
  4. `performanceRepairs.test.js`의 `var(--font-serif)` **개수 == 15** 단언은 정당한 사용마다 깨져 Claude가 스코프 검사로 완화했다(#863).
     앞으로 **개수 고정 단언 금지** — 계약(스코프·존재)만 검사.
  **산출**: 각 건 **수정 전후 수치**(First Load JS·gzip·cold build) 첨부 + 회귀 테스트 필수.
  **공통 규격**: base = 최신 `origin/main`(작업 전 `git rev-parse HEAD` 보고) · 자기 네임스페이스 브랜치 ·
  **게이트 한 문장 결합**(`npm test`까지 green이어야 커밋) · 보드는 **별도 커밋** · draft PR + 완료 보고(head 해시·수치).
  **하드리밋**: DB 스키마·시크릿·실화폐 금지 / **world·studies·`/home` 수정 금지**(리뷰는 하되 제안만) / 챕터 증식 금지.
  **사설 도구 원칙(2026-08-06 전환)**: 홍보·전환 유도 카피 신설 금지, 로그인은 벽이 아니라 권유.
