...
### doing
- 📚 R3 발주 중 — N5 order 13~18(ot-09-jinja·n5-10-kosoado·n5-13-numbers-time·
  ot-10-ramen·ot-12-menzei·n5-13b-date-counters), SPEC = #150 코멘트 5429565401.
  돈키호테 상호는 보류 특별 지시 포함. 오너가 Codex Cloud에 인라인 전달 후 PR 대기.
### todo
- 순회 지도(라운드당 6챕터): N5 R3~R5 → N4 R6~R9 → N3 R10~R12(확장 신규 저작 병행 검토)
  → N2·N1 R13~R17. **편성 확정사(2026-08-26 왕복)**: Codex 장기 스트림(R1) → Claude
  완전 무인(R2, 5428131058) → **오너 재전환 "ChatGPT한테 장기 업무" — R3부터 Codex-5
  정본 편성**(5429565401). 매 라운드: Claude SPEC 게시 → 오너 인라인 전달 → Codex PR →
  Claude 검수·머지·회신. 감사표 미제출 = 반려 유지. Claude 검수 스캔 틱이 PR을 집어간다.
