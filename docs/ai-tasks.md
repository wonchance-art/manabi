...
## Codex-1 (codex/*)
### doing
- **출시 후보 통합·중일 의미 검토 (2026-09-17, 오너 “좋아 진행해”)** — 기존 #1321에 검수된 #1322/#1323을 통합해 단일 후보로 정리. Preview·고정 QA 주소 최신화와 버전/실제 화면 확인, 출처 기반 40사례 재검토·실제 응답 평가 준비를 진행. 원래 작업 공간·운영 main/도메인·DB/env·교재 판본·SRS 보존. N5 감사 별도 세션은 아직 생성하지 않음.

### todo
- **검수 자동화 후속 (2026-09-17)** — 고정 QA 주소를 최신 후보1a21318f에 연결하고 공개 사전 검사 PASS. 남은 것은 OAuth 허용 목록·정상 인증 교사/학생 검수, 실제 모델 의미 평가·발행 N5 42과 커버리지. 화면 품질 회귀·중일 의미 자료 준비는 #1323 완료. 다음은 정상 계정/기기 확인과 독립 의미 검토·실제 모델 평가이며, N5 커버리지 감사는 별도 세션으로 분리 가능. PDF/신규 음성 보류. 병렬 세션은 아직 생성하지 않음. docs/qa-live-workflow.md.
- **통합 출시 후보 #1321 실사용 확인·운영 전환 (2026-09-17)** — #1316~#1320을 main 대상 draft 하나로 통합. 최신 #1321 Preview의 교사 가져오기/저장/재접속, 학생 암호 입장→원본→뜻/독음→직접 단어 저장→출처, 기존 개인 노트 누적 정리와 실제 iPad/Pencil은 로그인 후 확인. 합성 검수를 실계정으로 세지 않음. 운영 병합/승격·고정 alias는 별도 승인 범위. 원본 PR은 운영 통합 확인 전 보존. 아래 개별 후속의 현재 검토 단위는 #1321.
