...
- **🏁 가와구치코/후지 완성 선언**: 수집 #269→본생성 #276→배선 #278(첫 등산 버스·유람선 8호)→
  ...
  3호 = 제네바: 스냅샷 #285 검수·merge(해시 정합·계약 8/8) → Codex-2 본생성 SPEC 게시
  (5011571025 — POI 10종 좌표 표·무에트 도선 앵커). desc 10종 재고 완비
- **지역학(스터디즈) 트랙**: 🏁 일본학 6도메인 완간 + 한국학 4편(#274 개관·#280 역사·
  #282 경제 — 임진왜란 양국 교차 정합). **게임 반영 아이디어 보드 신설(#281 —
  docs/studies-game-ideas.md, 11개 후보 10점 평가·owner-gate, 추천 픽 ①+③ studiesRef 묶음)**
...
- **📱 여행 폰(오너 지시 #286)**: 여행 수첩 5번째 탭 — 게임 안 다이제틱 위키(지역학 검색·열람,
  studies 단일 진실원·게임 이탈 없음). 아이디어 보드 ①/③ 딥링크의 착지 기기로 확장 예정
### todo
- 일본 4도시 레거시 콘텐츠: 감사 완료(결손 0) — 다국어 UI 로드맵 확정 시 COPY 슬롯 이식과 1회 처리
- MSM 수도원 씬 카피 선례의 타 도시 액트 카피 확장 검토(IP 규율 내)
- 가와구치코 완성 후: 제네바 → 레만호(유럽 2차 순서), 후지 등산 액트 씬 골격 SPEC(자갈치 선례)
### done (최근)
