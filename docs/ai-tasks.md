...
## Codex-1 (codex/*)
### doing
- **뷰어 분석 복구 A 착수 (2026-09-09, 오너 설계 승인)** — `codex/viewer-analysis-resume-20260909`, 부모 #1296 head `19f5c5d5b7307ddc62383b018e077bebbc0d4560`. 누락/실패/불완전 줄을 합쳐 resume에 전달하고 문단 응답 중 정상 줄·교정·출처 보존, 중지/충돌/완료 검증. 범위: analysisCoverage 신규 순수 모듈·테스트, reanalysisPreservation/useReanalyze/ViewerPage, 격리 브라우저 검수 및 문서. DB·새 외부 전송 없음. 오프라인 채점 B와 옛 키/CSS는 별도 후속.
...
### todo
- **뷰어 신뢰성 후속 (Claude #1296 검수)** — 분석 resume missing 인덱스 병합 복원, 오프라인 큐 undo/outbox 잠금, `persistVocabGrade` 수렴, 옛 localStorage 키와 죽은 viewer-sheet CSS 정리. 원문 복귀 배포와 분리해 검증한다.
