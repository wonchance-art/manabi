...
- **🎨 v2-Q 뷰어 크롬 정돈 — 배지 폭이 제각각이던 건 취향이 아니라 **래퍼 부재**였다
  ...
  **배경도 테두리도 없고** 시리즈명은 `title` 툴팁일 뿐 **렌더되지 않는다**. 설계가 본
  ...
  계약 19종. **돌연변이 23종 전량 검출** — M19는 처음에 **생존**했다: 없는 요소의
  `indexOf`가 `-1`이라 「도구보다 앞에 온다」가 **공허 통과**했다. 존재 확인을 앞에 두고 재검출.
  green(3153) · lint 0 errors · **본문 무접촉 · 마이그레이션 0 · 로직 변경 0**. PR #1229.
