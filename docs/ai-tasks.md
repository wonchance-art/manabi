...
### todo
- **개인 노트 후속 (2026-09-15)** — 자유 필기 이미지/텍스트의 Gemini 전송 인식 API는 자동 승인 검토가 payload·목적지의 명시 승인 부재로 거절하여 미구현. 이번 개인 노트 코드·검수 문서의 공개 GitHub push도 별도 승인 부재로 거절되어 draft PR·미리보기 미생성. 공개 push/PR/미리보기 및 Gemini 선택 영역 전송 승인, 물리 iPad/Pencil·실제 계정 간 이어 쓰기, 외부 PDF/이미지 반입은 후속. 현재는 입력된 글자와 구조화된 표현의 한자 후보·일괄 정리를 제공한다.
