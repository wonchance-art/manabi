...
### doing
### todo
- **manabi 운영 후속 doing (2026-09-07)** — 오너 「모두 승인」으로 준비된 프로필 DB 수정 2건의 Codex 적용 및 검수용 인앱 브라우저 세션 사용을 명시적으로 승인했다. 원격 함수/정책 사전 조건 재확인 완료, 기존 역할 fingerprint 확보. 검증한 migration·격리 PostgreSQL12조건을 복원하고 승인 범위만 적용한다. 현재 브라우저는 로그아웃 상태라 오너의 검수용 계정 로그인을 요청했다. 기존 교재 migration 번호 정합화·병합은 Claude, 운영 승격은 기존 게이트 유지.
