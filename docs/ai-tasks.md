...
## Codex-1 (codex/*)
### doing
- **뷰어 문맥 뜻 선택 (2026-09-18, 오너 ㄱㄱ)** — #1321 후속. Chinese 단어창의 사전 후보/명시적 AI 문맥 후보 → 이 자료 토큰에만 저장 → 재접속/일본어 뜻 연결. 기존 교정 경로 재사용, 공유 사전·개인 어휘/SRS 자동 변경·DB 스키마 변경 없음. 오래된 응답/계정 전환/저장 실패/키보드 검수. 담당: ViewerPage/전용 의미·일본어 참조 UI, explain의 명시적 후보 분기, 전용 테스트/QA/문서.

### todo
- **실계정 의미 검수 후속 (2026-09-18)** — 정상 일반 계정의 중국어 4표본/자료 저장·교정·재접속·원문 복귀 확인. 东道主 문맥의 ‘접대하는 주인’이 같은 명사 첫 뜻 ‘주최국’으로 표시되는 의미 선택 문제는 미해결: 사전 뜻/문맥 후보 출처 분리·명시 선택·자료 한정 반영 설계 필요. 팀 암호 입장, 교사 설명판, 학생 원본 단어 저장/뜻 전달/복습 복귀, 일본어 나머지 표본·독립 감수·물리 iPad는 미검수. 개인 교정/SRS·공유 사전 자동 덮기 금지. docs/verification/authenticated-lexical-20260918.md.
