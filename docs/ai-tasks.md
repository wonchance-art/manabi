...
- **📚 우리 사전 '만남' 모델 — R1 완료·R2 진행(2026-08-22, 오너 "ㄱㄱ 착수")** — 외부
  ...
  **R3 노드 refs(오너 "진행해", RFC §4.6 신설)**: ⑫ 노드 `refs`/`refsLang` 스키마 +
  ...
  lint 0 err · 콘텐츠 게이트 오류 0 · vitest 1,960 + world 1,075 green. **잔여 =
  스크립트 4본(konbini·izakaya·ekiin·menzei) refs 저작**(居酒屋·屋台·お通し 정본
  추가 동반)·게이트/도어 노드·오버월드 desc, 서버 정본(§4.5)은 owner-gate.
