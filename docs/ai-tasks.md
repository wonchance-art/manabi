...
### doing
### todo
- **manabi 운영 전 실제 계정 검수 대기** — DB 수정 및 검수용 인앱 브라우저 사용은 오너 「모두 승인」으로 승인 완료했다. 현재 브라우저가 로그아웃 상태여서 직접 로그인만 요청했다. 실제 자료 저장·표현 복습·두 계정 권한 검수는 아직 미완료. 기존 교재 migration 번호 정합화·병합은 Claude, 자동 도메인 할당 분리·production 대기 빌드 검수는 운영 담당으로 유지한다.
