...
## Codex-1 (codex/*)
### doing
- **검수 자동화 2차 착수 (2026-09-17, 오너 “승인. 다음 작업 진행”)** — 별도 승인안 범위의 AGENTS/CLAUDE 정정 반영. 기존 #1322 작업 공간에서 공개 버전·고정 주소·인증 복귀 사전 검사 자동화와 정상 인증 교사/학생 검수 환경 설계. 앱/DB/env/alias/운영 병합은 이번 승인안에 포함하지 않음. 담당 파일은 두 규약·QA 전용 scripts/test·package 명령·검수 문서·자기 보드.

### todo
- **검수 자동화 후속 (2026-09-17)** — 고정 QA 주소/정상 인증의 교사·학생 계정, 승인 화면 비교/접근성, 실제 모델 의미 평가, 교재 커버리지 확장은 단계별 후속. AGENTS.md·CLAUDE.md 정정은 자동 승인 검토 거부 후 구체 제안에 대한 별도 오너 답변 대기; 파일 미수정. 병렬 세션은 우선 통합·품질/교재/제품 기능 3역할 제안이며 별도 세션 생성·기존 소유권 변경 없음. docs/quality-workflow.md.
