...
## 교재 순회 스트림 (claude/* — 완전 무인 · 구 Codex-5)
### doing
### todo
- 순회 지도(라운드당 6챕터): N5 R2~R5 → N4 R6~R9 → N3 R10~R12(확장 신규 저작 병행 검토)
  → N2·N1 R13~R17. **완전 무인 편성(오너 확정 2026-08-26, #150 코멘트 5428131058)**:
  Codex Cloud 제외, Claude가 주기 틱으로 감사·개보수·게이트·머지·완결 보고까지 수행.
  감사 규율 자기 적용 유지 — 모든 수정은 ⑴~⑺ 근거 명기, PR 본문에 챕터×항목 감사표,
  항목 밖 아이디어는 '제안' 레인 분리(무근거 수정 금지 — R1 원복 선례를 자신에게도).
- 스타일 캐논 미확정 2건(오너 확정 시 일괄 라운드로): ① 전사 장음 표기 ー/- 병존
  (실측 16:13) ② yomi 가나 클래스 — SCHEMA "전체 히라가나 독음" vs 실태(ドア·マナーモード
  등 가타카나 보존 다수 병존).
- ⚠ 오너 결정 대기(R3 창 착수 전): ot-12-menzei의 돈키호테 상호 — IP 하드리밋(상호
  무재현) vs 챕터 정체성(무대 자체가 그 가게). 임의 개변 금지, 결정 전 R3에서 해당
  챕터는 상호 표기만 보류하고 나머지 항목 감사.
...
- 📚 R2 — N5 order 07~12(ot-08-izakaya·n5-07-existence·n5-08-questions·ot-11-densha·
  n5-08-te-form·n5-09-adjectives) 감사·개보수, **무인 1호(본 PR 동봉)**. 수정 10곳:
