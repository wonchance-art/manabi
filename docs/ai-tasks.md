...
## Codex-1 (codex/*)
### doing
- 마르세유 EMEA 오버월드 게이트: 생샤를역 기준 도착 타일·보행 보정과
...
- **유럽 2차 라운드(오너 전권 위임 — 한 도시씩 완성)**: 1호 마르세유 — geo #260 검수·merge
  (2성분 BFS 85,296+141 독립 실측 정합) + 콘텐츠 배선·verifier 프로필·R4 스킨(#263) 완 —
  **20도시 등록**. 잔여 = 오버월드 EMEA 게이트(생샤를)만 → Codex-1 SPEC 게시 예정
- **지역학(스터디즈) 트랙 신설(오너 방향 — 전용 페이지 우선, 게임 적용 보류)**: 1기 골격+
  일본학 개관 merge(#256) — 후속 각론(역사·경제·문화)·한국학 개관은 대기 시간 트랙
- **렌더크래프트 R4 완결**: 엔진 #259 + 서브컬처 프롭 #262 검수·merge, tileSkins 10도시
  배정·배정표 회귀(#263)·가챠퐁/아케이드 도쿄 배치 — 지역 색감 라운드 종료
### todo
