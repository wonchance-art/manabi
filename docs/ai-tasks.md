...
### todo
- 브리즈번 geo 산출 시 verifier 프로필(브리즈번강 단면·스토리브리지 회랑 pin)+검수+배선
- 캔버라→멜버른 스펙 순차 게시(시드니는 owner-gate 메모리 결정 대기)
...
- **시드니 bbox·메모리 상한**: 로드맵 bbox 786×780 = 27.5MiB로 기본 24MiB 초과(Codex-2
  PREAUDIT 5003948464). 선택지 A) bbox 트림(예: 하버 중심 축소) B) 시드니 한정 상한 상향
  — 결정 전 시드니 수집·스펙 게시 보류(브리즈번→캔버라→멜버른은 상한 내라 정상 진행)
