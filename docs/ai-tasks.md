...
### doing
### todo
- 🗃️ 중국어 판별 캐시 RFC(발주 5312202647 §C4, 오너 ㄱㄱ 2026-08-17) — **RFC만, 구현
  착수 금지**: 재호출 실측·스키마 무변경 캐시+무효화 규칙·벤치 추정 → #150 코멘트.
  user_verified 보호 계약 충돌 금지. ← 최우선
...
## Claude (claude/*)
### doing
- **🀄 병음 일자 조판 — 크기 축소를 가로 압축으로 교체(2026-08-19, 오너 지적
  "아직도 일자가 아니라 글자마다 병음 위치가 다른데")** — #1058의 길이별 font-size
  축소(0.46/0.42/0.36em)가 남긴 문제: 크기가 다르면 병음마다 윗변·베이스라인·글자
  키가 달라져(윗변 지터 ±2px·키 최대 28% 편차) 한 줄로 안 읽힌다. 등배 복귀는
  불가(코퍼스 426문장·인접쌍 3,191 전수 — 등배 0.5em이면 겹침이 쌍 7.8%·문장
  35.7%·최대 12px). 단어 단위 병음(jieba 어절 rt)도 실측 기각 — 2자 어절에 긴 음절
  둘이 오면 더 나쁨(문장 43%). **해법: 크기는 전 음절 0.5em 하나로 통일하고 셀보다
  넓은 음절만 scaleX 가로 압축** — 세로 지표를 전혀 안 건드려 높이·베이스라인이
  전부 동일(일자)해지고 폭은 셀 안. 판정은 글자 '수'가 아니라 **추정 폭**(글자별
  전각비 실측표 + NFD 성조 제거 + 대문자 별도표) — 글자 수 기반의 잔여 겹침
  0.94%(màn màn류: m·w가 i·l의 3.5배 폭)까지 해소. 실측: 압축 후 **겹침 0쌍·최악
  여유 1.45px·추정 오차 ≤0.64px**, 압축 대상은 고유 음절의 29.9%(k 0.5대는
  chuāng·shuāng·zhuāng 계열 5음절뿐), 병음 윗변·아랫변 단일값(42/53)·글자 폭·피치
  20px 단일값·gap −3(네이티브 동일)·중앙 정렬 오차 0·ON=OFF 동일. 루비 압축은
  일본어 조판(JIS X 4051) 표준 관행. 수식 상수(RT_EM·TRACKING)와 CSS 실물의 일치를
  계약으로 고정, 유닛 8+계약 7.
