...
### done
- **실제 뷰어 전용 자동 검수 (2026-09-24)** — 구현 56692ea6. 기존 함수/합성 Auth·HTTP/PGlite를 재사용한 `qa:reader-app` 2엔진, Aa/집중/200%·자료 한정 뜻 저장/실패 재시도/재접속·원문 초점. 각각 8체크·기존 시각 기준3개·오류0/정리 통과(동일 앱 빌드의 로컬 점검). 최근 읽기/위치 기록만 계정·필드까지 검사해 허용하고 공유 사전/개인 뜻·SRS 변경 거부. reader8/classroom6/release14 구성으로 기존 범위 보존·중복 실행 제거. 3파일12단위/lint/diff PASS, 제품/DB/env 차이0. 최종 새 빌드/전체 CI·재검수→리뷰→다음 방향은 이 head의 PR #1321/#150/closeout 정본. 다음 제안은 legacy 자료의 동시 뜻 수정 보호. docs/verification/reader-app-qa-20260924.md.
