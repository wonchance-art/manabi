...
## Codex-1 (codex/*)
### doing
- **문장 기반 일본어 대조: 전송 범위 오너 확인 대기** — #1296의 원문 복귀·기존 후보 검증은 완료. 선택 문장/단어/뜻/품사를 명시적 조회 시 Gemini로 보내는 확장은 자동 승인 검토가 전송 승인을 확인하지 못해 보류했다. 기존 단어/뜻/자형 조회 유지; 문장 전송 코드 없음. 승인 뒤 문맥 캐시/재확인/보류 상태 및 정확도 검수를 별도 차수로 진행.

### todo
- **뷰어 신뢰성 B 후속 (오너 설계 승인)** — 분석 복구 A는 #1297로 완료. 다음은 오프라인 큐 저장 확인·미전송 항목 보존, undo/outbox 잠금, 서버 작업 식별·원자적 채점/취소 계약, `persistVocabGrade` 수렴. 옛 localStorage 키와 죽은 viewer-sheet CSS 정리는 별도. DB 계약은 검토 후 기존 협업 창구로 진행한다.
...
### done (최근)
- **뷰어 분석 복구 A 완료 (2026-09-09)** — #1297, 실행 `2932e88986395aacb0433881bafbcb81999b4c6c`. 누락/실패/불완전 줄 resume 연결, 문단 결과의 정상 줄·교정·ID 보존, 중지/충돌 안전 교체, 상태만 남은 자료 AI 미호출. 전체368파일/3,980개·새 테스트24개·변이 검출·로컬473페이지, CI34360526710 SUCCESS. Vercel READY, 배포21조건/오류0(320/390/768/1440px·복구·저장·복습·정확한 원문 복귀), 직접 화면 검수. 고정 preview 갱신·운영/DB/개인 기록 불변. 합성 fixture 검수이며 실계정 복구/물리 iOS는 별도. 상세 `docs/manabi-viewer-analysis-resume.md`, 최종 head는 CODEX_DONE.
- **저장 문맥·복습 원문 복귀 및 일본어 후보 검증 완료 (2026-09-09)** — #1296, 실행 `7a71c919c7c90f9555807ce89f98b74e0735aa2f`. 사용자/자료 권한을 확인하는 문맥 UUID, 인용과 토큰 대조·중복/유실 안내·요청 경합 보호, 주 문맥/링크 중복 제거. 자형 3종·품사/메타데이터 검증과 출처 표시, 최소 패널/저장 4등급 유지. 빌드 캐시 메모리 보완 후 로컬473페이지·CI34355199703 SUCCESS·Vercel READY. 배포10동작(320/390/768/1440px·저장·복습 왕복) 오류0, 화면 직접 검수, 비로그인 API401/private-no-store. 고정 preview 갱신, 운영/개인 기록 불변. 실계정 브라우저 연결 시간 초과로 해당 검수·물리 iOS는 미완료. 문장 Gemini 전송은 위 확인 대기. 상세 `docs/manabi-viewer-context-quality.md`, 최종 head/인계는 #150 CODEX_DONE.
