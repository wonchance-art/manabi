...
## Codex-1 (codex/*)
### doing
- **교재 설명·주의점 실계정 검수 보완 (2026-09-10)** — #1301, 최종 preview 6v9dwp8dg 실제 로그인 확인. 교재 확대/뜻 가리기/초점·위치 복귀·주의점 입력/취소 정상. 실제 사용에서 발견한 패널 내부 스크롤 잔존(다른 단어의 뜻 가림)·상하단 과 이동의 수업 문맥 유실을 같은 PR에서 보완 중. 관련30검사 통과, 브라우저 회귀·배포 확인 진행. 실자료에 검수 표현·주의점 기록 없음. 상세 `docs/verification/classroom-textbook-notes-20260910.md`.
- **수업 교재 학습 실계정 검수 (2026-09-10)** — #1300 구현·DB·배포 검수 완료. 새 preview qtr2r2rku 실제 로그인 화면/수업 복귀 주소 확인·교사 로그인 대기. 실제 교재→뜻→수업 추가→판 왕복은 격리 인증/DB 검수와 구분한다. 기존 실수업·개인 복습에 검수 입력 없음. 상세 `docs/manabi-classroom-study-flow.md`.
