...
## Codex-1 (codex/*)
### doing
- **수업 직접 열람·단어 저장·뜻 수정 전달 (2026-09-17, 오너 승인)** — #1321 head9ce690e3 위에서 후속. 개인 사본은 선택 보관으로 두고 수업 권한으로 원본 읽기→단어/문맥 저장→복습/원문 복귀를 연결. 이후 교사 뜻 수정의 원본·사본·변경 안내 일치, 개인 교정/SRS 보존. 허용: 수업/뷰어/단어 문맥 모델·API·SQL 신규 migration 파일·관련 검수/문서·자기 보드. 운영 DB 적용/환경/merge/alias는 제외. 실제 로그인 없이 로컬 SQL/RLS·합성 계정으로 검수.

### todo
- **통합 출시 후보 #1321 실사용 확인·운영 전환 (2026-09-17)** — #1316~#1320을 main 대상 draft 하나로 통합. 최종 g2xjncacd Preview의 교사 가져오기/저장/재접속, 학생 암호 입장→사본→뜻/독음→단어 저장→출처, 기존 개인 노트 누적 정리와 실제 iPad/Pencil은 로그인 후 확인. 합성 검수를 실계정으로 세지 않음. 운영 병합/승격·고정 alias는 별도 승인 범위. 원본 PR은 운영 통합 확인 전 보존. 아래 개별 후속의 현재 검토 단위는 #1321.
