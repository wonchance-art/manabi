...
> ## ⏸ 조건부 보류 (오너 확정 2026-08-31)
>
> **마이그레이션 품질은 양호하다** — 적용해도 위험이 없고, 적용하지 않아도 아무것도 깨지지 않는다
> (미적용 시 `/api/ai-relay`만 503, 앱 무영향). 보류 사유는 품질이 아니라 **쓸 주체가 없다**는 것이다:
> Codex-1~4 열이 비어 있고, Codex-5(Codex Cloud)는 GitHub 이슈조차 못 읽어 외부 HTTP·시크릿 보관
> 가능 여부가 미확인이며, 2026-08-31 확정된 C 편성(탐지/판단 분리)은 자동화를 쓰지 않는다.
> 지금 승인하면 오너는 마이그레이션 적용 + Vercel 시크릿 3종 등록을 치르고 **호출자 없는 채널**을 얻는다.
>
> **되살릴 조건 — 둘 중 하나가 참이 되면 재개한다:**
> 1. 에이전트 환경이 **외부 HTTP 호출 + 시크릿 보관**을 할 수 있다고 확인될 때
> 2. **리포 접근이 있는 상시 세션이 다시 여러 개** 돌 때(Codex-1~4 열 부활)
>
> 그때 이 문서와 `supabase/migrations/20260716074502_ai_relay_messages.sql`을 그대로 꺼내 쓰면 된다 —
> **테이블 신설이라 스키마 표류 위험이 없다**(다른 테이블 변경에 안 물린다).
> 아래 설계는 그 시점의 기준선으로 유효하므로 손대지 않는다.
