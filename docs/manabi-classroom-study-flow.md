...
## 실제 계정 수업 기록 왕복 — 2026-09-11

- 최신 26z7kitqi 배포에서 교사 실제 로그인 완료. 기존 검수용 수업의 오늘 날짜에 임시 노트 하나를 생성했다. 표현 입력 → 서버 저장 → 독음/뜻 분석 → 별도 판의 자동 수신을 확인했다.
- 그 노트의 뷰어에서 직접 찾기 → 사전 미등록 안내 → 독음/뜻 직접 입력 → `보여주고 기록` → 확대/뜻 가리기 → 원래 수업/날짜 복귀를 확인했다. 두 번째 표현도 별도 판에 새로고침 없이 반영됐고 노트를 다시 열어 서버 저장을 확인했다.
- 판의 글자 크기, 뜻 가리기/보이기, 이전 표현, 목록/한 표현, 최신 따라가기를 조작했다. 마지막에는 임시 노트만 소유자 삭제 UI로 제거했다. 오늘 판은 입력 대기로 돌아갔고 이전 날짜의 노트 1편/표현 4개는 유지됐다.
- 삭제 후 기존 노트의 원문/분석 JSON 지문 및 개인 단어 338개 전체 행 지문이 사전 값과 같았다. 임시 노트 잔존 0. 기존 추가 RPC가 갱신하는 검수 수업 루트의 마지막 표시 포인터/리비전은 바뀌므로 루트 JSON 전체가 불변이었다고 주장하지 않는다. 교재 연결/선택은 바꾸지 않았다.
- 이번 실제 쓰기는 수업 노트 뷰어에서 수행했다. 정식 교재를 연결한 출처 검증은 기존 격리 DB·브라우저 검사와 구분한다. 실제 교재 선택/뜻 확대/과 이동의 읽기 검수는 #1301에서 이미 완료했다.
- 기록 버튼 비활성화 뒤 확대를 닫으면 초점이 문서 본문으로 빠지는 문제를 발견해 #1301에서 보완했다. 실행·회귀·미리보기 결과는 `docs/verification/classroom-textbook-notes-20260910.md`에 기록한다.
