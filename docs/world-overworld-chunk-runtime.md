...
- ✅ 32×32 렌더 페이지 좌표·가시 범위·padding·이동 방향 prefetch
- ✅ 무복사 page source·entry/byte LRU·원자 가시 교체·stale bake 폐기
- ✅ Phaser RenderTexture bake adapter·실카메라 dirty 갱신·화면 내 물결 스프라이트 풀
- ✅ 기존 한일 448×384 격자의 임시 저장 청크 adapter(Tilemap 제거, 충돌·노드 좌표 불변)
- ✅ 전역 양자화 희소 노선 culling·청크 독립 VehicleLayer·페이지/저장 청크 경계 객체 연속성
- ✅ 부산–후쿠오카 기존 페리 노드의 실화면 route/시간표 fixture
- ✅ canonical `.owc` 4청크 결정적 패킹·체크인 산출물 검증·실화면 loader 교체

실화면은 `public/assets/overworld/korea-japan-fixture`의 4개 canonical 청크만 읽는다. 생성기는 현재
`buildPlayableGrid(decodeMap())`과 같은 표면·충돌 셀을 4+1+1bit로 패킹하고, 콘텐츠 manifest를 상대
경로순으로 기록한다. 개발·빌드 게이트의 `--check`는 소스에서 재생성한 바이트와 체크인 파일을 직접
비교한다. 렌더 표면은 `.owc`를 단일 진실원으로 사용하며, 충돌·노드 좌표는 승인된 마이그레이션 전까지
legacy 격자를 이중 운용한다. 전체 유효 셀·bbox 패딩 동일성 테스트가 이 과도기 계약을 잠근다.

`src/lib/world/overworldRenderPages.js`는 Phaser에 의존하지 않는다. 카메라 `worldView`를 가시 페이지,
padding, 방향 prefetch 순으로 정렬하고 각 페이지를 하나의 256×256 저장 청크 안 32×32 view로 바꾼다.
`OverworldRenderPager`에는 씬이 `bakePage`, `setPageVisible`, `destroyPage` 콜백을 주입한다. 새 가시
페이지를 전부 굽고 표시한 다음 이전 가시 페이지를 숨기므로 순간이동·로드 실패에도 빈 프레임이 없다.
manifest 세대가 바뀌거나 경쟁 카메라 요청·씬 종료가 발생하면 늦게 끝난 bake 결과는 파기한다.
...
수직 슬라이스는 `src/lib/world/overworldOverlay.js`에서 route 좌표를 타일 1/1024 정수로 고정한다.
`PhaserOverworldOverlayLayer`는 카메라 bounds+선폭 halo와 교차하는 전역 구간만 별도 Graphics에 그리고,
차량 스프라이트를 route/run ID로 소유한다. 따라서 렌더 페이지·256 저장 청크 경계를 지나도 같은 객체가
유지된다. 현재 실화면 fixture는 기존 `busan-port`↔`fukuoka-port` 노드와 월드 시각 시간표만 사용한다.
...
3. 32×32 렌더 페이지와 CityScene 동등 카메라 이동 테스트. **순수 런타임+Phaser adapter 완료.**
4. canonical 한일 4청크 실화면 교체, 해안·철도 경계 fixture, 차량이 페이지/저장 청크 경계를 넘는 테스트.
