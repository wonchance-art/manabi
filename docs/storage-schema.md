...
| `vocab-encounters:<lang>` | 만난 정본 표기의 정렬된 `string[]` JSON | `vocabEncounters.js`의 load/save/record; `NpcDialog`가 refs 저작 스텝 노출 시 기록, 완주 요약(목업 A)·도시 이해도(R2)가 소비 (rfc-vocab-encounter §4.2) | `lang`은 소문자 2자(ja 등)만 허용. 깨진 JSON·비배열은 빈 Set. 저장 시 비문자열·빈 표기 제거와 정렬, 전부 기존 표기면 재저장하지 않는다(멱등). 기록 원천은 저작 refs뿐이라 쓰기 시점 정본 대조는 없고(계약 테스트가 실재 보장), 소비처가 현재 정본 레지스트리와 교집합으로 유령 표기를 제외한다. 저장소 차단은 읽기 빈 Set·쓰기 `false`. |
